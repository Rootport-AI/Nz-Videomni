# Nz-LTX23 (AviUtl2 frontend)

A general-purpose AviUtl2 plugin (`.aux2`) that acts as a thin frontend for an
LTX 2.3 video-generation backend (`Nz-LTX23-backend`, developed separately and
frozen from this repository's perspective — it is never modified here).
`NzLTX23.aux2` registers a dockable window titled **Nz-LTX23** and hosts a
...
- **NAG (Negative Prompt) accordion**: a single collapsible "Negative Prompt"
  ...
  persists across restarts (`localStorage`, `nzltx23.nagNegativeText`); the
...
- **Connection settings**: the backend base URL is a persisted runtime
  setting (`settings.get` / `settings.set`, `%LOCALAPPDATA%\NzLTX23\
...
- **`.au2pkg.zip` packaging**: `scripts/package.ps1` bundles the
  single-file-embedded `NzLTX23.aux2` plus `Language/*.aul2` (and
  `package.ini` / `package.txt`) into `dist/NzLTX23-<version>.au2pkg.zip` in
  the layout AviUtl2 expects (`Plugin/NzLTX23/`, `Language/`).
...
Language/                 *.NzLTX23.aul2 (native UI strings, English/Japanese)
...
- CMake + Ninja. If they are not already available, `build.ps1` downloads a
  pinned portable copy into `%LOCALAPPDATA%\NzLTX23\buildtools` on first use
...
> `NzLTX23_tests.exe --test-suite-exclude=integration` directly, not `ctest`),
...
Output: `build\ninja-release\NzLTX23.aux2` and `build\ninja-release\NzLTX23_tests.exe`.
...
(`-DNZLTX_EMBED_WEBUI=ON`), so `NzLTX23.aux2` serves its UI from memory and
...
To point the build at specific tools, set `NZLTX_CMAKE`, `NZLTX_NINJA`, or
`NZLTX_VCVARS` to the full path of the respective executable/batch file.
...
> `build\ninja-release\CMakeFiles\NzLTX23.dir\webui_embedded.rc.res`). This is
...
> listening on `127.0.0.1:18620` (`Nz-LTX23-backend`'s `run.ps1` with
...
> `build\ninja-release\NzLTX23_tests.exe --test-suite-exclude=integration`.
...
a non-embedded build; the on-disk `Plugin\NzLTX23\webui\` folder layout from
...
#   <install>\data\Plugin\NzLTX23\NzLTX23.aux2
#   <install>\data\Language\*.NzLTX23.aul2 (from Language\)
# Also removes the old single-file <install>\data\Plugin\NzLTX23.aux2
# (avoids a double load), and any stale <install>\data\Plugin\NzLTX23\webui\
...
# Bundle the embedded .aux2 + Language files into dist\NzLTX23-<version>.au2pkg.zip:
...
After launching, the **Nz-LTX23** panel can be shown/docked from the AviUtl2
window menu. Plugin log lines appear in AviUtl2's in-app Log view **and** in
`%LOCALAPPDATA%\NzLTX23\logs\plugin.log`.
...
> `[NzLTX23\NzLTX23.aux2] trust=1`. Until approved, `RegisterPlugin` never runs
> and the plugin does not load (no log is written).

> **Right-click menu language.** The timeline's right-click menu (Nz-LTX23's
...
> setting**, not the Web UI panel's `LANGUAGE` toggle inside Nz-LTX23 — the two
...
> `Language\*.NzLTX23.aul2` file for whichever language AviUtl2 itself is set
