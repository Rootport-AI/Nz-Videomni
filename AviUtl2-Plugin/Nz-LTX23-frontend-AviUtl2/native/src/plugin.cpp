//----------------------------------------------------------------------------------
//  Nz-LTX23 - AviUtl2 general-purpose (.aux2) plugin, milestone M1.
//
...
//  https://app.nzltx23.local/index.html, and talks to native code over a
...
//  Constants
//---------------------------------------------------------------------
#define NZLTX_WINDOW_NAME L"Nz-LTX23"
...
constexpr UINT WM_NZLTX_POST_RESPONSE = WM_APP + 17;
...
nzltx::Bridge       g_bridge;
nzltx::WebViewHost  g_webview_host;
...
// Language files, see Language/*.NzLTX23.aul2). Falls back to the original
...
// WM_NZLTX_POST_RESPONSE path the async RPC responses already use
...
void PostJsonToPage(std::string json_utf8) {
    ...
    if (!::PostMessageW(g_plugin_hwnd, WM_NZLTX_POST_RESPONSE, 0,
...
void SnapshotSelection(EDIT_SECTION* edit, nzltx::SelectionSnapshot* snap,
    ...
    for (OBJECT_HANDLE o : objects) {
        ...
        if (edit->get_object_name != nullptr) {
            ...
            if (nm != nullptr && nm[0] != L'\0') {
                ...
                item.object_name = nzltx::WideToUtf8(nm);  // copy immediately
            }
        }
...
void EmitMenuInvoked(const char* action, EDIT_SECTION* edit) {
    try {
        ...
        nzltx::json_t data;
        data["action"] = action;
        data["selection"] = nzltx::MakeSelectionResult(snap);
        nzltx::json_t ev;
        ev["event"] = "timeline.menuInvoked";
        ev["data"] = std::move(data);
        PostJsonToPage(ev.dump());
        nzltx::LogInfo(std::wstring(L"timeline.menuInvoked pushed: action=") +
                       nzltx::Utf8ToWide(action));
    } catch (...) {
        // A C++ catch (not SEH) so object unwinding is allowed here (no C2712);
        // nothing may propagate out of a host-invoked plain callback.
        nzltx::LogError(L"EmitMenuInvoked failed (exception swallowed)");
    }
}
...
//     action carries the fixed action id (NZLTX_MENU_FALLBACK_FN below).
//
...
void OnMenuInvokedParam(void* param) {
    ...
    if (g_edit_handle != nullptr && g_edit_handle->call_edit_section_param != nullptr) {
        ...
        nzltx::LogWarn(L"menu-param: call_edit_section_param did not run; "
...
#define NZLTX_MENU_FALLBACK_FN(fn, action_id) \
...
// right-clicked object is a Nz-LTX23 provisional whose job is already complete,
...
NZLTX_MENU_FALLBACK_FN(OnMenu_ExtendVideo,       "extendVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_ReferenceVideo,    "referenceVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_ReferenceVideoChain, "referenceVideoChain")
NZLTX_MENU_FALLBACK_FN(OnMenu_VideoAudioToVideo, "videoAudioToVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_VideoAudioToLongA2v, "videoAudioToLongA2v")
NZLTX_MENU_FALLBACK_FN(OnMenu_AudioToLongA2v,    "audioToLongA2v")
NZLTX_MENU_FALLBACK_FN(OnMenu_ImageToVideo,      "imageToVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_AddImageKeyframe,  "addImageKeyframe")
NZLTX_MENU_FALLBACK_FN(OnMenu_ImageToClipChain,  "imageToClipChain")
NZLTX_MENU_FALLBACK_FN(OnMenu_EndWithThis,       "endWithThis")
NZLTX_MENU_FALLBACK_FN(OnMenu_AudioToVideo,      "audioToVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_AppendText,        "appendText")
NZLTX_MENU_FALLBACK_FN(OnMenu_InsertProvisionalResult, "insertProvisionalResult")
NZLTX_MENU_FALLBACK_FN(OnMenu_OutpaintVideo,     "outpaintVideo")
NZLTX_MENU_FALLBACK_FN(OnMenu_RetakeRange,       "retakeRange")
...
NZLTX_MENU_FALLBACK_FN(OnMenu_TextToVideoHere,             "textToVideoHere")
NZLTX_MENU_FALLBACK_FN(OnMenu_ImageFromCurrentFrame,      "imageFromCurrentFrame")
NZLTX_MENU_FALLBACK_FN(OnMenu_AddCurrentFrameKeyframe,    "addCurrentFrameAsKeyframe")
NZLTX_MENU_FALLBACK_FN(OnMenu_CurrentFrameToClipChain,    "currentFrameToClipChain")
NZLTX_MENU_FALLBACK_FN(OnMenu_InsertLatestResultHere,     "insertLatestResultHere")

#undef NZLTX_MENU_FALLBACK_FN

// A registerable timeline menu item: ASCII display key (run through Translate()
// into the Language/*.NzLTX23.aul2 Japanese label), the stable ASCII action id
...
// the embedded '\' makes the host render an "Nz-LTX23" submenu.
...
// display key. The '\' after "Nz-LTX23" makes the host render an "Nz-LTX23"
// submenu. The display keys are translated through Language/*.NzLTX23.aul2.
...
// translation keys in Language/*.NzLTX23.aul2 are untouched. (Keep this comment
...
const TimelineMenuItem kObjectMenuItems[] = {
    { L"Nz-LTX23\\\U0001F3AC Video: continue this video (v2v)",                             "extendVideo",       &OnMenu_ExtendVideo },
    { L"Nz-LTX23\\\U0001F3AC Video: generate using this video as reference (IC-LoRA)",      "referenceVideo",    &OnMenu_ReferenceVideo },
    { L"Nz-LTX23\\\U0001F3AC Video: long IC-LoRA reference for a clip chain",               "referenceVideoChain", &OnMenu_ReferenceVideoChain },
    { L"Nz-LTX23\\\U0001F3AC Video: audio-to-video from this video's sound",                "videoAudioToVideo", &OnMenu_VideoAudioToVideo },
    { L"Nz-LTX23\\\U0001F3AC Video: long a2v from this video's sound",                      "videoAudioToLongA2v", &OnMenu_VideoAudioToLongA2v },
    { L"Nz-LTX23\\\U0001F3AC Video: expand this video's canvas (Outpainting)",              "outpaintVideo",     &OnMenu_OutpaintVideo },
    { L"Nz-LTX23\\\U0001F3AC Video: redo the selected range (Retake)",                      "retakeRange",       &OnMenu_RetakeRange },
    { L"Nz-LTX23\\\U0001F5BC Image: generate a video from this image (i2v)",                "imageToVideo",      &OnMenu_ImageToVideo },
    { L"Nz-LTX23\\\U0001F5BC Image: add this image as a keyframe",                          "addImageKeyframe",  &OnMenu_AddImageKeyframe },
    { L"Nz-LTX23\\\U0001F5BC Image: generate a long video from this image (i2v Clip Chain)", "imageToClipChain", &OnMenu_ImageToClipChain },
    { L"Nz-LTX23\\\U0001F3AC\U0001F5BC: Generate a video ending with this",                 "endWithThis",       &OnMenu_EndWithThis },
    { L"Nz-LTX23\\\U0001F3B5 Audio: audio-to-video from this audio",                        "audioToVideo",      &OnMenu_AudioToVideo },
    { L"Nz-LTX23\\\U0001F3B5 Audio: long a2v from this audio",                              "audioToLongA2v",    &OnMenu_AudioToLongA2v },
    { L"Nz-LTX23\\\U0001F4DD Text: append to the main prompt",                              "appendText",        &OnMenu_AppendText },
    { L"Nz-LTX23\\\U00002B07 Insert this generated result now",                             "insertProvisionalResult", &OnMenu_InsertProvisionalResult },
};

const TimelineMenuItem kLayerMenuItems[] = {
    { L"Nz-LTX23\\\U00002728 Insert AI generation here (t2v)",                                    "textToVideoHere",           &OnMenu_TextToVideoHere },
    { L"Nz-LTX23\\\U0001F4F7 Generate a video from the current frame (i2v)",                      "imageFromCurrentFrame",     &OnMenu_ImageFromCurrentFrame },
    { L"Nz-LTX23\\\U0001F4F7 Add the current frame as a keyframe",                                "addCurrentFrameAsKeyframe", &OnMenu_AddCurrentFrameKeyframe },
    { L"Nz-LTX23\\\U0001F4F7 Generate a long video from the current frame (i2v Clip Chain)",      "currentFrameToClipChain",   &OnMenu_CurrentFrameToClipChain },
    { L"Nz-LTX23\\\U00002B07 Insert the latest generation result here",                           "insertLatestResultHere",    &OnMenu_InsertLatestResultHere },
};
...
void RegisterTimelineMenu(HOST_APP_TABLE* host, const TimelineMenuItem* items,
    ...
    if (reg_param != nullptr) {
        ...
        nzltx::LogInfo(std::wstring(L"register_") + kind + L"_menu_param: registered " +
        ...
        nzltx::LogInfo(std::wstring(L"register_") + kind +
                       L"_menu (non-param fallback): registered " +
                       std::to_wstring(count) + L" item(s)");
    } else {
        nzltx::LogWarn(std::wstring(L"register_") + kind + L"_menu[_param] unavailable "
...
void OnProjectLoad(PROJECT_FILE* project) {
    ...
    try {
        ...
        nzltx::LogInfo(L"timeline.projectLoaded pushed (project load handler)");
    } catch (...) {
        nzltx::LogError(L"OnProjectLoad failed (exception swallowed)");
    }
}
...
EXTERN_C __declspec(dllexport) bool InitializePlugin(DWORD version) {
    (void)version;
    nzltx::LogInfo(L"Nz-LTX23 InitializePlugin");
    return true;
}

EXTERN_C __declspec(dllexport) void UninitializePlugin() {
    nzltx::LogInfo(L"Nz-LTX23 UninitializePlugin");
    g_bridge.Shutdown();
    g_webview_host.Shutdown();
}
...
EXTERN_C __declspec(dllexport) void RegisterPlugin(HOST_APP_TABLE* host) {
    ...
    wcex.lpszClassName = NZLTX_WINDOW_NAME;
    wcex.lpfnWndProc   = WndProc;
    wcex.hInstance     = hinst;
    wcex.hbrBackground = reinterpret_cast<HBRUSH>(COLOR_WINDOW + 1);
    wcex.hCursor       = ::LoadCursorW(nullptr, IDC_ARROW);
    if (!::RegisterClassExW(&wcex)) {
        nzltx::LogError(L"RegisterClassExW failed");
        return;
    }
    ...
    if (hwnd == nullptr) {
        nzltx::LogError(L"CreateWindowExW failed");
        return;
    }
    ...
    g_status_label = ::CreateWindowExW(
        0,
        WC_STATICW,
        Translate(L"Loading Nz-LTX23 UI...").c_str(),
    ...
    // display name is translated (Language/*.NzLTX23.aul2); the window class
    // name / creation title above stay the fixed NZLTX_WINDOW_NAME identifier.
    const std::wstring display_name = Translate(NZLTX_WINDOW_NAME);
    ...
    g_bridge.Initialize([plugin_hwnd](const std::string& response) {
        ...
        if (!::PostMessageW(plugin_hwnd, WM_NZLTX_POST_RESPONSE, 0,
                            reinterpret_cast<LPARAM>(payload))) {
            delete payload;
        }
    });
    ...
    g_webview_host.SetReadyHandler([](bool success) {
        if (success) {
            nzltx::LogInfo(L"Nz-LTX23 WebView2 panel ready (navigation succeeded)");
        } else {
            nzltx::LogError(L"Nz-LTX23 WebView2 panel failed to become ready "
                            L"(navigation failed)");
        }
    });
    ...
    // '\' makes the host render an "Nz-LTX23" submenu.
    ...
    // and every alpha item below actually appears/fires under the "Nz-LTX23" submenu.
    ...
    if (host->register_project_load_handler != nullptr) {
        ...
        nzltx::LogInfo(L"register_project_load_handler: registered");
    } else {
        nzltx::LogWarn(L"register_project_load_handler unavailable (beta52) - orphan "
                       L"provisional scan must be user-triggered");
    }

    nzltx::LogInfo(L"Nz-LTX23 plugin registered (WebView2 host + edit handle)");
}
