...
constexpr wchar_t kVirtualHost[] = L"app.nzltx23.local";
constexpr wchar_t kVirtualOrigin[] = L"https://app.nzltx23.local";
constexpr wchar_t kResourceFilter[] = L"https://app.nzltx23.local/*";
constexpr wchar_t kIndexUrl[] = L"https://app.nzltx23.local/index.html";
...
// Load the embedded single-file Web UI (RCDATA "NZLTX_WEBUI_INDEX") from this
// DLL, if one was linked in (NZLTX_EMBED_WEBUI=ON at build time). Returns an
...
std::string LoadEmbeddedWebUi() {
    ...
    HRSRC res = ::FindResourceW(module, L"NZLTX_WEBUI_INDEX", RT_RCDATA);
    if (res == nullptr) {
        return std::string();
    }
...
// Extract the path component of a "https://app.nzltx23.local/<path>?<query>"
...
bool WebViewHost::Initialize(HWND parent, HWND fallback_label,
    ...
    // Prefer the single-file Web UI embedded in this DLL (NZLTX_EMBED_WEBUI=ON).
