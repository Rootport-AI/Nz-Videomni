...
// NzLTX23_tests.exe --test-suite-exclude=integration with no live backend.
//
...
std::string ApiBase() {
    return std::string(nzltx::kBackendBaseUrl) + nzltx::kBackendApiPrefix;
}
...
TEST_CASE("CrackUrl: single-key query is attached to the path exactly once") {
    const nzltx::detail::UrlParts parts = nzltx::detail::CrackUrl(
...
TEST_CASE("CrackUrl: multi-key query is preserved verbatim") {
    const nzltx::detail::UrlParts parts =
        nzltx::detail::CrackUrl(L"http://127.0.0.1:18620/api/v1/status?a=1&b=2");
    REQUIRE(parts.ok);
    CHECK(parts.host == L"127.0.0.1");
    CHECK(parts.port == 18620);
...
TEST_CASE("CrackUrl: no query string leaves the path unchanged") {
    const nzltx::detail::UrlParts parts =
        nzltx::detail::CrackUrl(L"http://127.0.0.1:18620/api/v1/status");
    REQUIRE(parts.ok);
    CHECK(parts.host == L"127.0.0.1");
    CHECK(parts.port == 18620);
...
TEST_CASE("CrackUrl: no path defaults to \"/\"") {
    const nzltx::detail::UrlParts parts =
        nzltx::detail::CrackUrl(L"http://127.0.0.1:18620");
    REQUIRE(parts.ok);
    CHECK(parts.host == L"127.0.0.1");
    CHECK(parts.port == 18620);
    CHECK(parts.path == L"/");
...
TEST_CASE("GET /status returns 200 with a running server body") {
    ...
    const nzltx::HttpResponse resp =
        client.RequestSync(ApiBase() + "/status", "GET", "", nzltx::kDefaultRequestTimeoutMs);
...
TEST_CASE("an unknown job id returns a 404 as a normal (non-transport) result") {
    nzltx::HttpClient client;
    const nzltx::HttpResponse resp = client.RequestSync(
        ApiBase() + "/jobs/does-not-exist", "GET", "", nzltx::kDefaultRequestTimeoutMs);
    REQUIRE(resp.transport == nzltx::TransportError::kNone);
...
TEST_CASE("RequestAsync delivers the response on a worker thread") {
    nzltx::HttpClient client;
    std::promise<nzltx::HttpResponse> promise;
    auto future = promise.get_future();
    client.RequestAsync(ApiBase() + "/status", "GET", "",
                        nzltx::kDefaultRequestTimeoutMs,
                        [&promise](nzltx::HttpResponse r) { promise.set_value(std::move(r)); });
    REQUIRE(future.wait_for(std::chrono::seconds(30)) == std::future_status::ready);
    const nzltx::HttpResponse resp = future.get();
    CHECK(resp.transport == nzltx::TransportError::kNone);
    CHECK(resp.status == 200);
}
...
TEST_CASE("full generate -> poll -> download flow saves a non-empty mp4") {
    ...
    const nzltx::HttpResponse gen =
        client.RequestSync(ApiBase() + "/generate", "POST", gen_body,
                           nzltx::kDefaultRequestTimeoutMs);
    REQUIRE(gen.transport == nzltx::TransportError::kNone);
    ...
    std::string status;
    for (int i = 0; i < 120; ++i) {
        const nzltx::HttpResponse poll = client.RequestSync(
            ApiBase() + "/jobs/" + job_id, "GET", "", nzltx::kDefaultRequestTimeoutMs);
        REQUIRE(poll.transport == nzltx::TransportError::kNone);
    ...
    const nzltx::DownloadResult dl = client.DownloadSync(
        ApiBase() + "/jobs/" + job_id + "/video", dest, nzltx::kDefaultDownloadTimeoutMs);
    REQUIRE(dl.transport == nzltx::TransportError::kNone);
    CHECK(dl.status == 200);
...
TEST_CASE("downloading a non-existent job reports a non-200 and writes no file") {
    ...
    ::DeleteFileW(dest.c_str());
    const nzltx::DownloadResult dl = client.DownloadSync(
        ApiBase() + "/jobs/does-not-exist/video", dest, nzltx::kDefaultDownloadTimeoutMs);
    REQUIRE(dl.transport == nzltx::TransportError::kNone);
    CHECK(dl.status != 200);
    CHECK(dl.size_bytes == 0);
