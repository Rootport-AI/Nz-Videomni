<#
.SYNOPSIS
    Configure and build the Nz-LTX23 native plugin (.aux2) and its test exe.
...
.DESCRIPTION
    ...
    therefore resolves cmake + ninja from (in order):
      1. Explicit env vars  NZLTX_CMAKE / NZLTX_NINJA (full path to the .exe)
      2. A Visual Studio-bundled copy, if present
      3. Whatever is already on PATH
      4. A per-user cache under %LOCALAPPDATA%\NzLTX23\buildtools
...
.PARAMETER RunTests
    ...
    doctest suite (invokes NzLTX23_tests.exe --test-suite-exclude=integration
    directly, not ctest, precisely so this works without a mock backend
    running). To also run "integration", start the mock backend
    (Nz-LTX23-backend's run.ps1 with config.yaml's model.backend: mock) and
    run build\<preset>\NzLTX23_tests.exe by hand with no filter (or ctest).
...
.PARAMETER NoEmbedWebui
    ...
    -DNZLTX_EMBED_WEBUI=OFF, so no npm install is required. The resulting
    NzLTX23.aux2 has no UI resource of its own; it falls back to the native
...
.PARAMETER EmbedWebui
    ...
    configured with -DNZLTX_EMBED_WEBUI=ON so that HTML is compiled into the
    DLL as an RCDATA resource. The resulting NzLTX23.aux2 serves its UI from
...
.PARAMETER EmbeddedHtml
    ...
    webui\dist-single\index.html (passed through to NZLTX_WEBUI_SINGLE_HTML).
#>
[CmdletBinding()]
param(
...
$CMakeVersion = "3.31.6"
$NinjaVersion = "1.12.1"
$CacheDir = Join-Path $env:LOCALAPPDATA "NzLTX23\buildtools"
...
function Resolve-CMake {
    if ($env:NZLTX_CMAKE -and (Test-Path $env:NZLTX_CMAKE)) { return $env:NZLTX_CMAKE }
...
function Resolve-Ninja {
    if ($env:NZLTX_NINJA -and (Test-Path $env:NZLTX_NINJA)) { return $env:NZLTX_NINJA }
...
function Resolve-VcVars {
    if ($env:NZLTX_VCVARS -and (Test-Path $env:NZLTX_VCVARS)) { return $env:NZLTX_VCVARS }
    $hit = Find-VsBundled "VC\Auxiliary\Build\vcvars64.bat"
    if ($hit) { return $hit }
    throw "vcvars64.bat not found. Set NZLTX_VCVARS to its full path."
}
...
if ($embed) {
    ...
    $configureArgs += "-DNZLTX_EMBED_WEBUI=ON"
    $configureArgs += "-DNZLTX_WEBUI_SINGLE_HTML=$htmlPath"
} else {
    # Ensure an earlier embedded configure in this build dir does not linger.
    $configureArgs += "-DNZLTX_EMBED_WEBUI=OFF"
}
...
$aux2 = Join-Path $buildDir "NzLTX23.aux2"
...
if ($RunTests) {
    ...
    $testExe = Join-Path $buildDir "NzLTX23_tests.exe"
