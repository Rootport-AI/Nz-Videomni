<#
.SYNOPSIS
    Deploy NzLTX23.aux2 (embedded Web UI build only) into the AviUtl2 Plugin
    folder.

.DESCRIPTION
    Embedded-only deploy (2026-07-17 one-off to embedded-only operation): the
    non-embedded layout (a bare .aux2 plus an on-disk Plugin\NzLTX23\webui\
    ...
    .aux2 and no longer copies a webui\ folder:
        ...
        <PluginDir>\NzLTX23\NzLTX23.aux2   (embedded Web UI resource inside)
    ...
    If a stale Plugin\NzLTX23\webui\ folder exists on the target from an
    ...
    The old single-file layout <PluginDir>\NzLTX23.aux2 is removed if present,
    ...
    subfolder of it):
        ...
        <PluginDir's parent>\Language\English.NzLTX23.aul2
        <PluginDir's parent>\Language\Japanese.NzLTX23.aul2
...
param(
    ...
    [string]$DistDir = "S:\OriginalApps\12_Nz-LTX23-AviUtl2\Nz-LTX23-backend\AviUtl2-Plugin"
)
...
# UTF-16LE resource name NZLTX_WEBUI_INDEX is NOT sufficient on its own: the
# L"NZLTX_WEBUI_INDEX" string literal passed to FindResourceW() in
# native/src/webview_host.cpp is compiled into the DLL's code unconditionally,
# regardless of -DNZLTX_EMBED_WEBUI, so its raw bytes are present in EVERY
...
$legacy = Join-Path $PluginDir "NzLTX23.aux2"
if (Test-Path $legacy) {
    Write-Host "Removing stale legacy plugin: $legacy"
    Remove-Item -Path $legacy -Force
}

# --- deploy into the NzLTX23 subfolder --------------------------------------
$destDir = Join-Path $PluginDir "NzLTX23"
...
$destAux2 = Join-Path $destDir "NzLTX23.aux2"
...
# Probe guard (2026-07-31): an investigation build (NZLTX_PROBE_VIDEO_ITEMS)
...
if ($DistDir) {
    ...
    $distAux2 = Join-Path $DistDir "NzLTX23.aux2"
...
if (Test-Path $languageSrc) {
    ...
    Get-ChildItem -Path $languageSrc -Filter "*.NzLTX23.aul2" | ForEach-Object {
