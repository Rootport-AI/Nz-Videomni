 ...
 * `reconcileFromTimeline` so a leftover `NzLTX23#<jobId>` tag from an
...
function AppShellBody({ nativeBridge }: AppShellProps) {
  ...
  // "Nz-LTX23" header title. "LTX 2.5" is not a real, backed model yet —
  ...
  const handleRoute = useCallback(async (command: RoutedMenuCommand) => {
    ...
    // ── W2 (⬇ insertProvisionalResult): the right-clicked object is a Nz-LTX23
    ...
    // by the object's `NzLTX23#<jobId>` name (the `menuInvoked` snapshot already
    ...
    if (action === "insertProvisionalResult") {
      ...
      const match = objectName ? /^NzLTX23#(.+)$/.exec(objectName) : null;
      if (match === null) {
        // (a) not a Nz-LTX23 reservation object.
    ...
    try {
      // X2(b): pass the ledger's terminal jobs so a stale `NzLTX23#<jobId>` tag
  ...
  // NzLTX23#… placeholders that survived a project reload, so the busy-guard and
    ...
    <div className="app-shell">
      <header className="app-header">
        {/* Tool-version dropdown (2026-08-19, mock — owner-directed): occupies
            the old static "Nz-LTX23" title's spot. Selecting "LTX 2.5" has no
