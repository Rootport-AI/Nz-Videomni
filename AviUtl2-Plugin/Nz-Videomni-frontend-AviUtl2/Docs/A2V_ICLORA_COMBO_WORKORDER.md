...
- 併読: [`DEVLOG.md`](DEVLOG.md) §42.8（相互排他の経緯調査の結論）／[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-25（実装完了記録。起票当時は旧§1-1）・同書§3-18（2026-07-19判定の訂正注記）／[`API_REFERENCE.md`](API_REFERENCE.md)（`GenerateChainRequest`の契約）／バックエンド [`../../../Docs/VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §34.5・§34.7（実機ゲートの前例）
...
- 同日、GPU実機ゲート合格済み（pose-control×3＋canny-control×1、1280×768／201フレーム、ピークVRAM 9.5GB）でオーナー受容済み（バックエンド [`../../../Docs/VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §34.7）。
...
- **mockでのE2E**: バックエンド [`../../../Docs/VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §34.5の前例（`clips=1` ＋ control ＋ `reference_video_id` ＋ `source_audio` → 202 → completed）に準拠する。
