...
| 生成結果 | 素材へ自然に到達する（意味論的に近い素材の場合） | 機構としては成立（実機ゲートM1〜M7全PASS）。**オーナー目視・試聴済み**——クリップの境目・末尾にモーフが出ることがあり、品質劣化として仕様上許容（推奨外） | 継ぎ目はすべて正順チェーンと同じ。**2つの素材の内容が離れていると最終クリップの中でクロスフェードやモーフになるが、仕様として許容**（オーナー目視は未了） | 素材へクロスフェードする既知問題 |
...
- **排他**: `retake`（既に窓の両端を占有）・`source_audio`（映像だけを凍結するため音声と食い違う）・`reference_video_id`（制御アダプタのセグメント別条件付けと競合）とは、どのモードでも併用不可。**`source_video`との併用（＝冒頭と末尾を与えた補間）は、クリップ1本（窓内モード）に限り可能**。**クリップ2本以上との`source_video`併用も2026-09-07から受理される**（ブリッジモード。それまでは422で拒否していた。上の表と本節のブリッジモードの説明を参照）。`clips[0].conditioning_images`との併用は可能。窓内モードでは凍結フレームがクリップの内側にあるため、極端に短いクリップでは頭と尾の凍結が窓を食い尽くして422になりうる（既定値では到達しない）。
