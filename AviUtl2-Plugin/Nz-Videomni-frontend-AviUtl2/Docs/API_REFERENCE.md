...
- **クリップ長169f/145fの品質警告は、クリップ1本（窓内モード）にのみ適用される**: `standard`（潜在22）なら169フレーム、`high_resolution`（潜在19）なら145フレームまでという「stage-2のタイル1枚に収めるのが望ましい」助言（422ではない）は、窓内モードの実験結果に基づくものであり、**複数クリップの逆順Chained・ブリッジモードには適用されない**（フロントエンドの警告バナーも`clips.length === 1`のときだけ表示する）。サーバーは何も判定しないので、いずれのモードでもGenerateはブロックしない。
