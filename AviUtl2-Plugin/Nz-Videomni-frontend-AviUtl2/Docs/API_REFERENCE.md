...
- **高速化フィールドの送信規約（Acceleration。6フィールド共通）**: `attention_backend`・`block_swap_prefetch`・`keep_resident`・`fused_gguf_dequant_kernel`・`vae_mode`・`keep_resident_embeddings`は、**サーバー既定から変わったときにだけ**リクエストへ加算される。**送出点は`webui/src/shell/accelerationSettings.ts`の`accelerationRequestFields()`ただ1つ**で、Single／Chained／バッチの各経路はその結果を展開しているだけである（すべて既定なら`{}`を返すので、リクエストJSONは高速化機能が入る前とバイト等価になる）。永続化は`localStorage`の`nzvideomni.acceleration`キーで、6フィールド全部が対象である。**モック（受理はするが効かない）扱いのフィールドはもう1つも無い**——最後まで残っていた`vae_mode`が2026-08-05に実機能になったため。
