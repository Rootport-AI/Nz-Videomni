...
- **`unsupported_features`(string[]、§3-98 P5で追加)**: そのベースモデルのエンジンが**扱えない機能の名前**。押しても必ず断られる操作を先回りで灰色にするために読む(強制するのは常にサーバー側の422 `FEATURE_UNSUPPORTED`)。**LTX 2.3 は空配列**、**LTX 2.5 は現在4件**(`two_stage_hq` / `outpaint` / `nag` / `prune_vaed`)である。**`chain` / `v2v` / `a2v` は 2026-08-23 に、`loras` / `reference_video` は 2026-08-24 に、`keep_resident` と `sage_attention` は 2026-08-25 に、`retake` と `end_source` は 2026-08-26 にこの配列から外れた**——LTX 2.5 でもクリップ連結・V2V継続・A2V(SingleタブのA2V・長尺A2V・バッチA2Vを含む)・スタイルLoRA・IC-LoRA(参照動画による制御。長尺を含む)・モデル骨格の常駐(`keep_resident`。既定 off のオプトイン)・SageAttention(`attention_backend`。既定 `"sdpa"` のまま)・撮り直し(`retake`)・素材（末尾）(`end_source`)が動くようになったためで、宣言を残すと**動くタブやパネルを灰色にしてしまう**。**2026-08-26 に `retake` と `end_source` が外れたことで、連結生成のモード名はこの配列から1つも無くなった**——いまここに並ぶ4件は、すべて単発生成の拒否表(`REJECT_TABLE`)から自動的に作られるものである。**フロントエンド側では、`retake` が外れると Edit タブの「撮り直し」サブタブ(とタイムラインの右クリックからそこへ入る導線)の灰色が解け、`end_source` が外れると Chained タブの「素材（末尾）」カードの灰色が解ける。** **Edit タブはサブタブ単位で灰色にする**——タブ全体が灰色になるのは`retake`と`outpaint`の両方が並んでいるときだけで、片方だけ動く今はタブは開き、動かない側(`outpaint`)のサブタブだけが灰色になる(`editSubTabsDisabledFor`。`chainPanelsDisabledFor`と同型の純関数で、サーバーの機能名`outpaint`とサブタブ名`outpainting`の読み替えもここだけで行う)。**フィールドそのものを返さないバックエンドは「制限なし」として扱うこと**(省略＝全部だめ、ではない)。**ただし、設定画面の Attention の選択が LTX 2.5 でも押せるようになったのは、この配列から `sage_attention` が外れたからではない**——設定画面(`SettingsPanel`)は`unsupported_features`をそもそも読んでおらず、Attentionの可否は`GET /status`の`acceleration.sage_available`だけで決まる。押せるようになったのは、**2026-08-25にバックエンドがその真理値表からLTX 2.5の例外(恒久的に`false`を返す上書き)を削除した**ためである(バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §77.4)。**この配列が減ってもグレーアウトが変わらない項目がある**という区別に注意すること。正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(b)。
...
> **ベースモデルにLTX 2.5を選ぶと、本節のフィールドのうち4個が422になる**(2026-08-25現在)。断られるのは`pipeline`(`"distilled"`以外)／`outpaint`／`nag_enabled`／`vae_mode`(`"default"`以外)で、いずれも**既定値と違うときだけ**エラーコード`FEATURE_UNSUPPORTED`(422)になり、**最初の1件だけ**が名指しされる。判定はジョブ作成前・素材の404より先。**`loras`と`reference_video_id`(および2つの強度)は2026-08-24に、`keep_resident`と`attention_backend`は2026-08-25に422を抜けて動作側へ移った。** 別に**「断らないが黙って無視する」5個**(`negative_prompt`／`guidance_scale`／`num_inference_steps`／`neg_method`／`vsf_scale`)があり、こちらはジョブが通常どおり走る(蒸留版2.5にCFGもステップ数の概念も無いため。無視したことはバックエンドの`logs/server.log`に1行残る)。**フィールド単位の正本**はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(d)の4分類表(全28件＝422系4／無視5／動作16／従属3)。連結生成側は別の表で、§5.2の注記と同 §6.10(f)(全34件＝422系3／無視5／動作23／従属3。**2026-08-26に`retake`と`end_source`が422から動作へ移った**)を見ること。**単発側のこの4分類は2026-08-26の改修でも動いていない**——撮り直しと素材（末尾）は連結生成のスキーマにしか無いフィールドだからである。
