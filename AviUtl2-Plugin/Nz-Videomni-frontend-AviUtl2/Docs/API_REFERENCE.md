...
最終更新: 2026-09-25 / 出典: 調査エージェント報告(2026-07-07)。エラーコード一覧・§3.12・§3.13は、モデル読み込み中の`/generate`・`/generate/chain`がジョブ作成前に同期で`409 PIPELINE_LOADING`を返すようになったことに伴う追記(2026-08-31。バックエンド`Videomni_Backend_Specification.md` §6.9(f)、実装は`DEVLOG.md` §98)。§12はフロントエンド実装完了(1.0.0-rc1)に伴う追記(2026-07-08)。§5.2・§12.4はバックエンドのchain拡張(clips24枠・chunked_upsample・chainへのloras/reference_video_id全面解禁)とポーリングタイムアウト延長に伴う追記・修正(2026-07-15)。§12.2は「未解消の穴」がN2/N3実装で解消済みになったことの追記(2026-07-17)。§5.1・§5.2・§7はNAG（Normalized Attention Guidance）解禁(バックエンドコミット2ae497b)に伴う追記(2026-07-28)。§10はバックエンド新設のMCPサーバー(`mcp_server/`、このAPIのもう一つのクライアント)に伴う追記(2026-07-28)。§5.1・§5.2・§7はVSF（Value Sign Flip。NAGに続く2つ目の非CFGネガティブプロンプト手法）の`neg_method`/`vsf_scale`追加とフロントエンド追随完了に伴う追記(2026-07-29)。§3.1・§5.1・§5.2・§7はAcceleration（生成の高速化。`attention_backend`＝SageAttentionのジョブ単位切替と、モックの`vae_mode`）の追加とフロントエンド追随・実機ゲート合格に伴う追記(2026-08-01)。§5.1・§5.2はモデル骨格のジョブ間キャッシュ(`keep_resident`。前処理の待ち時間を2本目以降から消す代わりにメインメモリを約20GB占有する切替)の追加とフロントエンド追随・エージェント実機ゲート合格に伴う追記(2026-08-03)。§5.1・§5.2・§7.1はGGUF逆量子化の1カーネル化(`fused_gguf_dequant_kernel`。生成結果を変えずに約17.5%短縮する切替)の追加と既定on反転に伴う追記と修正(2026-08-04)。§3.2・§5.2はRetake(`retake`)とStage-2クリップ長連動の窓上限を追記(2026-08-10)。§5.2・§5.4は長尺A2V（クリップ連結でのA2V＝音声から動画を生成する機能の複数クリップ解禁。`source_audio`の「clips=1本限定」撤廃）に伴う修正(2026-08-10)。§3.6・§5.2・§7はチェーン（`/generate/chain`）の参照動画（IC-LoRA）の複数クリップ解禁（長尺IC-LoRA。`reference_video_id`の「clips=1本限定」撤廃と`depth-control`専用の新エラーコード`LORA_DEPTH_CHAIN_UNSUPPORTED`追加、`GET /loras`への`preprocess`/`reference_downscale_factor`追加）に伴う追記(2026-08-11)。§2・§3.10・§5.2はEnd source（素材（末尾）。`/generate/chain`の`end_source`と、`POST /upload/video`応答の`frame_count`/`fps`）の追加に伴う追記(2026-08-16)。§5.2は同機能の**窓内モード**化（クリップ1本＝素材をクリップ自身の末尾として凍結し出力尺は変わらない・推奨／2本以上＝旧方式・非推奨）に伴う全面書き換え(2026-08-17)。§5.2は錨の固定強度`strength`（既定1.0・Stage-1のみソフト化可能・Stage-2は常にハード凍結）追加に伴う追記(2026-08-18)。§5.2・§5.4は同機能の逆順Chained化（クリップ2本以上＝`reverse`。出力尺はクリップ合計のまま・新422・旧方式は到達不能化）に伴う全面改稿(2026-08-18)。§5.2は同機能の第3弾（錨への素材音声の凍結。素材に音声トラックがあれば窓内モード・逆順Chainedいずれの錨クリップでも常にその音声を凍結し、`strength`は映像専用のつまみになった）に伴う追記(2026-08-18)。§3.2はSingleタブの賢い快適上限マーカー（`limits.single_comfort_token_budget`＝44880の新設。5つの高速化トグル全onのときだけ有効で、1つでもoffなら`spill_free_frames`が正）の追加に伴う追記(2026-08-18)。§3.5・§5.1・§5.2はLTX 2.5の高速化第3弾(SageAttention＝`attention_backend`)の開通に伴う追記・修正(2026-08-25。`unsupported_features`は6件へ、単発の422系は4個・連結の422系は5個へ。正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §77。**オーナーの目視ゲートG8は2026-08-25に合格し、その記録は同 §77.10**)。§5.1の`attention_backend`欄は、そのオーナー目視ゲートG8の合格(2026-08-25)を反映して更新した。§3.5・§5.1・§5.2・§5.4は撮り直し(Retake)と素材（末尾）(End source)のLTX 2.5開通に伴う修正(2026-08-26。`unsupported_features`は**4件**へ、連結の422系は**3個**へ〔全34件＝422系3／無視5／動作23／従属3〕。単発側の4分類は無変更。**連結生成のスキーマが表現できるモードは、これで全部LTX 2.5でも通る**。正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §78。**オーナーの目視ゲートG8は2026-08-30に合格＝同 §78.14**)。§3.5・§5.1は画角拡張(Outpainting)のLTX 2.5開通に伴う修正(2026-08-29。`unsupported_features`は**3件**へ、単発の422系は**3個**・動作は**17個**へ〔全28件＝422系3／無視5／動作17／従属3〕。連結側の4分類は無変更。**これで単発生成の側にも連結生成の側にも、断っている「モード」は1つも残っていない**。正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §79。**オーナーの目視ゲートG8′は2026-08-29に合格＝同 §79.11**)。§3.5・§5.1・§5.2は非CFGネガティブプロンプト(NAG／VSF)のLTX 2.5開通に伴う修正(2026-08-30。`unsupported_features`は**2件**へ、単発・連結とも422系は**2個**・無視は**2個**・動作は**24個／30個**・従属は**0個**へ〔単発 全28件＝422系2／無視2／動作24／従属0、連結 全34件＝422系2／無視2／動作30／従属0〕。**7つのフィールドが3つの分類から同時に「動作」へ移り、「従属」は両スキーマとも空になった**〔**空でも表は残す**〕。正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §80。**オーナーの目視ゲートG8は2026-08-30に合格＝同 §80.10**)。§3.2は快適上限マーカーのエンジン系統〔ベースモデルの世代〕別配信テーブル`limits.comfort_budgets`の新設に伴う追記(2026-08-31。従来の`single_comfort_token_budget`／`chain_comfort_token_budget`は表を持たない古いサーバー向けの互換値へ降格。線の正本はバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) §1.1、較正の記録は[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §84)。**§3.1・§5.1・§5.2・§7は`vae_mode`（PrunaVAED＝枝刈り版の映像VAEデコーダ）の失効記述の是正(2026-09-01)**——同フィールドは2026-08-05から実機能であり、フロントエンドも送信・永続化・操作UIを持つのに、本書は3箇所で「モック（受理のみ）」と書いたままだった。**旧§7.1「モックのみ＝『出すが無効化する』機能」の節は、唯一の項目だった`vae_mode`が実機能になってメンバーが1つも無くなったため削除し**、そこにあった高速化5フィールドの送信規約は§5.1の本文へ移した（LTX 2.5がアクティブな間の行ごと非表示・保存値の`default`正規化を含む）。台帳の記録はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-118、設計正本は同[`PRUNAVAED_WORKORDER.md`](../../../Docs/PRUNAVAED_WORKORDER.md) §6。**§2・§3.6・§5.2は、kohya形式LoRAの受け入れに伴う新エラーコード`LORA_FORMAT_UNSUPPORTED`(422)の追加と、撮り直し（Retake）でのスタイルLoRA併用の解禁に伴う追記(2026-09-02。§3.6には「エンジンのローダーが読めない書式のファイルも`GET /loras`の一覧には出る（断るのは生成時）」ことを明記した。正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §88、台帳は同[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-108・§3-62-02)。** **§3.5・§5.1・§5.2は、高速化フィールドの6つ目`keep_resident_embeddings`（LTX 2.5の埋め込み処理器〔embeddings processor＝プロンプトを読み取ったあとの内部表現を整える部品〕のジョブ間常駐）の新設に伴う追記・修正(2026-09-03。バックエンド台帳§3-114)。この1件だけは向きが逆で、**LTX 2.3の`unsupported_features`が初めて空配列でなくなった**——名指ししている部品がLTX 2.5にしか無いため、非対応を宣言するのはLTX 2.3の側である。4分類は単発が全29件〔422系2／無視2／動作25／従属0〕、連結が全35件〔422系2／無視2／動作31／従属0〕へ。正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(d)・(f)、実測は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §92、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §106)。** **§3.2は`comfort_budgets`のプロファイル直下へ`outpaint_budget`（画角拡張〔Outpainting〕の快適上限マーカーのトークン予算）が加わったことに伴う追記(2026-09-05。それまでフロントエンドが自前の定数表として持っていた線をサーバー配信へ合流させたもので、**線の値は動いていない**。`null`・欠落は「線なし」＝警告を出さない、が唯一の規則である。値の正本はバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) §9、実装と設計判断の記録は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §99、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §111)。** **§5.2は素材（冒頭）（`source_video`）と素材（末尾）（`end_source`）をクリップ2本以上で併用したときの新モード`"bridge"`（ブリッジモード）の追加に伴う全面追記(2026-09-07。それまで422で拒否していた組み合わせが受理へ転じた加算方向の変更で、フィールド・型・エラーコードの増減は無い。契約の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.2、実測とゲートは同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §102、設計の理由は同[`CHAIN_STAGE2_RESEARCH_NOTES.md`](../../../Docs/CHAIN_STAGE2_RESEARCH_NOTES.md) §11の【2026-09-07】の項、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §113、台帳のクローズ記録はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-90)。** **§2・§3.1・§3.20〜§3.22・§7は物体追尾（部分フィルタの枠を追跡AIに追わせる機能）の新設に伴う追記(2026-09-11。動画生成とは別の名前空間`/utils/track/*`にエンドポイントが3本増え、エラーコードが5つ増え、`GET /status`に`tracking`ブロックが1つ加わった。**生成の契約〔§5・§6〕には一切触れていない**——追尾はジョブを作らず、キューの枠を取らず、GPUも保存領域も使わない。機能全体の正本はバックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md)で、API契約は同書§4が正本である)。** **§2・§3.5・§5.1・§5.4はInpainting（マスクによる部分再生成。マスク動画の白い領域の内側だけを描き替える機能）の新設に伴う追記・修正(2026-09-14実装・2026-09-15クローズ。`GenerateRequest`に`inpaint`〔`InpaintSpec`〕が1つ加わり、エラーコードが7つ〔`INPAINT_*`。404はマスクが見つからない1つだけで、残る6つは422〕増えた。**エンドポイントは1本も増えていない**——Inpaintingは`POST /generate`の新しいモードである。LTX 2.5は`inpaint`を`FEATURE_UNSUPPORTED`(422)で断るため、`unsupported_features`は同エンジンで3件へ、単発側の4分類は全30件〔422系3／無視2／動作25／従属0〕へ動いた。**連結生成の契約〔§5.2〕には一切触れていない**。機能全体の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md)、実測とゲートは同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §105、クローズ記録は同[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-55-02、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §116である)。** **§2・§3.23・§5.1・§5.2・§10は、生成条件のmp4埋め込み（完成したmp4のコンテナの`comment`タグへ、`metadata.json`と同じJSONを書き込む機能。A1111〔Stable Diffusion web UI〕がPNGに生成条件を書き込むのと同じ考え方）と、その読み出し口`POST /utils/mp4-info`の新設に伴う追記(2026-09-24。単発・連結の両スキーマに`embed_mp4_metadata`が加わり、読み出し口専用のエラーコードが増えた。**既存のフィールドと応答の形は変わっていない**。契約の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.1・§6.2・§6.6・§6.8、実測とゲートは同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §115、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §124。あわせて§3.17に`joined.mp4`のタグの規則、§4の注意点に`peak_vram_mb`の読み先、§9にテストの使用例を1行ずつ足した)。** **§5.2の`stage2_window`と`retake`は、連結生成のStage-2の窓（潜在クリップ）に`w25`〜`w61`の13段が加わったことに伴う追記(2026-09-25。`stage2_window`は全16名になり、Retakeの窓長の上限は8×潜在フレーム数−7〔`standard`=169・`high_resolution`=145・`w61`=481〕として窓に連動する。台帳はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-165、記録は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116、フロントエンド実装は[`DEVLOG.md`](DEVLOG.md) §125)。**
...
- **アプリ1プロセス＋エンジンのワーカー、用途別の複数のvenv**(venvの数と内訳の正本はバックエンド `README.md` §0「環境分離ポリシー」の「venv の構成」。任意導入の`./.venv-utils`〔物体追尾〕もある)。
...
- `config.server.api_key`(既定 `null`)が設定されている場合のみ、下の列挙のエンドポイントに `Authorization: Bearer <key>` ヘッダが必要。未設定なら認証不要。
- 認証が掛かるのは `require_auth` 依存を持つエンドポイントのみ = `/generate`, `/generate/chain`, `/upload/*`, `/pipeline/load`, `/pipeline/unload`, `/jobs/{id}/join`, `DELETE /jobs/{id}`, `/utils/track/*`(§3.20〜§3.22、3本とも), `/utils/mp4-info`(§3.23)。**GET系(status/config/job/loras/models/`/loras/{name}/thumbnail`)と`POST /loras/reload`は認証不要**(`api/loras.py`の2本とも`require_auth`を持たない)。
...
| `LORA_FORMAT_UNSUPPORTED` | 422 | 指定したLoRAファイルの重み鍵の書式を、エンジンのローダーが読めない（**2026-09-02新設**）。**読めるのはA/B形式（`lora_A`/`lora_B`）とkohya形式（`lora_down`/`lora_up`＋`alpha`。鍵はドット区切りのみ）の2つだけ**で、DoRA・LoHa・LoKr・アンダースコア連結鍵のkohya（`lora_unet_…`）・LoRAの重み鍵を1つも持たないファイルは、ジョブを作る前にここで断る。`detail`に書式名と鍵の実例が入る。**それ以前は黙って空振りし、LoRA無しと同じ動画が出ていた。** 正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.8、実装記録は同[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-108 |
...
| `INPAINT_PREPROCESS_CONFLICT` | 422 | Inpainting（マスクによる部分再生成、§5.1の`inpaint`）で、参照動画を前処理（輪郭抽出・姿勢推定・深度推定）へ通す制御LoRAを併用した（**2026-09-14新設**）。センチネル緑で塗ったキャンバスは生の画素のまま制御LoRAへ渡す必要があるため。画角拡張の`OUTPAINT_PREPROCESS_CONFLICT`と同じ理由・同じ判定である。**以下7つの`INPAINT_*`はすべて2026-09-14新設で、機能全体の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md)、クローズ記録は同[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-55-02である** |
| `INPAINT_SOURCE_MISMATCH` | 422 | 要求の`width`/`height`が、対象動画の実寸を128の倍数へ切り上げた値（＝キャンバス）と一致しない。**対象動画の辺が256画素未満のときも同じ符号**（どちらも「この素材ではキャンバスを作れない」という同じ結論で、直し方も同じ＝素材を差し替える）。寸法を読み取れなかったときもここ |
| `INPAINT_MASK_NOT_FOUND` | 404 | `inpaint.mask_video_id`が保管庫に無い（アップロードが失敗したか、保存期間を過ぎて消えている）。**`INPAINT_*`のうち404はこれだけである** |
| `INPAINT_MASK_RESOLUTION_MISMATCH` | 422 | マスク動画の解像度が対象動画の実寸と一致しない。**マスクは引き伸ばさない**——拡大縮小すると境界が利用者の見ていない量だけ動くため |
| `INPAINT_MASK_FRAME_MISMATCH` | 422 | マスク動画の枚数が`num_frames`と一致しない。足りないまま通すと、ffmpegの既定の同期規則で最終フレームが繰り返され、窓の後半が「何も描き替えない」まま静かに通ってしまう |
| `INPAINT_WINDOW_OUT_OF_RANGE` | 422 | 要求した窓が対象動画に収まらない（撮り直しの`RETAKE_WINDOW_OUT_OF_RANGE`の鏡で、GPUを使う前に断る） |
| `INPAINT_LORA_INVALID` | 422 | 制御LoRAがちょうど1本でない。**実際に出るのは「2本以上」のときだけ**で、0本のときは`REFERENCE_REQUIRES_CONTROL_LORA`が先に断る |
...
| `TRACK_BUSY` | 409 | 追尾のセッションが既に1本開いている（**2026-09-11新設**。同時に開けるのは1本だけ。§3.20） |
| `TRACK_UNAVAILABLE` | 503 | 追尾の仕組みが導入されていない、またはワーカーを起動できない（**2026-09-11新設**。事前判定には `GET /status` の `tracking` を使う。§3.1） |
| `TRACK_SESSION_NOT_FOUND` | 404 | 知らないセッション番号（**2026-09-11新設**。閉じたあとの2度目の`DELETE`もここに落ちる。§3.22） |
| `TRACK_FRAME_INVALID` | 400 | 送ったフレームのバイト長がセッションの`frame_bytes`と合わない（**2026-09-11新設**。上限を超える`Content-Length`もここで断る。§3.21） |
| `TRACK_FAILED` | 503 | 追尾の途中でワーカーが落ちた、またはワーカーがエラーを返した（**2026-09-11新設**） |
| `LOCAL_ONLY` | 403 | `POST /utils/mp4-info`(§3.23)へ、このPC以外(ループバック〔`127.0.0.1`／`::1`〕以外)から要求が来た（**2026-09-24新設**）。サーバーを`--listen`でLANに開いていても断る |
| `MEDIA_NOT_FOUND` | 404 | `POST /utils/mp4-info`に渡したパスが存在しない、または通常のファイルでない（**2026-09-24新設**） |
| `MEDIA_UNREADABLE` | 422 | `POST /utils/mp4-info`に渡したファイルをffprobeが読めない、またはパスがネットワーク上の共有を指す形(UNCパス)である（**2026-09-24新設**。UNCパスはファイルに触れる前に断る） |

**追尾の5コードに入らない誤りは422である。** 本文の形そのものが誤っているとき（値の型が違う、必須の項目が無い、幅・高さ・枠の大きさが0以下、`search_factor`が受け付ける範囲の外）は、上の5つではなく通常の`VALIDATION_ERROR`(422)で返る。生成側の`FEATURE_UNSUPPORTED`は使わない。
...
- **`tracking`ブロック(2026-09-11新設)**: 物体追尾(§3.20〜§3.22)が使える状態かどうかだけを答える。使えるときは`{"available": true}`の1キーで、使えないときだけ`reason`が付く。**`reason`の値は`"not installed"`(重みが置かれていない＝`install-UETrack.bat`が未実行)と`"worker failed"`(ワーカーを起動できなかった)の2つだけ**である。フロントエンドはこれを読んでToolboxタブの追尾区画を灰色にし、案内を出す。**`queue`とは独立のブロックである**——追尾はキューの枠を取らないので、生成の混雑状況とは関係しない。
...
- `acceleration`ブロック（**2026-07-31追加**、`services/pipeline_manager.py::acceleration_status_block`）。生成の高速化（Acceleration）機能の能力公開。凍結対象ではない。
  - `attention_backends`: `attention_backend`（§5.1）が受け付ける値の一覧。**実装のある項目だけ**が並ぶ。**`vae_mode`はここには載らない**——このブロックはattentionの実装選択だけを公開するもので、可否フラグを持たない`vae_mode`（§5.1）は対象外だからである（枝刈り済みの重みが無いときはサーバー側がジョブ単位で降格する）。
...
- **`comfort_budgets`**(2026-08-31追加)は「快適上限マーカーの線を、エンジン系統〔ベースモデルの世代〕ごとに配信する表」。**この鍵が来ているなら、`single_comfort_token_budget`／`chain_comfort_token_budget`ではなくこちらが正である。** **下の例に出てくる値は例示であり、正本はバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) §1.1である。**
  ...
  - **`outpaint_budget`**(2026-09-05追加)は「画角拡張(Outpainting)の快適上限マーカーに使うトークン予算」。**プロファイル直下にあり、`rows`とは別のフィールドである**——`rows`が加速構成(`requires`)で選ぶ行の列なのに対し、こちらは**構成に依存しない固定線**なので行へ相乗りさせていない(裁定J1。理由の正本はバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) §9.2)。**`null`・欠落・0以下は「線が無い」の意味で、フロントは画角拡張の快適超過の警告を一切出さない**——較正と裁定の済んでいない系統に仮の数字を当てて「分かったふり」の表示をしないためである(オーナー裁定2026-09-05)。**読み方はひとつに固定する**: 必ず`webui/src/shell/outpaintBudget.ts`の`resolveOutpaintComfortBudget`を通して読むこと(戻り値は`number | null`)。**単発・連結の予算とは別軸なので、一方を他方で代用してはならない**(同 §9.4)。**値の正本は同 §9**である。
...
- **`unsupported_features`(string[]、§3-98 P5で追加)**: そのベースモデルのエンジンが**扱えない機能の名前**。押しても必ず断られる操作を先回りで灰色にするために読む(強制するのは常にサーバー側の422 `FEATURE_UNSUPPORTED`)。**LTX 2.5 は現在2件**(`two_stage_hq` / `prune_vaed`)、**LTX 2.3 は現在1件**(`keep_resident_embeddings`)である。**【2026-09-15 訂正】本行はいったん「LTX 2.5 は現在3件」と書き換えたが、それが真だったのは同じ 2026-09-15 の日中だけである**——`inpaint`は2026-09-14のInpainting(LTX 2.3専用の第1弾)で加わり、2026-09-15にLTX 2.5側の駆動部ができて**同じ日のうちに外れた**(バックエンド台帳`Docs/PENDING_TASKS_CLOSED.md` §3-150、実測は同`Docs/VERIFICATION_LOG.md` §107)。**この配列の歴史で「入ってから出ていった」名前は`inpaint`だけである。** **画面ではEditタブの「Inpainting」サブタブの灰色が一度かかり、同じ日のうちに解けた**——`featureScope.ts`の`inpaint`の行は残してある(翻訳表であって今日の制限の一覧ではないため)。**【2026-09-03 訂正】本行はそれまで「LTX 2.3 は空配列」と書いていたが、これが真だったのは 2026-09-03 より前についてだけである**——同日に加わった`keep_resident_embeddings`(埋め込み処理器のジョブ間常駐)は**LTX 2.5にしか無い部品**を名指しするので、**非対応を宣言するのはLTX 2.3の側**になった。**この配列に名前が「増えた」のはこれが初めてである**(ここまでの動きはすべてLTX 2.5側の減少だった)。**ただしこれで画面のグレーアウトは1つも変わらない**——`keep_resident_embeddings`は下の3つの表のどこにも現れない語で、Settingsのこの行は灰色ではなく**行ごと非表示**にする作法(`prune_vaed`と同じ。下の§5.1「高速化フィールドの送信規約」)で扱う。**この配列が動いてもグレーアウトが変わらない項目がある**という区別の、3例目である。**`chain` / `v2v` / `a2v` は 2026-08-23 に、`loras` / `reference_video` は 2026-08-24 に、`keep_resident` と `sage_attention` は 2026-08-25 に、`retake` と `end_source` は 2026-08-26 に、`outpaint` は 2026-08-29 にこの配列から外れた**——LTX 2.5 でもクリップ連結・V2V継続・A2V(SingleタブのA2V・長尺A2V・バッチA2Vを含む)・スタイルLoRA・IC-LoRA(参照動画による制御。長尺を含む)・モデル骨格の常駐(`keep_resident`。既定 off のオプトイン)・SageAttention(`attention_backend`。既定 `"sdpa"` のまま)・撮り直し(`retake`)・素材（末尾）(`end_source`)・画角拡張(`outpaint`)が動くようになったためで、宣言を残すと**動くタブやパネルを灰色にしてしまう**。**2026-08-26 に `retake` と `end_source` が、2026-08-29 に `outpaint` が外れたことで、モード名はこの配列から1つも無くなった**——いまここに並ぶ2件は、すべて単発生成の拒否表(`REJECT_TABLE`)から自動的に作られるもので、**どれもモードではなくエンジンが持っていない機能である**。**`nag` は 2026-08-30 に外れた**(非CFGネガティブプロンプトの開通)——**ただしこれで画面のグレーアウトは1つも変わらない。`nag` は下の3つの表のどこにも現れない語**で、NAG のトグルは `unsupported_features` を見ずに描かれていた(押せば422が返る、という関係だった)。**この配列が減ってもグレーアウトが変わらない項目がある**という区別の、2例目である。**フロントエンド側では、`retake` が外れると Edit タブの「撮り直し」サブタブ(とタイムラインの右クリックからそこへ入る導線)の灰色が解け、`end_source` が外れると Chained タブの「素材（末尾）」カードの灰色が解け、`outpaint` が外れると Edit タブの「画角拡張」サブタブの灰色が解ける。** **Edit タブはサブタブ単位で灰色にする**——タブ全体が灰色になるのは`retake`と`outpaint`の両方が並んでいるときだけで、**いまはどちらも並んでいないので、Edit タブの2つのサブタブは両方とも生きている**(`editSubTabsDisabledFor`。`chainPanelsDisabledFor`と同型の純関数で、サーバーの機能名`outpaint`とサブタブ名`outpainting`の読み替えもここだけで行う。**どちらのサブタブも灰色にしない状態が今の答えだが、この仕組みを消してはいけない**——将来また片方だけを断るベースモデルが載ったときに、そのまま効く)。**フィールドそのものを返さないバックエンドは「制限なし」として扱うこと**(省略＝全部だめ、ではない)。**ただし、設定画面の Attention の選択が LTX 2.5 でも押せるようになったのは、この配列から `sage_attention` が外れたからではない**——設定画面(`SettingsPanel`)は`unsupported_features`をそもそも読んでおらず、Attentionの可否は`GET /status`の`acceleration.sage_available`だけで決まる。押せるようになったのは、**2026-08-25にバックエンドがその真理値表からLTX 2.5の例外(恒久的に`false`を返す上書き)を削除した**ためである(バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §77.4)。**この配列が減ってもグレーアウトが変わらない項目がある**という区別に注意すること。正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(b)。
...
- **エンジンのローダーが読めない書式のファイルも、この一覧には出る**(2026-09-02)。一覧は書式で絞り込まない。書式の判定は生成時のLoRA名前解決で行い、そこで422 `LORA_FORMAT_UNSUPPORTED`(§2のエラーコード表)になる。一覧から消えるのは、そのファイルを置き場から取り除いたときだけ。
...
- **`joined.mp4`の`comment`タグ(2026-09-24〜)**: 元ジョブの生成条件(`metadata.json`と同じJSON)が入る。書き込むかどうかは元ジョブの`embed_mp4_metadata`に従い、書き込まないときも元動画のタグは引き継がない。規則の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.6、読み出しは§3.23。
...
### 物体追尾の3本(`/utils/track/*`、`api/tracking.py`)について — 2026-09-11新設

**名前空間が生成と分かれている。** 追尾はジョブを作らず、キューの枠を取らず、GPUも使わず、サーバーに保存物を1つも残さない。したがって`/jobs`系のポーリング(§4)も、`outputs/`も関与しない。呼び出すのはAviUtl2プラグインのネイティブ側だけで、操作パネル(webui)はこの3本を直接叩かない——プラグインが`timeline.trackObject`(ブリッジ契約v12、[`BRIDGE_CONTRACT.md`](BRIDGE_CONTRACT.md) §4.23)の中で往復する。**契約の正本はバックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md) §4**で、本節はその写しではなく、フロントエンドから見た要点である。

### 3.20 `POST /utils/track/sessions`(`api/tracking.py::open_track_session`)— 認証あり

追尾のセッションを1本開く。**同時に開けるのは1本だけで、2本目は409 `TRACK_BUSY`**。

- リクエスト(JSON): `{ "width": int, "height": int, "box": {"x": float, "y": float, "w": float, "h": float}, "search_factor": float|null }`
  - `width`/`height`は**プラグインが実際に描画した1枚目の実寸**。以後のフレームはすべてこの寸法として扱う。
  - `box`は追尾の種になる矩形(左上原点・画素)。`w`/`h`は0より大きいこと。
  - **`search_factor`は省略できる**。省略すると追跡AIのランタイムが持つ既定値になる。**サーバーが受け付ける上限は8.0**で、これを超える値は422。操作パネルが出すスライダーの範囲(2.0〜6.0)はこの内側にあり、8.0はUIの範囲ではなく外枠である。
- 応答: **201** で `{ "session_id": str, "width": int, "height": int, "frame_bytes": int }`。`frame_bytes`は1枚あたりのバイト数(幅×高さ×4)で、§3.21の検査はこの1つの数字と突き合わせる。
- エラー: `409 TRACK_BUSY`(既に1本開いている) / `503 TRACK_UNAVAILABLE`(未導入・ワーカーが起動できない) / `422`(本文の形の誤り)。
- **冷えたサーバーでは時間がかかる。** 最初のセッションを開く要求がワーカーの起動とモデル読み込みを引き起こすので、クライアント側のタイムアウトは余裕を持たせること。

### 3.21 `POST /utils/track/sessions/{session_id}/frame?frame=<int>`(`api/tracking.py::push_track_frame`)— 認証あり

フレームを1枚送って、枠を1つ受け取る。**1枚目も2枚目以降も同じエンドポイント**で、未初期化のセッションへ来た最初のフレームが初期化を兼ねる(応答の形は同じ)。

- 本文: **生のRGBAバイト列**(`Content-Type: application/octet-stream`)。JSONでもmultipartでもない。
- クエリ`frame`(必須・整数): AviUtl2のフレーム番号。**サーバーは中身を解釈せず、応答にそのまま載せ返すだけ**である(プラグインが応答と描画フレームを対応づけるためのもの)。
- 応答: **200** で `{ "frame": int, "box": {"x","y","w","h"}, "score": float }`。
- **「見失った」という状態はAPIに無い。** サーバーは`score`を生のまま返し、しきい値との比較・平滑化・中間点の間引きはすべてプラグイン側が行う。だから設定項目が増えてもこの契約は動かない。
- エラー: `400 TRACK_FRAME_INVALID`(バイト長が`frame_bytes`と合わない。**1枚あたりの上限を超える`Content-Length`は本文を読み込む前にここで断る**) / `404 TRACK_SESSION_NOT_FOUND` / `503 TRACK_FAILED`(ワーカーが落ちた・エラーを返した)。

### 3.22 `DELETE /utils/track/sessions/{session_id}`(`api/tracking.py::close_track_session`)— 認証あり

セッションを閉じる。本文は無し。応答は**200**で `{ "closed": true }`。

- **正常終了でも中止でも失敗でも、プラグインは必ずこれを送る。**
- **同じセッションを2度閉じると404 `TRACK_SESSION_NOT_FOUND`**。2度目を穏やかに200で返すと、呼び出し側の取り違えを隠してしまうため。
- **ワーカーのプロセスはセッションをまたいで常駐する**(モデルの読み込みは最初の1回だけ)。閉じて消えるのは対象ごとの追跡状態だけである。
- 放置されたセッションには期限があり、期限を過ぎたものは次に開く要求が来たときにその場で閉じられる(期限はソースコード上の定数で、設定項目ではない)。

### 3.23 `POST /utils/mp4-info`(`api/media_info.py::mp4_info`)— 認証あり

動画ファイルのコンテナの`comment`タグを読み出す(2026-09-24新設)。このアプリが生成した`output.mp4`／`joined.mp4`なら、`metadata.json`と同じJSON文字列が返る(書き込みの規則は§5.1の`embed_mp4_metadata`、契約の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.2・§6.6)。読み取りはffprobeに任せ、サーバーは中身を解釈しない。ジョブは作らない。

- リクエスト(JSON): `{ "path": "<サーバーを動かしているPC上の絶対パス>" }`
- 応答: **200** で `{ "comment": str | null }`。タグが無ければ`null`。
- **このPCからの要求だけに答える。** ループバック(`127.0.0.1`／`::1`)以外からの要求は`403 LOCAL_ONLY`。本文にサーバー側のパスを書かせる入口なので、`--listen`でLANへ開いていても外からファイルを探らせない。
- **ネットワーク上の共有を指すパス(UNCパス。`\\server\share\…`・`//server/share/…`・`\\?\UNC\…`など、区切り記号が混ざった書き方も含む)は、ファイルに触れる前に`422 MEDIA_UNREADABLE`で断る。** 判定はパスを解釈した結果のドライブ部分が`\\`で始まるかどうかで行うため、副作用として長いパス用の書き方(`\\?\C:\…`)とデバイスパス(`\\.\…`)も断られる。**ネットワークドライブを割り当てたドライブ文字(`Z:\…`など)は断らない**(文字列からは区別できない。既知の制約)。
- エラー: `404 MEDIA_NOT_FOUND`(存在しない・通常のファイルでない) / `422 MEDIA_UNREADABLE`(ffprobeが読めない・UNCパス) / `403 LOCAL_ONLY` / `422 VALIDATION_ERROR`(本文の形の誤り)。
- **ローカル運用向けの機能である。** プラグインの接続先を別のPCにしている場合、手元でドロップしたパスは接続先のPCには存在しないので読めない。
- 操作パネルでは「Toolbox」タブの「mp4 info」サブタブがこの入口を使う。
...
- `peak_vram_mb` は jobs API応答に含まれない(`metadata.json` か `logs/ltx_worker.log` のみ。`output.mp4`の`comment`タグにも、既定で`metadata.json`と同じJSONが入る＝条件は§5.1の`embed_mp4_metadata`、読み出しは§3.23)。
...
> **ベースモデルにLTX 2.5を選ぶと、本節のフィールドのうち2個が422になる**(2026-09-15現在)。断られるのは`pipeline`(`"distilled"`以外)／`vae_mode`(`"default"`以外)で、いずれも**既定値と違うときだけ**エラーコード`FEATURE_UNSUPPORTED`(422)になり、**最初の1件だけ**が名指しされる。判定はジョブ作成前・素材の404より先。**`loras`と`reference_video_id`(および2つの強度)は2026-08-24に、`keep_resident`と`attention_backend`は2026-08-25に、`outpaint`は2026-08-29に、**`nag_enabled`は2026-08-30に**422を抜けて動作側へ移った——`outpaint`は単発生成の側で「モード」を名指ししていた最後の1つである。** 別に**「断らないが黙って無視する」2個**(`guidance_scale`／`num_inference_steps`)があり、こちらはジョブが通常どおり走る(蒸留版2.5は工程数が固定で、CFGの概念も無いため。無視したことはバックエンドの`logs/server.log`に1行残る)——**ただしスキーマがこの2つを`pipeline="distilled"`のとき既定値に固定するので、LTX 2.5が受理しうるリクエストではこの行は出ない。** **フィールド単位の正本**はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(d)の4分類表(全30件＝422系2／無視2／動作26／従属0)。連結生成側は別の表で、§5.2の注記と同 §6.10(f)(全35件＝422系2／無視2／動作31／従属0)を見ること。**単発側のこの4分類は2026-08-26の改修では動かず(撮り直しと素材（末尾）は連結生成のスキーマにしか無いフィールドだからである)、2026-08-29の画角拡張の開通で動いた**——`outpaint`は逆に単発生成のスキーマにしか無い。**そして2026-08-30の非CFGネガティブプロンプトの開通では、単発側と連結側が同時に動いた**——7つのフィールド(`nag_enabled`／`negative_prompt`／`nag_scale`／`nag_tau`／`nag_alpha`／`neg_method`／`vsf_scale`)は両方のスキーマにあるためである。**この結果、422系の顔ぶれは単発側と連結側でまったく同じ2つになり、「従属」は両方とも空になった**(**空でも表そのものは残っている**——将来また422の背後に副パラメータができたときの置き場になるため)。**その「まったく同じ2つ」は2026-09-14のInpaintingで一度崩れ、単発側だけが3つになった**——`inpaint`は単発生成のスキーマにしか無いフィールドで、しかも**発見された制限ではなくオーナーの裁定による当面の線引き**だった。**【2026-09-15】その線引きは同じ日のうちに解消し、単発側と連結側はふたたびまったく同じ2つに戻っている**——LTX 2.5側の駆動部`engine25/inpaint25.py`が入り、`inpaint`は422から動作側へ移った(バックエンド台帳`Docs/PENDING_TASKS_CLOSED.md` §3-150、実測は同`Docs/VERIFICATION_LOG.md` §107)。
>
> **逆向きもある——LTX 2.3を選んでいるときに422になるフィールドが、2026-09-03から1つある**(バックエンド台帳§3-114)。`keep_resident_embeddings`(埋め込み処理器のジョブ間常駐)は**LTX 2.5にしか無い部品**を名指しするので、**断るのはLTX 2.3の側**である——既定(`false`)以外で送ると同じ`FEATURE_UNSUPPORTED`(422)になり、文面は切り替え先として「LTX 2.5」を名指しする。**これがLTX 2.3の初めての422フィールドである。** 判定の規律は2.5側と逐語で同じ(「フィールドが有るか」ではなく**既定値と違うか**・**最初の1件だけ**・ジョブ作成前)。**フロントエンドはこの422へ到達しない**——`unsupported_features`に名前が載っている間はSettingsの行ごと隠し、保存値を`false`へ書き戻すためである(下の「高速化フィールドの送信規約」)。
>
...
| `inpaint` | `InpaintSpec` \| null | **2026-09-14追加**。Inpainting（マスクによる部分再生成。マスク動画の白い領域の内側だけを描き替える）。`reference_video_id`が必須で、`outpaint`・`conditioning_images`・`crop_output`とは**排他**。`width`/`height`は**キャンバス**（対象動画の実寸を128の倍数へ切り上げた値）でなければならない。詳細は下の**InpaintSpec**。**LTX 2.3でもLTX 2.5でも使える**（2026-09-15。それ以前はLTX 2.5では422 `FEATURE_UNSUPPORTED`だった） |
...
| `vae_mode` | `"default"` \| `"prune_vaed"` `"default"` | **2026-07-31追加・2026-08-05から実機能**。映像を復元するVAEデコーダの選択で、`"prune_vaed"`は枝刈り版のPrunaVAED（軽いデコーダに差し替えて復号を速くする代わりに、出力品質がわずかに落ちる可能性がある）。**他の高速化項目と違い、これは絵が変わりうる選択である**。可否フラグは無く、枝刈り済みの重みが無ければサーバーがそのジョブだけ黙って通常のデコーダへ降格する（実際にどう扱われたかは`metadata.json`の`vae_mode_used`＝`"off"`／`"on"`／`"on->off"`）。既定と同じ`"default"`のときはリクエストにキーを載せない（フロント側`accelerationRequestFields()`は`"prune_vaed"`のときだけ足す）。永続化もされる（下の「高速化フィールドの送信規約」を参照）。**LTX 2.5では`unsupported_features`に`prune_vaed`が入っている間だけ422**（上の注記のとおり）で、フロントエンドはその間Settingsの行ごと隠すため、実操作でこの422へは到達しない。既存の`vram_optimization.vae_tiling`とは**無関係**。**LTX 2.5では本フィールドの指定自体が422になるが、`vae_mode_used`は引き続き書かれ、値は載っているデコーダの実名（例: `"conv"`）に変わる**——語彙の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.6。設計正本はバックエンド[`PRUNAVAED_WORKORDER.md`](../../../Docs/PRUNAVAED_WORKORDER.md) §6、実測は[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §52 |
...
| `embed_mp4_metadata` | bool `true` | **2026-09-24追加**。完成した`output.mp4`(と、そのジョブを結合した`joined.mp4`)のコンテナの`comment`タグへ、`metadata.json`と同じJSON文字列を書き込むか。**書き込むのは`metadata.json`を書き出す設定のときだけ**で、条件と規則の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.6である。**プロンプトもそのまま入る**。映像と音声は再エンコードせず、入れ物を組み直すだけで書き込む。書き込みに失敗してもジョブは完了扱いで、元のmp4が残る。`false`にすると何も書かず、結合でも元動画のタグを引き継がない。mp4を再エンコードするとタグは消える。読み出しは§3.23。**高速化フィールドと同じく、既定(`true`)と違うときだけ送る**(操作パネルは`false`のときだけ`embed_mp4_metadata: false`を載せる。保存と送出は`webui/src/shell/accelerationSettings.ts`)。LTX 2.3・LTX 2.5のどちらでも効く |
| `keep_resident_embeddings` | bool `false` | **2026-09-03追加・LTX 2.5専用**(バックエンド台帳§3-114)。LTX 2.5の**埋め込み処理器**(embeddings processor＝プロンプトを読み取ったあとの内部表現を整える部品)のCPU側の重み1本をジョブ間で保持し、毎ジョブのGGUF読み直しを省く。**生成結果は変わらない**——同じ部品を作り直さずに使い回すだけで、構築が速くなるだけである。代償はメインメモリで、常駐は**実測4.66GiB**(実機ゲートG-R2の実測。**正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §92**)。**`keep_resident`とは別のスイッチ**で、常駐する相手も別物(あちらはテキストエンコーダの状態辞書・約7.68GiB)なので、**両方onにするとメインメモリの増分は加算になる**。既定`false`のときはリクエストにキーを載せない(フロント側`accelerationRequestFields()`はONのときだけ`keep_resident_embeddings: true`を足す)。実際にどう扱われたかは`metadata.json`の`keep_resident_embeddings_used`(**`"on"`／`"off"`の2値だけ**。降格経路が無いので`"on->off"`は出ない)。**この項目だけは向きが逆で、LTX 2.3で既定以外にすると422になる**(上の注記)——フロントエンドはLTX 2.3がアクティブな間、Settingsの行ごと隠して保存値を`false`へ戻すため、実操作でこの422へは到達しない |
...
- **高速化フィールドの送信規約（Acceleration。6フィールド共通）**: `attention_backend`・`block_swap_prefetch`・`keep_resident`・`fused_gguf_dequant_kernel`・`vae_mode`・`keep_resident_embeddings`は、**サーバー既定から変わったときにだけ**リクエストへ加算される。**送出点は`webui/src/shell/accelerationSettings.ts`の`accelerationRequestFields()`ただ1つ**で、Single／Chained／バッチ／Edit（画角拡張・撮り直しの両サブタブ）の各経路はその結果を展開しているだけである（すべて既定なら`{}`を返すので、リクエストJSONは高速化機能が入る前とバイト等価になる）。永続化は`localStorage`の`nzvideomni.acceleration`キーで、6フィールド全部が対象である。**モック（受理はするが効かない）扱いのフィールドはもう1つも無い**——最後まで残っていた`vae_mode`が2026-08-05に実機能になったため。
  - **6つ目の`keep_resident_embeddings`は2026-09-03追加**（バックエンド台帳§3-114）。**規約そのものは`keep_resident`と同じ**（サーバー既定が`false`なので、ONのときだけ`keep_resident_embeddings: true`が載る）で、**この「既定なら鍵ごと出さない」性質が、LTX 2.3で422にならない理由でもある**。
  - **アクティブなベースモデルが断る項目は、隠す**（2026-09-01時点の作法）。`GET /models`の`unsupported_features`（§3.5）に`prune_vaed`が含まれている間は、Settingsから**PrunaVAEDの行ごと消す**（グレーアウトではない。422で断られる項目であって「受理はされるが効かない」たぐいではないため）。あわせて、非対応のベースモデルがアクティブだと観測した時点でブラウザの保存値を`default`へ書き戻す。**画面のコードにベースモデル名は書かず、判定は必ず`unsupported_features`から引く。** 送信側の`accelerationRequestFields()`はこの件で1行も変えていない（同じ結果を2か所で担保しないという判断）。経緯はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-139。
  - **同じ作法の2例目が`keep_resident_embeddings`で、こちらは向きが逆である**（2026-09-03）。`unsupported_features`に`keep_resident_embeddings`が含まれている間——**すなわちLTX 2.3がアクティブな間**——は、Settingsから**この行と注記をまとめて消し**、保存値を`false`へ書き戻す。**判定はここでもベースモデル名ではなく`unsupported_features`から引く**ので、画面のコードは「どちらのモデルが持っていないか」を知らないままである。送信側の`accelerationRequestFields()`はこの件でも1行も変えていない。実装の記録は[`DEVLOG.md`](DEVLOG.md) §106。
...
**InpaintSpec**(`api/models.py::InpaintSpec`、2026-09-14追加): `{mask_video_id, window_start_sec(既定0.0,≥0.0), blend_dilation_stage1(既定5,0-15), blend_dilation_stage2(既定2,0-15)}`。
...
- **余白（パッド）はフィールドに無い。** サーバーが対象動画をffprobeして`キャンバス − 実寸`から導く——素材ファイルそのものを実寸の唯一の正本にするためで、パッドを送れるようにすると照合先の無い2つ目の正本ができてしまう。同じ理由で、出来上がる動画の解像度も要求のフィールドではなく**対象動画の実寸**である（右と下の余白は最後に無劣化で切り落とされる）。
- **制御LoRAはちょうど1本**必要で、画角拡張と同じ`in-outpainting`を使う。0本なら`REFERENCE_REQUIRES_CONTROL_LORA`、2本以上なら`INPAINT_LORA_INVALID`。前処理を要求する制御LoRA（canny／pose／depth）との併用は`INPAINT_PREPROCESS_CONFLICT`。
- **AviUtl2フロントエンド専用の機能である**（Gradio検証UIにはInpaintingの画面が無い）。マスクはタイムラインの**部分フィルタ**の枠をAviUtl2本体に描かせて作る——その経路は[`BRIDGE_CONTRACT.md`](BRIDGE_CONTRACT.md) §4.24が正本。
- 送信例(AviUtl2フロントエンドが実際に組み立てる形。`webui/src/modes/edit/useInpaintForm.ts`):

```json
{ "prompt": "green lawn", "width": 1280, "height": 768, "num_frames": 121,
  "frame_rate": 30.0, "seed": -1,
  "reference_video_id": "vid_...",
  "loras": [ {"name": "in-outpainting", "strength": 1.0} ],
  "inpaint": { "mask_video_id": "vid_...", "window_start_sec": 2.0,
               "blend_dilation_stage1": 5, "blend_dilation_stage2": 2 } }
```

  - **`blend_dilation_*`は既定値のままでも常に送る**（画面の数字と送った値が食い違う経路を作らないため。オーナー裁定）。加速系フィールドの「既定なら鍵ごと出さない」規約とは逆向きなので注意。
...
| `vae_mode` | `"default"` \| `"prune_vaed"` `"default"` | §5.1と同義(2026-07-31追加・2026-08-05から実機能)。連結生成でも全クリップ・全ステージの復号に一律で効く |
...
| `embed_mp4_metadata` | bool `true` | §5.1と同義(2026-09-24追加)。連結生成では完成した連結動画(`output.mp4`)に書き込む。`to_clip_request`経由で各クリップの記録へも転記される |
| `keep_resident_embeddings` | bool `false` | §5.1と同義(2026-09-03追加・**LTX 2.5専用**)。`to_clip_request`経由で各クリップへ転記される。連結生成でも1つの設定がチェーン全体に効き、**埋め込み処理器の構築はジョブあたり1回**なので、効くのは`keep_resident`と同じく**次のジョブ**からである。**LTX 2.3で既定以外にすると422**になる点も単発生成と同じ。実測はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §92 |
| `stage2_window` | `"standard"` \| `"high_resolution"` \| `"full_length"` \| `"w25"`〜`"w61"`(3刻みの13名) `"standard"` | Stage-2のクリップ長(潜在フレーム数)プリセット(2026-08-10追加、2026-08-12に`full_length`を、2026-09-25に`w25`〜`w61`を追加。全16名)。`standard`=潜在22(既定)、`high_resolution`=潜在19。**`w25`〜`w61`は名前の数字が潜在フレーム数で、送り＝窓−4・重なり4の多タイル用**(数値の正本はバックエンド`chain_math.py`の`STAGE2_WINDOW_PRESETS`、記録は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116)。48fpsでは送りが奇数の窓(`w25`・`w31`…`w61`)は多タイル構成の大半が音声整合の422になる(同§116.4)。**`full_length`=潜在61(=481フレーム相当)・タイル数は必ず1**。Retake(`retake`、下記)の窓上限に連動し、上限は8×潜在フレーム数−7(`chain_math.retake_max_window_px(v_tile)`。`standard`=169、`high_resolution`=145、`w61`=481)。`full_length`指定時は**クリップ数1・`source_audio`必須**の2条件をAPI層が422で強制する(`source_video`・`retake`との排他は既存のペア排他制約が先に効くため、`full_length`固有のチェックとしては到達しない)。快適上限は`limits.spill_free_frames`(単発生成用の実測テーブル)側で判定され、Singleの既存の警告帯(琥珀色バナー)がそのまま実態に一致する。Single・Batchのa2vリクエストビルダーが常にこの値を固定送出し、Chained・Retakeのフォームには選択肢として出さない(フォームに出るのは`full_length`を除く15名) |
| `retake` | `RetakeSpec` \| null | Retake(選択範囲の撮り直し、2026-08-10追加)。`{video_id, window_start_sec, head_px=25, tail_px=24, regenerate_audio=true}`。窓長の正は`clips[0].num_frames`(8n+1、73〜上限。上限は`stage2_window`に連動し、8×潜在フレーム数−7)。`source_video`・`source_audio`・`reference_video_id`・`clips[0].conditioning_images`と排他。`clips`はちょうど1本必須。**LTX 2.5でも2026-08-26から使える**——仕様（窓長の範囲・のりしろ・排他・`metadata.json`の`retake`ブロック）はエンジン系統に依存せず、LTX 2.3と同一である。**窓は121フレーム以上を推奨**（73フレームでは前後の帯25＋24を除くと自由に作り直せるのが24フレームしか残らず、出来上がりが元とほとんど変わらない。**LTX 2.3から引き継いだ性質**で2.5固有ではない）。**LTX 2.5でのオーナーの目視ゲートG8は2026-08-30に合格した**——AviUtl2の操作パネルからの実機生成で、プロンプトどおりに再生成されること・継ぎ目が分からないこと・「Picture and sound」で音声も生成されること・「Picture only (keep the original sound)」で音声はそのままであることの4点が確認されている（バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) **§78.14**、実測は同 §78）。**スタイルLoRA（`loras`）は併用できる**（2026-09-02。撮り直しは1クリップのチェーンジョブなので、`loras`は元から素通しで受理される。フロントエンドが送るようになったのがこの日である）。**ただし制御系のIC-LoRAは使えない**——上記のとおり`retake`と`reference_video_id`は排他で、参照動画を送れないため`LORA_REQUIRES_REFERENCE`(422)になる。解禁は別テーマ（バックエンド[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §3-141）。記録は同[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-62-02・[`DEVLOG.md`](DEVLOG.md) §103 |
...
**EndSourceSpec**(`api/models.py::EndSourceSpec`): **`clips`の本数と`source_video`（素材（冒頭））の有無で挙動が4つに分かれる**（2026-08-17に窓内モード追加、2026-08-18に逆順Chained追加、2026-09-07にブリッジモード追加）。判定は`chain_math.compute_chain_layout`が1箇所で行い、`metadata.json`の`end_source.mode`として公開する（リクエスト側にモードを指定するフィールドは無い）。フロントエンドが使うのは**窓内モード（クリップ1本）**・**逆順Chained（クリップ2本以上で素材（冒頭）なし）**・**ブリッジモード（クリップ2本以上で素材（冒頭）あり）**の3つで、**旧方式（`internal_segment`）はAPIから到達不能な死蔵コード**である（後方互換とテスト・切り戻し専用の内部引数`end_source_mode_override`でのみ選べる。通常のリクエストでは絶対に選ばれない）。**素材（末尾）だけを使うときの推奨は窓内モード（クリップ1本）のみ**（2026-08-18のオーナー裁定）。逆順Chained（クリップ2本以上で素材（冒頭）なし）は受理されるが推奨外で、クリップの境目・末尾（錨直前）に品質劣化が出ることがあり、これは仕様として許容している（詳細は本節末尾）。**素材（冒頭）を併用するブリッジモードはこの裁定の対象外**で、用途の違う別のモードである。

| | **窓内モード（クリップ1本）＝推奨** | **逆順Chained（クリップ2本以上・素材（冒頭）なし）＝受理される（推奨外、2026-08-18〜）** | **ブリッジモード（クリップ2本以上・素材（冒頭）あり、2026-09-07〜）** | **旧方式（`internal_segment`）＝死蔵・到達不能** |
|---|---|---|---|---|
| 凍結フレームの置き場所 | **クリップ自身の末尾**（生成される窓の内側） | **最終クリップの末尾**（生成される窓の内側） | **最終クリップの末尾**（生成される窓の内側） | クリップ列の後ろに継ぎ足す内部区画 |
| 出力長 | **変わらない**（＝クリップの`num_frames`そのもの） | **変わらない**（＝クリップ合計） | **変わらない**（＝クリップ合計から素材（冒頭）の凍結分を引いた長さ。`source_video`併用時の通常の規則） | クリップ合計＋`context_frames` |
| 生成順 | クリップ1本を1つの窓として生成 | **タイムラインの逆順**（最終クリップから先に生成し、過去側へリレーする） | **タイムライン順**（普通の正順チェーンと同じ。最終クリップだけが両側で条件付けされる） | タイムライン順（内部区画は最後） |
| 生成結果 | 素材へ自然に到達する（意味論的に近い素材の場合） | 機構としては成立（実機ゲートM1〜M7全PASS）。**オーナー目視・試聴済み**——クリップの境目・末尾にモーフが出ることがあり、品質劣化として仕様上許容（推奨外） | 継ぎ目はすべて正順チェーンと同じ。**2つの素材の内容が離れていると最終クリップの中でクロスフェードやモーフになるが、仕様として許容**（2026-09-07のオーナー裁定で一括受容。実験的な機能という位置づけ） | 素材へクロスフェードする既知問題 |
| UIからの到達 | Chainedタブの「素材（末尾）」カード | 同カード＋クリップを2本以上に増やす | 同カード＋「素材（冒頭）」に動画を入れ、クリップを2本以上に増やす | 到達不可 |
...
**ブリッジモード**（`"bridge"`、2026-09-07追加）は、**素材（冒頭）と素材（末尾）を両方指定したクリップ2本以上**のときのモードである。クリップは**普通の正順チェーンとまったく同じ順序・同じ表**で生成され（`seg_generation_order`は先頭から順、`seg_head_source`は1つ前のクリップ、`seg_tail_source`は全クリップとも`null`）、**最終クリップだけが両側で条件付けされる**（頭＝1つ前のクリップからののりしろ、尾＝素材（末尾）の末尾の凍結フレーム）。つまり**遡って生成する継ぎ目を1つも作らない**ので、継ぎ目の性質は通常のChainedと同じである。想定する用途は**内容の似ている2本の動画の間の欠落区間を補完すること**で、2本の内容が離れていると最終クリップの中でクロスフェードや不自然なモーフになるが、**これは仕様として許容している**（2026-09-07のオーナー裁定）。**したがってフロントエンドは、このモードでは素材（末尾）由来の品質警告も遡り生成のヒントも出さない。**`metadata.json`の`end_source.mode`は`"bridge"`で、`generation_order`・`stage1_order`はどちらも正順の並びになる。実装・機械検証・実機ゲートの正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §102、設計の理由は同[`CHAIN_STAGE2_RESEARCH_NOTES.md`](../../../Docs/CHAIN_STAGE2_RESEARCH_NOTES.md) §11の【2026-09-07】の項である。

- **意味拡張1: `overlap_frames`（既存フィールド）**: 逆順Chainedの継ぎ目（セグメント間で共有される重なり潜在）の幅そのものとして使われる。窓内モードでは従来どおり`overlap_frames >= 2`が必須だが、**逆順Chainedとブリッジモードはこの下限を免除され`overlap_frames = 1`が合法になる**（内部区画を追加しないため音声のりしろ予算の消費が素のチェーンと同一で、下限の根拠が消滅するため）。フロントエンドは「素材（末尾）×2本以上×素材（冒頭）なし」への遷移でだけ既定値を1へ自動的に切り替える（逆方向の遷移では従来既定へ復帰する。**素材（冒頭）を付けたブリッジモードでは通常の既定値3のままである**）。
...
- **錨への素材音声の凍結（第3弾、2026-08-18追加）**: 素材（`video_id`側）に音声トラックがあれば、**窓内モード・逆順Chained・ブリッジモードのいずれの錨クリップでも常にその音声が凍結される（3モードとも扱いは同じ）**。新設のフィールドは無く、`EndSourceSpec`はv1から音声用のトグル・スライダーを持たない——「音声は素材からは取り込まれず独立して生成される」というv1〜第2弾の挙動そのものが変わった**既存フィールドの意味変更**である。**`strength`は映像専用のつまみになった**（音声のマスク値は`strength`の値に関わらず常に0.0）。凍結する音声潜在の本数（`n_end_a`）は、Retakeの尾側のりしろ凍結と同じスキャン規則（潜在の時間支持区間が帯の開始時刻以降にあるものを末尾から連続して数える）で決まり、単純な丸め計算ではない。素材ファイルの末尾基準でスライスするため、素材の潜在格子とタイムラインの格子の位相が一般には一致せず、**最大1潜在（40ミリ秒）の位置ずれ**が許容として残る。**フォールバック（自由生成）は「音声トラック無し／デコード不能」のときのみ**——`audio_frozen=false`・`audio_fallback_reason="no_audio"`になる。音声潜在が必要数に足りない構成（端数不足）はエラーにせず**取れた分だけ凍結する**（`audio_frozen=true`のまま`n_end_a_frozen`が`n_end_a`を下回る形で表れる）。**デジタル無音の検出はしない**——無音の素材は無音のまま凍結され、帯の入口で音が唐突に消える聴感もあり得るがそれは正常動作である。画像end source（静止画）はそもそも音声を持たないため本改修の対象外で、従来どおり音声は自由生成にフォールバックする。**配信される錨区間の音声は原波形のmuxではない**——A2Vや非再生成のRetakeが元の波形をそのまま貼り付けるのとは違い、本機能は**潜在を凍結**するため、配信音声は音声VAE＋ボコーダを1往復した音になる（cross-attentionを効かせるには潜在である必要があり、muxでは目的を達成できないため。同じ曲だが少しこもった音になるのが仕様）。切り戻しはバックエンドのモジュール定数`END_SOURCE_FREEZE_AUDIO`（既定`true`）で、`false`にすると本機能追加より前とバイト完全同一の出力に戻る。実装・機械検証・機械ゲートA1〜A9の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §65。
- **オーナー裁定（2026-08-18、テーマ完結）**: End sourceは**クリップ1本での使用を推奨**し、**複数クリップ（逆順Chained）は推奨外**の使い方と位置づける。実機ゲート後のオーナー目視・試聴で、複数クリップ時にクリップの境目・末尾（錨直前）へ系統的なモーフが出ることを確認しており、**この品質劣化は仕様として許容する**（根治にはモデル側の到着時刻拘束能力が要り、現行のLTX 2.3には無い）。UI上の警告文は**実装済み**である（`multiClipQualityWarning`。**素材（冒頭）を併用するブリッジモードでは出ない**）。品質を重視して複数クリップを終端付きで繋ぎたい場合は、End sourceをクリップ1本ずつ使い、生成物を次の素材にして過去へ遡って生成する**手動リレー**（AviUtl2タイムラインで組み合わせる。すべての継ぎ目が窓内モードの錨になる。音声錨の幅は映像の錨幅に比例し、錨8f〔UI既定〕なら音声7潜在≒約0.3秒・錨72fなら74潜在≒約3秒——音楽をしっかり引き継ぐには`context_frames`を大きくする必要があるが、UIは8固定のためAPI直叩きになる）が実用的な回避策になる。詳細・実験結果はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §64.7・§65.8、考察の正本は[`CHAIN_STAGE2_RESEARCH_NOTES.md`](../../../Docs/CHAIN_STAGE2_RESEARCH_NOTES.md) §11。
- **クリップ長169f/145fの品質警告は、クリップ1本（窓内モード）にのみ適用される**: `standard`（潜在22）なら169フレーム、`high_resolution`（潜在19）なら145フレームまで（一般には8×潜在フレーム数−7。`w61`なら481フレーム＝クリップの最大なので出ない）という「stage-2のタイル1枚に収めるのが望ましい」助言（422ではない）は、窓内モードの実験結果に基づくものであり、**複数クリップの逆順Chained・ブリッジモードには適用されない**（フロントエンドの警告バナーも`clips.length === 1`のときだけ表示する）。サーバーは何も判定しないので、いずれのモードでもGenerateはブロックしない。
- **`overlap_frames >= 2` はモード依存**: 窓内モードでは従来どおり必須（サーバーはのりしろ1を422にする）。**逆順Chained・ブリッジモードでは免除**（上記「意味拡張1」参照）。
- **排他**: `retake`（既に窓の両端を占有）・`source_audio`（映像だけを凍結するため音声と食い違う）・`reference_video_id`（制御アダプタのセグメント別条件付けと競合）とは、どのモードでも併用不可。**`source_video`との併用（＝冒頭と末尾を与えた補間）は、クリップ本数を問わず可能**である。クリップ1本なら窓内モード、**2本以上なら2026-09-07から受理されるブリッジモード**になる（それまでは2本以上を422で拒否していた。上の表と本節のブリッジモードの説明を参照）。`clips[0].conditioning_images`との併用は可能。窓内モードでは凍結フレームがクリップの内側にあるため、極端に短いクリップでは頭と尾の凍結が窓を食い尽くして422になりうる（既定値では到達しない）。
- **新たに422になる構成（2026-08-18、受理範囲の後方非互換変更）**: 新しいエラーコードは増やさず、422 `VALIDATION_ERROR`として返る。最終クリップの潜在数が`kv + n_end_v`（のりしろ幅＋錨の潜在数）以下になる構成——逆向きに運ぶべき新規生成内容がゼロになるため、通るクリップ長を名指しした422で拒否する（既定値では到達しない。**ブリッジモードにも同じ検査が掛かる**）。**旧方式では通っていた構成が逆順Chainedでは422になる**点に注意（例: 最終クリップが極端に短い構成）。**かつてここに①として並べていた「`source_video`×`end_source`×2クリップ以上」は、2026-09-07にブリッジモードとして受理へ転じたので、いまは無い。**
...
**総尺上限**: `MAX_CHAIN_TOTAL_PIXEL_FRAMES = 24×481 = 11544`(`api/models.py`の同名モジュール定数、2026-07-14に`8×481=3848`から拡張)。**`end_source`の帯（旧方式のみ発生）はこの上限に数えない**（2026-08-16）。窓内モード・逆順Chained・ブリッジモードのいずれも凍結分がクリップの内側にあるため、そもそも加算する帯が存在しない。
...
| Inpainting（マスクによる部分再生成） | `/generate` | inpaint指定（＋`reference_video_id`＋制御LoRA`in-outpainting`ちょうど1本。`width`/`height`は対象動画の実寸を128の倍数へ切り上げたキャンバス、窓の長さは`num_frames`。§5.1参照）。マスク動画の白い領域の内側だけを描き替え、外側は元の画素のまま残す。**LTX 2.3のみ**（2026-09-14追加） |
...
> **「表示だけで実際は効かない」罠を避けるための約束事**（過去に`fp8_transformer`が「statusには出るがworkerの挙動を変えない」状態で長く残った反省から、生成の高速化（Acceleration）の項目には次の2点を構造で担保してある）:
>
> 1. **実際に使われた方式は必ず記録される**。生成後に`outputs/{job_id}/metadata.json`の実効値欄（`attention_used`＝`"sdpa"`／`"sage"`／`"sage->sdpa"`、`keep_resident_used`、`fused_gguf_dequant_kernel_used`、`vae_mode_used`）で、要求どおりに適用されたか・降格したかを確認できる。フロント側の実機ゲートもログではなくこの値で判定する。**`vae_mode_used`だけはLTX 2.3とLTX 2.5で値の語彙が違う**（2.3はPrunaVAEDの可否、2.5は載っているデコーダの実名）——正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.6。
> 2. **`metadata.json`と`GET /jobs`の`request`エコーには、効かないフィールドも現れる**（`model_dump()`をそのまま出す既存の作法。`pipeline: "two_stage_hq"`がその例）。「ジョブ記録に残る」ことと「効く」ことは別である点に注意。
...
- 同時1ジョブ。実行中の新規投入は409。「混雑中」表示が必要（設計上は「予約」して現ジョブ完了後に自動送信する扱いを推奨。詳細は [DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md) M3参照）。
  - **追補（2026-09-11・追尾はこの制約の外）**: 物体追尾（§3.20〜§3.22）は生成ジョブとは独立で、CPUだけを使う。実行中のジョブを止めず、`JOB_BUSY`の対象にもならず、逆に生成が走っていても追尾は始められる（追尾どうしの「同時1本」は`TRACK_BUSY`という別の錠前で守る）。
...
- `tests/test_mp4_metadata.py` — `POST /utils/mp4-info`(§3.23)の使用例と、生成した mp4 の`comment`タグの往復。
...
- **`mcp_server/`**(2026-07-28新設) — このAPIのもう一つのクライアント。Claude CodeなどMCP(Model Context Protocol)クライアントから、Web操作パネル相当のツール群(本数と一覧の正本は`Docs/MCP_SERVER_DESIGN.md` §8)でバックエンドを直接操作できる(AviUtl2タイムライン連携は対象外)。フロントエンド(`.aux2`)実装への影響は無い別系統のクライアントだが、参考として挙げておく。詳細はバックエンド`README.md` §8、`Docs/MCP_SERVER_DESIGN.md`参照。
