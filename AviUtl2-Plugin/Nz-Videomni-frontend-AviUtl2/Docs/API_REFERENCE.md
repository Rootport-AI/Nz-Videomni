...
- **`unsupported_features`(string[]、§3-98 P5で追加)**: そのベースモデルのエンジンが**扱えない機能の名前**。押しても必ず断られる操作を先回りで灰色にするために読む(強制するのは常にサーバー側の422 `FEATURE_UNSUPPORTED`)。**LTX 2.3 は空配列**、**LTX 2.5 は現在8件**(`retake` / `end_source` / `two_stage_hq` / `outpaint` / `nag` / `prune_vaed` / `sage_attention` / `keep_resident`)である。**`chain` / `v2v` / `a2v` は 2026-08-23 に、`loras` / `reference_video` は 2026-08-24 にこの配列から外れた**——LTX 2.5 でもクリップ連結・V2V継続・A2V(SingleタブのA2V・長尺A2V・バッチA2Vを含む)・スタイルLoRA・IC-LoRA(参照動画による制御。長尺を含む)が動くようになったためで、宣言を残すと**動くタブやパネルを灰色にしてしまう**。**フィールドそのものを返さないバックエンドは「制限なし」として扱うこと**(省略＝全部だめ、ではない)。正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(b)。
...
> **ベースモデルにLTX 2.5を選ぶと、本節のフィールドのうち7個が422になる**(2026-08-24現在。連結生成そのもの・V2V継続・A2V・LoRA・参照動画は動く)。断られるのは`retake`／`end_source`／`nag_enabled`／`pipeline`(`"distilled"`以外)／`vae_mode`(`"default"`以外)／`attention_backend`(`"sdpa"`以外)／`keep_resident`で、いずれも**既定値と違うときだけ**エラーコード`FEATURE_UNSUPPORTED`(422)になり、**最初の1件だけ**が名指しされる。判定はジョブ作成前・素材の404より先。**`source_video`と`source_audio`は2026-08-23に、`loras`と`reference_video_id`(および2つの強度)は2026-08-24に422を抜けて動作側へ移った**ので、V2V継続・A2V・長尺A2V(複数クリップ)・Singleタブの`stage2_window="full_length"`・スタイルLoRA・IC-LoRA(クリップ別の参照窓＝長尺IC-LoRAを含む)はいずれもLTX 2.5で通る。**参照動画を添えるときは`width`/`height`が128の倍数であること**(系統に依存しない既存の検査)、**`depth-control`を2クリップ以上で使うと422**(`LORA_DEPTH_CHAIN_UNSUPPORTED`)という制約も従来どおり効く。**全34フィールドの4分類表(422系7／無視5／動作19／従属3)の正本はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.10(f)** で、先回りのグレーアウトに使う機能名は`GET /models`の`unsupported_features`(§3.5。現在8件)から取る。
