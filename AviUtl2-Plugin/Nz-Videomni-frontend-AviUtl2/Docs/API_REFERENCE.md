...
| `inpaint` | `InpaintSpec` \| null | **2026-09-14追加**。Inpainting（マスクによる部分再生成。マスク動画の白い領域の内側だけを描き替える）。`reference_video_id`が必須で、`outpaint`・`conditioning_images`・`crop_output`とは**排他**。`width`/`height`は**キャンバス**（対象動画の実寸を128の倍数へ切り上げた値）でなければならない。詳細は下の**InpaintSpec**。**LTX 2.5では422**（`FEATURE_UNSUPPORTED`） |
