...
>
> 1. **実際に使われた方式は必ず記録される**。生成後に`outputs/{job_id}/metadata.json`の実効値欄（`attention_used`＝`"sdpa"`／`"sage"`／`"sage->sdpa"`、`keep_resident_used`、`fused_gguf_dequant_kernel_used`、`vae_mode_used`）で、要求どおりに適用されたか・降格したかを確認できる。フロント側の実機ゲートもログではなくこの値で判定する。
