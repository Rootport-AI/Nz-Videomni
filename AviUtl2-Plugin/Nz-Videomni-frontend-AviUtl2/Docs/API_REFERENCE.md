...
- 認証が掛かるのは `require_auth` 依存を持つエンドポイントのみ = `/generate`, `/generate/chain`, `/upload/*`, `/pipeline/load`, `/pipeline/unload`, `/jobs/{id}/join`, `DELETE /jobs/{id}`, `/utils/track/*`(§3.20〜§3.22、3本とも)。**GET系(status/config/job/loras/models)は認証不要**。
