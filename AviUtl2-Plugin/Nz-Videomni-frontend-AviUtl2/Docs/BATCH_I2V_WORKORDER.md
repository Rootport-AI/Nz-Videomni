...
派生して残った将来課題は、**2026-09-02現在で1件**である——バックエンド[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §3-48（JobLedgerの50件級描画。実運用トリガー待ちへ分離）。**もう1件だった§3-47（バッチA2Vランナーの孤児化）は2026-09-02に解消してクローズした**——記録はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-47-02、実装記録は[`DEVLOG.md`](DEVLOG.md) §104（本書§6.5も同日に現行化済み）。
...
**（2026-09-02 現行化）バッチA2V側も同じ方式になった。** 起票当時（2026-07-31）は「A2V側のランナーは`useRef`保持のままなので、走行中にCreate画面が`remountTokens`でリマウントされると孤児化する」という残問題があり、本節はそれを新規起票の対象として書いていた。**2026-09-02にA2V側にも`modes/batch/runtime.ts`を新設して同じシングルトン方式へ移し、この非対称は解消した**——両バッチの寿命規則が揃い、「A2Vは孤児化する」という前提でピン留めしていた例外テスト（`shell/runLock.crossPanel.test.tsx`）も期待値ごと反転している。A2V側だけの意図的な違いは3点だけである（復元は走行中のときのみ／実行対象ゼロ時の同期即時解放を持たない／`outDirIsAuto`は走行入力から導出しない）。記録はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-47-02と[`DEVLOG.md`](DEVLOG.md) §104。**なお同じ段落が「共有ロックがアプリ再読み込みまで残る」と書いていた滞留のほうは、その後の2026-07-31の改修（[`DEVLOG.md`](DEVLOG.md) §54.9）で既に解消していた**——起票時の記述が失効したまま残っていたものである。
