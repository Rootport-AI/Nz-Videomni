...
```json
{ "id": 42, "ok": true, "result": { "pong": true, "pluginVersion": "1.0.0" } }
```
...
| `timeline.cancelTracking` | 同期(中止の合図を立てるだけ) | UI | v12(§4.23) |
| `timeline.trackProgress`(イベント、ネイティブ発) | — | ワーカー→UIへマーシャリング | v12(§4.23) |
| `timeline.renderMaskVideo` | 非同期(HTTPワーカースレッドへ投入。内部でUIスレッドの`call_edit_section_param`を繰り返し呼び、窓の1フレームごとにレンダリング→mp4書き込みを行う) | UI起点→ワーカー→レンダリング | v13(§4.24) |
| `timeline.maskProgress`(イベント、ネイティブ発) | — | ワーカー→UIへマーシャリング | v13(§4.24) |
...
- params: `{}`
- result: `{ pong: true, pluginVersion: string }`(`pluginVersion`は`kPluginVersion` = `"1.0.0"`)
- エラー: なし(常に成功)。
...
- **素材（冒頭）と素材（末尾）の併用（2026-09-07）もブリッジ契約に影響しない**: サーバー側に`"bridge"`という名前のモードが増えたが、**これは生成方式の名前であって本書が扱うブリッジ（WebUIとnativeの間の取り決め）とは別物である。** 変わったのはサーバーが受理する組み合わせの範囲だけで、アップロード応答の使い道も系統E（末尾合わせ）の純関数群も無改修、nativeも1行も変わっていない。契約は[`API_REFERENCE.md`](API_REFERENCE.md) §5.2、フロントエンド側の記録は[`DEVLOG.md`](DEVLOG.md) §113。
...
- result:
  ...
  - `selected: Array<{ layer, frameStart, frameEnd, effectName, filePath: string|null, objectName: string|null, mediaWidth: number, mediaHeight: number }>` — 選択中の各タイムラインオブジェクト。`filePath`/`objectName`は取得できない場合`null`。
    - **`frameStart`/`frameEnd`は「包含」である(2026-09-04に決着・解決済み)**: ネイティブは`OBJECT_LAYER_FRAME`の`start`/`end`を**素通しで**載せており(`bridge.cpp`の`GetSelectionEditProc`)、その`end`は**包含**(最後に占有するフレームそのものを指す)と確定した([SDK_REFERENCE.md](SDK_REFERENCE.md) §16 (h)が正本)。したがって**webui側が`frameEnd`を包含と読み、リボン長を`frameEnd - frameStart + 1`で算出しているのは正しい**(`timeline/prefillSeed.ts`・`jobs/fpsConvert.ts`・`timeline/retakeWindow.ts`)。[DEVLOG.md](DEVLOG.md) §44.4の「ネイティブ現物で`frameEnd`はinclusiveと確定した」という記録も**復権した**。
      - **経緯（同日中の訂正）**: 2026-09-04にいったん本項へ「`end`は**排他**と確定した——§44.4の包含と正面から衝突する・突き合わせは未了」と書いたが、**これは実機ログの誤読で、撤回した**。`end - start`が要求`length`と一致していたのは`end`が排他だったからではなく、**作られたオブジェクトが要求より1フレーム長かった**（エイリアスの`frame=`ヘッダに`length`をそのまま書いていた）ためである。**webui側は1行も変えていない**——直したのはネイティブの`NormalizeAliasObjectFrameHeader`が書く値だけで、`frame=0,length`→`frame=0,length−1`の1箇所である。詳細は[DEVLOG.md](DEVLOG.md) §107.11。
...
- **拡張(2026-09-11、contract v12＝`effectName`のフォールバック)**: `selected[]`の`effectName`に、**メディア3種(動画ファイル／画像ファイル／音声ファイル)にもテキストにも当たらないオブジェクトについて、エイリアスの先頭の効果名**が入るようになった。フィールドの増減は無く、**これまで空文字だった枠が埋まるだけ**である。

  - 直接の用途は物体追尾で、**部分フィルタを右クリックしたときに`effectName`が`"部分フィルタ"`になる**ことが、右クリック項目#21の種別ガードの成立条件である([`RIGHTCLICK_REDESIGN_SPEC.md`](RIGHTCLICK_REDESIGN_SPEC.md) §4-1・§4-3)。副作用として、部分フィルタ以外の効果オブジェクトにもそれぞれの効果名が載る。
  - **既存の4種判定は1つも動かない。** webui側の種別判定(`classifySelectionKind`)は知っている名前の集合の外をすべて「不明」へ落とすので、新しく入る名前は`"部分フィルタ"`を除きこれまでどおり「不明」として扱われる。
  - 判定の順序は「メディア → テキスト → 先頭の効果名」で、フォールバックはテキスト判定が外れた枝の中にある(`bridge.cpp`の`GetSelectionEditProc`。エイリアスがそもそも無いオブジェクトはこの枝に入らない)。**部分フィルタの中で効果の順序を入れ替えると先頭の効果名も変わりうる**点は、実機確認の対象として[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md) §9に挙げてある。
...
### 4.23 `timeline.trackObject` / `timeline.cancelTracking` / `timeline.trackProgress`(contract v12)

タイムラインに置いた**部分フィルタ**(画面の一部だけに効果を掛けるAviUtl2のフィルタオブジェクト)の枠を、バックエンドの追跡AIに追わせて、結果を**中間点**(値の時間変化を指定する点)として書き戻す。**機能全体の正本はバックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md)で、プラグイン側の処理手順・後処理・エイリアスの書き換え範囲は同書§5が正本**である。本節はブリッジ契約として交わす形だけを書く。

**責務の位置**: 推論はバックエンド、座標の換算とエイリアスの組み立てと後処理はネイティブ、設定の保持と表示は操作パネル。**操作パネルは座標を計算せず、エイリアスの文字列も組み立てない**ので、この3つの形さえ固定すれば設定項目が増えても契約は動かない。

#### `timeline.trackObject`(非同期)

- params(**全項目必須**。1つでも欠けるか型が違えば`BAD_REQUEST`):

  | 項目 | 型 | 意味 |
  | --- | --- | --- |
  | `layer` | `number` | 対象の部分フィルタが載っているレイヤー。 |
  | `frame` | `number` | 追尾の起点。**必ず選択スナップショットの`frameStart`(対象オブジェクトの開始フレーム)を渡すこと**——再生カーソルの位置ではない。AviUtl2の検索関数が「そのフレーム以降で最初に見つかったもの」を返す仕様なので、ここが対象自身の開始フレームであることが「最初に見つかるのは対象自身」の前提になる。 |
  | `searchFactor` | `number` | 探索範囲の広さ。受け付ける範囲は操作パネルのスライダーと同じ2.0〜6.0(サーバー側の外枠8.0とは別で、狭いほうがここ)。 |
  | `smoothing` | `number` | 動きの滑らかさ。0〜1。 |
  | `followSize` | `boolean` | 枠の大きさも追従させるか。 |
  | `lostScoreThreshold` | `number` | 見失いのしきい値。0〜1。 |
  | `lostBehavior` | `string` | **`"hold"`(直前の位置で止める)か`"continue"`(そのまま追い続ける)の2値のみ**。他の文字列は`BAD_REQUEST`。 |
  | `keyframeStride` | `number` | 中間点を打つ間隔(1以上)。先頭と末尾の中間点はこの値にかかわらず必ず残る。 |

- result: `{ ok: boolean, frames: number, keyframes: number, lostRanges: Array<{start, end}>, elapsedMs: number, cancelled: boolean }`
  - **`keyframes`は打った中間点の「件数」**であって、中間点そのものの配列ではない(値はエイリアスへ書き戻され、操作パネルは件数しか使わない)。
  - **`lostRanges`は絶対フレーム番号である。** ネイティブ内部の後処理はオブジェクト相対の位置で動くが、応答に載せる直前に対象の開始フレームを足して**AviUtl2のフレーム番号**へ直す。進捗イベントの`frame`と同じ規約で、どちらも利用者が画面で見る番号と突き合わせられる。
  - `cancelled`が真なら途中打ち切りだが、**そこまでの中間点は書き戻し済み**である。
- **二重起動は`TRACK_BUSY`で断る。** 追尾中にもう一度呼ばれた場合で、解除は処理の出口で自動的に行われる。
- エラー: `TRACK_BUSY` / `TRACK_SEED_INVALID` / `TRACK_WRITEBACK_FAILED` / `TRACK_FAILED`、および`BAD_REQUEST`(§5)。**バックエンド由来の`TRACK_UNAVAILABLE`／`TRACK_SESSION_NOT_FOUND`／`TRACK_FRAME_INVALID`は、包み直さずそのままの符号で届く**(REST側の定義は[`API_REFERENCE.md`](API_REFERENCE.md) §2・§3.20〜§3.22)。

#### `timeline.cancelTracking`(同期)

- params: `{}` / result: 中止の合図を立てるだけ。
- **合図を立てて即座に返る。** 追尾のループが1フレームごとに先頭で協調的に確認するので、実際に止まるのは次のフレームの境目である。

#### `timeline.trackProgress`(イベント、ネイティブ→WebUI)

```json
{ "event": "timeline.trackProgress", "data": { "frame": 0, "index": 1, "total": 240, "score": 0.0, "lost": false } }
```

- `frame`は**絶対フレーム番号**(AviUtl2のもの)、`index`は**1始まり**の通し番号、`total`は総フレーム数、`score`は追跡AIが返した生のスコア、`lost`は後処理のしきい値判定の結果。
- **間引きの規則**: 直前に送ってから**200ミリ秒以上**が経ったときだけ送る。**ただし最初のフレームと最後のフレームは必ず送る**(でなければ0%のまま始まり、100%に届かないまま終わったように見える)。
- **1秒あたりのフレーム数はここに入れない。** 操作パネルがイベントの到着時刻から自分で計算する。
- イベント機構そのものは§1.4のとおりで、ワーカースレッドからの投稿はUIスレッドへマーシャリングされる(§2)。

### 4.24 `timeline.renderMaskVideo` / `timeline.maskProgress`(contract v13)

Inpainting(マスクによる部分再生成。部分フィルタの枠の内側だけを描き替える機能)のためのマスク動画を、**AviUtl2本体に描かせる**。部分フィルタの位置・大きさ・角の丸め・ぼかし・マスクの種類・回転・中間点による動きをネイティブ側で再現計算する代わりに、黒背景の上に「枠の内側を白くする効果」を1つだけ足した部分フィルタの複製を重ねて実際に合成描画させ、その結果をそのままmp4へ書き出す。**機能全体の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md)で、一時オブジェクトの作り方・ソロ表示・後片付けの手順は同書§5が正本**である。本節はブリッジ契約として交わす形だけを書く。

**責務の位置**: マスクの意味づけ・アップロード・生成要求の組み立ては操作パネル、マスクの画素そのものを作るのはプラグイン。**操作パネルはマスクの画素に一切触れない**——プラグインが渡すのは書き出し済みのmp4のパスだけである。

#### `timeline.renderMaskVideo`(非同期)

- params(**全項目必須**。1つでも欠けるか型が違えば`BAD_REQUEST`。すべてAviUtl2の絶対フレーム番号で、両端を含む閉区間):

  | 項目 | 型 | 意味 |
  | --- | --- | --- |
  | `layer` | `number` | マスクの種にする部分フィルタが載っているレイヤー(0以上)。 |
  | `frameStart` | `number` | 部分フィルタの開始フレーム(0以上)。 |
  | `frameEnd` | `number` | 部分フィルタの終了フレーム(包含、`frameStart`以上)。 |
  | `windowStart` | `number` | 描画する窓の開始フレーム(0以上)。 |
  | `windowEnd` | `number` | 描画する窓の終了フレーム(包含、`windowStart`以上)。 |

  - **窓は部分フィルタの範囲を包含している必要はなく、重なってさえいればよい**(オーナー裁定D3。窓が部分フィルタより短くても長くても受け付ける——設計の正本は[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §4)。2つの区間が一切重ならないときは`BAD_REQUEST`。
  - **窓の長さの上限は4096フレーム**(`kMaskMaxWindowFrames`)。単発生成のAPI上限(481フレーム)より大幅に大きい値で、バグでしかありえない要求だけを止める安全弁である。長さの計算はint型のオーバーフローを避けるため64bit演算で行う。

- result: `{ filePath: string, width: number, height: number, frameCount: number }`。`width`/`height`は**プロジェクトの解像度**(マスクはシーンの合成結果そのものの描画のため)、`frameCount`は実際に書き出した枚数(＝窓の長さ)。`filePath`はそのまま`backend.uploadFile`へ渡せるローカルパスで、**操作パネルはこのファイルを開かず、画面にも出さない**(オーナー裁定。マスクは部分フィルタの枠そのものであり、プレビューに既に映っているため——[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §13)。
- 内部の処理(要約。手順の正本は[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §5): `layer_max+1`に黒背景のPNG、`layer_max+2`に部分フィルタの複製(最初の効果ブロックだけを残し、末尾に白化効果を1つ足したもの)を、**1回の編集区間の中で**作成する。作成後に両方の実際の位置・尺を読み直して検証し、片方でも要求どおりでなければ同じ編集区間の中で後始末して`MASK_FAILED`にする。この2つ**以外**のレイヤーをすべて一時的に無効化(ソロ表示)してから窓の各フレームを合成描画し、mp4へ書き出す。**削除とソロ解除は、削除を先に行う**。
- **受け入れる代償**: 「元に戻す」の履歴が、一時オブジェクトの作成と削除でそれぞれ1件ずつ、**合計2件増える**(利用者のオブジェクトは壊れない)。**AviUtl2の終了処理中だけは後片付けを諦め**、一時オブジェクトがタイムラインに残ることがある(追尾の`g_shutting_down`と同じ判断——UIスレッドを待つ`Bridge::Shutdown`と、そのUIスレッドを必要とする後片付けが、そのままでは互いを待ち合ってしまうため)。
- **マスクmp4は誰も消さない**——切り抜き(cutouts)と同じ扱いである。置き場所は§7を参照。
- エラー: `BAD_REQUEST` / `NO_EDIT_HANDLE` / `MASK_BUSY` / `MASK_SEED_INVALID` / `MASK_FAILED`(§5)。
- **単一のタイムラインジョブ枠を物体追尾(§4.23)と共有する**。v12で新設したこの枠は、v13でマスク描画にも広がった——追尾中に呼ぶと`MASK_BUSY`、逆にマスク描画中に`timeline.trackObject`を呼ぶと`TRACK_BUSY`で断られる。**エラーの符号は呼び出し側で決まる**(枠を握っているのがどちらであっても、`trackObject`は常に`TRACK_BUSY`、`renderMaskVideo`は常に`MASK_BUSY`を返す)。メッセージはどちらも固定文字列。
- **このバージョンには停止用のRPCが無い**。マスクは窓の全フレームが揃って初めて使えるため、途中で止めることは常に「失敗として捨てる」ことと同義であり、追尾の`timeline.cancelTracking`(そこまでの結果を書き戻す)とは性格が異なる(設計の理由は[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §18)。**マスク描画がジョブ枠を握っているあいだに`timeline.cancelTracking`を呼んでも**、握っているのが追尾でなければ何もせず`{cancelled:false}`を返す(§4.23の記述のとおり、これは失敗ではない)。

#### `timeline.maskProgress`(イベント、ネイティブ→WebUI)

```json
{ "event": "timeline.maskProgress", "data": { "frame": 0, "index": 1, "total": 240 } }
```

- `frame`は**絶対フレーム番号**(AviUtl2のもの)、`index`は**1始まり**の通し番号、`total`は窓の総フレーム数。
- **間引きの規則は`timeline.trackProgress`(§4.23)と同じ**: 直前に送ってから200ミリ秒以上が経ったときだけ送るが、**最初のフレームと最後のフレームは必ず送る**。
...
| `TRACK_FAILED` | 対象が見つからない・描画や通信が失敗した・バックエンドのエラーを分類できない(v12・§4.23) | `timeline.trackObject` |
| `MASK_BUSY` | 単一のタイムラインジョブ枠が塞がっている——追尾が走っているか、他のマスク描画が既に1本走っている(v13・§4.24) | `timeline.renderMaskVideo` |
| `MASK_SEED_INVALID` | 種にする部分フィルタが`layer`/`frameStart`に見つからない・部分フィルタでない(v13・§4.24) | `timeline.renderMaskVideo` |
| `MASK_FAILED` | 一時オブジェクトの作成・レイヤーの隔離・フレームの描画・mp4の書き出しのいずれかに失敗した(v13・§4.24。ネイティブは応答前に作成済みの一時オブジェクトと不完全なmp4を片付ける) | `timeline.renderMaskVideo` |
...
**上表に無いコードが`timeline.trackObject`から届くことがある。** 追尾はネイティブがバックエンドのRESTを直接叩く唯一のメソッドなので、**サーバー側のドメインエラー(`TRACK_UNAVAILABLE`／`TRACK_SESSION_NOT_FOUND`／`TRACK_FRAME_INVALID`)は包み直さず、その符号のまま応答へ載せる**(定義は[`API_REFERENCE.md`](API_REFERENCE.md) §2)。WebUI側は「知らないコードは原文のメッセージを見せる」という既存の扱いでそのまま受けられる。
...
| `ui.pickFile`/`ui.pickFolder`のWebUI側ローカル待機上限 | **無し**(`NO_LOCAL_TIMEOUT_METHODS`に列挙されたメソッドはタイマー自体を張らない) | ネイティブのファイルダイアログはユーザーが閉じるまで無期限に待つモーダルで「長すぎる」という概念が無いため。ユーザーが選択に10秒以上かけてもWebUI側が先にTIMEOUTで諦めることはない(ブリッジ破棄時は`DISPOSED`で解決)。同メソッドの集合には§4.23の`timeline.trackObject`と§4.24の`timeline.renderMaskVideo`も含む | `webui/src/bridge/types.ts`(`NO_LOCAL_TIMEOUT_METHODS`), `requestDispatcher.ts` |
...
| v11 | 素材fpsの取得と操作パネルへの流し込み(2026-09-01) | `timeline.getSelection`の`selected[]`への`mediaFps`(素材そのもののフレームレート、非nullable・`0`＝不明)追加(§4.13)。右クリックプリフィルのfps軸「素材に合わせる」を実際に機能させるために追加した。新規メソッド・新規エラーコードは無く、**追加は1フィールドだけ**である。**取得はMedia Foundation**(`MF_MT_FRAME_RATE`)で、native実装は新規モジュール`native/src/media_fps_probe.h/.cpp`(`FpsFromRatio`／`ProbeMediaFps`)と`bridge.cpp`の`GetSelectionEditProc`からの呼び出し、`bridge_core.h`の`SelectionItem::media_fps`と`MakeSelectionResult`のJSON化からなる。**nativeは生値を返し、整数へのスナップ(29.97→30)はwebuiの`prefillSeed.ts`が行う**。**mkv/webmでの取得失敗は正常系**(webuiがプロジェクトfpsへ落ちる)。doctestは新設`native/tests/test_media_fps_probe.cpp`ほかで**278→282ケース・アサーション1416→1452**(いずれも0失敗。実測値は`build.ps1 -Config Release -RunTests`で確認)。**MFのプローブが実ファイルを開く経路は実機でのみ確認可能**で、オーナーの実機ゲートG1〜G8待ちである(項目は台帳[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md)「2. 実装済み・ユーザーのテスト待ち」)（その後クローズ: [`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-13-02）。 |
| v12 | 物体追尾(2026-09-11) | `timeline.trackObject`(非同期)と`timeline.cancelTracking`(同期)の2メソッド新設、イベント`timeline.trackProgress`新設、エラーコード4つ(`TRACK_BUSY`／`TRACK_SEED_INVALID`／`TRACK_WRITEBACK_FAILED`／`TRACK_FAILED`)新設、`timeline.getSelection`の`effectName`へ先頭効果名のフォールバック追加(§4.13・§4.23・§5)。**既存メソッドの引数・応答は1つも変えていない**——`effectName`はこれまで空文字だった枠が埋まるだけで、フィールドの増減は無い。右クリックのアクションIDも`trackObject`が1つ増えただけである([`RIGHTCLICK_REDESIGN_SPEC.md`](RIGHTCLICK_REDESIGN_SPEC.md) §3-4 #21)。**この版で初めて、ネイティブがバックエンドのRESTを長時間往復し続けるメソッドが加わった**(1フレームごとに描画→送信→応答を繰り返す)ため、サーバー側のドメインエラーが包み直されずそのまま届く経路も新しい(§5)。機能全体の正本はバックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md)、REST側の契約は[`API_REFERENCE.md`](API_REFERENCE.md) §3.20〜§3.22である。 |
| v13 | Inpainting(マスクによる部分再生成、2026-09-14) | `timeline.renderMaskVideo`(非同期)メソッド新設、イベント`timeline.maskProgress`新設、エラーコード3つ(`MASK_BUSY`／`MASK_SEED_INVALID`／`MASK_FAILED`)新設(§4.24・§5・§7)。**既存メソッドの引数・応答は1つも変えていない。** v12が新設した単一のタイムラインジョブ枠を、物体追尾以外の用途(マスク描画)にも広げた初めての版で、`timeline.cancelTracking`(§4.23)は枠をマスク描画が握っているときに呼ばれても`{cancelled:false}`を返すことを本書へ明記した(挙動自体はv12から不変)。**このバージョンには停止用のRPCが無い**——半分だけ描けたマスクには使い道が無いため(理由は[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §18)。右クリックのアクションIDは`inpaintPartialFilter`・`inpaintVideo`の2つが増えた([`RIGHTCLICK_REDESIGN_SPEC.md`](RIGHTCLICK_REDESIGN_SPEC.md) §3-4 #22・#23)。機能全体の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md)である。 |
...
- `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/webui/src/shell/useFileDrop.ts` / `DropZone.tsx`(contract v7、各画面のドロップゾーン〔Toolboxの「mp4 info」を含む〕が共有するD&D受付ロジック/UI)
...
- `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/native/src/track_postprocess.h` / `track_postprocess.cpp`(contract v12、追尾の後処理6段〔見失いの印つけ・埋め・平滑化・大きさの固定・間引き・区間まとめ〕を担う純関数群。2026-09-11新規。**順序が固定であることと、入力が相対位置・出力の見失い区間が呼び出し側で絶対フレームへ直されること**は[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md) §5.5が正本)
- `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/native/tests/test_track_postprocess.cpp`(contract v12の後処理doctest)
- `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/native/src/alias_util.h` / `alias_util.cpp`(contract v12、部分フィルタの値と矩形の換算・エイリアスの5行だけの差し替え・書き戻しの復元。`NormalizeAliasObjectFrameHeader`もここ。**contract v13でマスク複製専用の純関数4つ(`KeepOnlyFirstEffectBlock`／`AppendEffectBlock`／`NormalizeAliasFrameBoundariesToRelative`／`ClipAliasFrameBoundaries`)と、白化効果の行を返す`WhiteningEffectBlockLines`が追加された**。**後者の定数（`effect.name=反転`／`輝度反転=1`）は2026-09-14のオーナー実機採取で確定済みで、書いていた値と採取物は一致していた**——原文の転記は[`SDK_REFERENCE.md`](SDK_REFERENCE.md) §16(k)、設計の正本は[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §5.2)
- `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/native/tests/test_alias_util.cpp`(contract v13、マスク複製の純関数群のdoctest)
