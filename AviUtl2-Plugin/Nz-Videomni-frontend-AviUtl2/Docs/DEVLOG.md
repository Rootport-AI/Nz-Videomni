...
- vitestは基準の実行（§111.4。実バックエンドを叩く`api/backend.integration.test.ts`を除外）で全緑である。**ファイル数・件数・基準との差はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §102.4が正本で、本節には書き写さない。** 差の内訳は**5件を足し、規則が消えて意味を失った2件を落とした**というものである。足した5件は、意味を反転させた2件（「両方は送れない」→「**`buildRequest`の結果に`source_video`と`end_source`の両方が載り、`validityReasons`が空**」／「素材（末尾）と2本以上が揃えば必ず警告」→「**素材（冒頭）が付いていれば遡り生成ではない**」）と、新設の3件（素材（冒頭）を付ける・外すでのりしろが3と1を往復すること／素材（冒頭）を先に付けた場合にのりしろ1が一度も現れないこと／`ChainedScreen`で素材（冒頭）を付けると遡り生成のヒントが消えること）である。**モック経由の受理テストは作っていない。** `mockBridge.ts`にはこの組み合わせの拒否がそもそも無く、通っても何の証明にもならないためである。
- `npm run lint`・`npm run typecheck`はどちらも基準どおりである（**警告本数を含む実数はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §102.4が正本**）。
- 配置は`build.ps1 -Config Release`→`deploy.ps1`で、`.aux2`のSHA-256がビルド出力・実機・バックエンドリポジトリ配布コピーの3値で一致している（**値の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §102.12**）。
...
## 114. キーフレーム画像の上限を10枚へ引き上げた — フロントエンドは配信値に追随しただけで済んだ（バックエンド台帳[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-147）＝オーナー実機ゲートG1〜G8 全合格（2026-09-08）

### 114.1 結論

**フロントエンドが変えたのは、画面の外にある3つの数字とテスト1本だけである。** 上限「5枚」はフロントに直書きされておらず、`GET /config`の`limits.max_conditioning_images`を読む動的な作りだったので、**サーバーが10を知らせるようになった時点で画面は10枚まで扱える。** 触ったのは、サーバーへ繋がる前に使うフォールバック値（`modes/single/defaultConfig.ts`）、モックが知らせる値（`bridge/mockBridge.ts`）、そしてEdit画面のテストが自前で持っていたスタブ（`modes/edit/EditScreen.retake.test.tsx`）の3箇所である。**「上限を読む場所」を1つに寄せ直す作業は要らなかった**——`useKeyframes`が配信値だけを見る作りは、最初からそうなっていた。

**本題はバックエンド側にある。** ①上限の正本を`config.py`の定数`MAX_CONDITIONING_IMAGES`ただ1箇所へ集約した（`limits.max_conditioning_images`は設定項目をやめ、その定数を返す**読み取り専用の算出項目**になった。古い`config.yaml`に`5`が残っていても無視される）。②単発生成と連結生成のバリデータを1つのヘルパーへ寄せ、**スナップ後に同じフレームへ落ちた2枚を422で拒否する**ようになった。**契約と仕様の正本はバックエンド側である**——契約は[`API_REFERENCE.md`](API_REFERENCE.md) §3.2・§5.1、仕様はバックエンド[`Videomni_Backend_Specification.md`](../../../Videomni_Backend_Specification.md) §6.2・§6.7で、**本節へは書き写さない。**

### 114.2 重複の422は、この画面からは踏めない

「同じ位置に2枚は置けない」という規則を、いまはサーバーが422で断る。ただし**この画面から踏むことはない**——KEYFRAMESの着地解決ルール（同じ目盛りへ落ちそうになったピンを、必ず次の空き目盛りへ滑らせる）が、重複したリクエストをそもそも組み立てないからである。カウンタの「max」も置ける格子点の数で自動的に頭打ちになる作りなので、**尺が短くて上限10に届かないことも画面の側で先に分かる**（`num_frames=9`なら置ける位置は`0`と`1`だけなので「0 / 2」と出る）。

この関係は設計メモ[`KEYFRAME_TIMELINE_DESIGN_NOTES.md`](KEYFRAME_TIMELINE_DESIGN_NOTES.md) §4を現行化して書き直した。同節は長らく「同じ位置への重複を防ぐ仕組みが、サーバーにもフロントにも存在しない」と書いていたが、**これはもう事実ではない**。あわせて、上限の正本が`config.limits.max_conditioning_images`だという記述（2026-09-07にいったん「誤り」と訂正されていた行）も、**二重構造そのものが解消したことを書く形へ**改めてある。

### 114.3 「キーフレームを追加」ボタンは既定の尺では7枚で止まる（規則は据え置いた）

**追加ボタンの規則は今回変えていない**（オーナー裁定）。この規則は「最後のピンと最終フレームの中点に置き、既存のピンは動かさない」というもので、押すたびに残りの尾を半分ずつ食う。既定の361フレームでは`0, 177, 265, 313, 337, 345, 353`の**7枚で「後ろに空きがありません」**となり、ボタンだけでは上限10へ届かない。**届かせるにはピンをドラッグして後ろを空ける**（そのヒントは以前から出ている）。規則を見直すかどうかは実機の使い勝手を見てから判断することになっており、受け皿はバックエンド台帳[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §3-146である。

> **追記（2026-09-17）**: その受け皿（台帳§3-146）は、オーナー裁定「実際に使って不便を感じなかった。8枚目以降で規則が変わるほうが認知の負担を増やす」により**現状維持のままクローズ**し、記録はバックエンド[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) **§3-146**へ移った。**追加ボタンの規則は据え置きで確定**である（実装は無変更。上限へ届かせたいときはピンをドラッグして後ろを空ける、という本文の説明もそのまま生きている）。**本文は追記専用の規律どおり不変**である。

**この性質は、上限まで埋めるテストの書き換えにそのまま出た。** `App.prefill.test.tsx`の「上限まで埋めて上限到達の文言を確認する」通しは、5を10へ書き換えるだけでは7枚で頭打ちになって落ちる。そこで、**1枚足すたびに、いま足した右端のカードのFRAME欄へ低い格子点（1, 9, 17, …）を打ち込んで尾を空け直す**形へ改め、止めているものが「後ろに空きがない」ではなく**枚数の上限**であることを明示的に確かめるようにした（`maximum 10 keyframes reached`が出て、`no room left`が出ないこと）。右クリック#10が差し替える先であるフレーム0のカードは、最後まで動かさない。

### 114.4 テストと状態

- **バックエンド**: `pytest -q`の全件が**2,246 passed / 25 skipped**。本テーマでの追加は**11件**（単発生成の検証6件・連結生成3件・Gradioの配線カナリア2件。連結生成側の枚数超過テストは今回が新設である）。
- **webui**: vitestは基準の実行（実バックエンドを叩く`api/backend.integration.test.ts`を除外）で**138ファイル・2,737件**が全緑・0 skipped。**本テーマでの件数の増減は無い**——§111時点の基準2,734件からの+3は、§113で5件足して2件落とした差である。`npm run typecheck`はエラー0、`npm run lint`はエラー0で警告31本と、いずれも基準どおりである。
- **オーナーの実機ゲートは2026-09-08に全項目合格した。** 手順と合格条件は[`REAL_BACKEND_CHECKLIST.md`](REAL_BACKEND_CHECKLIST.md) §4.14（G1〜G8）が正本である。画面側に関わるのはG3で、**LTX 2.3とLTX 2.5の両方で10枚のキーフレームを置いた生成が完走し、カウンタは「10 / 10」を示した**。§114.3が書いた「追加ボタンでは7枚で止まる」性質どおり、10枚目まではピンをドラッグして後ろを空けながら配置している。**見た目（ラベルの密度・カード一覧の縦長）についてオーナーからの追加の指摘は無く、別項の起票も行っていない。** G7（古い`config.yaml`を残したままでも`GET /config`が10を知らせる）は**単独の再実行を省略し、G3との同時確認と自動テストで充足と判定した**——判定理由の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §103.6である。
- **ゲート結果と設計判断の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §103、クローズ記録は[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-147である。** 起票時に本節が指していた台帳`PENDING_TASKS.md` §1-28は、クローズに伴い同書から削除されている。
- **実機ゲートの前に配布`.aux2`を再ビルドしてデプロイした。** G3が画面の見た目を見る項目なので、古い配布物のままでは判定にならないためである。

## 115. 物体追尾 — 部分フィルタの枠をUETrackに追わせ、結果を中間点として書き戻す（契約v12。バックエンド台帳[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-54-02）＝オーナー実機ゲート1〜8 全合格（2026-09-11）

### 115.1 結論

**フロントエンドはこの回で2つの初物を持った。** ①**ネイティブが、バックエンドのRESTを長時間にわたって往復し続けるメソッドを初めて持った**（`timeline.trackObject`。1フレームごとにプレビューを描画してPOSTし、フレーム数だけ繰り返す）。②**Toolboxが実タブへ昇格し、モックタブは1つも残っていない**（Phase 6で置いた2つの見た目だけの枠は、Editが2026-08-09に、Toolboxが本日、それぞれ実体を持った）。

**機能そのものの設計正本はバックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md)である**（プラグイン側は§5、操作パネルは§3）。**本節へは書き写さない。** ゲートの実数と実測値の正本は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §104である。

利用者から見た流れだけを1行で言えば、**プレビューで部分フィルタの枠を被写体に合わせ、タイムラインでその部分フィルタを右クリックして「🎯 物体追尾（部分フィルタを使用）」を選ぶ**と、Toolboxタブに進み具合が出て、終わると枠の位置と大きさに中間点が打たれている、というものである。効果（モザイク等）は利用者があとから自由に足す。

### 115.2 契約v12 — 追加専用で、既存のワイヤ形式は1文字も変えていない

- **`timeline.trackObject`**（非同期・通信用のワーカースレッド）— 引数は対象の指定（`layer`・`frame`）と設定6項目、戻り値は`{ok, frames, keyframes, lostRanges, elapsedMs, cancelled}`。`lostRanges`の**フレーム番号はAviUtl2の絶対番号**である（敵対的レビューで揃えた）。
- **`timeline.cancelTracking`**（同期）— 中止の合図を立てるだけで、追尾のループが先頭で協調的に確認する。
- **`timeline.trackProgress`**（ネイティブ発のイベント）— `{frame, index, total, score, lost}`。**直前の送信から200ミリ秒以上経ったときだけ送り、最初と最後のフレームは必ず送る。** 毎秒フレーム数はここに入れず、**操作パネルが到着時刻から自分で計算する。**
- **エラーコードは4つ新設した** — `TRACK_BUSY`／`TRACK_SEED_INVALID`／`TRACK_WRITEBACK_FAILED`／`TRACK_FAILED`。
- **`timeline.getSelection`の`effectName`にフォールバックが入った** — メディア3種にもテキストにも当たらないオブジェクトについて、エイリアスの先頭の効果名が入る。**フィールドの増減は無く、これまで空文字だった枠が埋まるだけ**である。右クリック項目の有効・無効は、この値が「部分フィルタ」かどうかで決める。

契約の正本は[`BRIDGE_CONTRACT.md`](BRIDGE_CONTRACT.md) §4.23（§3・§4.13・§5・§8・§10も同時に更新済み）、右クリック項目の通し番号（#21）は[`RIGHTCLICK_REDESIGN_SPEC.md`](RIGHTCLICK_REDESIGN_SPEC.md)である。

### 115.3 ネイティブ側 — 後処理は純粋な関数、書き戻しは5行だけ

- **`native/src/track_postprocess.{h,cpp}`（新設）** — 追尾の生の結果を中間点へ変える**6段の後処理**を、副作用の無い関数として置いた。順序は固定で、①しきい値で「見失い」の印を付ける、②直前の良好な枠で埋める（設定による）、③指数移動平均で滑らかにする、④枠の大きさを種に固定する（設定による）、⑤間引く（**先頭と末尾は必ず残す**）、⑥連続した見失いを区間にまとめる、である。タイムラインにもネットワークにも触れないので、doctestで総当たりに近い検査ができる。
- **`native/src/alias_util.{h,cpp}`の`PatchAliasPartialFilterKeyframes`（新設）** — エイリアスの**書き換えは5行だけ**である（`X`／`Y`／`サイズ`／`縦横比`の値の行4本と、`[Object]`区画の`frame=`の行1本）。**それ以外はバイト単位でそのまま残す**ので、利用者が追尾の前に付けていた効果もマスクの種類も温存される。実書式はオーナーの実機採取物が正本で、転記先は[`SDK_REFERENCE.md`](SDK_REFERENCE.md) **§16 (j)**である（`frame=`の端点が**両端を含む**という §16 (h) の確定知見が、境界が複数になっても変わらないことを含む）。
- **描画と送信のループ（`bridge.cpp`）** — 対象のレイヤーを一時的に無効化してから（効果が絵に焼き込まれないようにするため）、開始フレームから終了フレームまで、中止の確認→描画→送信→進捗の通知を繰り返す。**レイヤーの復元は出口で必ず走る形（RAII）**にしてあり、早期の打ち切りでも中止でも戻る。二重起動は`TRACK_BUSY`で断る。
- **書き戻しは「削除して作り直す」一択である** — AviUtl2 v2.0.54のSDKには既存オブジェクトの区間を増減するAPIが無いためである。**先に新しいエイリアスを組み立てて検証し**、そこで失敗したらタイムラインに触れないまま中断する。削除と作り直しは**同じ編集区間**で行うので、「元に戻す」1回で追尾前に戻る。作り直しに失敗したら即座に元のエイリアスで復元し、それも失敗したときだけ`TRACK_WRITEBACK_FAILED`を返す。**書き戻しの直前にエイリアスを照合する**ので、追尾中に対象が別のオブジェクトへ差し替わっていた場合は何もしない（敵対的レビューで足した）。
- **`http_client`はバイナリ本文のPOSTを扱えるように広げた。** 送るのは生のRGBAで、1080pなら1枚あたり約8.3メガバイトである。

### 115.4 操作パネル側 — Toolboxの実タブ化

- **`modes/toolbox/`を新設した** — `ToolboxScreen.tsx`（タブの器）・`ObjectTrackingSection.tsx`（画面）・`useObjectTracking.ts`（追尾の進行と結果の状態）の3本に、`shell/objectTrackingSettings.ts`と`shell/useObjectTrackingSettings.ts`（**設定7項目の正規化と`localStorage`への保存**。サーバーの設定には送らない）が付く。
- **`shell/ModeTabs.tsx`からモックタブの型を畳んだ。** これまでは`MockTabId`（`"toolbox"`）と`disabled`を判別子にした union で「パネルも`remountTokens`も`MenuTargetMode`も持たない見た目だけの枠」を型で塞いでいたが、**Toolboxがその3つすべてを持った時点で塞ぐ対象が消えた。** いまタブが灰色になる理由は**1つだけ**——読み込み中のベースモデルのエンジンがそのタブを実行できないとき（`disabledModes`＝`shell/featureScope.ts`の`disabledModesFor`。§111）である。
- **進み具合は「`123 / 240 フレーム ・ 18.4 fps`」の形で出す。** 見失い区間は「`120–134`」の行を並べ、終わったら「`240フレームを追尾し、中間点を120個打ちました（12.3秒）`」の1行を出す。停止で終わった場合も頭に「停止しました。」と付けて同じ形で出す。
- **追尾の仕組みが未導入の環境では設定を灰色にし、`install-UETrack.bat`の実行を案内する。** 判定は`GET /status`の`tracking`ブロックで、**サーバーの状態が分からないあいだ**（つながる前や、ベースモデルを切り替えている最中）は**最後に分かっていた可否をそのまま保つ**（敵対的レビューで足した。ここで可否を落とすと、モデル切替のたびに画面が灰色へ点滅する）。
- **追尾中にもう一度右クリックされたときは、画面を作り直さない。** 案内を出してToolboxタブへ切り替えるだけにする——作り直すと、走っている追尾の進捗の購読と応答の待ち合わせが捨てられ、**追尾自体は完走しているのに操作パネルだけが止まって見える**（§115.5の実機ゲート7）。

### 115.5 テストと状態

- **webui**: vitestは基準の実行（実バックエンドを叩く`api/backend.integration.test.ts`を除外）で**141ファイル・2,830件**が全緑・0 skipped（§114時点の基準は138ファイル・2,737件）。`npm run typecheck`はエラー0、`npm run lint`はエラー0で**警告31本と不変**である。
- **ネイティブ**: doctestは**358 passed / 6 skipped・アサーション1,825件で0 failed**（§107.11時点は292ケース・1,504アサーション）。新設は`test_track_postprocess.cpp`・`test_alias_util.cpp`・`test_bridge_core.cpp`への追加分である。
- **バックエンド**（参考）: pytest全件が**2,319 passed / 26 skipped**（§114時点は2,246 / 25）。skipの増分1本は`.venv-utils`でしか動かない実行時テストで、そちらでは4件が通る。実数の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §104.5である。
- **配布`.aux2`は1,358,336バイト・SHA-256 `63147501…`** で、ビルド出力・実機・リポジトリ配布コピーの**3値が一致**している。実機ゲートは画面と挙動を見る項目ばかりなので、ゲートの前にデプロイしてある。
- **オーナーの実機ゲート1〜8は2026-09-11に全項目合格した。** 右クリック項目の表示（1）、Toolboxの進捗・毎秒フレーム数・完了行（2。処理中は対象がグレーアウトする）、中間点とモザイクの追従・焼き込みの無いこと（3）、「元に戻す」1回で追尾前へ戻ること（4）、停止で途中まで書き戻しレイヤーが復元されること（5）、分割して後ろ側を再追尾しても前側が保たれること（6）、追尾中にAviUtl2を閉じても固まらないこと（8）である。**7（追尾中の再右クリック）だけは初回が不合格で**——「すでに別の追尾」と「追尾に失敗」が続けて出て操作パネルが止まって見えた（追尾そのものは完走していた）——§115.4の最後に書いた形へ直して再確認で合格した。
- **UIのフィードバックも同日に反映して再確認し、合格している**（右クリック項目名・説明文と一部補足文の削除・ラジオボタンの横並び・補足文の改訂・停止ボタンの「🚫停止」）。**見失いのしきい値の既定0.35は「実測に基づく暫定値」としてオーナーが受容し**、以後はユーザーの反応で調整する。
- **設計判断・実測値・ゲート結果の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §104、クローズ記録は[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-54-02である。** 起票時に台帳`PENDING_TASKS.md`側が持っていた§1-29（起票時は§3-54）は、クローズに伴い同書から削除されている。

## 116. Inpainting（マスクによる部分再生成）— 部分フィルタからマスク動画を描いて対象動画の内側だけを描き替える（契約v13。バックエンド台帳 §3-55）（2026-09-14）

### 116.1 結論

**タイムラインに置いた部分フィルタ（AviUtl2のメディアオブジェクトの一種。枠の内側だけに後から効果を掛ける）の枠を白黒のマスク動画へ焼き、その白い領域の内側だけを動画生成モデルに描き替えさせる機能を実装した。** 利用者から見た流れは1行で言えば、**部分フィルタとその内側を描き替えたい動画を、それぞれ別の右クリックでEditタブの「Inpainting」サブタブへ渡し、Generateを押すとマスクの描画・アップロード・生成の投入が続けて行われる**、というものである（順序はどちらの右クリックが先でも構わない）。マスクは常にAviUtl2本体に描かせ、利用者にもプラグインにも見せない。

機能そのものの設計正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md)である（利用者から見た流れは§1、操作パネルは§3、窓の規則は§4、プラグイン側の手順は§5）。**本節へは設計を書き写さない。** 実測値・段0スパイクの判定・実機ゲートの結果の正本は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §105である。

フロントエンドとしての要点は2つある。①**v12（物体追尾）が新設した単一のタイムラインジョブ枠を、追尾以外の用途（マスク描画）にも広げた初めての機能である**——2つの機能は同じ枠を取り合い、握っているのがどちらでも呼び出し側に応じた符号（`TRACK_BUSY`／`MASK_BUSY`）で断られる。②**Editタブの「Inpainting」サブタブが、1枚の操作パネルを2回の右クリックで満たすという、本書で初めての形で実タブに昇格した**——これに伴う「作り直さない」設計判断が§116.4の核心である。

### 116.2 契約v13 — 追加専用で、既存のワイヤ形式は1文字も変えていない

- **`timeline.renderMaskVideo`**（非同期・HTTPワーカースレッド）— 引数は種の位置（`layer`・`frameStart`・`frameEnd`）と窓の範囲（`windowStart`・`windowEnd`、すべて絶対フレーム・両端を含む閉区間）、戻り値は`{filePath, width, height, frameCount}`。窓は部分フィルタの範囲を包含している必要はなく、重なってさえいればよい。
- **`timeline.maskProgress`**（ネイティブ発のイベント）— `{frame, index, total}`。物体追尾の`trackProgress`と同じ間引き規則（200ミリ秒・最初と最後は必ず送る）で、`score`／`lost`に相当するフィールドは持たない（マスク描画は描けるか描けないかのどちらかしかないため）。
- **エラーコードは3つ新設した** — `MASK_BUSY`／`MASK_SEED_INVALID`／`MASK_FAILED`。
- **停止用の呼び出しは作っていない。** マスクは窓の全フレームが揃って初めて使えるため、途中で止めることは常に「失敗として捨てる」ことと同義であり、追尾の`cancelTracking`（そこまでの結果を書き戻す）とは性格が違う。AviUtl2を閉じたときの中止の合図だけを内部のフラグで持つ。

契約の正本は[`BRIDGE_CONTRACT.md`](BRIDGE_CONTRACT.md) §4.24（§3・§5・§7・§8・§10も同時に更新済み）、右クリック項目の通し番号（#22・#23）は[`RIGHTCLICK_REDESIGN_SPEC.md`](RIGHTCLICK_REDESIGN_SPEC.md)である。オブジェクト右クリックは16→18項目になった。

### 116.3 ネイティブ側 — 一時オブジェクト2つを1つの編集区間で作って消す

- **単一のタイムラインジョブ枠を物体追尾と共有する** — `g_track_running`だった変数を`g_timeline_job_running`／`g_timeline_job_kind`（none/track/mask）へ改名し、`exchange(true)`1か所で排他を保証する。返す符号は枠を握っている側ではなく**呼び出し側**で決まる（`trackObject`は常に`TRACK_BUSY`、`renderMaskVideo`は常に`MASK_BUSY`）。中止用に`g_mask_cancel`を新設し、`Bridge::Shutdown`で`g_track_cancel`の隣に1行足した。
- **ソロ表示の`SoloDisableCtx`を拡張した** — 走査上限を固定の`layer_max`から引数化する`scan_max`と、keep側を明示的に表示にする`force_enable_keep`を足した（既存呼び出し元はバイト同一）。**`SoloGuard`には`g_shutting_down`の例外を新設した**——これは`cutoutRange`の音声ソロにとっても、終了処理中のデッドロックを避ける修正を兼ねている。
- **一時オブジェクトは2つ、必ず`layer_max+1`→`layer_max+2`の順で1つの編集区間の中に作る** — シーン寸の黒いPNGを`create_object_from_media_file`で背景として`layer_max+1`に、部分フィルタの複製（最初の効果ブロックだけを残し、末尾に白化効果を1つ足したもの）を`create_object_from_alias`で`layer_max+2`に置く。**作成後に両方の実際の位置・尺を読み直して検証し**、要求どおりでなければ同じ編集区間の中で作った側を消して`MASK_FAILED`にする。後片付けはホストが返したエイリアス（複製側）とPNGのファイル名（背景側）による**指紋照合**で対象を引き直してから、1つの編集区間の中で行う。順序は**削除が先、ソロ解除が後**である。
- **エイリアス補助を新設した**（純関数・doctest対象）——`KeepOnlyFirstEffectBlock`（2つ目以降の効果ブロックを落とす）・`AppendEffectBlock`（白化効果のブロックを1つ足す）・`NormalizeAliasFrameBoundariesToRelative`（`frame=`の全値から先頭値を引いて相対化する。`get_object_alias`が絶対番号を返しても相対番号を返しても正しくなる）・`ClipAliasFrameBoundaries`（窓が部分フィルタより短いとき、中間点を保ったまま境界を窓長で切る）・`WhiteningEffectBlockLines`（白化効果のブロックそのものを返す）。
- **白化効果のバイト定数は「反転」効果＋「輝度反転」項目=1という仮の値**で、`native/src/alias_util.cpp`に`PROVISIONAL`ブロックとして書いてある。オーナーの実機採取（[`SDK_REFERENCE.md`](SDK_REFERENCE.md) §16 (k)〜(m)）が済むまでの暫定値であり、採取後に差し替える。
  - **【2026-09-15追記】この2行はもう暫定値ではない。** オーナーの実機採取（2026-09-14）で`effect.name=反転`／`輝度反転=1`が採取物と**完全に一致**したため、差し替えは不要だった。`alias_util.cpp`のブロックの見出しは`PROVISIONAL`から「実機採取済み（2026-09-14）」へ書き換えてあり、採取した`.object`の原文は[`SDK_REFERENCE.md`](SDK_REFERENCE.md) §16(k)に転記してある（(l)は実機で不要と確定、(m)は実行時のSDK読み出しの話なので手採取できないと確定）。
- **doctestは358 passed / 6 skipped・アサーション1,825件から、382 passed / 6 skipped・アサーション1,938件（24ケース増）へ拡張した**（0 failed）。新設は`test_alias_util.cpp`・`test_bridge_core.cpp`への追加分である。
- **敵対的レビューでMajorの指摘を5件受け、いずれも修正した** — 窓の長さ計算の整数オーバーフロー（64bit演算へ変更）、下限値検査の欠落、要求したレイヤーと実際に作られたレイヤーの不一致検査の欠落、背景オブジェクトの同一性判定が厳しすぎた点、ソロ表示が実際には切り替わっていなかった点である。

### 116.4 操作パネル側 — Editの「Inpainting」サブタブを、初めて2回の右クリックで満たす

- **`EditSubMode`に`"inpainting"`を足し、`MockSubTabId`を消した** — Toolbox昇格（§115.4）と同じ型で、常時無効のモックだったサブタブが実サブタブになった。
- **モジュール単一の保管庫`timeline/inpaintSlots.ts`を新設した** — 部分フィルタ・対象動画それぞれのスナップショットとバージョンカウンタ、サブタブ切替の要求、アップロード開始済みの目印、busyの各状態を持つ。
- **2つの右クリック経路（`inpaintPartialFilter`／`inpaintVideo`）はEdit画面を作り直さない。** `shell/AppShell.tsx`の早期分岐は、保管庫へ内容をpublishして`setMode("edit")`を呼ぶだけで、他の右クリック経路のように`remountTokens`を進めない——タブは常時マウントされているので画面を出すのはモード切替だけで足り、サブタブの自動選択は保管庫の`subTabRequest`を`EditScreen`が購読する形で行う。**この設計にした理由は、本機能が2回の右クリックを要求する初めての機能だから**である。2回の右クリックの間にリマウントを挟むと、画面自身が持つ状態（フレーム数欄の値・seed・進行中のアップロード・マスクの進捗）がそのたびに失われる。保管庫方式にしたことで、「どちらの順序で右クリックしても、2回目で1回目の内容が消えない」（実機ゲートG2）が成り立ち、対象動画の**二重アップロードも起きない**。
- **`useInpaintForm.ts`**（手本は`useRetakeForm.ts`）— マウント時に`getEditInfo`を1回だけ呼ぶ。対象動画のアップロードは対象の識別子が変わったときだけ動く反応的な一発物で、二重には走らない。フレーム数欄の既定値は、部分フィルタの保管庫バージョンが変わったとき（＝新しい部分フィルタが届いたとき）だけ再シードする。窓は`placeInpaintWindow`と`mapRangeToMaterialSec`で毎描画導出し、**`window_start_sec`は「窓の開始秒からトリム起点を引いた値」**（撮り直しと同じ式）。Generateを止める理由は9種（部分フィルタ無し・対象無し・アップロード中・アップロード失敗・トリム失敗・解像度不明・解像度不一致・窓が収まらない・マスク描画中）。`buildRequest`は`loras`に`in-outpainting`を先頭で装着し、`width`/`height`は素材寸を128の倍数へ切り上げた値、`inpaint`ブロックに`mask_video_id`と`window_start_sec`を積む。
  - **【2026-09-15追記】`inpaint`ブロックはこの2つだけではなくなった。** ゲート後の追加注文で`blend_dilation_stage1`／`blend_dilation_stage2`（マスク周囲の「のりしろ」。0〜15・既定5／2）が加わり、**既定のままでも常に送る**——画面に出ている数字と送る値が食い違う経路を作らないためで、加速系フィールドの「既定なら鍵ごと出さない」規約とは逆向きである。契約は[`API_REFERENCE.md`](API_REFERENCE.md) §5.1の**InpaintSpec**。
- **`InpaintingPanel.tsx`**（新規）— 対象動画カードと部分フィルタカードの2枚、Single生成と共通の`DurationField`（快適上限マーカーつき）、操作できないのりしろのモック区画、注意文4行、進捗1行を並べる。
  - **【2026-09-15追記】ゲート後の改修で、この並びは3か所変わった。** ①**のりしろは2区画になった**——「マスク周囲の「のりしろ」」（Stage-1／Stage-2の数値欄。**実際に効く**）が上に入り、モックのままなのは下の「時間軸の「のりしろ」の有無」だけである。②**注意文は4行から3行になった**（重複していた1行を削除）。③**マスクを作れなかったときの断りは、パネル内の行ではなくトーストで出す**。あわせて文言も直した——対象動画が未入力のときのGenerate不可理由は「Inpaintする動画をタイムラインの右クリックから入力してください。」、部分フィルタが消えているときは「部分フィルタが見つかりません。」、フレーム数が部分フィルタより短いときの注意は「部分フィルタよりも短いフレーム数が指定されています。動画の末尾側はInpaintされません。」である。のりしろ欄の下の注記は`Stage-2 × キャンバス（素材寸を128の倍数へ切り上げた値）の長辺 ÷ 64 ＋ 18px`を表示し、長辺が分からないあいだは数字を出さない文面へ切り替わる。
- **`shell/featureScope.ts`**に`FEATURE_UI.inpaint`と`editSubTab.inpainting`を追加し、`CONTAINER_TARGETS.whenAll`が見る機能数が3つになった——LTX 2.5を読み込んでいるあいだ、このサブタブを灰色にする経路である。
- **`bridge/types.ts`を契約v13へ、`mockBridge.ts`にハンドラを追加**。i18n文字列は英日両方に追加した。
- **vitestは141ファイル・2,830件から146ファイル・2,915件へ拡張し全緑**（`npm run typecheck`エラー0、`npm run lint`警告31本は不変）。（最終値は §116.5 の 2,924 件・doctest 384 件）
- **敵対的レビューでMajorの指摘を3件受けたが、いずれも「作り直さない」設計そのもので解消した** — 快適上限マーカーが再マウントのたびに消える、対象動画の二重アップロードが再マウントで起きる、利用者が編集したフレーム数欄やseedが再マウントで失われる、の3点である。あわせて、**マスク描画が進行中の右クリックには席を追加せず案内だけを出す**busyガードを足した。

### 116.5 完結（実機ゲート全合格・2026-09-15）
...
## 118. バッチパネルへi2vモードを追加した — 音声フォルダ不要、画像1枚につき動画を1本（バックエンド台帳[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-152）（2026-09-15〜16、クローズ済み）

### 118.1 結論

Create画面のバッチパネル（従来「バッチA2V」）に、音声フォルダを指定せず画像フォルダだけを指定したときの動作を追加した。スキャン時に**音声フォルダの有無だけ**でモードを決める——音声フォルダがあれば従来どおりのa2v（音声ファイル1本＝1行、`POST /generate/chain`）、無く画像フォルダだけがあればi2v（画像1本＝1行、`POST /generate`）。決めたモードは走行スナップショットへ凍結し、あとから設定パネルの中身が変わっても揺れない。あわせて、行画像の生成強度の規則をa2v・i2v共通の1つへ揃えた。設計の全体像・決定事項・理由は[`BATCH_A2V_I2V_MODE.md`](BATCH_A2V_I2V_MODE.md)が正本で、本節には書き写さない。

### 118.2 機械検証

vitest **147ファイル・2,974件**（`npx vitest run --exclude '**/backend.integration.test.ts'`。ベースライン146ファイル・2,925件からの+49件は新規機能テスト48件＋レビュー後追加1件）、`npm run typecheck`エラー0、lint警告31本（不変）。実機ゲートG1〜G12の結果の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §108（本節には書き写さない）。

### 118.3 引っかかりやすい点（次に触る人向け）

- **`BatchRow.wav`は、i2vモードでは画像ファイル名を保持する。** フィールド名は`wav`のままだが、i2v行では**スキャン元の画像名**が入っており、出力ファイル名（`{wav}`の語幹＋`.mp4`）も「処理中: ○○」の表示も、この`wav`の値から作られる。`image`列は利用者がSharedへ切り替えられるため、行の同一性の由来を`image`に持たせると「Sharedにした行の出力名が決まらない」という例外が生まれる——`wav`に出自を持たせているのはその例外を作らないためである。
- **行画像の強度は、Create画面冒頭のキーフレームカードの値を見る。** 具体的には`params.sharedConditioningImages?.[0]?.strength`で、readyなカードが無ければ既定0.8（`DEFAULT_STRENGTH`）。この値は**a2v・i2vの両モードで共通**であり、Sharedを選んだ行がカードの値をそのまま使うのと同じ規則になった。**従来のバッチa2vは行画像の強度が1.0固定だったが、この変更でそうではなくなった。**
- **モードはスキャン時に決まり、走行開始まで凍結される。** `start()`はフォルダの現在値からモードを再判定せず、直前のスキャンが確定した`scannedMode`をそのまま使う。フォルダを変えても、モードが黙って切り替わることはない（次項のとおり、フォルダを変えた時点で行自体がクリアされるため）。
- **音声フォルダ・画像フォルダのどちらを変えても、行とスキャン由来の状態は一括で消える。** 画像フォルダも行の生成元になった以上、片方だけを特別扱いする理由が無くなったための規則で、a2v側もこれまでよりわずかに神経質になった（画像フォルダを変えただけで、既にスキャン済みだった音声行も消える）。
...
> **追記（2026-09-18）**: 本節が「コメントにもホストの版名を書かない」と言い切っているのは、実際にやったことより強い。**版名が消えたのは実行時の2本の警告文である。** `REALDEVICE-VERIFY`のコメントのほうは版名を消したのではなく、**版名を唯一の根拠にするのをやめた**——確認した日付（beta52で2026-07、現行ホストで2026-09-04）と、実測の正本であるバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §97への参照を併記する形に改めたものである。`native/src/plugin.cpp`の別の`REALDEVICE-VERIFY`注記や、`native/src/bridge.cpp`・`native/src/webview_host.cpp`のコメントには、**当時そのホストで何を観測したかの記録として、beta52という版名がそのまま残っている**（記録は失効しないので外していない）。規則として読むなら「実行時に出る文言には版名を書かない。コメントで版名に触れるなら、日付と正本の節を添えて記録の形にする」である。**本文は追記専用の規律どおり不変**である。
...
## 123. バッチの行に書いた`<lora:>`タグが効くようになり、Settingsの快適上限表が4列になった — 合成してから1回だけ解析する・表は形だけ持ち数字は配信値から描く（オーナー依頼／バックエンド台帳 §3-1 の1箇条。2026-09-17）
...
- Settingsの快適フレーム上限の一覧表を、レガシー表1列から**「2.3 既定」「2.3 全on」「2.5」「2.5 Q6」の4列**にした。**この表が持っているのは「形」（どの解像度を行に並べ、どの出典を列にするか）だけで、数字はサーバーが配る`limits`から描く。**
  ...
  > **追記（2026-09-17）**: 2列目の見出しは同日中に「2.3 全on(Q4, Q6)」へ改めた。並走していた較正（バックエンド`COMFORT_LIMIT_TABLE.md` §11）で、LTX 2.3 は Q6_K の派生モデルでも線が動かないと分かったためで、列は足していない。表の下の「全on」の注記に、その根拠と、同じ実測で1280×768の361フレームだけ退避が出たことを1文ずつ足した。**本文は追記専用の規律どおり不変**である。

  > **追記（2026-09-18）**: 表の下の注記は、この追記で足した2文も含めて、同日のオーナー裁定ですべて撤去した（§123.3の追記）。
...
- **これまでのバッチはカードのプロンプトだけを解析していた。** 走行開始の時点で`loras`を凍結し、行の文字列はそのあと本文として足すだけだったので、行に書いたタグはただの文字列として送られていた（台帳§3-1の1箇条）。
- 直し方として「行を先に解析する」ではなく、**合成してから解析する**を選んだ。Create画面は「プロンプト欄の文字列を丸ごと解析する」という作りなので、バッチも**実際に送る文字列そのもの**を解析すれば、同じ入力から同じ`loras`が出る。**行ごとの例外規則をどこにも置かずに済む**のが、この順序の利点である。
- 規則の帰結は、**Add＝カードと行のタグの和集合（同じ名前は行に書いた強度が勝ち、並びは先に現れたほうの位置を保つ）／Replace＝行のタグだけ（行が空ならカードのタグ）**になる。モード別の分岐はコードに1行も書いていない——合成規則がそのまま答えを決めるからである。
- **重複の解消だけは別に足した。** タグ解析器は見つけたタグを順に並べるだけで**同名を解消しない**（Create・Chainは、制御LoRAとの結合を担う`combineLoras`の内側で解消していた）。そこで解消部を`dedupeLoraSpecs`として切り出し、バッチの合成経路も同じものを通すようにした。同じ名前を2件送るとサーバーが断るので、これは省けない。
- **挙動が変わるのは1点だけである**: タグを1つでも取り除いたとき、**空白の畳み込みが合成後の文字列全体に及ぶ**（従来はカード側の文字列にだけ掛かっていた）。タグが1つも書かれていなければ、送信本文は以前とバイト単位で同じである。
- **引っかかりやすい点**: 行に**制御アダプタ（IC-LoRA）の名前**を書くと、Create画面が持っている制御アダプタ用の選択UIと検査を通らず、書いたままAPIへ送られる。サーバーは名前で受け付けるので拒否はされないが、参照動画の設定と食い違う組み合わせを作れてしまう。行に書くのはスタイルLoRAに留めること（この注意は正本の`BATCH_A2V_CSV_SPEC.md` §8にも書いた）。
- テストは`batchRunner`側を増やし、**a2v行とi2v行の両方**で「和集合になる」「同名は行が勝つ」「Replaceは行のタグだけ」「Replaceで行が空ならカードのタグ」「タグが無ければキーそのものが出ない」を固定した。i2v行の送信本文がCreate画面の単発生成とバイト一致することを確かめる既存テストのヘルパも、新しい前処理へ揃えてある。**Chain画面のバッチi2v-longは別実装で、今回は変えていない**（合成のコメントが「A2Vも解析しない」を根拠にしていたのを、現行の事実へ直しただけである）。

### 123.3 Settingsの快適上限表 — 形だけを持ち、数字は配信値から描く

- **静的な表にしなかった理由は1つである。** レガシー表`limits.spill_free_frames`の実体はgit追跡外の`config.yaml`にあり、数値の正本であるバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md)が「この表の値をここ以外の文書へ書き写さないこと」と明記している。**写しを作らずに表を出す方法は、配信値を描き続けることだけ**である。
- そこで新設したモジュールには**行（解像度）と列（出典）の宣言だけ**を置き、1セルの値は`legacy`（レガシー表をその解像度の綴りで引く）・`budget`（配信されるトークン予算をフレーム数へ換算する）・`static`（較正済みだが配信されていない実測点）の3分岐で解く。`budget`の換算はCreateの快適上限マーカーと**同じ関数**を呼ぶ——同じ線であることが、この表が目安として意味を持つ根拠である。
- **唯一ハードコードしたのが「2.5 Q6」列**で、サーバーはtransformerの量子化を知らないため、この実測点を配る経路がそもそも存在しない。出典（`COMFORT_LIMIT_TABLE.md` §10）は定数のコメントに残し、正本の側にも「写しはここ1箇所」と書き添えた。載っていない解像度は「—」になる。
- **`budget`の行選びは、配信された`rows[]`の先頭を取るのではなく`resolveComfortRow`を通す。** 各行は`requires`（どの高速化設定のときに当たる行か）つきで並んでいて、**先頭が必ず全on行だとは限らない**。同関数は5項目を全onにした設定で`requires`を突き合わせて行を選び、係数の正規化と「その系統の行が無い」ときの`null`もまとめて引き受ける。生の`rows[]`を自前で走査してよい場所はここ1箇所という既存の約束にも合う。
- **表はアクティブなベースモデルの系統で選ぶ。** 対応表では`ltx`・`ltx25`のどちらも同じ1枚を指す（列に並べて比べるための表なので、載っているモデルが読み手に分かればよい）。対応に無い系統と、`GET /models`がまだ返っていない・オフラインを表す空文字のときは、**節ごと描かない**——どの系統のものとも言えない表を出すよりはよい、という判断である。
- **将来、別系統のエンジンが増えたときにやることは2つだけである**: その系統の表を1枚足し、系統→表の対応に1行足す。それまでは節が出ないので、誤った線を目安として見せてしまうことがない。
- 見た目は横スクロールするラッパに入れた。Settingsのパネルは幅が決まっているので、4列の表をそのまま置くとパネルごと広がる。ラッパの中だけが横に流れる形にしたので、**パネルの幅も縦スクロールも以前のままである。** 注記は表の下に4行——単発生成の目安であること、「全on」の中身、「2.5 Q6」は派生モデル1本の実測であること、「—」は測っていないことである。

  > **追記（2026-09-18）**: 表の下の注記4行は、実機ゲート後のオーナー裁定で撤去した（説明が冗長で、文書を読めば分かる内容の重複であるため）。表の上の1行の説明と表そのものは残している。「全on」の定義・「2.5 Q6」が製品の配る値でないこと・1280×768の361フレームの孤立した退避の断りは、バックエンド`COMFORT_LIMIT_TABLE.md` §1.1・§10・§11が正本である。**本文は追記専用の規律どおり不変**である。
- テストは、系統から表が引けること・3つの出典それぞれで値が出る／出ないこと・全on行から換算した値・どの行も列数が一致することを固定し、Settings側には「系統が空なら節そのものが無い」「LTX 2.5なら表と横スクロールのラッパがある」を足した。
...
### 124.2 Toolboxのサブタブと「mp4 info」

- **サブタブはEditタブと同じ型にした**（`modes/toolbox/ToolboxSubTabs.tsx`）。2つのパネルは常に読み込んだまま`hidden`属性で隠すだけなので、切り替えても道具の状態は消えず、走っている追尾も止まらない。サブパネルに`role="tabpanel"`を付けないのもEditと同じ理由である（アプリ全体のテストが「見えているタブの中身」を単数で取るため）。見た目のCSSはEditのものを参照せず、`ToolboxScreen.css`に自分の名前で写した——他のタブのCSS名に依存しないためである。
- **右クリックから追尾を始めると、Toolboxの画面が作り直されてサブタブは「Tracking」へ戻る。** その操作が必要とするタブなので、これは意図どおりである。
- **「mp4 info」が送るのはファイルのパスだけで、中身は送らない**（`modes/toolbox/Mp4InfoSection.tsx`）。ドロップ（`shell/useFileDrop.ts`、受け付ける拡張子は mp4・mov・webm・mkv）とファイル選択（`ui.pickFile`）のどちらも、得たパスを`POST /utils/mp4-info`へ渡すだけである。**したがってバックエンドが同じPCで動いているときにしか使えない**——入口の側も、このPC以外からの要求を断る。
- 右側の欄は**文字列を整形せずにそのまま出す**。バックエンドが書き込む時点で既に字下げ済みだからである。**コピー専用のボタンは置いていない**（欄の文字を選択してコピーする。A1111と同じで、オーナー裁定）。
- 表示の状態は「未選択」「読み取り中」「生成条件が見つかりませんでした」「読めませんでした」の4つである。入口のエラー3つのうち`MEDIA_NOT_FOUND`・`MEDIA_UNREADABLE`には日本語・英語の文を用意し、`LOCAL_ONLY`とそれ以外はサーバーの文をそのまま添える。**遅れて返ってきた古いファイルの答えが、あとから選んだファイルの結果を上書きしないように**、最後に出した要求の答えだけを採る。
- モックのブリッジには分岐を1つ足した（パスに`nometa`を含めば「タグ無し」、それ以外は固定の見本 JSON）。

### 124.3 Settingsの出力設定 — 保存物は「鍵が無ければ既定」で読む

- **値は加速設定と同じ保存物（`localStorage`の`nzvideomni.acceleration`）に同居させた。** Settings画面の「ジョブごとに要求へ載せる設定」の置き場がこの1つだからで、ファイル先頭に「加速以外の出力設定も持つ」と注記した。
- **後方互換は「鍵が無ければ既定」の1規則で守る。** 以前のビルドが書いた保存物には`embedMp4Metadata`の鍵が無い。読み取り関数は項目ごとに型を確かめ、無い・壊れている鍵は既定（On）で埋める。過去に加速設定の項目を1つずつ足してきたときと同じ作法で、利用者が何もしなくても以前の設定は1つも失われない。
- **要求へ載せる型は加速設定と分けた。** 送出点の`accelerationRequestFields()`は、加速設定の型に`{ embed_mp4_metadata?: false }`を足した形を返す。加速設定そのものの型には混ぜていない——快適上限マーカーが加速設定と配信テーブルの行を突き合わせるとき、この項目には突き合わせる相手が無いからである。送出点が1つなので、単発・連結・Edit・バッチのすべての生成要求に、呼び出し側を1行も変えずに乗る。
- 画面上は、パネル内の他の真偽値と同じ「ON／OFF」の2ボタン行にした。置き場所は加速設定の群の直後、接続先URLの直前である。

### 124.4 ベースモデル選択 — 状態は1箇所に置き、作り直しは`key`で起こす

- **状態は`AppShell`の`useBaseModels()`1つだけである。** 画面左上の`<select>`を`shell/BaseModelSelect.tsx`へ切り出し、ヘッダーとSettingsが同じ部品を使う。Settings側は選択肢・現在値・切り替えの処理・「サーバーが使用中か」を**propsで受け取るだけ**で、自分では状態を持たず、ジョブの文脈も読まない。**同じものを2箇所に見せるときは、状態を複製せず、持ち主から配る**——こうすれば2つの表示が食い違う経路がそもそも無い。
- **切り替えが終わったら「モデル」区画を作り直す。** この区画は`GET /models`を読み込み時にしか読まないので、そのままだと切り替え前のベースモデルのカテゴリと重みが残り、そのまま「読み込み」を押すと誤った組み合わせを送ってしまう。そこで区画に**読み込み済みのベースモデルのID**を`key`として渡した。IDが変わるとReactが区画を作り直し、新しいベースモデルの一覧を読み直す。`key`に使うのは「選ばれている値」ではなく「読み込みが終わった値」である——読み込み中に作り直すと、まだ古いベースモデルの一覧を読んでしまうからである。そのために`useBaseModels()`に読み込み済みのIDを返す`active`を足した。
- サーバーが使用中の間は、ヘッダーと同じくSettingsの選択も押せない。

### 124.5 引っかかりやすい点（次に触る人向け）

- **Settingsの選択はヘッダーと同じ`aria-label`を持つ。** テストで取るときは、Settingsのダイアログの内側に範囲を絞ること（`within(dialog)`）。
- **加速設定の型に項目を足すと、全項目をリテラルで組んでいるテストが型検査で一斉に落ちる**（今回は約10ファイル）。落ちた箇所を直すのが正しい手順で、型をゆるめて黙らせてはいけない。型検査は必ず`npm run typecheck`で行う（`npx tsc --noEmit`は偽の合格を出す）。
- **Toolboxに道具を足すときは、サブタブのIDを1つと、隠すパネルを1つ足す**だけでよい。道具ごとの状態は各道具の部品とフックが持ち、画面側が持つのは「どのサブタブを見せているか」だけである。
...
## 125. ChainedとRetakeのStage-2の窓が15段になり、ラベルに目安解像度が付いた — 数値は1つの関数で作り文言はi18nに置く・上限は式だけで決める（バックエンド台帳[`PENDING_TASKS_CLOSED.md`](../../../Docs/PENDING_TASKS_CLOSED.md) §3-165。2026-09-25、オーナー実機目視合格・完結）

### 125.1 結論

- **ChainedタブとEditタブのRetakeで、Stage-2（第2段・アップスケール工程）の窓（潜在クリップ）を15段から選べるようになりました。** 既存の`standard`（潜在22フレーム）・`high_resolution`（潜在19）に、`w25`〜`w61`（25〜61潜在フレームの3刻み13段）が加わっています。`full_length`はA2V専用なので、どちらのフォームにも出しません。
- **ラベルは「46f（LTX 2.5 1280×768）」の書式です。** 潜在フレーム数、読み込み中のエンジン名、そのエンジンの快適上限に収まる16:9の目安解像度を並べます。既存の2段もこの書式に揃えました。
- **Retakeの窓の上限は、窓の式（8×潜在フレーム数−7）だけで決まるようになりました。** `w61`では481フレームまで選べます。
- 裁定・機械ゲートの実数・較正・申し送りはバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116が正本で、窓ごとの目安解像度は同[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) 第12節が正本です。本書には書き写しません。

### 125.2 窓の写しとラベル — 数値は1つの関数、文言はi18n

- **窓の写しは`shell/tokenBudget.ts`の1か所です。** 型`Stage2Window`・表`STAGE2_WINDOW_PRESETS`・フォームに出す並び`STAGE2_WINDOW_OPTIONS`（15値）を置き、ChainedとRetakeの`<select>`はどちらもこの並びを`map`で描きます。選択肢を足すときに触るのはこのファイルと`api/types.ts`の送信型だけです。
- **ラベルは`stage2WindowOptionLabel`が作ります。** 目安解像度は既存の`chainComfortSize16x9`で求め、テンプレートの`{frames}`・`{engine}`・`{width}`・`{height}`を置き換えます。**文言はi18nの`optionTemplate`1本だけ**（ja「{frames}f（{engine} {width}×{height}）」、en「{frames}f ({engine} {width}×{height})」）で、数値の計算は関数の側にしかありません。目安解像度が0になる窓は「61f」だけ、エンジン名がまだ無い（最初の`GET /models`の前）ときは`{engine} `を省きます。
- **エンジン名は`/models`の`base_models[].display_name`をそのまま使います。** i18nに対応表は置いていません。`AppShell`が読み込み済みのベースモデルの表示名を引き、`ChainedScreen`と`EditScreen`へ`engineLabel`として渡します。
- **予算はChainedの快適上限マーカーと同じ行から取ります**（`resolveComfortRow`、一致する行が無ければ`chain_comfort_token_budget`）。刻みは、Chainedはフォームの刻み（参照動画ありなら128）、Retakeは64です。Retakeが行を引くために、`EditScreen`に`sageAvailable`を渡すようにしました。
- Gradioは同じ計算をPythonで写しています（`gradio_ui/comfort.py`）。同じ入力で同じ文字列になることを両側のテストで固定しています。

### 125.3 Chained — 超過警告は1文だけにした

- **「19に変えれば軽減」という固定の1文を削除しました**（i18nの`overBudgetShorterWindowHint`と、`standard`のときだけそれを出す分岐）。窓が15段になると「19に変えれば」は数ある選び方の1つでしかなく、狭い窓へ誘導する理由も無いからです。超過警告は`overBudgetWarning`の1文だけです。
- ヒント文は22と19の2択の説明をやめ、「クリップ長が長いほど継ぎ目が減るが、1回あたりの負荷が増える。ラベルの目安解像度を超えるとVRAM溢れで遅くなることがある」という一般的な説明にしました。V2Vの文脈フレームが窓に対して長すぎるときの文（`contextFramesTooLongForWindow`）は、「22に戻して」ではなく「より長いクリップ長を選ぶ」へ改めました。
- **快適上限マーカーは、どの窓でも式どおりに出ます**（既存の式が窓の潜在フレーム数を引数に取るので、コードの変更はありません）。

### 125.4 Retake — 上限は式だけ、アップサンプルは据え置き

- **上限は`retakeMaxWindowPx(窓の潜在フレーム数)`だけで決めます。** これまでは配信値`retake_window_max_frames`（169）との小さい方を取っていたので、広い窓を選んでも169で頭打ちになっていました。**配信値はフロントでは読まなくなりました**（型と既定値の169は、配信値の写しとして残してあります）。
- `timeline/retakeWindow.ts`の`RETAKE_WINDOW_MAX_PX`（169）は、既定の引数としてだけ使われる定数なので残しました。フォームは実際の上限を渡しています。
- **`chunked_upsample`は送らないまま**です。169以下の窓の出力を1バイトも変えないためです。そのぶん、481フレームの窓では一括アップサンプルになります。w61・481フレームで1回完走しました（peak 12,169 MB。バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.7）。出来はオーナーの目視で合格です（バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.7 の 5）。
- Retakeには快適上限マーカーも超過警告も足していません（裁定は選択肢とラベルまでです）。

### 125.5 引っかかりやすい点（次に触る人向け）

- **目安解像度は「その軸の上限」ではありません。** 旧テストは全窓で「高さから逆算した幅の上限＝目安の幅」の等号を断言していましたが、新しい窓の一部（w37・w40・w43・w49・w52・w55・w61）では成り立ちません（例: w61は1088×576ですが、576から逆算すると1152まで入ります）。断言は「逆算した上限≧目安の幅」に改めました。
- **i18nのキーを消すと、それを完全一致で引いているテストが落ちます。** 選択肢の文言を完全一致で比べていたテスト（`EditScreen.retake.test.tsx`など）は、15件の新しいラベルで取り直しました。
- 48fpsで奇数段の窓（w25・w31…w61）を選ぶと、多くの構成で止まります。ただしクリップ長の組み合わせによっては通り、そのときサーバーが受理するのは正しい挙動です（実例はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.7の6）。止まる構成では、音声つきならフロントの整合検査で、音声なしならサーバーの422で止まります。**新しい文言は足しておらず**、サーバーの文をそのまま出します（件数はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.4）。

### 125.6 状態

- **機械ゲート（型検査・vitest・lint）は緑です。** 実数の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.3です。
- **配布aux2のビルドと配置は済みです**（記録は同§116.3）。**オーナーの実機目視は合格です（2026-09-25）。** 目視の8項目と結果は同§116.7です。
...
## 126. Settingsの快適上限表にfp8の2列を暫定で足した — 2.5は固定の線を式で延ばし、2.3は実測の1点だけを置く（オーナー依頼。バックエンド台帳[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §1-31で配信に移すまでの暫定。2026-09-26、オーナー実機目視は未実施）

### 126.1 結論

- **Settingsの「快適上限の目安」表が6列になりました。** 並びは「2.3 既定｜2.3 fp8（既定）｜2.3 全on(Q4, Q6)｜2.5｜2.5 Q6｜2.5 fp8」です。新しいのは「2.3 fp8（既定）」と「2.5 fp8」の2列で、fp8 safetensors の transformer を選んだときの単発生成の快適上限を、解像度ごとに見られるようにしました。
- **「2.5 fp8」は、実測した3解像度を通る固定の線を、既存の式で6段すべての解像度へ延ばした目安です。** 「2.3 fp8（既定）」は1920×1088の実測1点だけで、他の解像度は「—」です。
- **この2列はフロントエンドの直書きで、暫定です。** サーバーはまだ重みの種別ごとの線を配っていません。配信値は変えておらず、台帳§1-31「快適上限のマニフェスト一本化」で配信に移したら、この直書きは消えます。
- 実測そのものの正本はバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) 第13節（較正の記録は同[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §119）です。**表に載せている線の値と実測点、表の実際の値の正本は本節です**（他の文書は本節を指します）。

### 126.2 何を変えたか

- **列の出典の種類を1つ足しました**（`shell/comfortDisplayTable.ts`）。これまでの`legacy`（レガシー表を引く）・`budget`（配信される線を式で延ばす）・`static`（実測点をそのまま置く）に、**`fixedBudget`（固定の線を式で各解像度へ延ばす）**が加わりました。解き方は`budget`と同じで、`resolveComfortRow`（全onの設定）から空間と時間の係数だけを借り、線だけ固定値を使います。その系統の行が無いときは`budget`と同じく「—」（`null`）です。
- **「2.5 fp8」列**（id `ltx25-fp8`）: `fixedBudget`・系統`ltx25`・線は定数`LTX25_FP8_SINGLE_BUDGET = 38760`（単発・全onのトークン予算）です。式は既存の`comfortFramesForBudget`（`floor(線÷マス数)`を潜在フレーム数nとし、`8(n−1)+1`で切り、下限9・上限は配信の`max_num_frames`）です。表の値は次のとおりです（上限481のとき）。

  | 解像度 | マス数 | 2.5 fp8 | 2.3 fp8（既定） |
  |---|---:|---:|---:|
  | 512×320 | 160 | 481（上限で切る） | — |
  | 960×576 | 540 | 481（上限で切る） | — |
  | 896×1152 | 1,008 | 297（実測と一致） | — |
  | 1280×768 | 960 | 313（実測と一致） | — |
  | 1920×1088 | 2,040 | 145（実測と一致） | 121（実測） |
  | 2560×1472 | 3,680 | 73 | — |

- **線を38,760にした理由**: 実測で2回とも快適だった上端は、1280×768が313フレーム（38,400トークン）、1920×1088が145フレーム（38,760）、896×1152が297フレーム（38,304）です。38,760は3つの中の最大値で、式に通すと3点をちょうど再現する唯一の値です。38,304では1280×768が305・1920×1088が137となり実測より低く出ます。39,424では321・305となり、測っていない段を快適と見せてしまいます。38,760は溢れた側の下端（40,320）より下にあります。
- **「2.3 fp8（既定）」列**（id `ltx-fp8-default`）: `static`で、定数`LTX_FP8_DEFAULT_FRAMES = { "1920x1088": 121 }`の1点だけです。サーバーの既定構成（加速はサーバー既定）での実測で、全onは測り切れていないため列名に「既定」を付けました。
- **文言**（`i18n/strings.ts`）: 列見出しのキー`comfortColumnLtxFp8Default`（ja「2.3 fp8（既定）」、en「2.3 fp8 (default)」）と`comfortColumnLtx25Fp8`（ja・en とも「2.5 fp8」）を足しました。表の上の説明`comfortHint`には、日本語と英語それぞれに「2.5 fp8 の列は、実測した3解像度（1280×768・1920×1088・896×1152）を通る線から式で他の解像度へ延ばした目安で、2.3 fp8 の列は1920×1088だけの実測（加速の設定はサーバー既定・全onは未計測）である」という趣旨の1文を足しています。
- **見た目**（`shell/SettingsPanel.css`）: `.comfort-table`の`min-width`を440pxから560pxにしました。6列で列が詰まらないためです。横スクロールのラッパの仕組みは§123のままです。
- **コメントも現行化しました。** ファイル冒頭の方針（「数字はここに書かない・例外は`static`」）と出典の種類の説明に`fixedBudget`を1行ずつ足し、`LTX25_Q6_FRAMES`の「この表で唯一ハードコードしてよい数字」は「配信されるまでの暫定として直書きしてよい数字（fp8の2列も同じ扱い・§1-31で配信に移す）」に改めました。
- **テスト**: `shell/comfortDisplayTable.test.ts`に5件を足しました——列が6本で新しい2列のidがあること／2.5 fp8の313・145・297・73と、512×320・960×576が上限で切れること／2.3 fp8（既定）は1920×1088が121で他の5行は`null`であること／`comfort_budgets: {}`のときと`rows: []`のときに`fixedBudget`が`null`になること。既存の「2.5 Q6」の検査は列をidで探す形に変え、分岐の数を数えるコメントを4つに直しました。`shell/SettingsPanel.test.tsx`には列見出し「2.5 fp8」「2.3 fp8 (default)」があることを足しました。
- **触っていないもの**: バックエンド・配信値・`featureScope.ts`・Create／Chained／Retakeの快適上限マーカーの計算・Gradioです。

### 126.3 なぜこの形か

- **2.3 fp8を線で延ばさなかったのは、既定構成が1本の線で表せないからです。** この列の実測はサーバーの既定構成（sdpa・常駐なし・PrunaVAEDなし）で、その構成はVAEデコーダの分割数の段差のせいでトークン数に対して単調にならない、とバックエンド[`COMFORT_LIMIT_TABLE.md`](../../../Docs/COMFORT_LIMIT_TABLE.md) 第8節が明記しています。コードの側でも`shell/comfortTable.ts`は、同じ理由で既定構成の配信行をわざと置いていません。1点から線を引いて他の解像度へ延ばすと、根拠の無い数字を見せることになります（計画段階の敵対的レビューの主要指摘1件をそのまま採りました）。
- **2.5 fp8を線で延ばしたのは、3幾何の快適側の上端がほぼ同じ高さにそろい、単一の値で表せる形だったからです**（同第13.1節）。全onの構成なので、既定構成のような段差の問題もありません。
- **直書きを増やしたのは暫定だからです。** サーバーは読み込んだ重みの種別（GGUFの量子化・fp8）を名乗っておらず、種別ごとの線を配る経路がまだありません。既存の「2.5 Q6」列と同じ性格の直書きで、台帳§1-31で種別ごとの行を配信するときに、3つの直書き（Q6の3点・fp8の2列）をまとめて吸収します。文書には線の値を書き写さず、本節を指す形にしました。

### 126.4 引っかかりやすい点（次に触る人向け）

- **`static`の列が2本になったので、列は`kind`ではなくidで探してください。** 以前のテストは「`static`の列はQ6だけ」という前提で`find((c) => c.source.kind === "static")`としていましたが、いまは「2.3 fp8（既定）」が先に当たります。`find((c) => c.id === "ltx25-q6")`のように、idで探すのが正しい書き方です。
- **「配信が無い」ときの試験は、`comfort_budgets: {}`か`rows: []`で書いてください。** `comfort_budgets`を`undefined`にすると、`resolveComfortRow`は古いサーバー向けの互換の行を返すので、`fixedBudget`の列にも値が出てしまいます。既存の`budget`列の試験と同じ書き方です。
- **表は単発生成の目安です。** 2.5 fp8の連結（窓22）の快適側は別の値（同第13.1節）ですが、この表は単発だけなので影響しません。

### 126.5 状態

- **機械ゲートは緑です（2026-09-26）。** `npm run typecheck`はエラー0、対象の4ファイル57件は合格です。全件（`npx vitest run --exclude "**/backend.integration.test.ts"`）は149ファイル3,057件中3,056件が合格で、残る1件（`App.trackRoute.test.tsx`）はCPUの負荷による時間切れで、単独で走らせ直すと合格しました。`npm run lint`はエラー0（既存の警告31本は不変）です。
- **配布aux2のビルドと配置は済みです。** `scripts/build.ps1 -Config Release -RunTests`が成功し（webuiの埋め込み・ネイティブdoctest 384件合格・aux2は1,460,736バイト）、`scripts/deploy.ps1`で実機のプラグインフォルダとリポジトリの配布コピー`AviUtl2-Plugin/NzVideomni.aux2`、言語ファイル2本を配置しました。記録はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §120です。
- **オーナーの実機目視は未実施です。** 項目は次の5つで、結果はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §120の欄に書きます。
  1. 表が6列で、「2.3 fp8（既定）」と「2.5 fp8」の列がある。
  2. 2.5 fp8は1280×768が313・1920×1088が145・896×1152が297。2.3 fp8（既定）は1920×1088が121で、他は「—」。
  3. 2.5 fp8の512×320と960×576は上限の481で切れている。
  4. 表の上の説明に、fp8についての1文が出る。
  5. 横スクロールで表が崩れない。
- コミットは dev に、コード（webui/src の5ファイル）・配布コピーのaux2・文書で行います。mainへのマージはオーナーの目視の後です。
