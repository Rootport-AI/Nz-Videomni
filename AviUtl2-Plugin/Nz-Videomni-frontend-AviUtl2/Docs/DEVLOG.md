...
**オーナーの実機ゲートR1〜R8は未了である**（結果は合格後にここへ追記する）。実機で見るには`build.ps1`→`deploy.ps1`での配置が要る（フロントエンドの配置先は実機と配布コピーの2箇所）。台帳の記録はバックエンド[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §3-140で、**クローズは実機ゲート合格後**である。

**状態**: 実装コミット済み（S1＝`78df6e1`〔純関数＋doctest〕／S2＝`36cd55e`〔挿入経路の差し替え〕）、本節を含む文書更新はS3のコミット。実機ゲート待ち。
