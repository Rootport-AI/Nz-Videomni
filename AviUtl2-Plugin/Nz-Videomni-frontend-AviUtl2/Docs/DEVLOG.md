...
- 見た目は横スクロールするラッパに入れた。Settingsのパネルは幅が決まっているので、4列の表をそのまま置くとパネルごと広がる。ラッパの中だけが横に流れる形にしたので、**パネルの幅も縦スクロールも以前のままである。** 注記は表の下に4行——単発生成の目安であること、「全on」の中身、「2.5 Q6」は派生モデル1本の実測であること、「—」は測っていないことである。
  ...
  > **追記（2026-09-18）**: 表の下の注記4行は、実機ゲート後のオーナー裁定で撤去した（説明が冗長で、文書を読めば分かる内容の重複であるため）。表の上の1行の説明と表そのものは残している。「全on」の定義・「2.5 Q6」が製品の配る値でないこと・1280×768の361フレームの孤立した退避の断りは、バックエンド`COMFORT_LIMIT_TABLE.md` §10・§11が正本である。**本文は追記専用の規律どおり不変**である。
