...
- **`chunked_upsample`は送らないまま**です。169以下の窓の出力を1バイトも変えないためです。そのぶん、481フレームの窓では一括アップサンプルになります。w61・481フレームで1回完走しました（peak 12,169 MB。バックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.7）。出来の目視は未実施です。
