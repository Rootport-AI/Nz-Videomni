...
**状態**: 追補の実装コミット済み（S1＝`9537855`〔`frame=`ヘッダの包含化＋テスト〕）。オーナーの実機ゲート（V-1〜V-3。**前提: プロジェクト24fps・生成fps24**）は未了。
