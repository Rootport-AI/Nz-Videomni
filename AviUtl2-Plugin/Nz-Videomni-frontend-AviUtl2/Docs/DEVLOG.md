...
## 125. ChainedとRetakeのStage-2の窓が15段になり、ラベルに目安解像度が付いた — 数値は1つの関数で作り文言はi18nに置く・上限は式だけで決める（バックエンド台帳[`PENDING_TASKS.md`](../../../Docs/PENDING_TASKS.md) §3-165。2026-09-25、オーナー実機目視待ち）
...
- 48fpsで奇数段の窓（w25・w31…w61）を選ぶと、多タイルの構成はほとんど通りません。音声つきならフロントの整合検査で、音声なしならサーバーの422で止まります。**新しい文言は足しておらず**、サーバーの文をそのまま出します（件数はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.4）。

### 125.6 状態

- **機械ゲート（型検査・vitest・lint）は緑です。** 実数の正本はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §116.3です。
- **配布aux2のビルドと配置は済みです**（記録は同§116.3）。**オーナーの実機目視は未実施です。** 目視の8項目と結果の欄は同§116.7です。
