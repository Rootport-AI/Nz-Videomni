...
## 114. キーフレーム画像の上限を10枚へ引き上げた — フロントエンドは配信値に追随しただけで済んだ（バックエンド台帳§1-28）（2026-09-08）
...
- **オーナーの実機ゲートは未実施である。** 手順と合格条件は[`REAL_BACKEND_CHECKLIST.md`](REAL_BACKEND_CHECKLIST.md) §4.14（G1〜G8）が正本で、**実行の前にバックエンドを作業ブランチ`feature/keyframe-limit-10`のツリーで起動し直すこと。** 古いサーバーのままだと6枚目以降が422で断られ、画面の不具合と読み違えてしまう。
- **配布`.aux2`の再ビルドとデプロイは、実機ゲートの前に行うこと。** G3が画面の見た目（カウンタ・カード一覧の縦長・ラベルの密度）を見る項目なので、古い配布物のままでは判定にならない。
