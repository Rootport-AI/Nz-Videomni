...
最終更新: 2026-09-04(§16 (h)を全面的に書き換え、**同日いったん「`OBJECT_LAYER_FRAME.end`は排他」と書いた判断を撤回して「包含と確定」へ訂正**した〔エイリアス`frame=a,b`の`b`も同じく包含。一次証拠は実機が書き出した`data\Alias`の4ファイルと`plugin.log`の5行で、本項へ転記してある〕。§3-140追補) / その前は 2026-09-04(§5.1のエイリアス書式を「XML形式」から「INI風」へ訂正し、§16へ実機確定知見(g)(h)〔`動画ファイル`の表示種別を決める`音声付き`キー〕を追加。バックエンド台帳§3-140) / 初版の出典: 調査エージェント報告(2026-07-07)。以降の追記はいずれも節ごとに日付を添えてある。
