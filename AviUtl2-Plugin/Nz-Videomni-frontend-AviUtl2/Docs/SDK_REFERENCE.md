...
最終更新: 2026-09-14(§16の(k)〜(m)をオーナーの実機採取と実機ゲートの結果で確定させた。(k)は採取した`.object`2本の原文を転記して白化効果の効果名・項目名を確定。(l)は実機で不要と確定。(m)は実行時のSDK読み出しの話なので手採取できないことを明記し、採取物は(k)の見本として使う扱いにした。あわせて`native/src/alias_util.cpp`の「PROVISIONAL」ブロックの見出しを「実機採取済み」へ書き換えた) / その前は 2026-09-14(§16に(k)〜(m)を追加。Inpainting〔台帳§3-55〕のマスク描画がプラグイン側で前提にしている3点を「実機採取待ち」として起票。採取後はここへ原文を転記し、`native/src/alias_util.cpp`の仮定数を差し替える) / その前は 2026-09-11(§16に(j)を追加。部分フィルタの中間点つきエイリアスの実機採取物2本〔`部分フィルタ_規定値.object`・`部分フィルタ_中間点2つ.object`〕を原文転記し、確定した規則を記載。(i)の一行を「採取済み」へ書き換え。バックエンド[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md) §5.7の該当箇所もあわせて更新) / その前は 2026-09-04(§16 (h)を全面的に書き換え、**同日いったん「`OBJECT_LAYER_FRAME.end`は排他」と書いた判断を撤回して「包含と確定」へ訂正**した〔エイリアス`frame=a,b`の`b`も同じく包含。一次証拠は実機が書き出した`data\Alias`の4ファイルと`plugin.log`の5行で、本項へ転記してある〕。§3-140追補) / 初版の出典: 調査エージェント報告(2026-07-07)。以降の追記はいずれも節ごとに日付を添えてある。
...
- **(k) 部分フィルタの中に足した「領域内を白にする効果」のエイリアス実書式が確定した（2026-09-14、オーナー実機採取。Inpainting、台帳§3-55）。** プラグインが仮の定数として書いていた`effect.name=反転`／`輝度反転=1`の2行は、採取物と完全に一致していた（書き方の変更は不要）。採取方法は(j)と同じ（右クリック→「エイリアスをファイルに保存」）。採取物は次の2本で、いずれも`D:\For_Videos\AviUtl2\aviutl2_v2.0.54\data\Alias\`配下、**UTF-8 BOM無し・CRLF**である。

  - `(k) 白化効果の実書式.object`（341バイト）
  - `(m) frame=の絶対・相対確認.object`（348バイト。中間点を2つ持たせたもので、**`frame=`行が`frame=0,75,146,224`である点だけが違い、`[Object.0]`以降は下記と1バイトも変わらない**）
  ...
  1. **効果名は「反転」**（`effect.name=反転`）。「単色化（白）」での採り直しは不要だった。
  2. **項目は5つで、`上下反転`・`左右反転`・`輝度反転`・`色相反転`・`透明度反転`の順に必ず全部書き出される。** いずれもチェックボックスで`=0`／`=1`の形を取る（(j)の規則4と同じで、既定値のままの項目も省略されない）。
  3. **白化に要るのは`輝度反転=1`の1行だけで、残りの4項目は既定値`0`のままでよい。** よってプラグインは今までどおり2行（`effect.name=反転`／`輝度反転=1`）だけを書き、残りは本体の既定に任せる（`BuildMediaObjectAlias`と同じ流儀）。定数の置き場は`native/src/alias_util.cpp`の該当ブロック（見出しは「実機採取済み（2026-09-14）」）で、上記6行は`native/tests/test_alias_util.cpp`に記録用のテストとして入れてある。
  4. **`[Object.1]`として`[Object.0]`（部分フィルタ本体）の後ろに並ぶ**——`AppendEffectBlock`が作る構造と同じである。

  設計の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §5.2。
- **(l) フィルタオブジェクト（部分フィルタ）が掛かる側のレイヤー番号 — 採取は不要と確定した（2026-09-14）。** **番号が若いレイヤーほど背後にあり、部分フィルタは背後（＝自分より若い番号）に掛かる**——オーナーの実機確認でこう決着したため、レイヤー3・5・7を使った採取は行わなかった。したがってプラグインの前提（黒い背景を`layer_max+1`、部分フィルタの複製を`layer_max+2`に置く）はそのまま正しい。`layer_max+2`への`create_object_from_alias`の自動確保と、`get_layer_enable`／`set_layer_enable`が`layer_max`を超える番号に効くことも、実機ゲートG6（マスク描画から生成まで完走した）の合格で裏づけられた。
- **(m) `get_object_alias`が返す`frame=`行が絶対番号か相対番号か — 手作業では採取できないと確定した（2026-09-14）。** これは**実行時のSDK読み出しがどちらの形で返すか**の話で、「エイリアスをファイルに保存」で採れるのは**本体が保存するときの形**（(j)のとおり絶対番号）だけである。両者は別物なので、手採取では決着しない。ただし**プラグインは先頭値を引いて相対へ正規化するので、どちらで返ってもマスクの複製は正しく作られる**（`NormalizeAliasFrameBoundariesToRelative`）。実機ゲートG6（追尾済みで枠が動く部分フィルタのInpaintingが完走し、描き替え領域が枠の動きに追従した）が、この正規化が実際に効いていることの裏づけである。この確認のために採ったファイル（`(m) frame=の絶対・相対確認.object`）は、中間点を2つ持つ白化済み部分フィルタの見本として(k)で使っている。
- **(n) WebView2 パネルをクリックしても、AviUtl2 の「作業中オブジェクト」は外れない（2026-09-16 実機確定）。** 操作パネル上のボタン押下を起点に `timeline.getSelection`（＝`get_focus_object`／`get_selected_object`）を叩いても、直前にタイムラインで選んだオブジェクトがそのまま返る。Toolbox タブの「🎯 追尾を開始」ボタンはこの性質に乗っている。採取の一次記録はバックエンド[`VERIFICATION_LOG.md`](../../../Docs/VERIFICATION_LOG.md) §110.5（G5）。設計の正本は同[`OBJECT_TRACKING_DESIGN.md`](../../../Docs/OBJECT_TRACKING_DESIGN.md) §3.4。
