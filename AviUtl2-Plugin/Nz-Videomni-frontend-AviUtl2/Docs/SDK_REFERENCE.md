...
- **(k) 部分フィルタの中に足した「領域内を白にする効果」のエイリアス実書式 — 【実機採取待ち】（Inpainting、台帳§3-55）。** 採取方法は(j)と同じ（右クリック→「エイリアスをファイルに保存」）。採るもの: 部分フィルタの中に「反転」を追加して「輝度反転」をオンにしたオブジェクトの`.object`原文（効果名・項目名・`[Object.1]`として並ぶ構造）。黒い背景の上でプレビューが**枠の内側だけ白・外側が黒**になること、ぼかしを上げると灰色の勾配になることも同時に目視する。反転で白にならなければ「単色化（白）」で同じ採取をやり直す。採取までは、プラグインは`effect.name=反転`／`輝度反転=1`の2行を仮の定数として書く（`native/src/alias_util.cpp`の"PROVISIONAL"ブロック。二次資料: AviUtl2向け解説で「反転」効果に「輝度反転」「色相反転」の項目があることは確認済み）。設計の正本はバックエンド[`INPAINTING_DESIGN.md`](../../../Docs/INPAINTING_DESIGN.md) §5.2。
- **(l) フィルタオブジェクト（部分フィルタ）が掛かる側のレイヤー番号 — 【実機採取待ち】。** 部分フィルタをレイヤー5に置き、色つきの図形をレイヤー3と7に置いて、どちらが変化するかを目視する。プラグインは「自分より番号の小さいレイヤーの合成結果に掛かる」と仮定し、黒い背景を`layer_max+1`、部分フィルタの複製を`layer_max+2`に置いている（逆なら`RenderMaskVideoWorker`の2つの名前つきローカルを入れ替えるだけ）。あわせて、`layer_max+2`への`create_object_from_alias`が自動確保されること、`get_layer_enable`／`set_layer_enable`が`layer_max`を超える番号に効くことも同じ機会に確かめる（§16(d)は`layer_max+1`までしか実証していない）。
- **(m) `get_object_alias`が返す`frame=`行が絶対番号か相対番号か — 【実機採取待ち】。** 中間点を2つ持つ部分フィルタで(k)の採取をやり直し、「読み出し→そのまま`create_object_from_alias`→再保存」の往復で`frame=a,b,c`行が保たれるかを確かめる。プラグインはどちらで返っても正しくなるよう、先頭値を引いて相対へ正規化してから複製を作る（`NormalizeAliasFrameBoundariesToRelative`）。
