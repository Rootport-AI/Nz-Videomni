...
const char kNamePrefix[] = "NzVideomni#";  // object_name = prefix + job_id
...
// Extract the job id embedded in an alias, if this is an NzVideomni provisional
// object. Returns true + *out on success. Two symmetric sources are tried:
//   1. the text-effect text item's trailing "[#<job_id>]" marker
//   2. the "NzVideomni#<job_id>" object_name token anywhere in the raw alias
...
bool AliasMatchesJob(const std::string& alias, const std::string& job_id) {
    ...
    // Fallback: the "NzVideomni#<job_id>" object_name token, if the scanner serialized
    // it into the alias. Delimit the end so "NzVideomni#abc" does not match "...abcd".
    const std::string src = StripUtf8Bom(alias);
...
std::vector<Reservation> DetectOrphans(
    ...
    for (const ScannedObject& o : scanned) {
        ...
        if (!ExtractJobId(o.alias, &jid)) {
            continue;  // foreign object (no NzVideomni marker) -> ignore
        }
