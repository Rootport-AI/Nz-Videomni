...
using nzvideomni::BackendRequest;
using nzvideomni::CaptureFrameRequest;
using nzvideomni::DownloadVideoRequest;
using nzvideomni::UploadFileRequest;
using nzvideomni::EditInfoResult;
using nzvideomni::HandleRequestJson;
using nzvideomni::InsertMediaParams;
using nzvideomni::InsertMediaResult;
using nzvideomni::MakeThumbnailRequest;
using nzvideomni::PickFileFilterEntry;
using nzvideomni::RequestContext;
...
RequestContext MakeContext(nzvideomni::EditInfoProvider edit,
...
TEST_CASE("ParseBackendRequest accepts a minimal GET") {
    ...
    CHECK(nzvideomni::ParseBackendRequest(params, &out, &err));
...
TEST_CASE("ParseBackendRequest rejects bad method / path / query / body") {
    std::string err;
    BackendRequest out;
    CHECK_FALSE(nzvideomni::ParseBackendRequest(
    ...
    CHECK_FALSE(nzvideomni::ParseBackendRequest(json::parse(R"({"path": "/x"})"), &out, &err));
}
...
TEST_CASE("ParseBackendRequest defaults timeoutMs to the built-in default") {
    ...
    const json params = json::parse(R"({"method": "GET", "path": "/status"})");
    CHECK(nzvideomni::ParseBackendRequest(params, &out, &err));
    CHECK(out.timeout_ms == nzvideomni::kDefaultBackendTimeoutMs);
}
...
TEST_CASE("BuildBackendUrl appends the already-prefixed path and query") {
    ...
    req.query = "a=1&b=2";
    CHECK(nzvideomni::BuildBackendUrl("http://127.0.0.1:18620", req) ==
...
TEST_CASE("MakeBackendRequestResult parses JSON bodies and passes through status") {
    json r = nzvideomni::MakeBackendRequestResult(202, R"({"job_id": "abc"})");
    CHECK(r["status"] == 202);
    CHECK(r["body"]["job_id"] == "abc");

    json empty = nzvideomni::MakeBackendRequestResult(204, "");
    CHECK(empty["status"] == 204);
    CHECK(empty["body"].is_null());

    json garbage = nzvideomni::MakeBackendRequestResult(500, "<<not json>>");
...
TEST_CASE("ParseDownloadVideo accepts a job id and derives path + file name") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseDownloadVideo(json::parse(R"({"jobId": "abc-123"})"), &out, &err));
    CHECK(out.job_id == "abc-123");
    CHECK(out.joined == false);
    CHECK(nzvideomni::DownloadVideoPath(out) == "/jobs/abc-123/video");
    CHECK(nzvideomni::DownloadFileName(out) == "abc-123.mp4");
}

TEST_CASE("ParseDownloadVideo honours joined=true") {
    DownloadVideoRequest out;
    std::string err;
    CHECK(nzvideomni::ParseDownloadVideo(json::parse(R"({"jobId": "j", "joined": true})"),
                                    &out, &err));
    CHECK(out.joined == true);
    CHECK(nzvideomni::DownloadVideoPath(out) == "/jobs/j/joined");
    CHECK(nzvideomni::DownloadFileName(out) == "j_joined.mp4");
}

TEST_CASE("ParseDownloadVideo rejects missing id and path-traversal ids") {
    DownloadVideoRequest out;
    std::string err;
    CHECK_FALSE(nzvideomni::ParseDownloadVideo(json::parse(R"({})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseDownloadVideo(json::parse(R"({"jobId": "../x"})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseDownloadVideo(json::parse(R"({"jobId": "a/b"})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseDownloadVideo(
...
TEST_CASE("ParseDownloadVideo accepts destDir/fileName/noClobber") {
    DownloadVideoRequest out;
    std::string err;
    CHECK(nzvideomni::ParseDownloadVideo(
...
TEST_CASE("ParseUploadFile accepts each valid kind and a file path") {
    for (const char* kind : {"image", "video", "audio"}) {
        ...
        CHECK(nzvideomni::ParseUploadFile(params, &out, &err));
        CHECK(out.kind == kind);
...
TEST_CASE("ParseUploadFile rejects bad kind / missing or empty file path") {
    UploadFileRequest out;
    std::string err;
    CHECK_FALSE(nzvideomni::ParseUploadFile(
        json::parse(R"({"kind": "thumbnail", "filePath": "a.png"})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseUploadFile(json::parse(R"({"filePath": "a.png"})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseUploadFile(json::parse(R"({"kind": "image"})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseUploadFile(
        json::parse(R"({"kind": "image", "filePath": ""})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseUploadFile(json::parse(R"([])"), &out, &err));
}
...
TEST_CASE("D2 a non-object query is rejected") {
    std::string err;
    std::string q;
    CHECK_FALSE(nzvideomni::BuildQueryString(json::parse(R"("a=b")"), &q, &err));
    CHECK(err == "'query' must be an object");
    CHECK(q.empty());
    CHECK_FALSE(nzvideomni::BuildQueryString(json::parse(R"([1,2])"), &q, &err));
    CHECK_FALSE(nzvideomni::BuildQueryString(json(nullptr), &q, &err));
...
TEST_CASE("D3 a non-string query value is rejected") {
    std::string err;
    std::string q;
    CHECK_FALSE(nzvideomni::BuildQueryString(json::parse(R"({"k": 1})"), &q, &err));
    CHECK(err == "'query' values must be strings");
    CHECK_FALSE(nzvideomni::BuildQueryString(json::parse(R"({"k": null})"), &q, &err));
    CHECK_FALSE(nzvideomni::BuildQueryString(json::parse(R"({"k": true})"), &q, &err));
...
TEST_CASE("D4 an upload without a query leaves UploadFileRequest.query empty") {
    ...
    CHECK(nzvideomni::AppendQueryToUrl(base, out.query) == base);
}

TEST_CASE("D5 BuildQueryString percent-encodes both keys and values") {
    std::string err;
    std::string q;
    CHECK(nzvideomni::BuildQueryString(json::parse(R"({"a b": "x/y"})"), &q, &err));
    CHECK(q == "a%20b=x%2Fy");
    CHECK(nzvideomni::BuildQueryString(json::parse(R"({"k": "v"})"), &q, &err));
    CHECK(q == "k=v");
    // Unreserved characters pass through untouched; '&' and '=' do not.
    CHECK(nzvideomni::BuildQueryString(json::parse(R"({"a-b_c.d~e": "1&2=3"})"), &q, &err));
    CHECK(q == "a-b_c.d~e=1%262%3D3");
    // An empty object yields an empty (URL-neutral) query.
    CHECK(nzvideomni::BuildQueryString(json::parse(R"({})"), &q, &err));
    CHECK(q.empty());
}
...
TEST_CASE("D6 a null 'query' is treated as no query at all") {
    ...
    std::string err;
    up.query = "stale=1";
    CHECK(nzvideomni::ParseUploadFile(
    ...
    CHECK(nzvideomni::BuildBackendUrl("http://h", req) == "http://h/x");
}
...
TEST_CASE("D8 AppendQueryToUrl returns the URL completely unchanged on an empty query") {
    ...
    for (const char* u : urls) {
        CHECK(nzvideomni::AppendQueryToUrl(u, "") == std::string(u));
    }
}

TEST_CASE("D9 AppendQueryToUrl joins with '?' and BuildQueryString joins with '&'") {
    CHECK(nzvideomni::AppendQueryToUrl("http://h/upload/video", "a=1") ==
          "http://h/upload/video?a=1");
    std::string err;
    std::string q;
    CHECK(nzvideomni::BuildQueryString(
    ...
    CHECK(nzvideomni::AppendQueryToUrl("http://h/api/v1/upload/video", q) ==
...
TEST_CASE("UploadPath maps a kind to its endpoint tail") {
    CHECK(nzvideomni::UploadPath("image") == "/upload/image");
    CHECK(nzvideomni::UploadPath("video") == "/upload/video");
    CHECK(nzvideomni::UploadPath("audio") == "/upload/audio");
}

TEST_CASE("FileNameFromPath returns the last component for both separators") {
    CHECK(nzvideomni::FileNameFromPath("C:\\a\\b\\frame_10.png") == "frame_10.png");
    CHECK(nzvideomni::FileNameFromPath("/var/tmp/clip.mp4") == "clip.mp4");
    CHECK(nzvideomni::FileNameFromPath("bare.wav") == "bare.wav");
    CHECK(nzvideomni::FileNameFromPath("").empty());
}

TEST_CASE("ContentTypeForExtension covers the v3 mapping (case-insensitive)") {
    CHECK(nzvideomni::ContentTypeForExtension("a.png") == "image/png");
    CHECK(nzvideomni::ContentTypeForExtension("a.PNG") == "image/png");
    CHECK(nzvideomni::ContentTypeForExtension("a.jpg") == "image/jpeg");
    CHECK(nzvideomni::ContentTypeForExtension("a.jpeg") == "image/jpeg");
    CHECK(nzvideomni::ContentTypeForExtension("a.webp") == "image/webp");
    CHECK(nzvideomni::ContentTypeForExtension("a.mp4") == "video/mp4");
    CHECK(nzvideomni::ContentTypeForExtension("a.mov") == "video/quicktime");
    CHECK(nzvideomni::ContentTypeForExtension("a.webm") == "video/webm");
    CHECK(nzvideomni::ContentTypeForExtension("a.mkv") == "video/x-matroska");
    CHECK(nzvideomni::ContentTypeForExtension("a.wav") == "audio/wav");
    CHECK(nzvideomni::ContentTypeForExtension("a.mp3") == "audio/mpeg");
    CHECK(nzvideomni::ContentTypeForExtension("a.m4a") == "audio/mp4");
    CHECK(nzvideomni::ContentTypeForExtension("a.aac") == "audio/aac");
    CHECK(nzvideomni::ContentTypeForExtension("a.flac") == "audio/flac");
    CHECK(nzvideomni::ContentTypeForExtension("a.ogg") == "audio/ogg");
    CHECK(nzvideomni::ContentTypeForExtension("a.bin") == "application/octet-stream");
    CHECK(nzvideomni::ContentTypeForExtension("noext") == "application/octet-stream");
    // A dot only in a directory component is not an extension.
    CHECK(nzvideomni::ContentTypeForExtension("C:\\a.dir\\file") ==
          "application/octet-stream");
}

TEST_CASE("BuildMultipartHeader/Footer produce the exact RFC-7578 framing") {
    const std::string boundary = "TestBoundary123";
    const std::string header = nzvideomni::BuildMultipartHeader(boundary, "file",
    ...
    CHECK(nzvideomni::BuildMultipartFooter(boundary) == "\r\n--TestBoundary123--\r\n");
    CHECK(nzvideomni::MultipartContentType(boundary) ==
...
TEST_CASE("ParseCaptureFrame treats an absent or empty params as no frame") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseCaptureFrame(json::object(), &out, &err));
    CHECK(out.has_frame == false);
    CHECK(nzvideomni::ParseCaptureFrame(json::parse("null"), &out, &err));
    CHECK(out.has_frame == false);
}
...
TEST_CASE("ParseCaptureFrame reads an explicit integer frame") {
    CaptureFrameRequest out;
    std::string err;
    CHECK(nzvideomni::ParseCaptureFrame(json::parse(R"({"frame": 42})"), &out, &err));
...
TEST_CASE("ParseCaptureFrame rejects a non-integer frame") {
    CaptureFrameRequest out;
    std::string err;
    CHECK_FALSE(nzvideomni::ParseCaptureFrame(json::parse(R"({"frame": 1.5})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseCaptureFrame(json::parse(R"({"frame": "10"})"), &out, &err));
}

TEST_CASE("CaptureFileName combines the frame number with a unique suffix") {
    CHECK(nzvideomni::CaptureFileName(10, "20260707_120000_001") ==
...
TEST_CASE("ParsePickFileKind accepts image/video/audio/imageOrVideo and rejects others") {
    ...
    for (const char* k : {"image", "video", "audio", "imageOrVideo"}) {
        ...
        CHECK(nzvideomni::ParsePickFileKind(params, &kind, &err));
        CHECK(kind == k);
    }
    CHECK_FALSE(nzvideomni::ParsePickFileKind(json::parse(R"({"kind": "document"})"),
                                         &kind, &err));
    CHECK_FALSE(nzvideomni::ParsePickFileKind(json::parse(R"({})"), &kind, &err));
    CHECK_FALSE(nzvideomni::ParsePickFileKind(json::parse(R"({"kind": 3})"), &kind, &err));
    CHECK_FALSE(nzvideomni::ParsePickFileKind(json::parse(R"([])"), &kind, &err));
}

TEST_CASE("PickFileFilter matches the backend allow-list and appends All files") {
    const std::vector<PickFileFilterEntry> img = nzvideomni::PickFileFilter("image");
    REQUIRE(img.size() == 2);
    ...
    const std::vector<PickFileFilterEntry> vid = nzvideomni::PickFileFilter("video");
    REQUIRE(vid.size() == 2);
    CHECK(vid[0].pattern == "*.mp4;*.mov;*.webm;*.mkv");
    CHECK(vid[1].pattern == "*.*");

    const std::vector<PickFileFilterEntry> aud = nzvideomni::PickFileFilter("audio");
    REQUIRE(aud.size() == 2);
    CHECK(aud[0].pattern == "*.wav;*.mp3;*.m4a;*.aac;*.flac;*.ogg");
    CHECK(aud[1].pattern == "*.*");

    // Unknown kind still yields the All-files catch-all only.
    const std::vector<PickFileFilterEntry> other = nzvideomni::PickFileFilter("other");
...
TEST_CASE("PickFileFilter for imageOrVideo leads with a combined filter, then image-only/video-only/All files") {
    const std::vector<PickFileFilterEntry> combo = nzvideomni::PickFileFilter("imageOrVideo");
...
TEST_CASE("ParseMakeThumbnail requires a filePath and defaults maxDim to 256") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseMakeThumbnail(json::parse(R"({"filePath": "C:\\a\\x.png"})"),
                                    &out, &err));
    CHECK(out.file_path == "C:\\a\\x.png");
    CHECK(out.max_dim == 256);

    CHECK_FALSE(nzvideomni::ParseMakeThumbnail(json::parse(R"({})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseMakeThumbnail(json::parse(R"({"filePath": ""})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseMakeThumbnail(json::parse(R"([])"), &out, &err));
}
...
TEST_CASE("ParseMakeThumbnail honours maxDim and clamps into [16, 1024]") {
    MakeThumbnailRequest out;
    std::string err;
    CHECK(nzvideomni::ParseMakeThumbnail(
    ...
    CHECK_FALSE(nzvideomni::ParseMakeThumbnail(
...
TEST_CASE("ComputeThumbnailSize preserves aspect ratio and never upscales") {
    ...
    nzvideomni::ComputeThumbnailSize(1920, 1080, 256, &w, &h);
    CHECK(w == 256);
    CHECK(h == 144);
    // Portrait downscale.
    nzvideomni::ComputeThumbnailSize(1080, 1920, 256, &w, &h);
    CHECK(w == 144);
    CHECK(h == 256);
    // Already small enough: 1:1, no upscale.
    nzvideomni::ComputeThumbnailSize(100, 80, 256, &w, &h);
    CHECK(w == 100);
    CHECK(h == 80);
    // Square exactly at the limit.
    nzvideomni::ComputeThumbnailSize(256, 256, 256, &w, &h);
    CHECK(w == 256);
    CHECK(h == 256);
    // Extreme aspect ratio keeps the short edge at least 1.
    nzvideomni::ComputeThumbnailSize(1000, 3, 256, &w, &h);
    CHECK(w == 256);
    CHECK(h == 1);
    // Degenerate inputs -> 0x0.
    nzvideomni::ComputeThumbnailSize(0, 10, 256, &w, &h);
    CHECK(w == 0);
    CHECK(h == 0);
}
...
TEST_CASE("Base64Encode matches known vectors with correct padding") {
    auto enc = [](const std::string& s) {
        return nzvideomni::Base64Encode(
    ...
    CHECK(nzvideomni::Base64Encode(bytes, sizeof(bytes)) == "AP8Q");
    CHECK(nzvideomni::Base64Encode(nullptr, 0) == "");
}

TEST_CASE("MakeDataUrl builds the data: prefix with the mime and base64") {
    CHECK(nzvideomni::MakeDataUrl("image/jpeg", "Zm9v") == "data:image/jpeg;base64,Zm9v");
    CHECK(std::string(nzvideomni::kThumbnailMimeType) == "image/jpeg");
}
...
struct FakeSettingsStore {
    ...
    nzvideomni::SettingsGetProvider Getter() {
        return [this]() -> std::string { return base_url; };
    }

    nzvideomni::SettingsSetProvider Setter() {
        return [this](const std::string& candidate) -> nzvideomni::SettingsSetOutcome {
            set_called = true;
            nzvideomni::SettingsSetOutcome outcome;
...
using nzvideomni::AliasMatchesJob;
using nzvideomni::CutoutRangeRequest;
using nzvideomni::ExtractAudioRequest;
using nzvideomni::InsertProvisionalOutcome;
using nzvideomni::InsertProvisionalParams;
using nzvideomni::ProvisionalTextAlias;
using nzvideomni::ReplaceObjectRequest;
using nzvideomni::ResolveProvisionalParams;
using nzvideomni::ScannedObject;
using nzvideomni::SelectionItem;
using nzvideomni::SelectionSnapshot;
using nzvideomni::UpdateObjectTextRequest;
using nzvideomni::UpdateProvisionalTextParams;
// I3 (timeline.updateProvisionalReservation + placement geometry).
using nzvideomni::PlacementResolveInput;
using nzvideomni::PlacementResolveResult;
using nzvideomni::ProvisionalPlacement;
using nzvideomni::ResolveProvisionalPlacement;
using nzvideomni::UpdateProvisionalReservationParams;
using nzvideomni::UpdateReservationOutcome;
using nzvideomni::UpdateReservationRequest;
// I13 (timeline.deleteProvisionalByJob).
using nzvideomni::DeleteProvisionalRequest;
// Replace-insert (timeline.insertMediaForJob).
using nzvideomni::ReplaceMediaForJobOutcome;
using nzvideomni::ReplaceMediaForJobRequest;
...
TEST_CASE("getSelection formats the snapshot with nullable filePath/objectName") {
    ...
    CHECK(r["selected"][0]["objectName"] == "NzVideomni#job1");
...
TEST_CASE("ParsePlaybackRange reads the first two fields of the real 4-field CSV") {
    ...
    REQUIRE(nzvideomni::ParsePlaybackRange("0.000,10.700,\xe5\x86\x8d\xe7\x94\x9f\xe7\xaf\x84\xe5\x9b\xb2,0",
                                      &start, &end));
    CHECK(start == doctest::Approx(0.0));
    CHECK(end == doctest::Approx(10.7));

    // Two seconds trimmed off the head: the FIRST field moves.
    REQUIRE(nzvideomni::ParsePlaybackRange("2.000,10.700,\xe5\x86\x8d\xe7\x94\x9f\xe7\xaf\x84\xe5\x9b\xb2,0",
                                      &start, &end));
    ...
    REQUIRE(nzvideomni::ParsePlaybackRange("3.333,10.700,\xe5\x86\x8d\xe7\x94\x9f\xe7\xaf\x84\xe5\x9b\xb2,0",
                                      &start, &end));
...
TEST_CASE("ParsePlaybackRange ignores everything past the second field") {
    ...
    REQUIRE(nzvideomni::ParsePlaybackRange("1.500,4.250", &start, &end));
    ...
    // break the parse.)
    REQUIRE(nzvideomni::ParsePlaybackRange("1.500,4.250,play range,0", &start, &end));
    CHECK(start == doctest::Approx(1.5));
    CHECK(end == doctest::Approx(4.25));
    REQUIRE(nzvideomni::ParsePlaybackRange("1.500,4.250,mode,0,extra,fields", &start, &end));
    CHECK(start == doctest::Approx(1.5));
    CHECK(end == doctest::Approx(4.25));
    // Surrounding whitespace is tolerated (a value read straight off an alias
    // line can carry a trailing space / CR).
    REQUIRE(nzvideomni::ParsePlaybackRange(" 1.500 , 4.250 ,mode,0\r", &start, &end));
    CHECK(start == doctest::Approx(1.5));
    CHECK(end == doctest::Approx(4.25));
}
...
TEST_CASE("ParsePlaybackRange rejects anything it cannot read with certainty") {
    ...
    const auto rejected = [&](const char* raw) {
        CHECK_FALSE(nzvideomni::ParsePlaybackRange(raw, &start, &end));
    ...
    CHECK_FALSE(nzvideomni::ParsePlaybackRange("1.0,2.0", nullptr, &end));
    CHECK_FALSE(nzvideomni::ParsePlaybackRange("1.0,2.0", &start, nullptr));
}

TEST_CASE("ParsePlaybackRange accepts a zero-length window (end == start)") {
    double start = 0.0;
    double end = 0.0;
    REQUIRE(nzvideomni::ParsePlaybackRange("4.000,4.000,mode,0", &start, &end));
...
TEST_CASE("ParsePlaybackSpeedPercent normalizes the percentage to a 1.0 scale") {
    double speed = 0.0;
    REQUIRE(nzvideomni::ParsePlaybackSpeedPercent("100.00", &speed));
    CHECK(speed == doctest::Approx(1.0));
    REQUIRE(nzvideomni::ParsePlaybackSpeedPercent("200.00", &speed));
    CHECK(speed == doctest::Approx(2.0));
    REQUIRE(nzvideomni::ParsePlaybackSpeedPercent("50.00", &speed));
    CHECK(speed == doctest::Approx(0.5));

    speed = 1.0;
    CHECK_FALSE(nzvideomni::ParsePlaybackSpeedPercent("", &speed));
    CHECK_FALSE(nzvideomni::ParsePlaybackSpeedPercent("fast", &speed));
    CHECK_FALSE(nzvideomni::ParsePlaybackSpeedPercent("100%", &speed));
...
TEST_CASE("ParseInsertProvisional accepts a full request and rejects bad params") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseInsertProvisional(
    ...
    CHECK_FALSE(nzvideomni::ParseInsertProvisional(
    ...
    CHECK_FALSE(nzvideomni::ParseInsertProvisional(json::parse(R"([])"), &out, &err));
}
...
TEST_CASE("ParseInsertProvisional handles the optional textPrefix (spec 5-5)") {
    ...
    REQUIRE(nzvideomni::ParseInsertProvisional(
        json::parse(R"({"jobId": "j1", "displayText": "hi", "numFrames": 40,
                        "genFps": 30, "placement": "C", "cursorLayer": 2,
                        "cursorFrame": 100})"),
        &out, &err));
...
TEST_CASE("ParseUpdateProvisionalReservation handles the optional textPrefix") {
    ...
    std::string err;
    REQUIRE(nzvideomni::ParseUpdateProvisionalReservation(
    ...
    REQUIRE(nzvideomni::ParseUpdateProvisionalReservation(
        ...
        &out, &err));
    CHECK(out.text_prefix == "RSV: ");
    CHECK_FALSE(nzvideomni::ParseUpdateProvisionalReservation(
        json::parse(R"({"oldJobId": "", "newJobId": "n1", "displayText": "hi",
                        "numFrames": 40, "genFps": 30, "placement": "A",
                        "materialLayer": 2, "materialFrameEnd": 130,
                        "textPrefix": 5})"),
...
TEST_CASE("BuildProvisionalPlaceholder embeds the job id and pins the length") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalPlaceholder("job-XYZ", "a nice prompt", 37);
    // object_name is the exact-match search key.
    CHECK(built.object_name == "NzVideomni#job-XYZ");
    ...
    const ProvisionalTextAlias reserved =
        nzvideomni::BuildProvisionalPlaceholder("job-XYZ", "a nice prompt", 37, "RSV: ");
    CHECK(reserved.alias.find("RSV: a nice prompt") != std::string::npos);
    CHECK(reserved.object_name == "NzVideomni#job-XYZ");
...
TEST_CASE("insertProvisional builds the alias, calls the provider and echoes name") {
    ...
    CHECK(j["result"]["objectName"] == "NzVideomni#j9");
    // The provider received a job-tagged, length-pinned alias.
    CHECK(captured_name == "NzVideomni#j9");
...
TEST_CASE("ParseResolveProvisional accepts a full request and rejects bad params") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseResolveProvisional(
    ...
    CHECK_FALSE(nzvideomni::ParseResolveProvisional(
...
TEST_CASE("resolveProvisional replaces a present placeholder (mode replaced)") {
    ...
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("resume", "jobR");
...
TEST_CASE("resolveProvisional maps a replace failure to PROVISIONAL_FAILED") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("x", "jobF");
    RequestContext ctx = AvailableCtx();
    ctx.scan_objects = [built]() {
        ScannedObject o;
        o.layer = 1;
...
TEST_CASE("ParseUpdateProvisionalText accepts a full request and rejects bad params") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseUpdateProvisionalText(
    ...
    CHECK_FALSE(nzvideomni::ParseUpdateProvisionalText(
...
TEST_CASE("updateProvisionalText updates a matched placeholder") {
    const ProvisionalTextAlias built = nzvideomni::BuildProvisionalTextAlias("p", "jobU");
...
TEST_CASE("ParseDeleteProvisionalByJob accepts a jobId and rejects missing/empty") {
    nzvideomni::DeleteProvisionalByJobParams out;
    std::string err;
    REQUIRE(nzvideomni::ParseDeleteProvisionalByJob(
        json::parse(R"({"jobId": "job-77"})"), &out, &err));
    CHECK(out.job_id == "job-77");
    // Missing jobId.
    CHECK_FALSE(nzvideomni::ParseDeleteProvisionalByJob(json::parse("{}"), &out, &err));
    CHECK_FALSE(err.empty());
    // Empty jobId.
    CHECK_FALSE(nzvideomni::ParseDeleteProvisionalByJob(
...
TEST_CASE("deleteProvisionalByJob deletes the matched placeholder (deleted=true)") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("done", "jobD");
...
TEST_CASE("deleteProvisionalByJob reports deleted=false when the provider fails") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("done", "jobF");
    RequestContext ctx = AvailableCtx();
    ctx.scan_objects = [built]() {
        ScannedObject o;
        o.layer = 2;
...
TEST_CASE("ParseInsertMediaForJob accepts jobId+filePath and rejects missing/empty/non-string") {
    nzvideomni::InsertMediaForJobParams out;
    std::string err;
    REQUIRE(nzvideomni::ParseInsertMediaForJob(
        json::parse(R"({"jobId": "job-9", "filePath": "C:\\a.mp4"})"), &out, &err));
    CHECK(out.job_id == "job-9");
    CHECK(out.file_path == "C:\\a.mp4");
    // Missing jobId.
    CHECK_FALSE(nzvideomni::ParseInsertMediaForJob(
    ...
    // Non-object params.
    CHECK_FALSE(nzvideomni::ParseInsertMediaForJob(json::parse("[]"), &out, &err));
}
...
TEST_CASE("insertMediaForJob replaces in place when the job's marker is found") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("done", "jobR");
...
TEST_CASE("insertMediaForJob reports usedFallback when the replace provider fell back") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("done", "jobFB");
    RequestContext ctx = AvailableCtx();
    ctx.scan_objects = [built]() {
        ScannedObject o;
        o.layer = 1;
...
TEST_CASE("insertMediaForJob reports INSERT_FAILED when the replace provider fails") {
    const ProvisionalTextAlias built =
        nzvideomni::BuildProvisionalTextAlias("done", "jobX");
    RequestContext ctx = AvailableCtx();
    ctx.scan_objects = [built]() {
        ScannedObject o;
        o.layer = 2;
...
TEST_CASE("ParseUpdateProvisionalReservation accepts a full request") {
    UpdateProvisionalReservationParams out;
    std::string err;
    CHECK(nzvideomni::ParseUpdateProvisionalReservation(
...
TEST_CASE("scanProvisionals returns every NzVideomni placeholder and ignores foreign objects") {
    const ProvisionalTextAlias a = nzvideomni::BuildProvisionalTextAlias("clip A", "jobA");
    const ProvisionalTextAlias b = nzvideomni::BuildProvisionalTextAlias("clip B", "jobB");
    RequestContext ctx = AvailableCtx();
    ctx.scan_objects = [a, b]() {
        ...
        oa.frame_end = 140;
        oa.alias = a.alias;
        ScannedObject foreign;  // a non-NzVideomni object -> ignored
        foreign.layer = 2;
...
TEST_CASE("ParseCutoutRange accepts a full request and rejects bad params") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseCutoutRange(
    ...
    CHECK_FALSE(nzvideomni::ParseCutoutRange(
    ...
    CHECK_FALSE(nzvideomni::ParseCutoutRange(
        json::parse(R"({"layer": 0, "frameStart": 0, "frameCount": 1,
                        ...
                        "soloKeepLayers": ["x"]})"),
        &out, &err));
    CHECK_FALSE(nzvideomni::ParseCutoutRange(json::parse(R"([])"), &out, &err));
}

TEST_CASE("MakeCutoutResult mirrors the contract v5 result shape") {
    const json r = nzvideomni::MakeCutoutResult("C:\\tmp\\cut.mp4", 1280, 720, 48, true);
...
TEST_CASE("ParseExtractAudio accepts a full request and rejects bad params") {
    ...
    std::string err;
    CHECK(nzvideomni::ParseExtractAudio(
    ...
    CHECK_FALSE(nzvideomni::ParseExtractAudio(
...
TEST_CASE("MakeExtractAudioResult mirrors the contract v5 result shape") {
    const json r = nzvideomni::MakeExtractAudioResult("C:\\tmp\\a.wav", 1.25, 48000, true);
...
TEST_CASE("IsAudioStreamPresent flags silent / empty ranges as no audio") {
    CHECK(nzvideomni::IsAudioStreamPresent(1000, 0.5));
    CHECK(nzvideomni::IsAudioStreamPresent(1, nzvideomni::kAudioSilenceThreshold));
    CHECK_FALSE(nzvideomni::IsAudioStreamPresent(0, 1.0));      // no samples
    CHECK_FALSE(nzvideomni::IsAudioStreamPresent(1000, 0.0));   // pure silence
    CHECK_FALSE(nzvideomni::IsAudioStreamPresent(1000, nzvideomni::kAudioSilenceThreshold / 2));
}
...
TEST_CASE("ParsePickFolder treats absent params / absent-or-null title as no title") {
    nzvideomni::PickFolderRequest out;
    std::string err;
    CHECK(nzvideomni::ParsePickFolder(json::object(), &out, &err));
    CHECK(out.has_title == false);
    CHECK(nzvideomni::ParsePickFolder(json::parse(R"({})"), &out, &err));
    CHECK(out.has_title == false);
    CHECK(nzvideomni::ParsePickFolder(json::parse(R"({"title": null})"), &out, &err));
    CHECK(out.has_title == false);
}
...
TEST_CASE("ParsePickFolder reads an explicit title and rejects a non-string one") {
    nzvideomni::PickFolderRequest out;
    std::string err;
    CHECK(nzvideomni::ParsePickFolder(json::parse(R"({"title": "Pick a folder"})"), &out, &err));
    CHECK(out.has_title == true);
    CHECK(out.title == "Pick a folder");
    CHECK_FALSE(nzvideomni::ParsePickFolder(json::parse(R"({"title": 5})"), &out, &err));
}

// --- fs.listFiles --------------------------------------------------------------

TEST_CASE("ParseListFiles requires a non-empty folderPath and defaults filters") {
    nzvideomni::ListFilesRequest out;
    std::string err;
    CHECK(nzvideomni::ParseListFiles(json::parse(R"({"folderPath": "C:\\wavs"})"), &out, &err));
    CHECK(out.folder_path == "C:\\wavs");
    CHECK(out.extensions.empty());
    CHECK(out.with_audio_duration == false);

    CHECK_FALSE(nzvideomni::ParseListFiles(json::parse(R"({})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseListFiles(json::parse(R"({"folderPath": ""})"), &out, &err));
    CHECK_FALSE(nzvideomni::ParseListFiles(json::parse(R"([])"), &out, &err));
}
...
TEST_CASE("ParseListFiles parses the extensions array and withAudioDuration") {
    nzvideomni::ListFilesRequest out;
    std::string err;
    CHECK(nzvideomni::ParseListFiles(
...
TEST_CASE("ParseListFiles rejects a non-array extensions and a non-boolean withAudioDuration") {
    nzvideomni::ListFilesRequest out;
    std::string err;
    CHECK_FALSE(nzvideomni::ParseListFiles(
...
TEST_CASE("MakeListFilesResult formats each entry and preserves input order") {
    std::vector<nzvideomni::ListedFile> files;
    nzvideomni::ListedFile a;
    a.name = "b.wav";
    a.path = "C:\\wavs\\b.wav";
    a.size_bytes = 12345;
    a.mtime_ms = 1700000000000.0;
    a.duration_sec = 2.5;
    nzvideomni::ListedFile b;
    b.name = "a.wav";
    b.path = "C:\\wavs\\a.wav";
    b.size_bytes = 999;
    ...
    const json r = nzvideomni::MakeListFilesResult(files);
...
TEST_CASE("MakeListFilesResult on an empty input yields an empty files array") {
    const json r = nzvideomni::MakeListFilesResult({});
...
TEST_CASE("ParseProbeAudioDuration requires a non-empty filePath") {
    nzvideomni::ProbeAudioDurationRequest out;
    std::string err;
    CHECK(nzvideomni::ParseProbeAudioDuration(json::parse(R"({"filePath": "C:\\a.wav"})"), &out,
                                         &err));
    CHECK(out.file_path == "C:\\a.wav");

    CHECK_FALSE(nzvideomni::ParseProbeAudioDuration(json::parse(R"({})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseProbeAudioDuration(json::parse(R"({"filePath": ""})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseProbeAudioDuration(json::parse(R"({"filePath": 5})"), &out, &err));
}

// --- fs.probeMediaInfo -----------------------------------------------------

TEST_CASE("ParseProbeMediaInfo requires a non-empty filePath") {
    nzvideomni::ProbeMediaInfoRequest out;
    std::string err;
    CHECK(nzvideomni::ParseProbeMediaInfo(json::parse(R"({"filePath": "C:\\a.mp4"})"), &out,
                                     &err));
    CHECK(out.file_path == "C:\\a.mp4");

    CHECK_FALSE(nzvideomni::ParseProbeMediaInfo(json::parse(R"({})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseProbeMediaInfo(json::parse(R"({"filePath": ""})"), &out, &err));
    CHECK_FALSE(
        nzvideomni::ParseProbeMediaInfo(json::parse(R"({"filePath": 5})"), &out, &err));
}

TEST_CASE("MakeMediaInfoResult reports all-zero for an unavailable snapshot") {
    nzvideomni::MediaInfoSnapshot snap;  // available == false; other fields untouched
    snap.duration_sec = 9.0;  // should be ignored because available == false
    snap.width = 1920;
    snap.height = 1080;
    const json r = nzvideomni::MakeMediaInfoResult(snap);
...
TEST_CASE("MakeMediaInfoResult passes through the probed values when available") {
    nzvideomni::MediaInfoSnapshot snap;
    snap.available = true;
...
TEST_CASE("ParseResolveDroppedFiles reads __droppedPaths in order") {
    const nzvideomni::ResolveDroppedFilesRequest out = nzvideomni::ParseResolveDroppedFiles(
...
TEST_CASE("ParseResolveDroppedFiles yields an empty result when the key is absent, empty, "
          "or the wrong shape") {
    // No key at all.
    CHECK(nzvideomni::ParseResolveDroppedFiles(json::parse(R"({})")).file_paths.empty());
    // Explicit empty array.
    CHECK(nzvideomni::ParseResolveDroppedFiles(json::parse(R"({"__droppedPaths": []})"))
              .file_paths.empty());
    // Not an array at all.
    CHECK(nzvideomni::ParseResolveDroppedFiles(json::parse(R"({"__droppedPaths": "C:\\a.png"})"))
              .file_paths.empty());
    CHECK(nzvideomni::ParseResolveDroppedFiles(json::parse(R"({"__droppedPaths": null})"))
              .file_paths.empty());
...
TEST_CASE("ParseResolveDroppedFiles silently skips non-string and empty-string elements") {
    const nzvideomni::ResolveDroppedFilesRequest out = nzvideomni::ParseResolveDroppedFiles(json::parse(
...
TEST_CASE("MakeResolveDroppedFilesResult pairs each path with its derived file name") {
    nzvideomni::ResolveDroppedFilesRequest req;
    req.file_paths = {"C:\\dir\\clip.mp4", "clip_no_dir.wav"};
    const json result = nzvideomni::MakeResolveDroppedFilesResult(req);
...
TEST_CASE("MakeResolveDroppedFilesResult yields an empty files array for no dropped paths") {
    const nzvideomni::ResolveDroppedFilesRequest req;  // default-constructed, empty
    const json result = nzvideomni::MakeResolveDroppedFilesResult(req);
...
TEST_CASE("InjectDroppedPathsIntoRequest injects into an EMPTY params object "
    ...
    const std::string resp = nzvideomni::InjectDroppedPathsIntoRequest(
...
TEST_CASE("InjectDroppedPathsIntoRequest passes a non-object / unparsable request through unchanged") {
    CHECK(nzvideomni::InjectDroppedPathsIntoRequest("not json at all", {"C:\\a.png"}) ==
          "not json at all");
    CHECK(nzvideomni::InjectDroppedPathsIntoRequest("[]", {"C:\\a.png"}) == "[]");
    CHECK(nzvideomni::InjectDroppedPathsIntoRequest(R"("just a string")", {"C:\\a.png"}) ==
          R"("just a string")");
}
