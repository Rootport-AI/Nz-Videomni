...
using nzvideomni::BuildSettingsJson;
using nzvideomni::NormalizeBaseUrl;
using nzvideomni::ParseSettingsJson;
using nzvideomni::SettingsStore;
...
std::wstring TempSettingsPath(const wchar_t* name) {
    ...
    path += L"nzvideomni_test_settings_";
    path += name;
    path += L".json";
    return path;
}
...
TEST_CASE("ParseSettingsJson falls back to the default on malformed JSON") {
    CHECK(ParseSettingsJson("not json at all") == nzvideomni::kBackendBaseUrl);
    CHECK(ParseSettingsJson("") == nzvideomni::kBackendBaseUrl);
    CHECK(ParseSettingsJson("[1,2,3]") == nzvideomni::kBackendBaseUrl);
}

TEST_CASE("ParseSettingsJson falls back to the default on a missing/invalid baseUrl") {
    CHECK(ParseSettingsJson("{}") == nzvideomni::kBackendBaseUrl);
    CHECK(ParseSettingsJson(R"({"baseUrl": 123})") == nzvideomni::kBackendBaseUrl);
    CHECK(ParseSettingsJson(R"({"baseUrl": "not-a-url"})") == nzvideomni::kBackendBaseUrl);
}
...
TEST_CASE("SettingsStore defaults to kBackendBaseUrl before Load()") {
    ...
    CHECK(store.GetBaseUrl() == nzvideomni::kBackendBaseUrl);
}
