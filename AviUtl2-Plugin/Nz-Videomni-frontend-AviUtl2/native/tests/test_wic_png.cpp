...
TEST_CASE("EncodeRgbaToPngFile writes a valid PNG with the right dimensions") {
    ...
    std::string err;
    const bool encoded = nzvideomni::EncodeRgbaToPngFile(dest, px.data(), width, height, &err);
...
TEST_CASE("EncodeRgbaToPngFile rejects invalid arguments") {
    ...
    CHECK_FALSE(nzvideomni::EncodeRgbaToPngFile(dest, nullptr, 4, 4, &err));
    const std::vector<unsigned char> px(16, 0);
    CHECK_FALSE(nzvideomni::EncodeRgbaToPngFile(dest, px.data(), 0, 4, &err));
}
...
TEST_CASE("MakeJpegThumbnail decodes a PNG and downscales it to a JPEG") {
    ...
    std::string enc_err;
    REQUIRE(nzvideomni::EncodeRgbaToPngFile(src, px.data(), width, height, &enc_err));
    ...
    std::string err;
    const bool ok =
        nzvideomni::MakeJpegThumbnail(src, 64, &bytes, &tw, &th, &sw, &sh, &err);
...
TEST_CASE("MakeJpegThumbnail does not upscale a small source") {
    ...
    int sh = 0;
    std::string err;
    REQUIRE(nzvideomni::MakeJpegThumbnail(src, 256, &bytes, &tw, &th, &sw, &sh, &err));
...
TEST_CASE("MakeJpegThumbnail fails cleanly on a missing/undecodable file") {
    ...
    CHECK_FALSE(
        nzvideomni::MakeJpegThumbnail(missing, 128, &bytes, &tw, &th, &sw, &sh, &err));
...
TEST_CASE("uploadFile code path: encode a PNG and POST it to /upload/image") {
    ...
    std::string enc_err;
    REQUIRE(nzvideomni::EncodeRgbaToPngFile(png, px.data(), width, height, &enc_err));

    // 2. Derive the URL / filename / content-type exactly like the bridge.
    const std::string url = std::string(nzvideomni::kBackendBaseUrl) +
                            nzvideomni::kBackendApiPrefix + nzvideomni::UploadPath("image");
    const std::string filename = "upload_probe.png";
    const std::string content_type = nzvideomni::ContentTypeForExtension("upload_probe.png");
    CHECK(content_type == "image/png");

    // 3. Upload through the multipart client.
    nzvideomni::HttpClient client;
    const nzvideomni::HttpResponse resp = client.UploadFileSync(
        url, png, "file", filename, content_type, nzvideomni::kUploadTimeoutMs);
