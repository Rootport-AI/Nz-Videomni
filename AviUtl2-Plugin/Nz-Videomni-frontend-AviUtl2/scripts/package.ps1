<#
.SYNOPSIS
    Package the (embedded) NzVideomni.aux2 and its Language files into a
    distributable .au2pkg.zip (milestone M7c release packaging).

.DESCRIPTION
    Produces  dist\NzVideomni-<Version>.au2pkg.zip  with the multi-file plugin
    ...
    the zip root; AviUtl2 extracts them relative to its application-data folder:
        ...
        NzVideomni-<Version>.au2pkg.zip
        |--- Plugin\NzVideomni\NzVideomni.aux2
        |--- Language\English.NzVideomni.aul2
        |--- Language\Japanese.NzVideomni.aul2
        |--- package.ini
        |--- package.txt
...
#
# A single-needle search for the UTF-16LE resource name NZVIDEOMNI_WEBUI_INDEX is
...
# the DLL's code unconditionally regardless of -DNZVIDEOMNI_EMBED_WEBUI, and its
...
if ($embedded) {
    Write-Host "Embedded Web UI resource detected in NzVideomni.aux2."
} else {
...
$pluginStage   = Join-Path $stage "Plugin\NzVideomni"
...
Copy-Item -Path $aux2 -Destination (Join-Path $pluginStage "NzVideomni.aux2") -Force

$languageSrc = Join-Path $RepoRoot "Language"
$langFiles = Get-ChildItem -Path $languageSrc -Filter "*.NzVideomni.aul2" -ErrorAction SilentlyContinue
if (-not $langFiles) { Write-Warning "No Language\*.NzVideomni.aul2 files found; package will ship without them." }
...
uninstallSubFolderFile=Plugin\NzVideomni\NzVideomni.aux2
"@
$packageTxt = @"
Nz-Videomni $Version
...
$zipPath = Join-Path $OutDir "NzVideomni-$Version.au2pkg.zip"
