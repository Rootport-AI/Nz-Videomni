...
// `NzVideomni#<jobId>` tag on the timeline whose job is already terminal in the
...
describe("App / X2 reservation release (予約詰まり修正)", () => {
  ...
  it(
    ...
    async () => {
      // The exact post-422 timeline state: a leftover `NzVideomni#job-done` tag whose
      ...
      // The `NzVideomni#job-done` marker is preserved (🎞 insert still needs it) —
...
// seat + its `NzVideomni#pending-…` placeholder must be rolled back via the screen's
