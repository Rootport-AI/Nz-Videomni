  ...
  it(
    ...
    async () => {
      const generateRes = await fetch(`${BASE_URL}${API_PREFIX}/generate`, {
        ...
        body: JSON.stringify({
          prompt: "Nz-Videomni M2 WebUI integration test probe",
          width: 384,
          height: 256,
          num_frames: 17,
          frame_rate: 24,
  ...
  it(
    ...
    async () => {
      ...
      const firstRes = await fetch(`${BASE_URL}${API_PREFIX}/generate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(generateBody("Nz-Videomni M3 busy-probe job 1")),
      });
      ...
      try {
        const secondRes = await fetch(`${BASE_URL}${API_PREFIX}/generate`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(generateBody("Nz-Videomni M3 busy-probe job 2 (should be rejected)")),
        });
  ...
  it(
    ...
    async () => {
      ...
      const generateRes = await fetch(`${BASE_URL}${API_PREFIX}/generate`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Nz-Videomni M4 I2V integration test probe",
          width: 384,
          height: 256,
          num_frames: 17,
          frame_rate: 24,
  ...
  it(
    ...
    async () => {
      ...
      const { strippedPrompt, loras } = parseLoraPrompt(
        "Nz-Videomni M5 LoRA integration test probe <lora:Pixar_Toon:1.0>",
      );
      expect(strippedPrompt).toBe("Nz-Videomni M5 LoRA integration test probe");
  ...
  it(
    ...
    async () => {
      const chainRes = await fetch(`${BASE_URL}${API_PREFIX}/generate/chain`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Nz-Videomni M6 chain integration test probe: a cat, then a dog joins in",
          width: 384,
          height: 256,
          frame_rate: 24,
          seed: 4246,
  ...
  it(
    ...
    async () => {
      ...
      const chainRes = await fetch(`${BASE_URL}${API_PREFIX}/generate/chain`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Nz-Videomni M6 A2V integration test probe",
          width: 384,
          height: 256,
          frame_rate: 24,
          seed: 4247,
  ...
  it(
    ...
    async () => {
      ...
      const sourceRes = await fetch(`${BASE_URL}${API_PREFIX}/generate`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: "Nz-Videomni M6 V2V source integration test probe",
          width: 384,
          height: 256,
          num_frames: 49,
          frame_rate: 24,
      ...
      try {
        ...
        const chainRes = await fetch(`${BASE_URL}${API_PREFIX}/generate/chain`, {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            prompt: "Nz-Videomni M6 V2V continuation integration test probe",
            width: 384,
            height: 256,
            frame_rate: 24,
            seed: 4249,
