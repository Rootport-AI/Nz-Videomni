...
export interface AppLimits {
   ...
   * ため、`spill_free_frames` が正である。2026-08-31 の再測定は3解像度
   * （1280×768・1920×1088・2560×1472）が対象で、うち2つが動いた——
   * 512×320・960×576 は API 上限 481 で頭打ちのため測定対象外（据え置き）。
   * 「`ltx` に requires 空の行を足せば全構成で賢くなる」というのは誤り。
   *
