 ...
 * is a `useCallback` closing over the live Create form (`crop_output`, the
 * reference video, the control LoRA), which would break the batch's "frozen
 * at `start()`, immune to later UI edits" contract, drag in fields an i2v row
 * must not send (D7: no `crop_output`), and be called from `runtime.ts` —
 * outside React — after the Create screen may already have unmounted. The
 * common fields are byte-equivalent to `toGenerateRequest()` all the same
 * (same key order, same omission rules), which `buildI2vGeneratePayload.test.ts`
 * pins directly against the hook.
 *
 ...
 */
import type { ConditioningImage, GenerateRequest, LoraSpec } from "../../api/types";
 ...
 *
 * Deliberately absent: `crop_output` (D7), `source_audio`, `stage2_window`,
 * `pipeline`, `num_inference_steps`, `guidance_scale`, `overlap_frames`,
