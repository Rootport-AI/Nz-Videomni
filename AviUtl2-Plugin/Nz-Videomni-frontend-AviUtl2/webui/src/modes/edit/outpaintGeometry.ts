 ...
 * 直書きされていた定数と一致する（つまりこの式はその定数の一般化）。
 *
 * **どちらの段が「見えるなじみ幅」を決めるかは機能によって違う**:
 *
 *  - 画角拡張（Outpainting）では stage 2 が {@link stage2FromStage1} で 5:2 に
 *    連動し、常に stage 1 以下の幅にしかならないので、**stage 1 だけで決まる**。
 *  - Inpainting では 2 つの段を独立に指定できて、最後に混ぜるのは仕上げ段
 *    なので、**帯幅は stage 2 が決める**（実測の正本は `VERIFICATION_LOG.md`
 *    §105.3。そちらは定数項 18px も込みで、呼び出し側が足す）。
 *
