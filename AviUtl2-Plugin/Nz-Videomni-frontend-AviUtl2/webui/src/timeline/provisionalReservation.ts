 ...
 * to tell a Generate-前 ⏳生成予約 (object name `NzVideomni#pending-…`) apart from a
 * job-bound ⏳生成中 (`NzVideomni#<jobId>`) after a project reload (§2). */
 ...
 * `NzVideomni#pending-…` placeholder when the Generate submit rejected synchronously
 ...
 * placeholder re-discoverable by job id (the `NzVideomni#<id>` object_name double-tag
 ...
 * reservation seat from whatever `NzVideomni#…`-tagged placeholders survived a
 * project reload.
 *
 * `timeline.scanProvisionals` returns every NzVideomni-tagged object as an orphan
 * candidate (`{ jobId, layer, frame }`), where `jobId` is the token after
 * `NzVideomni#`. It is text-blind — it cannot read the placeholder's 4-stage body
...
export async function reconcileFromTimeline(
  ...
  // X2(b) 台帳認識: a job-bound `NzVideomni#<jobId>` tag whose job is already
