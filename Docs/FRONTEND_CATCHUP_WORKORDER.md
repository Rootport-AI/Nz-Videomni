...
> **（2026-07-15 グループ3完了・歴史記録追記）グループ3（設計判断項目＋品質負債・項目11〜14）を実装完了し、これでフロントエンド追随の全グループ（1〜3）が完了した。** 着手前にオーナーへ設計判断を仰ぎ、項目11（A2VのCreate移設）は「Gradio式の任意音声添付欄／distilled固定パラメータ／ChainからA2Vサブモード削除」の3方針で確定・実装した（単発A2Vは既存の`submitChain`とバッチA2V共有の`buildA2vChainPayload`で`POST /generate/chain`へ送信。`A2vChainPayload`が`GenerateChainRequest`の構造的スーパーセットのためAPI型定義の改修は不要）。項目12（Width/Heightスナップ）はGradio式（手入力自由・ステッパー時のみスナップ）に統一。項目14（README類のM1表記負債）はルート／webuiのREADMEとCMakeLists.txtを1.0.0-rc1へ更新。項目13（実バックエンドでの通し確認）は**実GPU生成をオーナーが後日実施する前提のため、ブロッカー除去（deploy／run-aviutlスクリプトの旧環境パス恒久修正）とチェックリスト新設（`Nz-LTX23-frontend-AviUtl2\Docs\REAL_BACKEND_CHECKLIST.md`）までを完了とし、実GPU通し自体は最終ゲートとして残置**している。品質ゲート: webui `npm run typecheck`エラー0、vitest 48ファイル548件パス、`npm run build`成功。フロントエンド側の記録は`Nz-LTX23-frontend-AviUtl2\Docs\DEVLOG.md`§10を参照。
>
> **（2026-07-15 パリティ再棚卸し・歴史記録追記）グループ1〜3完了後、フロント側で追加のGradio↔フロント パリティ差分（N1〜N13）を再棚卸しし、`Nz-LTX23-frontend-AviUtl2\Docs\WEBVIEW2_PARITY_BACKLOG.md`の「未実装の改修項目」に実装難易度＋依存順序つきで整理した（実装は次セッション以降）。うちN12（`two_stage_hq`＝非量子化モデル用の高品質モード）はオーナー決定で将来の研究課題へクローズしたため、能動的な実装対象は12件（N1〜N11・N13）である（詳細は同バックログのN12項目を参照）。未着手タスク全体は同`Docs\PENDING_TASKS.md`に二段階で集約している。フロントエンド側の記録は`Nz-LTX23-frontend-AviUtl2\Docs\DEVLOG.md`§11を参照。**
>
...
>
> **（2026-07-17 全件実装完了＋大規模UIリデザイン・歴史記録追記）2026-07-15〜17のセッションで、パリティ再棚卸し（N1〜N13）のうちN9（将来の研究課題へ降格）を除く全件の実装を完了した。** さらにその後、オーナーの実機使用感を踏まえた2日間の大規模UIリデザイン（サブタブ廃止・ジョブレーン廃止・予約機構廃止→台帳〔JobLedger〕化など、機能追加ではなくUIの置き場所・見せ方の作り直し）を実施した。フロントエンド追随として本書が起票したグループ1〜3・パリティN項目は、以上ですべて決着済み（能動的な実装対象は残っていない。N9=研究課題降格・N12=クローズのみ残置）。詳細はフロントエンド`Nz-LTX23-frontend-AviUtl2\Docs\DEVLOG.md`§13を参照。なお、このリデザインセッション中に`backend/main.py`へアクセスログフィルタ2種（`StatusAccessFilter`／`JobsListAccessFilter`）を追加しているが、これは「バックエンドは一切変更しない」という凍結方針に対するオーナー承認済みの唯一の例外である（生成ロジック`api`/`engine`には一切手を入れていない、ログ出力抑制のみ）。revertしないこと。
...
- **重要な発見（バックエンド側の残課題）**: Gradioのバッチランナー（バックエンド`services`配下のバッチ実行経路）は、チェーンリクエストに`chunked_upsample`（アップサンプル処理を小分けにしてVRAM溢れを防ぐオプション、グループ1・項目2で追加）を明示送信していない既存のギャップがあることが判明した。フロントエンドのバッチA2Vはこの点に先んじて常に明示送信する実装にしてあるため実害はないが、**バックエンド側のGradioバッチ実行経路にも`chunked_upsample`の明示送信を追加することが望ましい**（本ワークオーダーのスコープ外につき、バックエンド側の別途対応が必要な残課題として記録する）。
  - **追記（2026-09-18）**: この残課題は片づいた。Gradioのバッチにも専用のチェックボックス（既定オン）が付き、`chunked_upsample`を真偽どちらでも明示送信するようになった。クローズ記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-160、検証は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §114である。
