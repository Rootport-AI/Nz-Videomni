...
**G3（オーナー実機real）は全項目合格した。** Depth（通常経路・A2V経由）は2026-08-03にオーナー実機で合格。Deblurは一度OOM（ジョブ`92a95e94`）が発生したが、原因究明のうえ修正し、1280x768/257fの完走を実機ゲート（ジョブ`bda91a2b`）で確認したうえで、**目視によるG-fix3判定（くっきり具合の確認）も2026-08-04に合格した**（A/Bペア`c322130d`対`eceea948`で視覚差なし・大条件`d4bedb69`も品質問題なし）。あわせて大条件の参照エンコード中に観測したVRAM reservedの膨張（実確保の13倍）は、参照動画ローダーのGPU cat連結による2乗則膨張が真因と判明し、第3修正で根治済みである（SHA完全一致確認。§49.10）。Deblurのピーク実測13.9GBは16GB推奨環境に収まるためVRAM警告は不要とオーナーが裁定した。**G4（既存canny/pose/upscalerの回帰）も2026-08-04に合格**——改修前コミット`24c67fd`と現行`e273052`で同一条件3本ずつ生成しSHA-256完全一致。**第3修正・G-fix3・G4に対応する差分は `e273052`〜`9701fbe` でコミット・プッシュ済み。webuiも再ビルド・再デプロイ済み。** 定義・詳細は§7に置く。
...
- **残タスク**: なし。UIヒント文の「出力と同じ解像度で処理」という不正確な表現は2026-08-04に「縮小せずに条件付けに使う」へ修正済み（`gradio_ui/i18n.py`・webui `strings.ts` の両方。webuiは再ビルド・再デプロイ済み）。VRAM実測の結果、**警告文言自体の追加は不要**とオーナーが裁定していた（§49.11）ため、本テーマは表現修正の完了をもって完結した。
