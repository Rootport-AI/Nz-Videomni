...
**G3（オーナー実機real）のうちDepth（通常経路・A2V経由）は2026-08-03にオーナー実機で合格した。Deblurは同日OOMが発生し調査中である。G4（既存canny/pose/upscalerの回帰）は未実施である。** デプロイも行っていない。定義・進捗は§7に置く。

---

## 7. 残ゲートの定義

**G2は2026-08-03に実施し全項目PASSした**（実測は§6.5）。**G3はDepthが合格・Deblurが調査中、G4は未実施である。**

### 7.1 △ G3 オーナー実機（real環境・実GPU）── Depth合格・Deblur調査中
...
3. ⬜ **`deblur`** を1本以上。ぼけた動画（**デフォーカスぼけ**であること。モーションブラーでは効かない）を参照に渡し、§4のプロンプト2段構成でくっきりした出力が得られること。**2026-08-03 オーナー実機で `t2v` 1280x768/257f（ジョブ `92a95e94`）を試したところ、生成開始から約50秒後にCUDA out of memoryが発生。落下地点を調査中。**
4. ⬜ **DeblurのVRAM実測**。`reference_downscale_factor=1` のため、stage-1の総トークン数は**参照なしを1として、縮小係数2で1.25倍・係数1で2.0倍（既存の制御系比で約1.6倍）**になる。実測値を`config.yaml`の`limits.spill_free_frames`（根拠[`RESOLUTION_DURATION_CAPABILITY.md`](RESOLUTION_DURATION_CAPABILITY.md) §8.4）と突き合わせ、**ヒント文のVRAMに関する記述を確定させる**（現在は「他のアダプタよりVRAM消費が大きくなります」という具体値のない固定文言にしてある）。表をdeblur向けに分けるかは実測後判断。**項目3のOOM調査待ちのため未着手。**

- **項目3のOOM**: 計画段階で洗い出していたリスクR4（`reference_downscale_factor=1`＝縮小係数1でstage-1の総トークン数が既存の制御系比で約1.6倍に増える）が現実化した可能性がある。1280x768/257fという解像度・フレーム数がDeblurの縮小係数1という条件下でどこまで持つかは、G3着手前の時点で実測していなかった（実測はG3で行う計画だった。§7.1原文）。原因の切り分け（純粋な解像度・フレーム数の限界か、他要因の重なりか）はこれから行う。
