...
> **【ステータス 2026-08-03】実装完了・機械検証（pytest・自己診断・型検査・vitest）全PASS・G1ゲート（深度前処理の性質チェック）全項目PASS・G2ゲート（モック通し）全項目PASS。コミット・プッシュ済み（backend `fd6d43f` / frontend `d375028`）。**G3（オーナー実機real）のうちDepthは合格**（通常経路・A2V経由とも2026-08-03オーナー実機確認。参照動画の特徴を踏襲した動画を問題なく生成）。**Deblurは実機でOOM**（`t2v` 1280x768/257f、ジョブ`92a95e94`、生成開始約50秒後にCUDA out of memory）が発生したが、**原因を究明し修正済み**（落下地点＝参照動画のVAEエンコード、真因＝torch 2.9.1のbf16 Conv3dのim2colフォールバック。係数1のときだけchannels_last_3dへ切り替え・復元する対処で1280x768/257fが270.02秒で完走することを実機ゲートで確認。ジョブ台帳・実測はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.9）。**目視によるG-fix3判定（くっきりした出力かどうかの確認）は未実施。** また大条件の参照エンコード中にVRAM reservedが実確保の13倍まで膨張する現象を観測していたが、**真因（参照動画ローダーのGPU cat連結による2乗則膨張）を特定し、第3修正（CPU組み立て版への置換）で根治した**（SHA完全一致確認済み。バックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.10）。第3修正自体のコミット・プッシュは未実施。**G4（既存canny/pose/upscalerの回帰）は未実施。** デプロイも未実施。**「完了」ではない。**
>
...
### 6.6 G3は一部合格・Deblur OOMは修正済み・G4は未実施

**G3（オーナー実機real）のうちDepth（通常経路・A2V経由）は2026-08-03にオーナー実機で合格した。DeblurはOOM（ジョブ`92a95e94`）が発生したが、原因究明のうえ修正し、1280x768/257fの完走を実機ゲート（ジョブ`bda91a2b`）で確認した。目視によるG-fix3判定（くっきり具合の確認）はまだ実施していない。あわせて大条件の参照エンコード中に観測したVRAM reservedの膨張（実確保の13倍）は、参照動画ローダーのGPU cat連結による2乗則膨張が真因と判明し、第3修正で根治済みである（SHA完全一致確認。§49.10）。G4（既存canny/pose/upscalerの回帰）は未実施である。** デプロイ・第3修正自体のコミットも行っていない。定義・進捗は§7に置く。

---

## 7. 残ゲートの定義

**G2は2026-08-03に実施し全項目PASSした**（実測は§6.5）。**G3はDepthが合格・Deblur OOMは修正済み（完走確認・目視G-fix3待ち）、G4は未実施である。**

### 7.1 △ G3 オーナー実機（real環境・実GPU）── Depth合格・Deblur OOM修正済み（目視待ち）

以下の4点を確認する。**深度前処理が実際に走って `control_depth.mp4` が書き出されることの確認も、ここが初出となる**（G2はモックのため前処理が実行されない。§6.5末尾）。
...
- **残タスク**: UIヒント文の「出力と同じ解像度で処理」はG3のVRAM実測後の文言確定時に「縮小せずに条件付けに使う」へ修正する（`gradio_ui/i18n.py`・webui `strings.ts` の両方。webuiは再ビルド・再デプロイ要）。**reservedの膨張現象は第3修正で根治済み（§49.10）のため、この文言確定はいつでも着手できる状態にある。**
