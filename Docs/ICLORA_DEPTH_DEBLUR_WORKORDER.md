...
> **【ステータス 2026-08-03】実装完了・機械検証（pytest・自己診断・型検査・vitest）全PASS・G1ゲート（深度前処理の性質チェック）全項目PASS・G2ゲート（モック通し）全項目PASS。ただし G3（オーナー実機real）・G4（既存canny/pose/upscalerの回帰）は**未実施**であり、本テーマは**実機ゲート待ち**の状態である。デプロイも未実施。**「完了」ではない。**
>
...
`reference_downscale_factor="1"` は「**参照動画を出力と同じ解像度のまま処理する**」ことを意味する。既存のcanny/pose（union-control）とpixel-spatial-upscalerはいずれも2であり、参照を半分に縮めてから使う。Deblurが1であることは次の2つの帰結を持つ。

1. **エンジン側の縮小係数ガードを緩める必要があった**（§5.1）。従来は「1以下は拒否」という実装で、これは事実上「Deblurを弾く」ことを意味していた。
2. **VRAM消費が増える**。参照が縮まないぶん、stage-1（1段目の生成）が扱うトークン数がおよそ2倍になる。ヒント文にこの旨を明記した（実測はG3で行う。§7.3）。
...
3. **Deblurの書式とVRAM**: プロンプトの2段構成の実例、デフォーカスぼけ専用でモーションブラーには効かないこと、参照を出力と同じ解像度で処理するためVRAM消費が他のアダプタより大きいこと。
...
- **項目4は既存仕様の維持確認である。** 複数クリップのチェーンで制御系を使えるようにすることは本テーマのスコープ外で、フロントエンド[`PENDING_TASKS.md`](../../Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md) §3-7が管理している。
...
4. **DeblurのVRAM実測**。`reference_downscale_factor=1` のためstage-1のトークン数がおよそ2倍になる（既存の制御系＝係数2に対して1.25倍→2.0倍の増加にあたる）。実測値を既存の「スピルの起きない解像度・フレーム数の表」と突き合わせ、**ヒント文のVRAMに関する記述を確定させる**（現在は「他のアダプタよりVRAM消費が大きくなります」という具体値のない固定文言にしてある）。
