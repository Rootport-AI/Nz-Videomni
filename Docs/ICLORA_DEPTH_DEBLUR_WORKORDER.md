...
- **項目4は既存仕様の維持確認である。** 複数クリップのチェーンで制御系を使えるようにすることは本テーマのスコープ外であり、**台帳未起票**である（複数同時適用は[`PENDING_TASKS.md`](../../Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md) §4-26＝旧§3-7で、この件〔チェーンでの制御系利用〕とは別テーマ）。
...
### 6.6 ✅ G3・G4とも全項目合格（テーマ完結）

**G3（オーナー実機real）は全項目合格した。** Depth（通常経路・A2V経由）は2026-08-03にオーナー実機で合格。Deblurは一度OOM（ジョブ`92a95e94`）が発生したが、原因究明のうえ修正し、1280x768/257fの完走を実機ゲート（ジョブ`bda91a2b`）で確認したうえで、**目視によるG-fix3判定（くっきり具合の確認）も2026-08-04に合格した**（A/Bペア`c322130d`対`eceea948`で視覚差なし・大条件`d4bedb69`も品質問題なし）。あわせて大条件の参照エンコード中に観測したVRAM reservedの膨張（実確保の13倍）は、参照動画ローダーのGPU cat連結による2乗則膨張が真因と判明し、第3修正で根治済みである（SHA完全一致確認。§49.10）。Deblurのピーク実測13.9GBは16GB推奨環境に収まるためVRAM警告は不要とオーナーが裁定した。**G4（既存canny/pose/upscalerの回帰）も2026-08-04に合格**——改修前コミット`24c67fd`と現行`e273052`で同一条件3本ずつ生成しSHA-256完全一致。**デプロイ・第3修正以降の実装差分のコミットは未実施**（残作業はヒント文言の表現修正のみ）。定義・詳細は§7に置く。

---

## 7. 残ゲートの定義

**G2は2026-08-03に実施し全項目PASSした**（実測は§6.5）。**G3・G4とも2026-08-04に全項目合格し、本テーマの検証は完結した。**

### 7.1 ✅ G3 オーナー実機（real環境・実GPU）── 全項目合格

以下の4点を確認した。**深度前処理が実際に走って `control_depth.mp4` が書き出されることの確認も、ここが初出だった**（G2はモックのため前処理が実行されない。§6.5末尾）。

1. ✅ **`depth-control` の通常経路**を1本以上。参照動画をアップロードして生成し、深度が制御として効いていること（元の動きや構図を保ったまま内容が置き換わること）を目視で確認する。制御追従度（`conditioning_attention_strength`）は公式推奨の0.6を出発点にする。**出力ディレクトリに `control_depth.mp4` が生成されていること**もあわせて確認する。**2026-08-03 オーナー実機確認済み（合格。参照動画の特徴を踏襲した動画を問題なく生成）。**
2. ✅ **`depth-control` のA2V経由**を1本以上。音声から動画を作る経路でも同じ前処理が通ること。**2026-08-03 オーナー実機確認済み（合格）。**
3. ✅ **`deblur`** を1本以上。ぼけた動画（**デフォーカスぼけ**であること。モーションブラーでは効かない）を参照に渡し、§4のプロンプト2段構成でくっきりした出力が得られること。**2026-08-03 オーナー実機で `t2v` 1280x768/257f（ジョブ `92a95e94`）を試したところ、生成開始から約50秒後にCUDA out of memoryが発生。原因を究明し修正した**——落下地点は参照動画のVAEエンコード、真因はtorch 2.9.1のbf16 Conv3dのim2colフォールバック。係数1のときだけchannels_last_3dへ切り替える対処で、同条件（ジョブ`bda91a2b`）が270.02秒で完走することを確認した。**「くっきりした出力が得られること」の目視判定（G-fix3）も2026-08-04にオーナー実機で合格した**——A/Bペア（`c322130d`対`eceea948`、同一シード）で視覚差なし、タイル継ぎ目のちらつきは知覚不能レベル。大条件`d4bedb69`もデブラー効果が正しく出ており品質問題なし。
4. ✅ **DeblurのVRAM実測**。`reference_downscale_factor=1` のため、stage-1の総トークン数は**参照なしを1として、縮小係数2で1.25倍・係数1で2.0倍（既存の制御系比で約1.6倍）**になる。**実測は完了した**——1280x768/257f（ジョブ`bda91a2b`）で参照エンコード区間のallocatedが1450→peak 3006MB（差分約1.56GB。オフライン予測1.2GB＋動画テンソル0.36GBとほぼ一致）。**同条件でreservedが39,846MBまで膨張する現象も観測していたが、真因は参照動画ローダー（wheelの`load_video_conditioning`）のGPU cat連結による2乗則膨張と判明し、第3修正（CPU組み立て版への置換）で根治した——ジョブ`d4bedb69`でSHA完全一致・大条件のジョブ全体peak reservedは39,846MB→13,932MBに縮小（詳細はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.10）。ジョブ全体のピーク実測は13.9GB（既存アダプタ比+約0.4GB）で16GB推奨環境に収まるため、`config.yaml`の`limits.spill_free_frames`表を分けるほどの逸脱はなく、ヒント文のVRAM警告は不要とオーナーが2026-08-04に裁定した。**

- **項目3のOOMの原因**: 落下地点は**参照動画のVAEエンコード**（非タイル・`empty_cache()`の保護外）。真因は**torch 2.9.1のbf16 Conv3dがcontiguousレイアウトでim2colフォールバックを取り、「入力ch×27×出力体積×2B」の一時行列を確保すること**（実測が予測式と比率1.00で一致）。計画段階のリスクR4（stage-1の総トークン数が約1.6倍に増える見立て）自体は正しかったが、実際にOOMを引き起こしたのはstage-1本体ではなく参照エンコードのConv3d実装の挙動だった。詳細な原因究明の経緯・第1修正の不合格・第2修正の内容・ジョブ台帳はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.9参照。
- **残タスク**: UIヒント文の「出力と同じ解像度で処理」という不正確な表現を「縮小せずに条件付けに使う」へ修正する（`gradio_ui/i18n.py`・webui `strings.ts` の両方。webuiは再ビルド・再デプロイ要）。VRAM実測の結果、**警告文言自体の追加は不要**とオーナーが裁定した（§49.11）ため、残るのはこの表現修正のみである。
- **`StateDictRegistry`との関係**: `StateDictRegistry`が将来GPU常駐に変わった場合、本修正（Conv3d重みのchannels_last_3d切替と復元）の安全性——重み参照が他ジョブ・他経路と分離されていること——を再検証すること。

### 7.2 ✅ G4 既存アダプタの回帰（2026-08-04合格）

`canny-control`・`pose-control`・`pixel-spatial-upscaler-x2` が本改修前と同じに動くこと。**縮小係数2の経路がバイト単位で不変**であることを確認した。§5.1でメタデータの読み取り方を全走査に変え、§5.3で `frame_cap` をドライバに足しているため、これらが既存経路に影響していないことを実測で押さえる必要があった。

- **実施者**: オーナー実機。
- **比較基準**: 改修前コミット `24c67fd` の同一条件（896×512/121f・シード424242・8ステップ・sage/prefetch on・resident off・同一プロンプト・参照動画バイト一致）出力とのbyte一致。
- **結果**: 現行コミット `e273052` で同一条件3本を再生成し、**3本ともSHA-256完全一致**（canny: `d770eceb`→`6c1b866e`／pose: `70c3d305`→`e809ead7`／upscaler: `fb87dce7`→`77462587`）。手順・詳細はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.11。
...
- [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) **§49**（本テーマの検証記録・実測値の正本。**§49.11**にG4・G-fix3の実測値）／**§28.1**（制御追従度の3つのノブの切り分け。深度0.6推奨の出典もここで照合済み）／**§22**（Phase Cの全ゲート詳細）
