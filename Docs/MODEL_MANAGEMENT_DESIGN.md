...
- 正本の親: 2026-07-04〜05のオーナー要望と★スコープ絞り込み決定2つ（要旨は下の「スコープ」行と§0のカテゴリ設計の根拠に転記済み）。※元文書 `MODEL_MANAGEMENT_FUTURE_WORKORDER.md` は2026-07-27の文書整理で削除した（残っていた将来項目は`checkpoint_path`の後始末1件のみで、[`PENDING_TASKS.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/PENDING_TASKS.md) §3-26の同時実施項目へ移した）。
