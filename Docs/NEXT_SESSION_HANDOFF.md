...
## ▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-11 **reference動画付きIC-LoRAのchain対応（α版・clips=1限定）＝実装完了**＝客観ゲート全PASS（pytest 526 passed / 0 failed / 1 skipped）・独立レビュー must-fix ゼロ・mock E2E PASS・**GPU実機目視ゲート✅完了・ユーザー受容（2026-07-12）**・**コミット未実施（ユーザー判断待ち）**／2026-07-12追記: **GUIスピナー退行（`1e18dea`起因）を`show_progress="hidden"`で修正し、オーナー実機確認も完了＝VERIFICATION_LOG §35クローズ**）

> **本ブロックが最新の正本。** これ以降の▶節はすべて歴史記録。食い違ったら本ブロックが正。詳細＝`VERIFICATION_LOG.md` §34（reference動画付きIC-LoRAのchain対応）・§35（GUIスピナー退行の修正）。旧「次セッションの課題」節（下記に残る2026-07-11 A2V＋LoRA併用の解禁ブロックの末尾）に記載していた「reference付きIC-LoRAのチェーン対応」課題は**本セッションで実装完了**（α版・clips=1限定）。
...
2. **筆頭課題＝コミット／プッシュ（オーナー判断）**。本セッションの変更一式は依然未コミットのまま。**2コミットへの分割案あり＝①機能追加（reference動画付きIC-LoRAのchain対応・α版）②GUIスピナー修正（`show_progress="hidden"`）**。
3. **次セッションの課題＝バッチA2V機能の設計＆実装（正本＝[`BATCH_A2V_WORKORDER.md`](BATCH_A2V_WORKORDER.md)）**。2026-07-11〜12にオーナーと合意した新機能＝ゆっくり実況・VOICEROID実況制作者向けに、i2v（画像から動画を生成する機能）の口パクアニメーションを音声素材100〜200個ぶん就寝中に一括生成するGUI機能。表（マニフェスト）が正典＝ファイル名による音声⇔画像⇔プロンプトの自動結びつけは不採用・表情プリセットも不採用（LTX 2.3は自然言語プロンプトが効くため定型化は表現の幅を狭める、というオーナー判断）。表の列＝queue#／wav名／Duration／image（ドロップダウン）／Prompt／Stat。CSVマニフェストはWebView2フロントエンド（AviUtl2拡張）と共通仕様にし相互運用する方針。Gradio 6.19.0の `gr.Dataframe` にセル内ドロップダウンが無いことは実ソースで確認済み＝`.select`／`.edit`イベントと既存Jobsタブ（`jobs_table`/`on_job_select`）と同型の下部編集パネル方式が有力。**未決の設計論点5点（Replace切替時の挙動・Prompt列の編集方式・Skipの閾値・バッチAPIの実装層・CSVマニフェスト形式）が残っており、次セッション冒頭でオーナーに確認すること**（詳細は正本の§4）。**上記2（コミット／プッシュ）との優先順位はオーナー判断**＝本ドキュメントの都合で勝手に順位付けしない。
