...
**研究課題へ格下げ（オーナー決定・2026-07-11）**: 「control系アダプタ名をプロンプトに `<lora:...>` として手打ちした場合、1回分のアップロードが無駄になる」既知の軽微事項（nit・上記「テスト・独立レビュー」節参照）は、次セッション課題から除外し将来の研究課題へ格下げした。ユーザーからイシューが上がってから対応する方針（着手条件＝ユーザー報告起点）。
