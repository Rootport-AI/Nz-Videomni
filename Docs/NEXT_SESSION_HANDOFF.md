...
**課題の一覧と状態は台帳 [`PENDING_TASKS.md`](PENDING_TASKS.md) が正本**で、件数も項目番号も本書には書かない。**2026-08-30 に起票したのは §3-127（`scripts/setup.ps1` の完了案内が実在しない `.au2pkg.zip` を案内している）と §3-128（`engine/venv-engine.freeze.txt` が `hf-xet` を固定していない）の2件**で、どちらもインストール導線を実機で通したときに見つかったものである（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.4・§82.7）。
