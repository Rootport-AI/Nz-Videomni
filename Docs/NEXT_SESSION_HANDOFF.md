...
**第2弾は第1弾と種類の違う作業だった、という点が後続に効く。** 第1弾は「無視していた項目を効かせる」作業だったので `unsupported_features` が動かなかったが、**第2弾は「422 で断っていた機能を開ける」作業なので、この配列が 8件 → 7件になった。この配列が動いたのは第2弾が初めてである。** 画面のグレーアウトも1つ解けたが、**フロントエンド本体の変更は0行**である（Settings 画面は `unsupported_features` を読んでいないため。触ったのはモックの2箇所だけ）。**あわせて既存の罠が1つ消えた**——これまでは LTX 2.5 を選んだまま設定画面で常駐を on にすると、以後の**すべての**生成が 422 で断られていた。実測の正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §76（**RSSピークが従来 18.3GiB と過小報告されていた件は同 §76.2(3)**、新設 pytest の走らせ方は同 §76.4）。
...
- **未押し込みは高速化第2弾のぶん4本である**——`f72c8e7`（C0＝計測下地）／`6a10960`（C1＝engine25 の開通）／`69b58ec`（C2＝アプリ側の開通）／本コミット（C3＝文書の総更新）。**本数の正本は `git status -sb` の `ahead` 表示**なので、押し込む前には必ずそれを見ること。押し込みはオーナー承認のうえで行う運用なので、**エージェントは以後もコミットまでで止めること。**
