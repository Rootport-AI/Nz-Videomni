...
| **バックログ（ユーザー処置 2026-07-05・二次トリアージ済み）** | **近い将来にやる**: ①GUI への V2V/A2V 露出（音声スムージング UI 要件含む） ②モデル管理（**スコープ絞り込み済み=ドロップダウン選択のみ・ディレクトリ再編なし**。正本=[`MODEL_MANAGEMENT_FUTURE_WORKORDER.md`](MODEL_MANAGEMENT_FUTURE_WORKORDER.md) 冒頭★注記）＋**コンソールログ改修**（httpx ポーリング行の抑制＋工程名つき進捗/it 速度/所要時間。調査=[`CONSOLE_LOG_FORGE_NEO_RESEARCH.md`](CONSOLE_LOG_FORGE_NEO_RESEARCH.md)） ③**IC-LoRA 制御の効き具合（strength）可変化** ④GUI 細目（言語/テーマ永続化・動的行・デフォルト negative 欄）。**将来の改善事項（一旦クローズ）**: 高品質モード実配線（D節）・IC-LoRA 他制御タイプ（depth/Motion-Track 等）・**A2V×V2V 併用**（便利だがコストメリット薄・ユーザー判断）。**研究課題（改善事項より低優先）**: リップシンク強化（ガイダンス差し込み案=VERIFICATION_LOG §25.5 保留）・V2V 音声継ぎ目の浅い凹み（§24.7）・IC-LoRA 前処理キャッシュ・**A2V 複数クリップ音声窓割り**（複雑なわりに用途不明・ユーザー判断）。**不要と確定**: A2V 音声トリミング指定（フロントエンド AviUtl2 と機能重複・サーバー側では持たない・ユーザー決定 2026-07-05） |
