...
## ▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-04 Gradio GUI追いつきセッション末・**次セッションはこのブロックだけ読めば現在地が分かる**）
...
### このセッションでやったこと — **検証用Gradio GUIのバックエンド追いつき**
...
- **バックエンドの pipeline／guidance_scale 消費**＝`two_stage_hq` の実効化。現状 `services/ltx_runner.py:849-862` の payload がこれらをエンジンに渡さず常にdistilled経路のため、GUIは当該オプションを**表示するが無効化＋「バックエンド対応待ち」注記**にしてある。
