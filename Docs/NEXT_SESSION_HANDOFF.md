...
**LTX 2.5 で使えるのは、基本生成（テキストから動画・画像から動画）・クリップ連結（Chained）・V2V継続（素材（冒頭）に動画を使って続きを作る）・A2V（音声から動画。Single・長尺・バッチのいずれも）・スタイル LoRA・IC-LoRA（参照動画による制御。クリップ別の参照窓＝長尺 IC-LoRA を含む）である。** まだ無いのは Retake・素材（末尾）・Outpainting・NAG／VSF・高速化技術（SageAttention・`keep_resident`・PrunaVAED）で、要求すると 422 で断り、画面側でもパネルが灰色になる。**Chained タブは LTX 2.5 でも開き、その中で灰色のまま残るのは素材（末尾）の1カードだけ**である（`unsupported_features` は8件）。後続は台帳[`PENDING_TASKS.md`](PENDING_TASKS.md) §3-102。

**A2V までは、すべてオーナーの実機確認に合格している。** 基本生成（v1）は 2026-08-22（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §69.21）、クリップ連結は 2026-08-23（同 §72.10）、V2V継続と A2V は 2026-08-24（同 §73.10）。**長尺A2V はオーナー裁定で合格扱い**（裁定の全文と理由は同 §73.10(2)）。**スタイル LoRA と IC-LoRA（2026-08-24 実装）は機械ゲート・実機ゲートとも全項目合格だが、オーナーの目視だけがまだ済んでいない**（見る5項目は同 §74.10）。16GB の回避策は 1 段も使っていない（1920×1088・169フレームの V2V まで確認済み）。実測の正本は同 §69（v1）・§72（連結生成＋固定ベンチマーク B1〜B4）・§73（V2V・A2V＋B5〜B8）・§74（LoRA・参照動画＋B9〜B13）、API 契約は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10、設計正本は[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §5.6・§8.5、方式の差分は[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) 12節である。

### 残っているオーナー作業（2件）

1. **スタイル LoRA と IC-LoRA の目視**（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §74.10 の5項目）。検証用の合成画像6枚と動画16本が `outputs/ltx25-stage3-gate/` にあり、AviUtl2 の操作パネルから自分で作って見てもよい。**見るときの注意が2つある**——スタイル LoRA は**トリガー語（そのLoRAの合言葉）をプロンプトに入れないと効きが穏やか**で、輪郭線制御（canny）は**鮮明な参照素材でないとエッジがほとんど出ない**（同 §74.9）。
2. **`git push`（押し込み待ち）。** 連結生成（第1段）・V2V／A2V（第2段）・LoRA／IC-LoRA（第3段）の実装と記録まで、コミットは積んである。押し込みはオーナー承認のうえで行う運用のため未実施である。

### 次に着手する候補

台帳の「1. 近日中の改修項目」に残っているのは §1-4 だけ（オーナー自身が README のスピードガイドを書く作業であり、AI エージェントが実装するタスクではない）。したがって次のテーマは「3. 将来の研究課題」から選ぶ。

**次のテーマは §3-102 の「高速化技術の LTX 2.5 前倒し」である。** 順番はオーナー裁定（2026-08-23）の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」で決まっており、**必須級（クリップ連結・V2V・A2V・スタイル LoRA・IC-LoRA）は 2026-08-24 で出尽くした**。対象は SageAttention（`attention_backend`）・モデル骨格の常駐（`keep_resident`）・PrunaVAED（`vae_mode`）と、いま無視して先へ進めている 2 つ（fused カーネル・先読み block swap）で、アダプタの `CHAIN_REJECT_TABLE` と `REJECT_TABLE` から該当行が外れることが完了の合図である。**回帰の物差しは揃っている**——固定ベンチマーク B1〜B4（同 §72.7）・B5〜B8（同 §73.7）・B9〜B13（同 §74.8）を同じ条件で走らせ直せば、「速くなったが絵が変わっていないか」を mp4 の SHA-256 一致で判定できる。**速度を比べるときは交互対比較で測ること**（同一セッションで A・B・A・B と交互に走らせて対で差を取る。理由は同 §43.6・§73.11(2)）。
