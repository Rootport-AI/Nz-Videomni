...
1. **ユーザーの GUI 実機確認（main マージ後にサーバー再起動して 3 点）**: ①共有プロンプト空欄で「Generate chain」→黄色い警告ポップアップ ②チェーン生成中の進捗テキストに「クリップ n/N」 ③クリップ1とクリップ2に**別々の個別プロンプト**を入れて生成→出力後半（V2V なら約 0:02 以降）で内容が切り替わる＝クリップ2の実在の目視確認。
