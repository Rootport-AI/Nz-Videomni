...
## ▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-06 **GUI プロンプト欄の一本化 ＋ ネガティブ欄グレーアウト**＝実装完了・客観ゲート PASS・**最終目視／実機ゲート＋マージ承認待ち**）

> **本ブロックが最新の正本。** これ以降の▶節はすべて歴史記録。食い違ったら本ブロックが正。

| 項目 | 状態 |
|---|---|
| リポジトリ | **branch `feature/prompt-unification`（未マージ・push／main マージはユーザー承認待ち）**・commit `fd35877`・回帰 PASS（pytest **422 passed / 1 skipped**）。作業ツリー clean |
...
| **残＝ユーザー宿題** | ①**プロンプト一本化の最終目視／実機ゲート 4 点**（上部に下書き欄1つ・全タブで表示／Style LoRA タブでサムネイル選択→上部欄にタグ／連結で LoRA タグ除去の警告／ネガティブ欄グレーアウト）＝ユーザー帰宅後 ②**画風/キャラ LoRA 対応の最終目視ゲート＝未完了**（S0 スパイク動画 6 本＝session scratchpad `dspike_out`／GUI 実機操作＝Style LoRA タブ・Reload・クリック挿入）③前回持ち越しの GUI 実機確認 3 点（黄トースト／クリップ n/N／クリップ別プロンプトでクリップ2の実在目視）。いずれもユーザー実施待ち |
