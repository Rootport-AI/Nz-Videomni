...
**LTX 2.5 で使えるのは、基本生成（テキストから動画・画像から動画）・クリップ連結（Chained）・V2V継続（素材（冒頭）に動画を使って続きを作る）・A2V（音声から動画。Single・長尺・バッチのいずれも）・スタイル LoRA・IC-LoRA（参照動画による制御。クリップ別の参照窓＝長尺 IC-LoRA を含む）・高速化4つ（GGUF逆量子化の1カーネル化・先読み block swap〔いずれも既定 on〕・モデル骨格の常駐＝`keep_resident`〔既定 off のオプトイン〕・SageAttention＝`attention_backend`〔既定は `"sdpa"` のまま〕）である。** まだ無いのは Retake・素材（末尾）・Outpainting・NAG／VSF・残る高速化1つ（PrunaVAED）で、要求すると 422 で断り、画面側でもパネルが灰色になる。**Chained タブは LTX 2.5 でも開き、その中で灰色のまま残るのは素材（末尾）の1カードだけ**である（`unsupported_features` は**6件**）。後続は台帳[`PENDING_TASKS.md`](PENDING_TASKS.md) §3-102。

**ここまでの機能は、高速化第3弾（SageAttention）を除いてすべてオーナーの実機確認に合格している**（第3弾は**目視待ち**。下記の「残っているオーナー作業」を参照）。 基本生成（v1）は 2026-08-22（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §69.21）、クリップ連結は 2026-08-23（同 §72.10）、V2V継続と A2V は 2026-08-24（同 §73.10）、**スタイル LoRA と IC-LoRA は 2026-08-25**（同 §74.12。AviUtl2 の操作パネルから両方を試し、いずれも効果があることを確認して合格）。**長尺A2V はオーナー裁定で合格扱い**（裁定の全文と理由は同 §73.10(2)）。**使うときの注意が2つある**——スタイル LoRA は**トリガー語（そのLoRAの合言葉）をプロンプトに入れないと効きが穏やか**で、輪郭線制御（canny）は**鮮明な参照素材でないとエッジがほとんど出ない**（同 §74.9）。16GB の回避策は 1 段も使っていない（1920×1088・169フレームの V2V まで確認済み）。実測の正本は同 §69（v1）・§72（連結生成＋固定ベンチマーク B1〜B4）・§73（V2V・A2V＋B5〜B8）・§74（LoRA・参照動画＋B9〜B13）、API 契約は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10、設計正本は[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §5.6・§8.5、方式の差分は[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) 12節である。
...
**高速化第3弾（2026-08-25 実装）は機械ゲート・実機ゲートとも全項目合格したが、これだけはオーナーの目視ゲート（G8）が残っている＝目視待ちである。** SageAttention（`attention_backend`）を LTX 2.5 でも開通させた——**既定は `"sdpa"` のまま**で、利用者が設定画面で明示的に `sage` を選んだときだけ効く。**実測は 1280×768・2クリップ×121フレームの連結生成で 1.1017倍**（交互3組の平均。合格線 1.10倍に対してぎりぎりの合格）。**本弾だけは「選ぶと出来上がる絵の細部が変わる」種類の高速化なので、固定ベンチマークの SHA-256 一致では合否を出せない**——判定は、すでに実績のある LTX 2.3 の sage を先に較正して作った **PSNR（ピーク信号対雑音比。元の絵とどれだけ違うかを1つの数値にまとめた指標で、単位は dB。数値が大きいほど元の絵に近い）と SSIM（構造類似度。2つの映像がどれだけ同じ構造をしているかを 0〜1 で表す指標で、1 に近いほど同じ）の相対の帯**で行った（768p 単発で PSNR 29.082dB／SSIM 0.891、512×320 で PSNR 25.171dB／SSIM 0.863。いずれも合格）。**当初の絶対基準〔SSIM 0.98／0.96〕は較正で LTX 2.3 自身が割ることが判明したため、オーナー再裁定で相対の帯・幾何別へ作り直してある**（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77.2(3)）。**第2弾と同じく 422 の側からの移動なので `unsupported_features` は 7件 → 6件になったが、設定画面のグレーアウトが解けた理由はこれとは独立である**——設定画面（`SettingsPanel`）は `unsupported_features` をそもそも参照しておらず、attention の選択が押せるかどうかは `GET /status` の `acceleration.sage_available` だけで決まる。C2（`cb97613`）で真理値表から LTX 2.5 の恒久 `false` 上書きを削除したため実探索になり、true が返るようになった——**これが設定画面のグレーアウトが解けた理由である**（正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77.9(5)）。**`sage` で 422 になっていた罠（2.3 で選んだ設定がブラウザに残り、2.5 に切り替えると全ジョブが 422）も消えたが、`vae_mode` 側には同型の罠が残っている**（同 §77.7(f)・台帳 §3-102 の判断材料(9)）。実測の正本は同 §77。


### 残っているオーナー作業（目視1件・押し込み1件）

**2026-08-25 の時点でオーナー側に残っているのは、①高速化第3弾（SageAttention）の目視ゲート G8 と、②そのぶんの押し込み（`git push`）の2つである。**

- **① 目視ゲート G8（未了）＝高速化第3弾（SageAttention）。** これだけは**選ぶと出来上がる絵の細部が変わる**種類の高速化なので、機械の検証では採否を決められない。**素材は `outputs/ltx25-sage-gate/g8_material/` に置いてある**（sdpa／sage の対が3組〔A＝1280×768 の連結・B＝1280×768 の単発 257フレーム・E＝512×320 の単発〕、sage 単独が2本〔C＝1920×1088・D＝IC-LoRA〕、静止画 26枚、読み方を書いた `README.md`）。**あわせてお願いしたいことが2つある**——(i) **音の継ぎ目を1箇所、耳で確かめてほしい**（VRAM 最大形状の sage で、接合1箇所の音声の不連続の指標が閾値 6.0 に対し 6.2256＝3.8% 超過。ただし**同じシード・同じ形状の sdpa 凍結版でも同じ接合が自身の最悪 3.5634 を記録**しており、映像側は両方とも全接合が「連続」判定なので、機械では白黒がつかない）、(ii) **B の対の秒数の差〔157.27秒 対 113.64秒〕を速度の成果として読まないでほしい**（B の sdpa 側はサーバー再起動直後の1本目にあたる。速度の正しい読み方は A の対＝119.51秒 対 108.31秒）。詳細は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77.6。
- **それ以外の目視は全て決着している。** 最後まで残っていたスタイル LoRA と IC-LoRA の目視は 2026-08-25 に合格した（同 §74.12）。**高速化第1弾・第2弾にはもともと目視作業が無い**（出力がビット単位で不変であることを機械で確認済みのため）。
- **② 押し込み。** 高速化第1弾のぶん（実装5本＋文書2本）と第2弾のぶん（6本）は 2026-08-25 に押し込み済みである。**未押し込みは高速化第3弾のぶん4本**——`1fb4fb8`（C0＝仮想環境への `sageattention` 導入と品質計測の下地）／`3235f15`（C1＝engine25 の開通）／`cb97613`（C2＝アプリ側の開通）／本コミット（C3＝文書の総更新）。**本数の正本は `git status -sb` の `ahead` 表示**なので、押し込む前には必ずそれを見ること。押し込みはオーナー承認のうえで行う運用なので、**エージェントは以後もコミットまでで止めること。**

### 次に着手する候補

台帳の「1. 近日中の改修項目」に残っているのは §1-4 だけ（オーナー自身が README のスピードガイドを書く作業であり、AI エージェントが実装するタスクではない）。したがって次のテーマは「3. 将来の研究課題」から選ぶ。

**高速化技術の LTX 2.5 前倒しは、第3弾（SageAttention）で打ち止めになった。** 順番はオーナー裁定（2026-08-23）の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」で決まっており、**必須級（クリップ連結・V2V・A2V・スタイル LoRA・IC-LoRA）と高速化の第1弾（fused カーネル・先読み block swap）は 2026-08-24 に、第2弾（モデル骨格の常駐）と第3弾（SageAttention）は 2026-08-25 に終わっている**（第3弾はオーナー目視待ち）。**残る PrunaVAED（`vae_mode`）は LTX 2.5 版の枝刈り済み重みが存在しないため先送り**なので、段階分けの上では次は準必須級（Retake・素材（末尾）・Outpainting）に入ることになる——**ただし着手の順序と時期はオーナーの判断による。**
