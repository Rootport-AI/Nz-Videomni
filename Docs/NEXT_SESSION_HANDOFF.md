...
| **次セッション** | **ユーザー目視ゲート → マージ判断が入口**。目視対象＝`outputs/visual_review/` の #10 vs #14（追従度 1.0 vs 0.6・本命ペア）・#15（参照 0.8 の滲み確認）・#16 vs #17（補助ペア）。受容なら push／main マージ（ユーザー承認）。以降のバックログは下記「バックログの正本」行を参照 |
| 残る宿題（次セッションをブロックしない） | ①IC-LoRA strength の目視ゲート 4 本（上記） ②前回持ち越しの GUI 実機確認 3 点（事前チェック拒否の黄色トースト／チェーン進捗「クリップ n/N」／クリップ別プロンプトでクリップ2の実在目視）＝**ユーザーが後日実施すると表明（2026-07-06）** |
