...
2. **残課題（意図的スコープ外）＝reference付き IC-LoRA（参照動画による条件付けアダプタ）のチェーン対応は未実装**。§32.1 の決定どおり、チェーン（`POST /generate/chain`）は `reference_video_id` を持たない構造のため v1 のスコープ外とした。対応する場合はチェーンの構造自体の拡張が必要でスコープが広がる。
3. **既知の軽微事項（nit）＝control系アダプタ名をプロンプトに `<lora:...>` として手打ちした場合、1回分のアップロードが無駄になる**。音声または画像をアップロードしたあとサーバー側の422（`LORA_CONTROL_UNSUPPORTED_IN_CHAIN`）で拒否されるため（拒否メッセージ自体は正しくローカライズされる）。Style ギャラリーのサムネイルをクリックする通常の操作では発生しないため、対応は見送り。
