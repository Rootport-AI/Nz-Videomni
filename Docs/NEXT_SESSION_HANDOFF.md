...
### 残っているオーナー作業（押し込み1件・README 執筆1件）

**2026-08-25 の時点でオーナー側に残っているのは、①未押し込みコミットの押し込み（`git push`）と、②台帳 §1-4 の README スピードガイド執筆の2つだけである。目視・実機確認はすべて決着した。**

- **① 押し込み。** 高速化第1弾のぶん（実装5本＋文書2本）と第2弾のぶん（6本）は 2026-08-25 に押し込み済みである。**未押し込みは高速化第3弾のぶん7本**——`1fb4fb8`（C0＝仮想環境への `sageattention` 導入と品質計測の下地）／`3235f15`（C1＝engine25 の開通）／`cb97613`（C2＝アプリ側の開通）／`8176668`（C3＝文書の総更新）／`5290706`（C3 への敵対的レビュー指摘25件の修正）／`ad57513`（本書のグレーアウト機序の誤記訂正）／**本コミット（目視ゲート G8 合格の記録）**。**本数の正本は `git status -sb` の `ahead` 表示**なので、押し込む前には必ずそれを見ること。押し込みはオーナー承認のうえで行う運用なので、**エージェントは以後もコミットまでで止めること。**
