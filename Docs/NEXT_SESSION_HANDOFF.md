...
### 次セッションの課題（2026-07-14時点・未着手）

Clip Chain拡張の完結を受けて、次セッションの課題は以下の2件。**優先順位はオーナー判断とする。**

① **アップサンプルの時間チャンク化**（768p長尺で発生するスピル・OOMの根本対策。stage-1完了直後の一括アップサンプルを時間方向にチャンク化して最大のVRAMピークを削る。Clip Chainタブに「Chunked upsample mode (for VRAM 16GB)」チェックボックスをオプトインで新設し、既定オフでは現状経路がそのまま動く）: 正本＝[`CHUNKED_UPSAMPLE_WORKORDER.md`](CHUNKED_UPSAMPLE_WORKORDER.md)。**着手前にアップサンプラーの時間受容野・stage-2タイル分割の流用可否・API→サービス層→エンジンのフラグ配線の裏取りが必要**（同ワークオーダー§3・§4参照）。
