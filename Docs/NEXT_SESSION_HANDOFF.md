...
## ▶▶▶▶▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-05 GUI 経由クリップ連結＋結合の症状解消 完了・branch `fix/v2v-join-silent-loudnorm`・**OPEN＝ユーザー目視ゲート＋push/mainマージ承認**）

> **本ブロックが最新の正本。** これ以降の▶節はすべて歴史記録（直下の「2026-07-05 GUI V2V/A2V 露出＋モデル管理＋ログ改修」ブロック以降を含む）。食い違ったら本ブロックが正。詳細＝**VERIFICATION_LOG §27**。
...
- pytest 基準の新値＝**346 passed / 1 skipped**（343+1 → +3）。既存有音経路テスト緑＝非退行。
- コミット列（branch `fix/v2v-join-silent-loudnorm`）: `c7ad906`（fix）→`6ba83ae`（test）→`1422543`（i18n）→docs コミット。

### OPEN（次のアクション）

1. **ユーザー目視ゲート**: ①output.mp4（6.3 秒）にクリップ2の内容が含まれるかの実物目視（H3 の最終確定） ②joined.mp4（`outputs/e4a34bf5-941f-4ed5-a9cc-ed6d82783964/joined.mp4`）の目視試聴。
2. **push／main マージ＝ユーザー承認待ち**。

---

## ▶▶▶▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-05 GUI V2V/A2V 露出＋モデル管理＋ログ改修 完了・mainマージ済・**次セッション＝GUI 経由クリップ連結＋結合の症状解消**）（歴史記録）

> **（歴史記録）本ブロックは上位の「2026-07-05 GUI 経由クリップ連結＋結合の症状解消 完了」ブロックに置き換わった。** 本文の「★次セッション＝GUI 経由のクリップ連結＋結合の症状解消」は**実施済み**（結果＝最新ブロック・VERIFICATION_LOG §27）。これ以降の▶節はすべて歴史記録。食い違ったら最新ブロックが正。詳細＝**VERIFICATION_LOG §26**。

本セッションは **Fable5 親＋Opus 子の並行オーケストレーション**（フェーズ1＝3子・フェーズ2＝1子・各自 worktree・GPU 所有権は親が逐次貸与）で 3 題目を実装し、G3 目視ゲート→フィードバック対応→再ゲートまで通した。

### フェーズ1＋2 完了サマリ

| 題目 | ゲート結果 | コミット列／merge SHA |
|---|---|---|
| **コンソールログ改修**（子C） | S3 実機: byte-match 3系統一致・peak_vram **8440** 再現・httpx ポーリング行 **68%→0** | `afc66aa`→`a3d7f3b`→`199e8da`→merge **`b77a812`** |
| **モデル管理ドロップダウン**（子B・正本=[`MODEL_MANAGEMENT_DESIGN.md`](MODEL_MANAGEMENT_DESIGN.md)） | S4 実機: 別名二重登録 swap→同 seed→**SHA 一致×3**・未知名 404 | `8c762b9`→`4613b7e`→`a062274`→`e3e3601`→`42f36b2`→(rebase `5ad77d3`)→merge 済 |
| **GUI V2V/A2V 露出＋結合 API**（子A・正本=[`mockups/JOIN_API_PROPOSAL.md`](mockups/JOIN_API_PROPOSAL.md)） | 実機 e2e: V2V **190.5s**/peak 9889・join **3.2s**（尺誤差 0.000）・A2V **185.2s**/r=1.0000 | `13b5501`→`6fbe763`→`214c26f`(rebase `dce6fc9`→`8b08b5c`→`c7fce67`)→merge **`f999374`** |
| **G3 フィードバック対応 F1-F5**（子F・`fix/g3-feedback`） | GPU 窓④: **byte-match×3**（V2V 再検証 output.mp4 が §24.7 合格素材とバイト一致）・単発ステップ進捗実機動作・uvicorn ポーリング **0** | `fc9ce2d`→`33f1be4`→`3bd6100`→`2a955ac`→`821db57`→merge **`b8d9236`** |

- 新設 API: `GET /models`・`POST /pipeline/load`（optional モデル指定・省略時 byte 同一）・`POST /jobs/{id}/join`・`GET /jobs/{id}/joined`（`services/join_manager.py`）。
- F1-F5 の要点: F1 uvicorn ポーリング GET 選択的抑制／F2 wheel の tqdm を `engine/progress_shim.py` へ実行時差し替え（wheel 無改変・数値非干渉）でステップ単位進捗／F3 GUI 進捗整形（`JobResponse` に optional `stage`）／F4 V2V 継続の作法ガイド（§24.7 反映）／F5 クロスフェード既定 150→**300ms**＋GUI ドロップダウン 150/300/500。
- pytest **343 passed / 1 skipped**（本セッション累計 212→343）。

### クローズ項目（本セッション・ユーザーゲート）

- **A2V（GUI 経由）＝完璧（リップシンク含む）・i2v 操作/出力＝完璧・joined.mp4/ログ/GUI 進捗表示＝完璧**（フィードバック対応後の再ゲートで 3 件クローズ）。
- **A2V 音量 +3dB＝違和感なし → 研究課題へ格下げ（クローズ）**。
- **モデル管理の実代替モデル DL＝しない方針（ユーザー決定）**＝切替配線は別名二重登録で実証済み・実重み投入は将来ユーザー任意。

### ★次セッション＝GUI 経由のクリップ連結＋結合の症状解消（分析・仮説まで本セッション実施済み）（実施済み・歴史記録＝最新ブロック・VERIFICATION_LOG §27 参照）

ユーザー症状（GUI 操作時）: ①1クリップ目だけ生成され結合版が作られない ②2クリップ目が作られない ③「Create joined version」で **503＋ffmpeg `[loudnorm] Value -inf for parameter 'I' out of range`**。

確定事実（親の一次調査）: 対象 job `03c0a691`＝**正常 completed**（clips=2・total 225f・new_frames_px=152・source_fps=16→24 リサンプル・source_had_audio=true）。元動画（upload `ed7d8b41`）の音声は**デジタル無音**（volumedetect mean_volume **-91.0 dB**）。

仮説（次セッションで検証・詳細＝VERIFICATION_LOG §26.4）:
- **H1（③の本命・確度高）**: `join_v2v` の2パス loudnorm が無音音声の実測 I=**-inf** を第2パスに渡し ffmpeg が拒否→503。対処方向＝実測が範囲外なら loudness 整合をスキップしクロスフェードのみ実施。
- **H2（①）**: 結合版は手動ボタン（設計どおり）＋③で失敗＝「作られない」に見えた複合。
- **H3（②）**: クリップ2は metadata 上生成済み（new_frames_px=152＝参照フレーム 73 が置換されるため新規≈6.3s）。GUI 側の尺説明不足で「2クリップ目が無い」に見えた可能性。**実物の目視確認は次セッションで先に実施**。
- 副次: 無音音声の 16fps 素材という**新しい入力クラス**（検証マトリクス外）。

---

## ▶▶▶▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-05 audio-to-video 完結・mainマージ済）（歴史記録）

> **（歴史記録）本ブロックは上位の「2026-07-05 GUI V2V/A2V 露出＋モデル管理＋ログ改修 完了」ブロックに置き換わった。** これ以降の▶節はすべて歴史記録。食い違ったら最新ブロックが正。
...
| 項目 | 状態 |
|---|---|
| リポジトリ | branch **`feature/gradio-gui-catchup`（main へ未マージ）**。base＝main `8bdf90a`（Phase C マージ済）。コミット列 `1c5b146`（S1土台）→`feea189`（S2プリセット）→`4c2c356`（S3多キーフレーム）→`b4a09e1`（S4 IC-LoRA参照）→`6f00ebc`（S5クリップ連結）→`88f7c1a`（リファクタ＝パッケージ化・挙動変更ゼロ）→`f766f88`（S6 Jobs/Settings）。**push／マージはユーザー承認待ち** |
| 検証用Gradio GUI | ✅**客観ゲート全PASS・目視ゲートは未実施（OPEN）**。旧 `gradio_ui.py`（Phase 1初期機能のまま186行）を、Phase 3／IC-LoRA Phase B/C のバックエンド機能を検証できる**4タブ構成**へ作り替え＝`gradio_ui/` パッケージ（9モジュール）。数値＝VERIFICATION_LOG §23 |
| IC-LoRA Phase A/B/C | ✅完了・mainマージ済（Phase C＝merge `8bdf90a`・G5ユーザー受容済 2026-07-04。正本＝各STATUS・VERIFICATION_LOG §20/§21/§22） |
| 目視ゲート | **GUIぶんが未クローズ**（下記チェックリスト参照）。生成機能側（Phase C含む）の目視は全クローズ済み |
| **次セッション** | **video-to-video 継続の設計→実装**（ユーザー決定 2026-07-04）。設計の入口・調査結果＝[`FEATURE_RESEARCH_2026-07-04.md`](FEATURE_RESEARCH_2026-07-04.md)。同書を読めば再リサーチ不要で設計に入れる |

### このセッションでやったこと — **①検証用Gradio GUIのバックエンド追いつき ②フロントエンド計画のための機能リサーチ**

本セッションは前半＝Gradio GUI 追いつき（下記）、後半＝**本家 LTX Desktop との機能比較＋ video-to-video／audio-to-video／プロンプト強化／高品質モードの深掘りリサーチ**を実施し、**優先順位をユーザー決定**した（①video-to-video＝次セッションで設計→実装／②audio-to-video＝近い将来／③静止画・プロンプト強化＝不要／④高品質モード＝遠い将来）。リサーチ全文＝[`FEATURE_RESEARCH_2026-07-04.md`](FEATURE_RESEARCH_2026-07-04.md)。GUI 側の作業は以下。

**進め方**: ①必要機能の洗い出し → ②HTMLモックアップ（アーティファクト）で**仕様をユーザーと合意** → ③実装、の順で進めた。ユーザー仕様修正3点を反映＝(1)既定言語は英語・日本語はSettingsから切替、(2)ダークモード既定、(3)Generateボタンは右カラムの操作パネルへ。実装後にOpusサブエージェントで**完全性監査**（仕様 vs API表層全体）を実施＝9件反映。

**GUI構成（`gradio_ui/` パッケージ）**: Generate（プリセット／多キーフレーム／IC-LoRA参照）・Clip Chain（8スロット）・Jobs・Settings の4タブ＋上部ステータスバー。事前チェック（÷128・拡張子・サイズ・overlap／総フレーム＝サーバーと同じ `chain_math` 流用）とエラー整形（`format_api_error` 15コード）をUI側に実装。GUIは**凍結APIの薄いクライアント**に徹する（AviUtl2統合の予行）。

**客観ゲート（PASS）**: pytest **147 passed / 1 skipped**（skipは既存の `ltx_core` import skip）。モックバックエンド統合スモーク＝`GET /ui` 200（ダークjs・213コンポーネント）／`POST /generate`→completed→mp4 8094 bytes／`POST /generate/chain`（2クリップ）→completed→mp4 17750 bytes。詳細＝**VERIFICATION_LOG §23**。

### 開いているゲート（次セッションで消化・この順）

> **補記（2026-07-04 セッション終盤・ユーザー判断）**: 外出中で目視できないため、**目視ゲート未消化のまま push／main マージを承認**（このGUIはテスト用であり多少の不具合は許容、との判断）。よって下記 1・2 は**マージ後の宿題**として残る。
>
> **さらに補記（2026-07-04・ユーザー明言）**: GUI 目視ゲート（下記1）と実機 e2e（下記2）は引き続き **OPEN** だが、**ユーザーは「後日実施する」と明言**しており、**次セッション（video-to-video の設計→実装）をブロックしない**。次セッションは 1・2 の完了を待たずに着手してよい。

1. **ユーザー目視ゲート（最重要・未実施）**: 実サーバー起動→4タブ全操作→言語／テーマ切替の見た目→**720p生成の音声付き再生をGUI内で**確認。客観PASS≠目視ゲートの規律。
2. **実バックエンドe2e（未実施）**: 720pプリセット・多キーフレーム・pose-controlの÷128違反／遵守・クリップ連結を実機で通す。
3. ~~push／main マージ~~ → **実施済（2026-07-04 ユーザー承認）**。

### 持ち越し（将来項目・記録のみ）

> **優先順位の整理（2026-07-04 ユーザー決定）**: **video-to-video ＝ 次セッションの本題へ昇格**（上表の「次セッション」行・詳細は [`FEATURE_RESEARCH_2026-07-04.md`](FEATURE_RESEARCH_2026-07-04.md) B節）。**audio-to-video ＝ 近い将来**（同 C節）。**静止画生成・プロンプト強化 ＝ 不要とユーザー決定**（同 E節）。**高品質モード（`two_stage_hq`）＝ 遠い将来**だが、非蒸留×量子化 dev 重み（`unsloth/LTX-2.3-GGUF` の `ltx-2.3-22b-dev-Q4_K_M.gguf` 等）という着手の入口は同 D節に記録済み。

- **バックエンドの pipeline／guidance_scale 消費**＝`two_stage_hq` の実効化（＝上記「高品質モード＝遠い将来」）。現状 `services/ltx_runner.py:849-862` の payload がこれらをエンジンに渡さず常にdistilled経路のため、GUIは当該オプションを**表示するが無効化＋「バックエンド対応待ち」注記**にしてある。着手の入口＝[`FEATURE_RESEARCH_2026-07-04.md`](FEATURE_RESEARCH_2026-07-04.md) D節。
- **`GET /jobs/{id}/metadata`**（GUIで peak_vram_mb／backend 表示用）＝**新規エンドポイント不追加の方針**で意図的に見送り。
- `gr.BrowserState` による言語／テーマ永続化（固定secret＋実機検証が必要）。
- `gr.render` による動的キーフレーム／クリップ行（現状は固定スロット）。
- Settings のデフォルト negative prompt 設定（S6では死んだUIを避けるため意図的に見送り）。

### 得られた知見（記録）

- **gradio 6**: theme／js／css の指定が `gr.Blocks()` → `mount_gradio_app()`／`launch()` へ移動した。
- **`build_ui` は uvicorn がlistenする前に走る**ため `/config` はビルド時に取れない → `demo.load` で取得。
- **`gr.I18n` はブラウザロケール依存のみ**＝実行中の言語切替はラベルレジストリ＋`gr.update` 一括更新で実現。
- クリップ連結の総フレーム上限はサーバーと同じ `chain_math` を流用（独自算術を発明しない）。

---

## ▶▶▶▶▶▶▶▶ 最新ステータス（2026-07-04 Phase C実装セッション末）（歴史記録）

> **（歴史記録）本ブロックは上位の「2026-07-04 Gradio GUI追いつきセッション末」ブロックに置き換わった。** これ以降の▶節（Phase C準備末・2026-07-03以前・2026-07-01以前）はすべて歴史記録。食い違ったら最新ブロックが正。
...
## ▶▶▶▶▶▶▶ 最新ステータス（2026-07-04 Phase C準備セッション末）（歴史記録）

> **（歴史記録）本ブロックは上位の「2026-07-04 Phase C実装セッション末」ブロックに置き換わった。** これ以降の▶節（2026-07-03以前・2026-07-01以前）はすべて歴史記録。食い違ったら最新ブロックが正。
