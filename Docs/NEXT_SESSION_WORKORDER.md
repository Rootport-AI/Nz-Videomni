# 次セッション ワークオーダー — Phase 1 残タスク（Gradio 手動確認を除く）
...
## タスク③ keep=1 常駐モードの新設（opt-in・keep=0 既定は不変）

> **方針（2026-07-02 ユーザー決定）**: 既定 keep=0 は**壊さない**。その上で **opt-in の keep=1 常駐モードを新設**する（将来のクリップ連結=Phase3 の連続 I2V で遅延累積を消すための先行投資）。既定 keep=0 経路には分岐を増やさない最小侵襲設計とする。
> **進め方**: ①read-only 実現可能性調査（out-of-place 移動が first-party `engine/` 内か wheel 側かの切り分け＝**実装可否の分水嶺**）→ ②監督/ユーザーで go/no-go → ③go なら branch で実装＋byte-match/VRAM/commit 検証。※調査結果は本節に追記する。
