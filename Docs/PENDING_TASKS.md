...
- 作成: 2026-07-15／最終更新: 2026-09-01（オーナーの台帳見直しにより§3-4・§3-10・§3-15・§3-37・§3-45・§3-46・§3-89・§3-91・§3-102・§3-118の10件をクローズし、§3-52を再訪条件つきの§4-31へ移した。同日それ以前に、🎞ボタンで挿入したオブジェクトが青一色になる症状を§3-140として起票し、軽微修正4件がオーナーの実機目視ゲートに合格したため§2は節ごと削除済み。同日さらに、§3-42を「1.86Mpx超の解像度におけるSingle長尺クリップの実現」と改めて再訪条件つきの§4-32へ移し〔§3-42は欠番〕、§3-13〔素材fpsの取得〕を新しい調査結論〔Media Foundation方式で実現可能〕とオーナー方針で書き直し、その方針の覚え書きを§3-71・§3-72へ1行ずつ足した）
...
**§3-97（マルチエンジン化）・§3-98（LTX 2.5対応）・§3-102（LTX 2.5でまだ使えない機能の対応）は本書では欠番**——記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-97・§3-98・§3-102にある。**§3-98はv1＝基本生成のスコープでのクローズだが、その残りを引き取っていた§3-102も2026-09-01にクローズした**ので、いま本書に生きているのは下表の各項目である。
...
**本テーマ由来の3件は2026-09-01にクローズした**——LTX 2.5を選んでいる間に`vae_mode`を選べてしまい422になる件＝[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-139、LTX 2.5でまだ使えない機能の対応＝同§3-102、フロントエンド`API_REFERENCE.md`の`vae_mode`の失効記述＝同§3-118。本表は本書§3の生きている項目だけを並べる表である。
...
#### 3-13. 素材fpsの取得と操作パネルへの流し込み（Media Foundation方式で実現可能・実装は未着手）

- **概要**: 右クリックプリフィルのSettings方針「素材に合わせる」を、fpsについても選択オブジェクトが参照する素材そのもののフレームレート（例えば30フレーム毎秒で撮影した動画素材を、プロジェクト24フレーム毎秒のタイムラインに置いている場合の30という値）で操作パネルへ流し込めるようにすること。サイズ軸は選択素材の実寸（`mediaWidth`/`mediaHeight`）を取得できるため既に実現しているが、fps軸は現在**プロジェクト全体のフレームレート**（`timeline.getSelection`が返す`rate`/`scale`から求めた値）を代用している。
- **調査結論（2026-09-01。従来の「軽い拡張では実現できない」を覆した）**: AviUtl2 SDKの`get_media_info`（`MEDIA_INFO`にfpsもフレーム総数も無い）では取れないが、**WindowsのMedia Foundation**（SourceReader＋`GetNativeMediaType`＋`MF_MT_FRAME_RATE`）なら素材fpsを**分数のまま**（29.97＝30000/1001）取得できる。必要なライブラリ（mfplat／mfreadwrite／mfuuid）はビルドで全てリンク済みで、同一プロセス内にMedia Foundationを使う実績と作法もある（`native/src/mf_mp4_writer.cpp`）。**当初見積もりの「入力プラグイン経由の中規模ネイティブ実装」は不要**で、**合計200行前後の小規模**で済む。
- **方針（オーナー決定2026-09-01）**: ①取得は**Media Foundation方式**とする（シェルプロパティ`System.Video.FrameRate`は1000倍の整数で丸めが入り、mkvはシェル拡張の有無という環境依存があるため採らない）。②流し込み先は`timeline.getSelection`応答への`mediaFps`追加（契約v11・optional）——既存設計（`mediaWidth`等の並び・プリフィルの純関数）に整合する。③**非整数fpsは整数へスナップする**（29.97→30、23.976→24）。④取得に失敗したときは、現行どおりプロジェクトfpsへフォールバックする。
- **着手時の注意点**: AviUtl2はbeta10でMedia Foundationのファイルリーダーを削除しL-SMASH Worksが標準構成であるため、**「AviUtl2が読めるファイル」と「MFがfpsを答えられるファイル」は同じ集合ではない**（mp4/movは確実・mkv/webmは不確実で、取得失敗は異常ではなく正常系として扱う）。`MFStartup`はプロセスで1回。UIスレッドとのスレッド配置は実装計画時に決める。
- **現在の措置**: FPS軸の選択肢「素材に合わせる」は機能を持たず、Settingsのボタンはグレーアウトして残してある（撤去はしない）。保存済み設定値が「素材」だった場合はfps軸を「プロジェクトに合わせる」へ自動フォールバックする。サイズ軸の3択は有効（素材実寸は取得できるため）。
- **状態**: 未着手（着手時期未定・実装プランは未策定で、方針だけが決まっている段階）。**優先度が高くない理由は、多くの制作フローで素材とプロジェクトのfpsが一致しており現状の代用でも実害が小さいことであって、規模の大きさではなくなった。**
- **出典**: Microsoft Learn（`MF_MT_FRAME_RATE`・`System.Video.FrameRate`）、`native/src/mf_mp4_writer.cpp`（同一プロセス内でMedia Foundationを使う既存の作法）、[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §41.7・§47、[`RIGHTCLICK_REDESIGN_SPEC.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/RIGHTCLICK_REDESIGN_SPEC.md) §5-13、[`BRIDGE_CONTRACT.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/BRIDGE_CONTRACT.md) §4.13、`AviUtl_ExEdit2 SDK`の`get_media_info`／`MEDIA_INFO`、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-34。
...
**新しい422経路へ実操作で到達できると分かったとき、または短クリップのクライアント側ゲート（X3）を緩める改修をしたとき** → 422後に仮オブジェクトの予約が詰まらないことを実機で確認する（詳しくは[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-45〔旧§4-23。同書には2026-09-01クローズの別項目§3-45-02もあるが、本項が指すのは無印の§3-45である〕）。422経路`REFERENCE_REQUIRES_CONTROL_LORA`は単発・chainの両方に存在する（バックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.7）が、フロントエンドは参照動画に制御LoRAの同伴を必須とするゲートを持つため、通常の操作ではこの422に到達しない。**このゲートを迂回して実操作で422を出せると分かったら、予約詰まりの再検証を実施する**。

#### 3-33. アップロードに時間がかかる状況ができたとき → `referenceUploading`ゲートの再検証

**長尺・大容量の参照動画や低速なストレージ・リモートのバックエンドでアップロードに時間がかかる状況が生じたとき、またはアップロード中の進捗表示・スピナーを設ける改修をしたとき** → 「アップロードが確定するまでGenerateを無効化する」ゲートを実機で確認する（詳しくは[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-46〔旧§4-24。同書には2026-09-01クローズの別項目§3-46-02もあるが、本項が指すのは無印の§3-46である〕）。ローカルでは速すぎて「まだ確定していない一瞬」を捉えられず、検証不能・暫定合格でクローズしている（単体挙動は自動テストで確認済み）。
...
- **覚え書き（オーナー決定2026-09-01）**: 素材fpsのプリフィルでは**非整数fpsを整数へスナップする**方針が決まった（本書§3-13の方針）。ただしこれはプリフィルの値の話であり、**プロジェクトのfpsが29.97のままなら本項の422はそのまま起きる**——スナップは本項の代替にならない。
- **状態**: 将来の改修項目（着手時期未定）。
...
- **覚え書き（オーナー決定2026-09-01）**: 素材fpsのプリフィルでは**非整数fpsを整数へスナップする**方針が決まった（本書§3-13の方針）。**本項の音ズレは書き出し側の切り捨てなのでスナップでは解消しない**が、素材が29.97でもプリフィルが30を入れる分、利用者が非整数fpsを指定する機会は減る。
- **状態**: 将来の改修項目（着手時期未定）。
...
- **出典**: `engine/gguf/ic_lora_common.py`（`_SUFFIX_A`と`load_ic_lora_pairs`）、[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §71.6（14本の照合結果と、kohya形式2本を判定対象外とした理由）、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-102（LoRAのベースモデル軸の判断材料(2)）。
...
- **なぜ効くか**: いまは PrunaVAED（`vae_mode`）が LTX 2.5 で422になる（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-102）ため選べる値が実質1つで実害は小さいが、**LTX 2.5版の枝刈り済み重みが上流に出て開通したときには確認の穴になる。**
- **完了条件**: LTX 2.5 のジョブの`metadata.json`に`vae_mode_used`が実使用値で書かれること。
- **同種の課題**: engine25固有の情報が`metadata.json`に載らないという、より広い論点が[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-102の判断材料(4)にある。**そちらとまとめて直すのが自然である。**
- **状態**: 未着手（将来の改修項目）。
- **出典**: バックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §84.6、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-102。
...
### 4-31. 失敗ジョブの`metadata.json`が必要になったとき → 失敗経路でも書き出す（起票：2026-08-03、本節へ移動：2026-09-01）
...
### 4-32. 1.86Mpx超の解像度におけるSingle長尺クリップの実現（起票：2026-07-27、本節へ移動：2026-09-01）

**1.86Mpx（Chainedのstage-2固定窓設計の解像度上限）を超える解像度で、長尺クリップの需要が実際に生じたとき** → 単発生成（`/generate`）へチャンク化アップサンプルとstage-2のタイル化を移植できるかを検証する。

- **何が塞いでいるか＝前提の変更**: LTX 2.5でChainedの窓22が1088pでも快適上限に収まり、ドリフトもほぼ不可視だと実証されたため、**「高解像度×長尺」という需要の大半はChainedが回収した**。SingleとChainedの役割分担（Single＝解像度自由・尺制約／Chained＝解像度制約・尺自由）を守るというオーナーの戦略判断で本節へ降ろした（2026-09-01）。**移植してもSingleはAPI上限481フレームを超えられない**という天井も変わらない。
...
- **クローズした項目の跡地に説明文は残さない**（記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)および`DEVLOG.md`等の出典へ移す）。**例外は「読み違えを防ぐ1行」だけ**——たとえば「§3-97・§3-98・§3-102は本書では欠番であり、記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)の同番号にある」のように、番号を見た人が誤った結論に至るのを防ぐ短い注記は置いてよい（§3冒頭の1行がこの例外にあたる）。
