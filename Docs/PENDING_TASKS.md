...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了：2026-08-23に連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を実装してオーナー合格。2026-08-24にスタイルLoRAとIC-LoRA〔長尺を含む〕を実装して全ゲート合格し、2026-08-25にオーナー合格。同じく2026-08-24に高速化第1弾〔fusedカーネルと先読みblock swap〕、2026-08-25に高速化第2弾〔keep_resident＝テキストエンコーダの常駐〕も実装し全ゲート合格**）
...
- **残っているもの（本項の実体）**: 下表の7件。**行の数は`unsupported_features`の7件（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`／`sage_attention`）と1対1に並ぶが、右端の「断っているフィールド」列は1対1ではない**——NAG／VSFの行に並べた`neg_method`／`vsf_scale`は422で断っているフィールドではなく、「無視して先へ進める」側にある（422になるのは`nag_enabled`だけである。下記の判断材料(1)）。**行を機能名の一覧として読み、フィールド単位の正本は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10(d)・(f)の4分類表を見ること。** 上から順に着手する（オーナー裁定2026-08-23の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」に沿う）。**必須級は2026-08-24で出尽くし、同日に高速化第1弾（fusedカーネル・先読みblock swap）が、翌2026-08-25に高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）が完了した。次に着手するのは表の先頭＝高速化第3弾（SageAttention）である。**

| 段階 | 残項目 | 断っているフィールド |
|---|---|---|
| **高速化技術の前倒し（第3弾）** | SageAttention。出力の細部が変わるので**PSNR（ピーク信号対雑音比。元の絵とどれだけ違うかを1つの数値で表す指標で、単位はdB。数値が大きいほど元の絵に近い）を基準にした判定**が要る（§43の注意書きと同じ） | `attention_backend` |
...
| 見るところ | 現在値 | 完了時にどうなるか |
|---|---|---|
| `GET /models`の`unsupported_features`（LTX 2.5） | **7件**（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`／`sage_attention`） | 実装した機能の名前が1つ消える。0件になれば本項は完了。**`keep_resident`は2026-08-25にここを抜けた（高速化3弾の中では初めての事例。この配列そのものは2026-08-23・2026-08-24にも動いている）** |
| `CHAIN_REJECT_TABLE`（連結生成用） | **6行**（`retake`／`end_source`／`nag_enabled`／`pipeline`／`vae_mode`／`attention_backend`） | 該当行が1行消える。**`source_video`／`source_audio`は2026-08-23に、`loras`／`reference_video_id`は2026-08-24に、`keep_resident`は2026-08-25にここを抜けて動作側へ移った** |
| `REJECT_TABLE`（単発生成用） | **5行**（`pipeline`／`outpaint`／`nag_enabled`／`vae_mode`／`attention_backend`） | 同上 |
