...
| §3-131 | LTX 2.5の`metadata.json`に`vae_mode_used`が書かれない | 方針確定・実装未着手 |
...
**同じ2026-09-01に、さらに2件が本表から外れた**——§3-123（G8の街路の単体動画の境界勾配比）は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-123へ移してクローズし、§3-103（拡散デコーダ版の映像VAE＝DiffVAE）は再訪条件つきで本書§4-33へ降ろした。**本書側の2番号はどちらも欠番である。**
...
- **方針（オーナー決定2026-09-01。実装は未着手）**: **`vae_mode_used`は「どのデコーダで復号したか」の記録と位置づけ、engine25が実際に載せた映像VAEの種別を報告する経路を新設する。** engine25は`is_diffusion_video_vae`の判定に基づくデコーダの実名（現行の構成なら`"conv"`）を求め、ワーカーの`done`イベント → アダプタ → `metadata.json`のリレーでそれを流す。**LTX 2.3の`"on"`／`"off"`／`"on->off"`という値の意味は従来どおりで、2.3側は無改修である。** 将来DiffVAE（本書§4-33）やLTX 2.5版のPrunaVAEDが開通したときは、その実名がそのまま入る。
- **完了条件**: LTX 2.5 のジョブの`metadata.json`に、実際に使った映像VAEのデコーダ名が`vae_mode_used`として書かれること（LTX 2.3の値の意味と書き出しが変わっていないことも同時に確かめる）。
...
- **状態**: **方針確定・実装未着手**（将来の改修項目）。
- **出典**: バックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §84.6、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-102、[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §10（`is_diffusion_video_vae`と既定デコーダの現状）、オーナー決定（2026-09-01）。
...
### 4-33. 拡散デコーダ版の映像VAE（DiffVAE）を採用するかどうか（起票：2026-08-22、本節へ移動：2026-09-01）
...
- **決定性は「壊れる」のではなく、条件つきで保てる**（本項の旧記述の訂正）: 実機重みは `default_num_inference_steps=1`・`model_output_type="x0"` で反復せず、拡散ブロックはデコーダ重みの約2%である。雑音は `torch.Generator` で固定できる。環境によって出力が変わる原因は、**`AUTO_TILING` のタイル表が実行時の空きVRAMから決まること**——1ステップ時の雑音はタイルごとに独立に引かれるため、タイル表が乱数の消費順序を決める。**明示 `TileSizeConfig` と固定 `generator` を組み合わせれば環境非依存になる。** 正本は同 RESULTS.md §5・§6。
