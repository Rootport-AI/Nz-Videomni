...
| 見るところ | 現在値 | 完了時にどうなるか |
|---|---|---|
| `GET /models`の`unsupported_features`（LTX 2.5） | **7件**（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`／`sage_attention`） | 実装した機能の名前が1つ消える。0件になれば本項は完了。**`keep_resident`は2026-08-25にここを抜けた（この配列が動いた最初の事例）** |
...
**一方、高速化第2弾（2026-08-25）ではこの3つの表がそろって動いた。** `keep_resident`は**もともと422で断っていた機能**なので、開通させると3表すべてから1行ずつ消え、`unsupported_features`は8件→7件になった——**この配列が動いたのは第2弾が初めてである。** つまり上の「動く表が違う」という区別は、第1弾（第2弾ではない側）と第2弾を並べて見ると具体的に確かめられる。**画面のグレーアウトも1つ解けたが、フロントエンド本体の変更は0行である**（Settings画面は`unsupported_features`を読んでいないため。触ったのはモックの2箇所だけ）。
...
- **なぜ起票したか（効果の見積り）**: **費用はテキストエンコーダと同格である。** 第2弾が消したテキストエンコーダの構築は6〜8秒で、それが0.4秒になって**ジョブ全体が約25%短縮**した。埋め込み処理器はその半分ほどの費用があり、しかも**`keep_resident`をONにしてもこの区間はまったく縮まない**（別の登録簿を使っているため。実測でもON／OFFを問わず3.3〜3.9秒で安定していた。G0の基準線では最大5.7秒）。つまり**第2弾の取りこぼしがそのまま残っている**。
