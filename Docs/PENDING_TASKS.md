...
- **判断材料（0）高速化技術を入れる前に、比べる物差しを作ってある**: 固定ベンチマーク**B1〜B4**（幾何・シード・プロンプト・画像・経路をすべて固定した4本）を[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) **§72.7**に台帳化した。高速化技術を1つ入れるたびにこの4本を走らせ直し、mp4のSHA-256が一致すれば「絵は1バイトも変わっていない」と言い切れる。**この物差しがあるので、高速化を必須級の途中へ前倒しで割り込ませても回帰を見失わない。**
...
- **状態**: 部分完了（連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を2026-08-23に実装した。連結生成の本体は同日オーナー目視も合格し、V2V・A2Vの実機ゲートは[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §73にある。残り8件は未着手で、次に着手するのは表の先頭＝Style LoRAである）。
