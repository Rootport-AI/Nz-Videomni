...
- 作成: 2026-07-15／最終更新: 2026-09-14（§2を立て直し、§3-55を§2-10へ移設、マスク受け渡し契約の正本を`INPAINTING_DESIGN.md`へ移した。現在の構成は**§2 実装済み・ユーザーのテスト待ち／§3 将来の研究課題／§4 スコープ外**の3節。§1〔近日中の改修項目〕は該当項目が無いため、運用規則どおり見出しごと削除してある）
...
#### 2-10. Inpainting（マスクによる部分再生成）— 実装済み・オーナーの実機採取と実機ゲート待ち（§3-55から移設、2026-09-14）
...
- [ ] **(k) 白化効果の実書式**: 部分フィルタに「反転」を足して「輝度反転」をオンにしたオブジェクトの`.object`原文を右クリック→エイリアス保存で採る（枠の内側が白・外側が黒になること、ぼかしで灰色の勾配になることも目視） → プラグインが書く白化効果のバイト定数（効果名・項目名）が確定し、`native/src/alias_util.cpp`の`PROVISIONAL`ブロックを実測値へ差し替えられる（[`SDK_REFERENCE.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/SDK_REFERENCE.md) §16(k)）。
- [ ] **(l) 描画順の確認**: 部分フィルタをレイヤー5に、色つきの図形をレイヤー3と7に置いて、どちらが枠の内側に現れるか目視する → 黒い背景と部分フィルタの複製を`layer_max+1`／`layer_max+2`のどちらに置くべきかが確定し、仮定と逆なら`RenderMaskVideoWorker`の2つのレイヤー番号を入れ替える（同§16(l)）。
- [ ] **(m) `frame=`の絶対/相対確認**: 中間点を2つ持つ部分フィルタで(k)の採取をやり直し、「読み出し→そのまま`create_object_from_alias`で作成→再保存」の往復で`frame=a,b,c`の並びが保たれるか確かめる → `get_object_alias`が絶対番号を返すか相対番号を返すかが確定し、`NormalizeAliasFrameBoundariesToRelative`による正規化が実際に必要かどうかが裏づけられる（同§16(m)）。
...
- [ ] **G1 右クリック**: 部分フィルタと動画をそれぞれ右クリックする → 部分フィルタには「🖌 Inpainting」、動画には「🎬 Video」の項目が出て、対象外の種別には出ないか既存の案内で断られること。
- [ ] **G2 順序不問**: #22→#23、#23→#22の両順序で右クリックする → どちらの順序でもカード2枚が揃い、2回目で1回目の内容が消えず、対象動画の二重アップロードが起きないこと。
- [ ] **G3 解像度不一致**: プロジェクトと異なる解像度の動画を対象動画にする → 実数入りの専用文言が出てGenerateが押せなくなること。
...
- [ ] **G6 動くマスクでの生成**: 中間点で枠が動く部分フィルタでGenerateする → 完成動画の解像度・尺が対象動画と一致し、描き替え領域が枠の動きに追従し、音声が元のままであること。
- [ ] **G7 追尾との排他**: マスク描画中に物体追尾を、追尾中にマスク描画をそれぞれ試す → 後から呼んだ方が`TRACK_BUSY`／`MASK_BUSY`で断られ、先発は完走すること。
- [ ] **G8 LTX 2.5**: LTX 2.5への切り替え中・切り替え後の画面を見る → 読み込み中はInpaintingサブタブが灰色になること。
- [ ] **G9 部分フィルタの消失**: カード表示後に部分フィルタを削除してGenerateを押す → 案内文で止まり生成が投入されないこと。
- [ ] **G10 回帰**: `outputs/inpaint_regression/`のツールで改修前後のフィンガープリントをLTX 2.3・2.5の両エンジンで比較し、撮り直し・追尾・cutoutRangeも操作する → 出力SHA-256が両エンジンで改修前と一致し、他機能も従来どおり動くこと。
...
- [ ] **③G10の改修前SHAの採取方法**: mainを一時的に起動して`outputs/inpaint_regression/`のツールで改修前のフィンガープリントを採るかどうかの判断。
