...
- 作成: 2026-07-15／最終更新: 2026-09-11（現在の構成は**§1 近日中の改修項目／§3 将来の研究課題／§4 スコープ外**の3節。§2〔実装済み・ユーザーのテスト待ち〕は該当項目が無いため、運用規則どおり見出しごと削除してある）
...
### 1-29. 物体追尾（部分フィルタの枠をUETrackに追わせる）（起票：2026-09-11）

- **概要**: 重量級の動画生成AIとは別のモジュールとして軽量ユーティリティAIの置き場を新設し、最初の道具として**単一物体追跡AI（UETrack Base・CPU固定）**を載せる。利用者はAviUtl2のプレビューで部分フィルタの枠を被写体に合わせ、タイムラインの右クリックから追尾を呼ぶ。成果物は部分フィルタへの中間点の書き戻しだけで、サーバーには保存物を残さない。効果（モザイク等）は利用者が自由に足す。
- **設計の正本は[`OBJECT_TRACKING_DESIGN.md`](OBJECT_TRACKING_DESIGN.md)である**（責務分担・API契約・プラグイン側の手順・エイリアスの書式・設定項目・検証の関門・否決した案。**本項には写さない**）。
- **段取り**:
  - **M0 設計書と台帳** — 設計書の初版を`feature/object-tracking`へpushし、オーナーがGitHubで確認する。**同時にオーナーへ実機採取を依頼する**（下記）。
  - **M1 バックエンド** — 導入経路（`install-UETrack.bat`・記述ファイル・専用仮想環境の作成）を先に作り、続いてスパイク（CPU速度とスコア分布の実測・重みの絞り込み）、重みの新設リポジトリへの配置、ワーカーとAPIの実装。
  - **M2 プラグイン** — 右クリック項目・追尾のループ・後処理・エイリアスの書き戻し。出口でオーナーの実機確認を通す。
  - **M3 操作パネル** — Toolboxタブの実タブ化と物体追尾の画面。
  - **M4 文書とmerge** — 関連文書の更新と、オーナー合格後のmainへのmerge。
- **オーナーへの依頼（M0と並行）**: **部分フィルタに中間点を2つ打ち、位置と大きさを変えたうえで、エイリアスをファイルへ保存して渡してほしい。** 中間点つきのエイリアスの実書式はリポジトリ内に一次資料が無く、この採取物が正本になる（設計書の該当箇所には〔採取待ち〕の印を付けてある）。あわせて実機確認の依頼項目は設計書の「検証の関門」節にまとめてある。
- **状態**: 近日中の改修項目（設計は確定済み）。作業は`feature/object-tracking`ブランチで進め、実装が済んだら運用規則どおり§2〔実装済み・ユーザーのテスト待ち〕を立て直して移す。
- **関連**: マスクの受け渡し契約は§3-55が正本（本項からは切り離してある）。
...
- **概要**: AviUtl2側で作ったマスク動画を入力として、映像の一部だけを部分再生成する機能。
- **本項がマスク受け渡し契約の正本である**: 次の3点が、マスクを扱う全機能の共通契約である。**応用先（§4-8(C)の`conditioning_attention_mask`の露出・§4-25のAviUtl2側で作ったマスク動画の受け口）は、契約そのものを各項で作り直さないこと。**
  - (a) マスク加工（モザイク等）はAviUtl2ネイティブのフィルタ効果側で行い、マスク自体はバックエンドへ返さない。**インペイント用途のときだけ返す。**
  - (b) マスク形式はピクセル精度1本に統一する。潜在解像度への丸めは受け手（バックエンド）側の責務とする。
  - (c) 受け渡しは1chグレースケール動画として、既存のuploadsレールを流用する案が有力。非可逆コーデック対策として、受信側での二値化を契約に含める。
- **マスクはAviUtl2側のオブジェクトから作る**: 追尾（§1-29）で動かした部分フィルタでも、利用者が手で作ったオブジェクトでも、他者のプラグインが作ったものでも、契約から見ればいずれも同じ「AviUtl2側のオブジェクト」である。**追跡AIの出力（矩形）から直接マスク動画を作る経路は設けない**（矩形を白く塗るだけの処理をサーバーへ置く必然性が無く、保存物の議論を追尾側へ持ち込むことになるため。理由の正本は[`OBJECT_TRACKING_DESIGN.md`](OBJECT_TRACKING_DESIGN.md)）。
- **前提**: §1-29（物体追尾）の第1弾の完成。
- **設計メモ**: 潜在空間は空間1/32・時間1/8であるため、マスクは32px角のブロック精度に丸まる（「髪の毛一本だけ」といった精密指定はできないという期待値調整をUXに織り込む必要がある）。上記(b)のとおり、マスクはピクセル精度のまま受け取り、バックエンド側で丸める。
- **土台**: 時間軸版のRetakeは実装済み（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-73）で、本項は領域版にあたる。固定しているwheel（`ltx_pipelines`）に`RetakePipeline`（動画から初期latentを作りmaskで部分デノイズする）が実在するので、ゼロからの発明にはならない。
- **フロントエンド側の見込み規模**: マスク動画を組み立ててアップロードする導線の新設。追尾そのもののUXは§1-29で作るため、本項の範囲外である。
- **状態**: 将来の研究課題（着手時期未定・§1-29の完成待ち）。
...
- **何が塞いでいるか**: **マスクの受け渡し契約が未確定であること。契約の正本は本書§3-55**（ピクセル精度1本・1chグレースケール動画・uploadsレール・受信側の二値化）で、**本項はその応用先の1つである**——契約を本項で作り直さないこと。受け口の入口設計は§4-25と同じ話である。
...
- **本項は§3-55のマスク契約の応用先である**: **受け渡しの形（ピクセル精度1本・1chグレースケール動画・uploadsレール・受信側の二値化）は本書§3-55が正本**で、本項が扱うのは**その入口をAviUtl2側の既存プラグインへ繋ぐこと**である。ほかの応用先は§4-8(C)（`conditioning_attention_mask`の露出）である。
