...
### 2-3. 埋め込み処理器の常駐（`keep_resident_embeddings`）の画面目視 — 本書§3-114の移送先（起票：2026-08-25／実装：2026-09-03）

LTX 2.5の埋め込み処理器（embeddings processor＝プロンプトを読み取ったあとの内部表現を整える部品）をジョブ間で常駐させる切替を、Settingsの「生成の高速化」へ追加した。**実装・機械ゲート・実機ゲート（API側）はすべて合格済み**で、方式・実測値・合否の正本はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §92である（本書には書き写さない）。**出力のSHA-256が凍結値と一致しているので、生成物そのものの目視ゲートは設けていない**——残るのは画面の見え方だけで、**この表がその正本**である。

- [ ] **U1**: ベースモデルにLTX 2.3を選んだ状態でSettingsを開く → 「生成の高速化」に「埋め込み処理器の常駐（LTX 2.5）」の行が**見えない**（グレーアウトではなく、行ごと消える）。
...
素材（冒頭）＝V2V（動画から続きを生成する機能）を使った連結生成で、`duration_seconds`（尺）が実際に届くmp4より長い値を返していた食い違いを直した。**自動ゲート（モックによる三者一致テスト＝実ファイル・`metadata.json`・ジョブ状態API）は合格済み**で、方式と結果の正本はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §93である（本書には書き写さない）。**フロントエンドは1行も変えていない**——値が直れば履歴カードの表示はひとりでに正しくなる。
