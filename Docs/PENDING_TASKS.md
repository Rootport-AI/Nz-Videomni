...
- 位置づけ: **セッション開始時に「次に何をすべきか」を確認するための台帳**。プロジェクト全体（バックエンド `Nz-Videomni` と、フロントエンド `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2`）の課題をここへ一本化している。優先度の高い順に次の4つへ分けている（**運用規則は末尾「本台帳の位置づけ（運用規則）」節に一本化**してある）。
  ...
  2. **実装済み・ユーザーのテスト待ち** — 実装は完了しているが、オーナー本人による実機・目視・実GPUでのテストがまだ済んでいないもの。全項目が合格して空になった節は見出しごと削除する運用のため、その時点では本節が無い場合がある（**現在は§2-1の1件がある**）。復活させるときは**チェックリスト形式**で書く——各項目を「何を操作して確認するか → どうなれば合格か」の1〜2行にし、`- [ ]`の箇条書きを画面・機能ごとの小見出しでまとめる。経緯や完了宣言はここに書かず[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)か各出典へ落とし、本節からは「経緯: 〜」の一言参照にとどめる。テストではなく仕様の是非をオーナーが判断する項目は§2-3（オーナー判断待ち）に分離する。
...
実装は完了しているが、オーナー本人による実機・目視・実GPUでのテストがまだ済んでいないもの。**合格したら本節から除去し、記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)と各出典へ落とす。**

### 2-1. LTX 2.5のSageAttention（`attention_backend`）の目視ゲート（G8）

**素材は`outputs/ltx25-sage-gate/g8_material/`に置いてある**——`sdpa`と`sage`の対が3組（**A**＝1280×768・2クリップ×121フレーム／**B**＝1280×768・257フレーム単発／**E**＝512×320・121フレーム単発）、`sage`単独が2本（**C**＝1920×1088・2クリップ×169フレーム／**D**＝512×384・IC-LoRA canny）、静止画26枚、読み方を書いた`README.md`。

#### 絵の見比べ

- [ ] **Aの対（`sdpa`／`sage`）を見比べる** → 構図と被写体が同じままで、変わっているのが質感やノイズの出方だけなら合格。
- [ ] **Bの対を見比べる** → 同上。**Bの2本の秒数の差を速さの成果として読まないこと**（`sdpa`側はサーバー再起動直後の1本目にあたる。速さの正しい読み方はAの対のほう）。
- [ ] **Eの対を見比べる** → 小さい画面ほど絵の変化は大きく出る。ここが許容できないなら「小さい画面では`sage`を選ばない」という案内を足す判断になる。
- [ ] **CとDの`sage`単独2本を見る** → VRAMが最大になる形状と、参照動画つきの経路で、絵が破綻していなければ合格。

#### 音の聴き取り（1箇所だけ）

- [ ] **Cの接合を1箇所、耳で聴く** → 継ぎ目が耳障りでなければ合格。自動判定では閾値をわずかに超えたが、**同じシード・同じ形状の`sdpa`版でも同じ接合が自身の最悪値を記録**しており、映像側は両方とも全接合が「連続」判定なので、機械では白黒がつかない。

- **不合格だったときにどうするか**: 既定は`"sdpa"`のままなので**利用者に影響を出さずに保留できる**。案内文を足す・特定の形状で勧めない、といった対応で足りるのか、開通そのものを見送るのかはオーナー裁定による。
- **経緯・依頼内容の正本**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77（依頼の全文と注意2点は同 §77.6）。本書§3-102も参照。
...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**6件**） | 部分完了。**高速化の前倒しは2026-08-25の第3弾（SageAttention）で打ち止め**（`unsupported_features`は7件→6件。**ただし第3弾はオーナー目視待ち＝本書§2-1**）。残るPrunaVAEDはLTX 2.5版の枝刈り済み重みが出るまで先送りなので、次に入るのは準必須級（Retake／End source／Outpainting）だが、着手の順序と時期はオーナー判断待ち |
...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了：2026-08-23に連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を実装してオーナー合格。2026-08-24にスタイルLoRAとIC-LoRA〔長尺を含む〕を実装して全ゲート合格し、2026-08-25にオーナー合格。同じく2026-08-24に高速化第1弾〔fusedカーネルと先読みblock swap〕、2026-08-25に高速化第2弾〔keep_resident＝テキストエンコーダの常駐〕と高速化第3弾〔SageAttention〕も実装し全ゲート合格——ただし第3弾はオーナー目視待ち**）
...
- **済み（2026-08-25実装。ただし**オーナー目視待ち**）**: **高速化第3弾＝SageAttention（`attention_backend`）**。**既定は`"sdpa"`のまま据え置いた**（利用者が設定画面で明示的に`sage`を選んだときだけ効く）。**本弾だけは「選ぶと出来上がる絵の細部が変わる」種類の高速化なので、固定ベンチマークのSHA-256一致では合否を出せない**——機械ゲートG0〜G3と実機ゲートG5（全11項）は全項目合格したが、**オーナーの目視ゲート（G8）は未了である**（依頼内容と素材は本書§2-1、実測と判定の作り方は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77が正本）。`unsupported_features`は7件→6件になり、設定画面のattentionの選択がLTX 2.5でも押せるようになった（**押せるようになったのは`GET /status`の`sage_available`が真になったためで、この配列が減ったからではない**。同 §77.4）。**あわせて既存の罠が1つ消えたが、`vae_mode`側には同型の罠が残っている**（下記の判断材料(9)）。
- **オーナー合格**: 連結生成は2026-08-23、V2V継続とA2Vは2026-08-24、**スタイルLoRAとIC-LoRAは2026-08-25**（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.10・§73.10・§74.12）。**長尺A2Vはオーナー裁定で合格扱い**（裁定の全文と理由は同 §73.10(2)）。**いずれもAviUtl2の操作パネルからの実機検証で判定している。**
- **残っているもの（本項の実体）**: 下表の6件。**行の数は`unsupported_features`の6件（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`）と1対1に並ぶが、右端の「断っているフィールド」列は1対1ではない**——NAG／VSFの行に並べた`neg_method`／`vsf_scale`は422で断っているフィールドではなく、「無視して先へ進める」側にある（422になるのは`nag_enabled`だけである。下記の判断材料(1)）。**行を機能名の一覧として読み、フィールド単位の正本は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10(d)・(f)の4分類表を見ること。** 上から順に着手する（オーナー裁定2026-08-23の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」に沿う）。**必須級は2026-08-24で出尽くし、同日に高速化第1弾（fusedカーネル・先読みblock swap）が、翌2026-08-25に高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）と高速化第3弾（SageAttention）が完了した**（第3弾はオーナー目視待ち）。**高速化の前倒しはここで打ち止めで、残るPrunaVAEDはLTX 2.5版の枝刈り済み重みが出るまで先送りである。したがって次に着手するのは準必須級（Retake／End source／Outpainting）だが、着手の順序と時期はオーナーの判断による。**
