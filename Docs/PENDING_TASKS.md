...
- 作成: 2026-07-15／最終更新: 2026-09-02（**§3-39・§3-117をクローズし、§3-142を起票した**。クローズ分の移設先は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-39-02・§3-117）
...
| §3-142 | 逆順ChainedでIC-LoRA参照動画の窓が生成順に配られる（LTX 2.3のみ） | 未着手（優先度低） |
...
#### 3-142. 逆順ChainedでIC-LoRA参照動画の窓が生成順に配られる（LTX 2.3のみ）（起票：2026-09-02）

- **概要**: `engine/pipeline/chain_pipeline.py:1701-1702`は、参照動画の窓を`next(ref_windows, None)`で**生成順**に消費している。逆順Chained（素材（末尾）×クリップ2本以上）はセグメントを末尾から先頭へ向けて生成するため、この経路でIC-LoRA参照動画を併用すると、**最後に生成されるクリップへ窓0（参照動画の先頭区間）が渡ってしまう**——窓とセグメントの対応がタイムライン上の位置ではなく生成順で決まる。
- **LTX 2.5は正しい**: `engine25/chain25.py:2745`は`ref_conds[i]`をタイムライン添字`i`で直接引く設計で、生成順に依存しない。**2.3固有の実装差である。**
- **実害の見立て**: 映像側のみの問題（音声側は無関係）。ただし**逆順Chained×IC-LoRA参照動画という組み合わせ自体、現行のフロントエンド操作ではほぼ到達不能**とみられる——着手時はまず実操作でこの組み合わせに到達できるかどうかの確認から始めること。
- **出自**: [`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-117（逆順Chainedの音声の継ぎ目調査）の副産物として2026-09-02のコード読解で発見。
- **状態**: 未着手（将来の研究課題・優先度低）。
- **出典**: `engine/pipeline/chain_pipeline.py:1701-1702`、`engine25/chain25.py:2745`、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-117。
