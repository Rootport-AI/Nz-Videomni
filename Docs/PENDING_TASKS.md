...
| §3-144 | 参照エンコードのタイル化の残件 | 未着手（トリガー待ち） |
...
#### 3-144. 参照エンコードのタイル化の残件（起票：2026-09-05）

- **トリガー**: 参照つきの長尺（単一クリップで361フレーム超に相当する長さ）でIC-LoRAの制御の精度が落ちたと体感したとき、またはその報告が上がったとき。
- **内容**: [`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-76（参照動画のVAEエンコードのしきい値自動タイル化）は、オーナー裁定により最小仕上げで完結させた。繰り延べたのは次の3つである。
  1. **LTX 2.5に残る小さな実差の原因の切り分け**——タイル化した参照の潜在に、LTX 2.3より大きい誤差が残っている。有力仮説（時間方向の重なりが2.5のVAEの受容野に足りていない）と、切り分けの実験設計（タイル幾何のクロスと重なりの掃引）は出典にまとめてある。2.5が落ちた場合の分岐は、着手前にオーナーの裁定を取ること。
  2. **本番の制御動画コンテンツでの測り直し**（当初計画のB-1改）——決着済みの測定は生の動画を入力に使っており、canny（輪郭線）・pose（姿勢）系の本番の入力である前処理済みの制御動画ではない。
  3. **快適バナーの線の再較正**（当初計画のゲートC相当）——しきい値は導入時にバナーの線に合わせて固定してあり、線そのものは動かしていない。あわせて、しきい値の下（217〜361フレーム相当の帯）に意図的に残してある軽いスピル（VRAM不足時のメインメモリへの退避）を解消するかどうかも、この機会に判断する。
- **着手時の注意**: 測定設計の落とし穴が3点（出力側の二値化・判定定数の置き方・生成物どうしを比べる指標）、いずれも出典に書いてある。**外から持ってきた判定定数をそのまま使わないこと**——今回それで測定が一度破綻している。
- **出典**: バックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) **§101**（本テーマ全体の正本。残件と再訪材料の一覧は§101.11）、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-76（クローズ記録）。
- **状態**: 将来の研究課題（トリガー待ち）。
...
- **本項の扱い**: **実装するかどうかはオーナー判断であり、着手は決まっていない。** 直すなら`GET /models`の`unsupported_features`をGradio側でも読み、含まれる機能のコントロールを隠す（またはグレーアウトする）形になる。**表駆動化そのものは2026-09-05にフロントエンド側で実装され、オーナーの画面目視まで含めて決着している**（記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-135、実装と設計判断は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §99。**この表はWebUI専用でGradioからは使えない**ので、⑤を直すときの参考にはなっても再利用はできない）。
...
- **関連**: [`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-133（参照動画つき単発生成の実測。**単発側の線は動かないという結論で決着済み**であり、この25,000とは別軸のままである）・同書§3-76（参照動画VAEエンコードのタイル化。**しきい値をこの25,000から機械的に割り出した固定値で導入しており、線そのものは動かしていない**——再較正は本書§3-144の担当である）。
