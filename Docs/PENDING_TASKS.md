...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**4件**） | 部分完了。**準必須級のRetakeとEnd sourceは2026-08-26に実装・全機械ゲート合格**（`unsupported_features`は6件→4件。**連結生成のモード名は1つも残らなくなった**）。**ただしオーナーの目視ゲートG8は未了**——チェックリストは本書§2、素材は`outputs/ltx25-rtes-gate/g8_material/`。残るのは準必須級のOutpainting（棚卸し済み・別設計で作り直し。§3-102の判断材料(10)）・実験的なNAG／VSFと非蒸留モデル・先送りのPrunaVAED（LTX 2.5版の枝刈り済み重みが出るまで） |
...
| 見るところ | 現在値 | 完了時にどうなるか |
|---|---|---|
| `GET /models`の`unsupported_features`（LTX 2.5） | **4件**（`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`） | 実装した機能の名前が1つ消える。0件になれば本項は完了。**`keep_resident`（第2弾）と`sage_attention`（第3弾）は2026-08-25に、`retake`と`end_source`は2026-08-26にここを抜けた。連結生成のモード名が1つも残らなくなったので、この配列はいま`REJECT_TABLE`（単発生成用）をそのまま並べた式になっている**——**2つの表が食い違いようがない形**であり、chain側から付け足す名前はもう存在しない |
...
| `REJECT_TABLE`（単発生成用） | **4行**（`pipeline`／`outpaint`／`nag_enabled`／`vae_mode`） | 同上 |
...
- **LTX 2.5では別の論点が絡む**: `prune_vaed`は**LTX 2.5では`unsupported_features`に残っている4件の1つ**である（LTX 2.5版の枝刈り済み重みが存在しないため先送り。本書§3-102）。加えて`vae_mode`には**`sage`で消えたのと同型の罠**が残っている——LTX 2.3で選んだ設定がブラウザに残ったまま2.5へ切り替えると全ジョブが422になる（同§3-102の判断材料(9)、[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77.7(f)）。**UIへ出すなら、この罠の始末とセットで考えること。**
