...
- 作成: 2026-07-15／最終更新: 2026-09-11（現在の構成は**§3 将来の研究課題／§4 スコープ外**の2節。§1〔近日中の改修項目〕と§2〔実装済み・ユーザーのテスト待ち〕は、どちらも該当項目が無いため、運用規則どおり見出しごと削除してある）
...
- **マスクはAviUtl2側のオブジェクトから作る**: 追尾（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-54-02）で動かした部分フィルタでも、利用者が手で作ったオブジェクトでも、他者のプラグインが作ったものでも、契約から見ればいずれも同じ「AviUtl2側のオブジェクト」である。**追跡AIの出力（矩形）から直接マスク動画を作る経路は設けない**（矩形を白く塗るだけの処理をサーバーへ置く必然性が無く、保存物の議論を追尾側へ持ち込むことになるため。理由の正本は[`OBJECT_TRACKING_DESIGN.md`](OBJECT_TRACKING_DESIGN.md)）。
- **前提**: 物体追尾の第1弾は完成済みである（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-54-02。2026-09-11にクローズ）。
...
- **フロントエンド側の見込み規模**: マスク動画を組み立ててアップロードする導線の新設。追尾そのもののUXは物体追尾の第1弾（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-54-02）で作り終えているため、本項の範囲外である。
- **状態**: 将来の研究課題（着手時期未定。前提だった物体追尾の第1弾は完成済みである）。
