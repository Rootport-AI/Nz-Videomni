...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り8件。**高速化技術の行を第2弾・第3弾・先送りの3行へ分割したため、前回より件数は増えているが、作業そのものは前進している**） | 部分完了。**次に着手するのは高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）**（高速化第1弾＝fusedカーネルと先読みblock swapは2026-08-24に完了。スタイルLoRAとIC-LoRAは2026-08-25のオーナー目視合格で完了） |
...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了：2026-08-23に連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を実装してオーナー合格。2026-08-24にスタイルLoRAとIC-LoRA〔長尺を含む〕を実装して全ゲート合格し、2026-08-25にオーナー合格。同じく2026-08-24に高速化第1弾〔fusedカーネルと先読みblock swap〕も実装し全ゲート合格**）
...
- **残っているもの（本項の実体）**: 下表の8件。**行の数は`unsupported_features`の8件（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`／`sage_attention`／`keep_resident`）と1対1に並ぶが、右端の「断っているフィールド」列は1対1ではない**——NAG／VSFの行に並べた`neg_method`／`vsf_scale`は422で断っているフィールドではなく、「無視して先へ進める」側にある（422になるのは`nag_enabled`だけである。下記の判断材料(1)）。**行を機能名の一覧として読み、フィールド単位の正本は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10(d)・(f)の4分類表を見ること。** 上から順に着手する（オーナー裁定2026-08-23の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」に沿う）。**必須級は2026-08-24で出尽くし、同日に高速化第1弾（fusedカーネル・先読みblock swap）も完了した。次に着手するのは表の先頭＝高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）である。**

| 段階 | 残項目 | 断っているフィールド |
|---|---|---|
| **高速化技術の前倒し（第2弾）** | モデル骨格の常駐（`keep_resident`）のテキストエンコーダ側。**既定OFFのオプトイン**として入れる想定 | `keep_resident` |
...
| 見るところ | 現在値 | 完了時にどうなるか |
|---|---|---|
| `GET /models`の`unsupported_features`（LTX 2.5） | **8件**（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`／`sage_attention`／`keep_resident`） | 実装した機能の名前が1つ消える。0件になれば本項は完了 |
| `CHAIN_REJECT_TABLE`（連結生成用） | **7行**（`retake`／`end_source`／`nag_enabled`／`pipeline`／`vae_mode`／`attention_backend`／`keep_resident`） | 該当行が1行消える。**`source_video`／`source_audio`は2026-08-23に、`loras`／`reference_video_id`は2026-08-24にここを抜けて動作側へ移った** |
| `REJECT_TABLE`（単発生成用） | **6行**（`pipeline`／`outpaint`／`nag_enabled`／`vae_mode`／`attention_backend`／`keep_resident`） | 同上 |
...
- **状態**: 部分完了。**高速化第1弾（fusedカーネル・先読みblock swap）は2026-08-24に完了**（全ゲート合格・出力ビット不変のため目視ゲート無し。[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §75）。連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕は2026-08-23に実装し**オーナー合格済み**（連結生成は2026-08-23の目視〔[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.10〕、V2V継続とA2Vは2026-08-24にAviUtl2の操作パネルからの実機検証で合格し、長尺A2Vはオーナー裁定により合格扱い〔同 §73.10〕）。**スタイルLoRAとIC-LoRA〔長尺を含む〕は2026-08-24に実装して全ゲート合格し、2026-08-25にAviUtl2の操作パネルからの実機検証でオーナー合格した**（同 §74.12）。**残り8件は未着手で、次に着手するのは表の先頭＝高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）である。**
