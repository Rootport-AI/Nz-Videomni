...
- 作成: 2026-07-15／最終更新: 2026-09-14（§2を立て直し、§3-55を§2-10へ移設、マスク受け渡し契約の正本を`INPAINTING_DESIGN.md`へ移した。現在の構成は**§2 実装済み・ユーザーのテスト待ち／§3 将来の研究課題／§4 スコープ外**の3節。§1〔近日中の改修項目〕は該当項目が無いため、運用規則どおり見出しごと削除してある。同日夜、Inpainting（§2-10）にオーナーの実機採取(k)〜(m)と実機ゲートG1〜G10の結果を反映し、§3-149を新設した）
- 位置づけ: **セッション開始時に「次に何をすべきか」を確認するための台帳であり、セッションの入口は本書ただ 1 つである**（引き継ぎ専用の文書＝`NEXT_SESSION_HANDOFF.md`・`NEXT_SESSION_WORKORDER.md`のような役割の重複する文書は、新設しない）。プロジェクト全体（バックエンド `Nz-Videomni` と、フロントエンド `AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2`）の課題をここへ一本化している。優先度の高い順に次の4つへ分ける（**運用規則の正本は末尾「本台帳の位置づけ（運用規則）」節**）。
  ...
  2. **実装済み・ユーザーのテスト待ち** — 実装は済んでいて、オーナー本人の実機・目視・実GPUテストが未了のもの。書式は**チェックリスト形式**である——各項目を「何を操作して確認するか → どうなれば合格か」の1〜2行にし、`- [ ]`の箇条書きを画面・機能ごとの小見出しでまとめる。テストではなく仕様の是非をオーナーが判断する項目は「オーナー判断待ち」の小見出しへ分ける。**全項目が合格して空になったら、本節は見出しごと削除する**（次に確認待ちの項目が出た時点で節ごと立て直す）。**現在は1件ある。**
...
#### 2-10. Inpainting（マスクによる部分再生成）— 実装済み・実機ゲート一部合格／一部再確認待ち（§3-55から移設、2026-09-14）

**概要**: AviUtl2のタイムラインに置いた部分フィルタの枠を白黒のマスク動画へ焼き、その内側だけを動画生成モデルに描き替える機能。2026-09-14に段0スパイク→バックエンド→プラグイン→操作パネルの実装まで1日で通した。機構は**画角拡張（Outpainting）経路の流用**（緑塗り＋ラプラシアンピラミッドブレンド）である。設計正本は[`INPAINTING_DESIGN.md`](INPAINTING_DESIGN.md)、実測値の正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105、フロントエンド側の実装記録は[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §116。**`feature/inpainting`ブランチで作業中・mainへは未マージ。**

**(a) 実機採取（nativeの仮定数を差し替えるための一次資料）**

- [x] **(k) 白化効果の実書式**: 採取済み・確定。`.object`原文は`[Object.1] effect.name=反転`（5項目）・`輝度反転=1`で、プラグインが書いていた仮の2行と一致した。`native/src/alias_util.cpp`の`PROVISIONAL`ブロックは差し替え不要（[`SDK_REFERENCE.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/SDK_REFERENCE.md) §16(k)）。
- [x] **(l) 描画順の確認**: 不要と確定。オーナーが直接確認——AviUtl2は番号の小さいレイヤーを奥に合成し、部分フィルタは自分より奥（番号の小さい）レイヤーへ効くため、黒背景を`layer_max+1`、複製を`layer_max+2`に置く仮定はこのまま成立する。G6（動くマスクでの生成、実機合格）が裏づけ。
- [x] **(m) `frame=`の絶対/相対確認**: 手作業での採取は不能と判明。プラグインは`frame=`を絶対・相対どちらの書式でも読めるよう正規化しており、往復確認そのものが不要。G6（中間点で動く部分フィルタへの追従）が実地の裏づけ。採取済みファイルはフィクスチャとして保管する。

**(b) 実機ゲート**（手順と合格条件の正本は[`REAL_BACKEND_CHECKLIST.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/REAL_BACKEND_CHECKLIST.md) §4.16。結果は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105へ記録する）

- [ ] **G1 右クリック**: 部分フィルタと動画をそれぞれ右クリックする → 部分フィルタには「🖌 Inpainting」、動画には「🎬 Video」の項目が出ること（メニューはSDKの仕様でどの種別を右クリックしても表示され、対象外の種別を選んだときの断りはクリック後に出る——「対象外の種別には出ない」という従来の合否文言は誤りだったため修正した）。動画側の項目は「🎬 Video: 選択範囲を撮り直す (Retake)」のすぐ下に並ぶ（メニュー順序を変更）。**再確認待ち**（LTX 2.5選択中にInpaintingサブタブが見当たらなかった件はLTX 2.5未対応が原因とオーナーが確認済みで、変更はしない）。
- [x] **G2 順序不問**: 部分フィルタ→動画、動画→部分フィルタのどちらの順でも右クリックする → どちらの順序でもカード2枚が揃い、2回目で1回目の内容が消えず、対象動画の二重アップロードが起きないこと。**合格。**
- [ ] **G3 解像度不一致**: プロジェクトと異なる解像度の動画を対象動画にする → 実数入りの専用文言が出てGenerateが押せなくなること。**不具合を検出**（部分フィルタが無い状態でも解像度不一致の注意文が出ていた）→ **両方のカードが揃っているときだけ判定するよう修正済み・再確認待ち。**
- [x] **G4 窓とフレーム数**: 部分フィルタがリボン端にある場合や、フレーム数を部分フィルタより短くした場合を確認する → フレーム数欄の既定値・窓のずれ・注意文・快適上限マーカーがいずれも正しく出て、短くしてもGenerateは止まらないこと。**合格（2026-09-14）**
- [x] **G5 マスク描画**: Generateを押して進捗とその後のタイムラインを確認する → 進捗表示・一時オブジェクトの後片付け・「元に戻す」・書き出したmp4の白黒がいずれも正しいこと。**合格（2026-09-14）**
- [x] **G6 動くマスクでの生成**: 中間点で枠が動く部分フィルタでGenerateする → 完成動画の解像度・尺が対象動画と一致し、描き替え領域が枠の動きに追従し、音声が元のままであること。**合格**（`run_inpaint`の実GPU初回実行）。
- [x] **G7 追尾との排他**: マスク描画中に物体追尾を、追尾中にマスク描画をそれぞれ試す → 後から呼んだ方が`TRACK_BUSY`／`MASK_BUSY`で断られ、先発は完走すること。**コード検査による合格とオーナーが裁定**（両ジョブとも手操作では速すぎて排他を再現できないため、単一ジョブ枠＋機械検証で足りると判断）。
- [x] **G8 LTX 2.5**: LTX 2.5への切り替え中・切り替え後の画面を見る → 読み込み中はInpaintingサブタブが灰色になること。**合格。**
- [ ] **G9 部分フィルタの消失**: カード表示後に部分フィルタを削除してGenerateを押す → 案内文で止まり生成が投入されないこと。**不具合を検出**（案内文が一瞬で消えて読めなかった）→ **トースト表示へ変更済み・再確認待ち。**
- [x] **G10 回帰**: 画角拡張の固定シード出力を改修前（main）と改修後（feature）でLTX 2.3・2.5の両系統で比較 → **両系統ともSHA-256がビット一致（合格・2026-09-14）**。結果は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105.8。撮り直し・追尾・cutoutRangeの手動操作は未確認（オーナー任意）。

**(c) オーナー判断待ち**

- [ ] **①膨張の既定値**: 既定5/2のまま据え置くか、仕上げ段（stage-2）の膨張を下げて描き替わる帯を狭めるかの判断。根拠は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105.3（帯幅は仕上げ段の膨張だけで決まり、下げれば狭まる）。
- [ ] **②スパイクの目視**: `outputs/inpaint_spike/runs/replace_full/restored.mp4`・`remove_full/restored.mp4`を見て、画角拡張経路の流用という判定に同意するかどうか。
- [x] **③G10の改修前SHAの採取方法**: 監督がmainの作業ツリーから一時サーバーを立てて採取し、featureと比較して決着（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105.8）。
...
#### 3-55. Inpainting（マスクによる部分再生成）／マスク受け渡し契約の正本（起票：2026-08-05）

**§2-10へ移設（2026-09-14、実装済み）。** マスクの受け渡し契約の正本は本項から[`INPAINTING_DESIGN.md`](INPAINTING_DESIGN.md) §6へ移った。応用先（§4-8(C)・§4-25）は同書を参照する。
...
#### 3-146. webuiのキーフレーム「追加」規則の見直し（起票：2026-09-08）

- **概要**: 「キーフレームを追加」は「最後のピンと最終フレームの中点に置き、既存のピンは動かさない」規則（フロントエンド`KEYFRAME_TIMELINE_DESIGN_NOTES.md` §3、2026-07-18オーナー決定）のため、既定の361フレームでは7枚（0, 177, 265, 313, 337, 345, 353）で「後ろに空きがありません」となり、上限10に「追加」ボタンだけでは届かない（ピンをドラッグして空ければ届く）。上限10化の実機で使い勝手を見てから、規則を変えるか（例: 後ろに空きが無いときだけ最も広い隙間の中点へ置く）据え置くかを判断する。**オーナー裁定（2026-09-08）: 上限10化のテーマでは規則を変えない。**
- **出典**: フロントエンド`KEYFRAME_TIMELINE_DESIGN_NOTES.md` §3、`webui/src/modes/single/keyframeGrid.ts`の`nextAddPosition`。
...
- **本項は§3-55のマスク契約の応用先である**: **受け渡しの形（ピクセル精度1本・1chグレースケール動画・uploadsレール・受信側の二値化）は[`INPAINTING_DESIGN.md`](INPAINTING_DESIGN.md) §6が正本**で、本項が扱うのは**その入口をAviUtl2側の既存プラグインへ繋ぐこと**である。ほかの応用先は§4-8(C)（`conditioning_attention_mask`の露出）である。
...
- **何が塞いでいるか**: **利用可能な物体認識・追尾マスクプラグインの調査が未着手**で、それらが吐き出すマスク動画の形式が§3-55の契約に合うかどうかも分かっていない。着手はこの調査から始める。
- **出典**: オーナー発案。
