...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了：2026-08-23に連結生成の本体を実装**）

- **概要**: LTX 2.5で使えない機能を、1つずつ2.5でも使えるようにするのが本項。**1テーマにまとめてあるが、着手は機能ごとに切り出してよい**（互いに独立しているため）。**本項はテーマ全体としては未完なので、クローズ済み台帳へは移していない。**
- **できるようになったもの（2026-08-23）**: **連結生成の本体（Chained＝複数クリップを1本に繋ぐ生成）**。素のChainedは2.5でも走り、`GET /models`の`unsupported_features`から`chain`が外れた。実測は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72、API契約は[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10(f)、設計への反映は[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §5.6・§8.5、方式の差分は[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) 12節。
- **残っているもの（本項の実体）**: 下表の9件。上から順に着手する想定である（オーナー裁定2026-08-23の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」に沿う）。

| 段階 | 残項目 | 断っているフィールド |
|---|---|---|
| **必須級** | V2V継続（素材（冒頭）） | `source_video` |
| **必須級** | A2V（音声から動画） | `source_audio` |
...
- **状態**: 部分完了（連結生成の本体は2026-08-23に実装済み。残り9件は未着手）。
- **出典**: [`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §5.6・**§8.5（連結生成の実績表）**、[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §69.9・**§71（窓サイズのスイープとIC-LoRA互換検証の実測）**・**§72（連結生成の全ゲートと固定ベンチマークB1〜B4）**、[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) **12節（LTX 2.5での実装と2.3との差分5点）**、[`LTX25_RESEARCH_NOTES.md`](LTX25_RESEARCH_NOTES.md) **11節（設計への持ち帰り）**、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-98（クローズ済みの親テーマ）、[`../Videomni_Backend_Specification.md`](../Videomni_Backend_Specification.md) §6.10（(d)＝単発生成の全28件・**(f)＝連結生成の全34件**）、本書§3-108（kohya形式LoRAのローダー非対応）・**§3-110（連結音声の音量）**。
...
#### 3-110. LTX 2.5の連結音声がLTX 2.3より約10倍大きい（原因未特定・要オーナー試聴）（起票：2026-08-23）

- **概要**: 同じ題材・同じ幾何（2クリップ×73フレーム・512×320）で連結生成を行うと、**LTX 2.5が生成する音声の音量はLTX 2.3の約10倍ある**。継ぎ目以外の区間の音の大きさ（中央値）で比べると、LTX 2.3が0.0015〜0.0018でほぼ無音なのに対し、LTX 2.5は0.0158である。**継ぎ目そのものの実測値は3本ともほぼ同じ**（0.0082〜0.0102）なので、継ぎ目の品質の話ではなく**音量そのもの**の話である。
- **これは不具合と決まったわけではない**: LTX 2.5のほうが「音を出すべき場面できちんと音を出している」だけかもしれない。**まずオーナーの試聴で、大きすぎて割れていないか・LTX 2.3と並べたときに違和感が無いかを確かめる**のが最初の一歩である。対象は`outputs/ltx25-chain-g5/runs/b1|b2|b4/output.mp4`（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.9）。
- **原因の候補（未切り分け）**: ①音声VAE（音声の圧縮展開器）そのものの違い、②蒸留の違い（LTX 2.5は蒸留モデルで、音声側の学習のされ方が違う可能性）、③文脈の与え方の違い（連結では各クリップが同じプロンプトの下で音を作るため、2.5のほうが強く反応している可能性）。**単発生成（Single）でも同じ差が出るかどうかを先に見る**と、①②と③を切り分けられる。
- **副次的な影響（記録）**: この音量差のせいで、継ぎ目の判定ハーネスがLTX 2.3側で**偽陽性**を出す。ハーネスは「継ぎ目での音の跳ね」を「継ぎ目以外での跳ねの中央値」で割った比で判定するので、分母が極端に小さいLTX 2.3では比が跳ね上がる（実測7.02／6.88／6.67で閾値6.0超）。**同じハーネスを使う人は、比だけでなく境界の実測値と非境界の最大値を必ず並べて見ること**（詳細と3つの根拠は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.8(2)）。
- **受入条件**:
  - (a) オーナーの試聴により「割れている／うるさすぎる」かどうかの判断が出ていること。**問題なしと判断されたら、その旨を記録して本項をクローズする**（コード変更は不要）。
  - (b) 問題ありと判断された場合は、単発生成（Single）でも同じ差が出るかを実測し、**音量差がLTX 2.5全体の性質なのか連結生成に固有なのかを切り分けている**こと。
  - (c) 音量を調整する実装を入れる場合は、同一シードでの出力が**音声以外はビット単位で不変**であること（mp4のSHA-256は変わるので、映像トラックだけを取り出して比較する）。調整量は固定値ではなく理由の述べられる値であること。
- **状態**: 未着手（将来の研究課題）。**急ぎではない**——生成そのものは成功しており、AviUtl2のタイムライン上で音量を下げれば実用上は困らない。
- **出典**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.8(1)・§72.8(2)・§72.9、本書§3-102（親テーマ）。
