...
| §3-127 | `scripts/setup.ps1`の完了案内が案内する`.au2pkg.zip`が、いまの配布物と食い違っている（実体は`AviUtl2-Plugin\NzVideomni.aux2`のドラッグ＆ドロップ） | 未着手・オーナー判断待ち。**文面を実在するパスへ差し替えるだけの小さな改修**である（利用者は必ずこの文を読み、存在しないファイルを探すことになる）。見つけた経緯は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.4 |
| §3-128 | `engine/venv-engine.freeze.txt`が`hf-xet`を固定しておらず、直接指定の段で入った1.6.0がそのまま残る | 未着手・裁定待ち（**固定するのか、意図的に外すのか**）。freezeの記載と実環境の食い違いで、開発機の古い`.venv-engine`には`hf-xet`が無い。詳細は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.7 |
