...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**2件**） | 部分完了。**残る2件（`two_stage_hq`＝非蒸留モデル／`prune_vaed`＝PrunaVAED）はどちらも外部要因待ちで、いま着手できる課題は1つも無い。** **オーナーの目視・実機確認は全テーマ分が決着している**（監督裁定2件の追認だけがオーナー判断として残る）。実装の年表と目視の結果の参照先は本項の「実装の年表」「オーナー確認」の行にある |
...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了。残りは`two_stage_hq`と`prune_vaed`の2件だけで、どちらも外部要因待ちである。実装の経過は下記「実装の年表」の参照先を見ること**）

- **概要**: LTX 2.5で使えない機能を、1つずつ2.5でも使えるようにするのが本項。**1テーマにまとめてあるが、着手は機能ごとに切り出してよい**（互いに独立しているため）。**本項はテーマ全体としては未完なので、クローズ済み台帳へは移していない。**
- **実装の年表**: 本項は2026-08-23から2026-08-30にかけて、オーナー裁定の段階分け（必須級 → 高速化技術の前倒し → 準必須級 → 実験的）の順に開通させてきた。**段ごとのコミット・通過したゲート・設計上の要点は[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md) §8.5、実測と既知の制限は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §69〔v1〕・§72〔連結生成〕・§73〔V2V・A2V〕・§74〔スタイルLoRA・IC-LoRA〕・§75〜§77〔高速化第1〜3弾〕・§78〔撮り直しと素材（末尾）〕・§79〔画角拡張〕・§80〔NAG／VSF〕が正本である。** **本書では年表を繰り返さない**——同じ年表を何箇所にも置いた結果、合格が出るたびに一部だけが古くなる事故を起こしたためである。
- **オーナー確認**: **全テーマ分が決着しており、目視・実機確認で未了のものは1件も無い。** 目視ゲートを設けた4テーマの結果は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) **§77.10**〔高速化第3弾＝SageAttention〕・**§78.14**〔撮り直しと素材（末尾）〕・**§79.11**〔画角拡張。既定値2点の裁定は同 §79.12〕・**§80.10**〔NAG／VSF〕、操作パネルからの実機確認は同 §72.10・§73.10〔**長尺A2Vはオーナー裁定で合格扱い**＝同 §73.10(2)〕・§74.12にある。**残っているオーナー判断は1つだけである——撮り直しと素材（末尾）の監督裁定2件（R-1の天井の読み・M8のキーフレーム述語）の追認で、目視ゲートの合格宣言にこの2件についての明示の言葉が含まれていなかったためである**（同 **§78.14(4)**）。
...
- **★いま着手できる課題は1つも残っていない（2026-08-30以降）**: 残る2件はどちらも「外部要因が揃わないと着手できないもの」である——非蒸留モデルは**それを回せる計算機が手元に無く**、PrunaVAEDは**上流にLTX 2.5版の枝刈り済み重みが存在しない**。**つまり本項について「次に何をするか」を計画で決める段階は終わっており、次に動くのは前提が外から変わったときである。**
...
- **状態**: 部分完了。**いま着手できる課題は1つも残っていない**——残る2件（非蒸留モデル／PrunaVAED）はどちらも外部要因待ちで、次に動くのは前提が外から変わったときである。**オーナーの目視・実機確認は全テーマ分が決着している**（上記「オーナー確認」の行）。**本項をクローズしないのは、`unsupported_features`にこの2件が残っているからである。**
...
- **劣化そのものは受容済みである**: 逆順Chainedは2026-08-18のオーナー裁定で「推奨外・劣化は仕様として許容する」と位置づけられており、**2026-08-30の目視ゲートG8でオーナーがこの継ぎ目を実際に聴き取ったうえで「想定の範囲内なので合格」と裁定した**（裁定の正本は同 **§78.14(3)**）。**したがって本項に残っているのは原因の究明だけで、案内文もフロントエンドの文言も変えていない。**
