...
#### 3-102. LTX 2.5でまだ使えない機能の対応（起票：2026-08-22／**部分完了：2026-08-23に連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を実装**）
...
- **完了判定**: 機能が1つ実装できるたびにアダプタの対応表（`services/engines/ltx25/adapter.py`の`REJECT_TABLE`＝単発生成用と`CHAIN_REJECT_TABLE`＝連結生成用の**2つある**）から1行外すと、`GET /models`の`unsupported_features`から自動的に消える。**「対応表から1行消えて、テストが対応表と実装のずれを検出しなくなること」が完了の合図である。**
- **状態**: 部分完了（連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕を2026-08-23に実装した。連結生成の本体は同日オーナー目視も合格。**V2V継続・A2V・長尺A2Vは機械ゲートG1〜G4・実機ゲートG5とも全項目合格で、オーナーの目視4件と試聴2件だけが残っている**〔[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §73・目視欄は §73.9〕。残り8件は未着手で、次に着手するのは表の先頭＝Style LoRAである）。
