...
- 作成: 2026-07-15／最終更新: 2026-09-07（現在の構成は**§2 実装済み・ユーザーのテスト待ち／§3 将来の研究課題／§4 スコープ外**の3節。§1〔近日中の改修項目〕は該当項目が無いため、運用規則どおり見出しごと削除してある）
...
### 2-9. 素材（冒頭）と素材（末尾）を同時に指定した2本以上の連結（本書§3-90の移送先。起票：2026-08-18／実装：2026-09-07）

素材（冒頭）＝Start sourceと素材（末尾）＝End sourceを両方指定した2本以上の連結が使えるようになった。**全クリップを正順に生成し、最終クリップだけを両側（頭＝1つ前のクリップからののりしろ、尾＝素材（末尾）の末尾の凍結フレーム）で条件付けする**形である。**内容の似ている2本の動画の間の欠落区間を補完するための機能**で、2本の内容が離れていると最終クリップの中でクロスフェードやモーフになるが、これは仕様として許容している（そのため品質の警告文も出さない）。**実測値・ゲート結果の正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §102で、本書には書き写さない。**

**自動ゲートはすべて合格している**（バックエンドとフロントエンドのテスト・型検査・lint、両エンジンの実機ジョブ、既存経路の非退行）。作業ブランチは `feature/start-end-bridge` で、`.aux2` は実機とバックエンドリポジトリ配布コピーの2箇所へ配置済みである。

**共通の下ごしらえ**: **バックエンドを `feature/start-end-bridge` のツリーで起動し直しておく。** 古いツリーのままだと、画面が送れるようになった組み合わせをサーバー側が 422 で断るため、実装の不具合と読み違えてしまう。AviUtl2 も起動し直すこと。

**生成結果の目視**（Hugging Faceのプライベートデータセット `Rootport/Nz-LTX23-eval-clips` の `round8-start-end-bridge/` にある2本。素材の2本は同じデータセットの `round7-start-end-materials/`）

- [ ] **V1（クリップのつなぎ目）**: LTX 2.3 の動画を再生し、1本目から2本目、2本目から3本目のつなぎ目を見る → **これまでの普通の正順の連結生成と同じ程度のつなぎ目に見えれば合格。**
- [ ] **V2（最終クリップの中での着地）**: 同じ動画の最後のクリップを見る → **cand2 の冒頭の絵へ向かって近づいていき、着地が唐突でなければ合格。** cand1 と cand2 は別のシードで作った別の動画なので、自動車の細部や背景が走っているうちに移り変わって見えるのは想定内である（形の崩れではなく、動きとして自然かどうかで判断する）。
- [ ] **V3（音のつながり）**: 同じ動画を音つきで聴く → **最後が cand2 の冒頭の音へ向かって着地し、途切れや急な切り替わりが無ければ合格。**
- [ ] **V4（LTX 2.5）**: LTX 2.5 の動画で V1〜V3 を繰り返す → **同じ基準で合格すればよい。** 見比べて好みを言えるようにするのが、2本を同じ指示で作ってある理由である。

**操作パネル（Chained画面）**

- [ ] **U1（送信できる）**: Chained で素材（冒頭）に動画、素材（末尾）に素材を入れ、クリップを2本以上にする → **送信できない理由が表示されず、のりしろの幅が3のままで、遡って生成する旨のヒントと複数クリップの品質警告がどちらも出ず、Generate が通れば合格。**
- [ ] **U2（素材（冒頭）を外す・付け直す）**: U1の状態から素材（冒頭）を外す → **ヒントが出てのりしろの幅が1へ変わる。** そのまま付け直す → **ヒントが消えてのりしろの幅が3へ戻る。両方向とも変われば合格。**
- [ ] **U3（クリップ1本は従来どおり）**: クリップを1本に戻し、素材（冒頭）と素材（末尾）を両方指定する → **これまでどおりの見え方・操作感のまま生成できれば合格**（この組み合わせは以前から使えたもので、今回の改修で挙動を変えていない）。

**オーナー判断待ち**

- [ ] **J1（merge の可否）**: 上の目視に合格したら、作業ブランチ `feature/start-end-bridge` を main へ merge するかどうかを判断する。**判断材料**: 既存経路が無変化であることは同一シードでの出力の完全一致で実証済みで（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §102）、覆す場合もブランチを捨てるだけで main は無傷である。

**合格後**: ①本項を `3-90` として[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移送しクローズする。②本節が空になるので、運用規則どおり見出しごと削除する。③同書冒頭の「空になった節の扱い」の段落へ、§2をまた節ごと削除した旨と§2-9が欠番になる旨を追記する。④本書§3-96の1行注記と、[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) §11の参照先を移送先へ付け替える。
