...
- 作成: 2026-07-15／最終更新: 2026-09-16（§3-151〔バッチパネルのLTX 2.5での実機確認〕をオーナーの実機確認と裁定でクローズし[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-151へ。同日、§4-14〔バッチA2VパネルのCrop欄が未露出〕をオーナーが直接着手し、実装・実機ゲート全合格でクローズして同書 §3-153 へ移した〔記述の誤りも同書で訂正〕。あわせて台帳外のオーナー注文「Toolboxタブの追尾開始ボタン」を同書 §3-154 として記録。同日、追加のオーナー注文「出力クロップ欄の自由入力化」も実装・実機ゲート全合格で同書 §3-155 として記録。現在の構成は**§3 将来の研究課題／§4 スコープ外**の2節で、§1・§2 は該当項目が無いため見出しごと削除してある）
