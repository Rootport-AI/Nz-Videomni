...
- 作成: 2026-07-15／最終更新: 2026-09-02（**§3-112をクローズし、本書から除去した**——GPUメモリの確保方針の環境変数`PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`は、Windowsでは一度も効いていなかったので、改名も新しいつまみの追加もせず**設定行ごと全部削除する**ことで解消した〔オーナー裁定〕。移設先は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-112）
