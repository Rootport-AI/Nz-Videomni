...
**§3-97（マルチエンジン土台）と§3-98（LTX 2.5対応）は2026-08-22にクローズし、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-97・§3-98へ移した**（オーナーの実機目視全項目合格とオーナー承認による）。本書の当該2番号は欠番である。**§3-98のクローズはv1＝基本生成のスコープでのクローズ**なので、LTX 2.5でまだ使えない機能の対応は本書§3-102として、拡散デコーダ版VAEの採否は本書§3-103として生きている。以後、この2テーマの経緯を引くときは番号だけで書かず、必ず`PENDING_TASKS_CLOSED.md`というファイル名を添えること。
...
- **経緯**: LTX 2.5対応（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-98）のPhase 1（インストーラを2系統対応へ抽出する作業）で発見した。**その改修は2.3経路の出力と`uv`の引数列を1文字も変えない方針だったため、この不具合も挙動を温存したまま持ち越してある**（直すと「変えない」という保証が崩れ、抽出が正しかったことの確認ができなくなるため）。
...
- **概要**: LTX 2.5対応（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-98）のv1は基本生成（T2V／I2V）だけを実装し、それ以外は422で明示的に断る形にした。ここで断っている機能をLTX 2.5でも使えるようにするのが本項である。**1つのテーマとしてまとめて起票してあるが、着手は機能ごとに切り出してよい**（互いに独立しているため）。
...
- **概要**: LTX 2.5 の映像VAE（映像の圧縮展開器）には**畳み込みデコーダ版（conv）**と**拡散デコーダ版（DiffVAE）**の2種類があり、LTX 2.5対応（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-98）のv1は前者を既定にした。後者を採るかどうかを判断するのが本項。ファイル自体は `models/LTX25/VAE/diffvae/` へ退避してあり、カテゴリの走査は非再帰なので自動認識の対象にならない（置いてあるが選べない状態）。
...
- **これは新しい不具合ではない**: オーナー自身の `logs/ltx_worker.log`（2026-08-20・マルチエンジン化の実装より前）に、まったく同じ段差が記録されている——ジョブ `713ff845` がピーク8,442／予約8,972（66.42秒）、続くジョブ `9e3b415c` がピーク9,260／予約9,732（73.82秒）。LTX 2.5対応（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-98）の改修が持ち込んだものではないことは、この既存ログで裏取り済みである。
