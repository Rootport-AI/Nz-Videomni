...
- 作成: 2026-07-15／最終更新: 2026-09-02（**fps全入口整数スナップ（§3-71・§3-72対策）の実機ゲート待ちで§2節を立て直した（§2-2。同日の§2-1〔G-P1〕は既に合格・削除済みのため番号は引き継がず次番号を採った）**。実装記録はフロントエンド`DEVLOG.md` §105・バックエンド`VERIFICATION_LOG.md` §91）
...
### 2-2. fps全入口整数スナップの実機ゲート（§3-71・§3-72対策、起票：2026-09-02）

非整数fps（29.97/23.976）対策として、webuiのfps全入口整数スナップ（正本は`modes/single/paramUtils.ts`の`snapFrameRate`）・右クリックプリフィル時のみのトースト・MCP/Gradioの`_snap_frame_rate`（範囲内のみ丸め、範囲外は素通し）を実装済み。自動検証はエラー0・全緑（設計要点と件数の正本はバックエンド[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §91、実装記録はフロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §105）。以下は`build.ps1`→`deploy.ps1`での実機配置後にオーナーが確認する項目で、**この表が実機ゲートの正本**である（他文書はここを参照し、二重に書き写さないこと）。

- [ ] **G1**: 29.97fpsのAviUtl2プロジェクトで、fpsポリシー既定（project）のまま右クリック→「✨ここにAI生成を挿入」→Create画面が開く → Create欄のfpsが30・「29.97→30」のトーストが1枚出て6秒で消える。
- [ ] **G2**: fpsポリシー＝素材に切り替え、23.976fpsの動画オブジェクトを右クリックしてCreate画面を開く → 欄が24・トーストの元値表示が23.98。
- [ ] **G3**: 30fpsぴったりのプロジェクトでG1と同じ操作をする → 欄は30のまま・トーストは出ない（偽陽性が無いことの確認）。
- [ ] **G4**: 29.97fpsのプロジェクトで、既存クリップの「動画延長」からChain画面を開く → 共通fpsが30・トースト1枚・仮オブジェクトの長さとDURATIONが一致する。
- [ ] **G5**: Create画面のfps欄に29.97を貼り付け／120と入力／欄を空にする、の3操作をそれぞれ試す → 順に30／60／1（空欄は現行と同じ挙動）になり、いずれもトーストは出ない。
- [ ] **G6**: Retake・Outpainting両パネルのfps欄に29.97／0／200をそれぞれ入力する → 両パネルとも順に30／1／60になる。
- [ ] **G7**: 29.97fpsのプロジェクトで、音声付き2クリップのChain生成を実行する → 422エラーが出ない（§3-71の再現ケースが消えていることの確認）。
- [ ] **G8**: 30秒を超える生成物をAviUtl2で再生する → 末尾まで映像と音声にズレが無い（§3-72の再現経路が消えていることの確認）。
- [ ] **G9**: MCP経由で`submit_generate(frame_rate=29.97)`を実行し、`job_status(job_id)`でリクエストのエコーを確認したうえで直ちに`cancel_job`する → `request.frame_rate`が30になっている（submitの戻り値にはfpsが載らないためjob_statusが必須。**real検証はGPU使用前に毎回オーナーの了承を得る規律に従い、確認後は即キャンセルして生成そのものは走らせないこと**）。
- [ ] **G10**: Gradio同梱UIの単発／Chainのfps欄に29.97を入力して送信する → payloadは30になり、欄の表示も整数のまま・Chainでは幾何プリチェックによるローカル拒否が出ない。
- [ ] **G11**: 言語をja/en相互に切り替えてG1を再実行する → トースト文言が両言語とも自然に表示される。

**合格後**: 本書§3-71・§3-72を[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移送する。
...
- **クライアント側の緩和策は実装完了・実機ゲート待ち（§2-2参照）**。
...
- **状態**: 未着手（将来の研究課題）。
- **出典**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §79.2、`outputs/ltx25-outpaint-prep/RESULTS.md`（複写: [`Outputs-archive/ltx25-outpaint-prep/RESULTS.md`](Outputs-archive/ltx25-outpaint-prep/RESULTS.md)）の G0-d。
