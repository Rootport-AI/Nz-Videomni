...
### 2-7. Edit画面へ加速設定を載せた件の実機確認と、画角拡張の新しい警告位置の目視 — 本書§1-27の移送先（起票：2026-09-04／実装・再較正：2026-09-05）

Edit画面（画角拡張〔Outpainting〕・撮り直し〔Retake〕）が、生成要求にSettingsの「生成高速化」（加速設定）を載せるようになった。**あわせて画角拡張の快適上限のトークン予算を、加速を全てonにした構成で測り直してある**（`ltx25`の線だけが動き、`ltx`は据え置き）。**数値の正本は[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §9、較正と改修の記録は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §98とフロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §110で、本書には書き写さない。**

**自動ゲートはすべて合格している**（型検査・vitest・lint、計測点20点の加速照合と境界再現走、ビルドとデプロイのSHA-256 3値一致）。`.aux2`は実機とバックエンドリポジトリ配布コピーの2箇所へ配置済みなので、**AviUtl2を起動し直せばそのまま確認できる。**

**共通の下ごしらえ（V1〜V4）**: Settingsの「生成高速化」を**全部on**にしておく。生成が終わったら、そのジョブの`metadata.json`（`outputs/<ジョブID>/`）を開いて`*_used`で終わる項目を見る——**ここが実際に使われた構成である。**

**画角拡張（Outpainting）**

- [ ] **V1（加速が載る）**: 画角拡張で1本生成する → `metadata.json`の`attention_used`が`sage`、`block_swap_prefetch_used`・`keep_resident_used`・`fused_gguf_dequant_kernel_used`が`on`になっていれば合格（**改修前はどれもサーバー既定の値だった**）。
- [ ] **V2（既定へ戻せる）**: Settingsの加速を既定へ戻して、もう1本生成する → `metadata.json`の`*_used`が既定の構成に戻っていれば合格。**Settingsで選んだとおりに動くこと**が見たい点である。
- [ ] **V5（LTX 2.3 の警告位置）**: LTX 2.3 で、1280×768の素材へ余白を左右320・上下160（＝キャンバス1920×1088）与え、尺だけを動かす → **153コマでは快適上限の警告が出ず、161コマで出れば合格。**
- [ ] **V6（LTX 2.5 の警告位置 — 線が上がったことがこの1点で見える）**: LTX 2.5 へ切り替え、余白を左右320・上下128（＝キャンバス1920×1024。**較正で測ったのと同じ形**）に変えて、尺だけを動かす → **185コマでは警告が出ず、193コマで出れば合格。** **185コマは、再較正の前はちょうど警告が出ていた尺である**——ここで警告が消えていれば、線が上がったことが画面から直接見える。

**撮り直し（Retake）**

- [ ] **V3（加速が載る）**: 撮り直しで1本生成する → `metadata.json`の`*_used`がV1と同じようにSettingsの選択を反映していれば合格。**撮り直しにも同時に載せたので、こちらも1本見ておく。**
- [ ] **V4（エンジンを切り替えても断られない）**: ベースモデルを LTX 2.3 と LTX 2.5 で切り替えながら、画角拡張と撮り直しをそれぞれ1本ずつ走らせる → **422（設定を断るエラー）が1度も出なければ合格。** 各エンジンが受け付けない設定は`AppShell`が画面から隠して書き戻す作りなので、**利用者の操作だけで断られる状態には入らないはず**、というのがここで確かめたいことである。

**オーナー判断待ち**

- [ ] **J1（最重要）— 全on構成で測った固定線を、加速を切っている利用者にも当てたままでよいか。** 画角拡張の線はエンジン系統だけで決まる固定線で、単発生成・連結生成のマーカーが持つ「構成が合わなければレガシー表へ落ちる」仕組みが無い（[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §9.2）。**加速を切っている利用者には、線が楽観側に出る。**
  - **判断材料**: ①**LTX 2.3 は線が動いていない**ので、この系統では懸念そのものが生じない。②**LTX 2.5 は線を1段上げたが、2026-09-04にサーバー既定の構成で行った掃引でも、新しい線の位置まで快適だった**（[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §9.3の旧留保。当時は「境界を掴めていないので保守的に据え置く」という理由で採らなかっただけである）——**つまり加速を切っている利用者にも実測の裏づけがある。** ③**残るのは「全on構成で測った線を、構成を問わず当てる」という設計そのものの是非**で、これは実測では決まらない。④**覆す場合の費用は小さい**——定数1つの値を戻して再配置するだけである。
- [ ] **J3 — 較正中に出た想定外2件の扱い**（どちらも採用した線には影響しない。詳細は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §98.8）。①LTX 2.3 のある1点だけ、判定軸によって答えが割れた（**メモリは膨らんだが速度は落ちなかった**点。機序は解明済みで、境界の端でもない）。②2026-09-04の記録が書いていた「LTX 2.3 の第2段階の窓は全点で判定不能になる見込み」という**予測が外れていた**（今回の較正で状況が変わったのではなく、**2026-09-04自身のデータでも既にそうだった**）。**当時の一次記録は書き換えず、食い違いの指摘を2026-09-05側の一次記録に置いてある**——この扱いでよいかの確認である。

**合格後**: ①本項を`3-143`として[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移送しクローズする。②同書§3-134の「後続の起票」の参照を付け替える。③一次記録（`outputs/comfort-calib-2026-09-05/`のRESULTSとREADME）を`Docs/Outputs-archive/comfort-calib-2026-09-05/`へ複写する（**この複写はまだ作っていない**——[`Outputs-archive/comfort-calib-2026-09-04/`](Outputs-archive/comfort-calib-2026-09-04/)と同じ形で、スナップショットである旨の注記だけを足して作る）。④同書冒頭の「空になった節の扱い」の段落へ、§2をまた節ごと削除した旨と§2-7が欠番になる旨を追記する。
