...
- [ ] **G4 窓とフレーム数**: 部分フィルタがリボン端にある場合や、フレーム数を部分フィルタより短くした場合を確認する → フレーム数欄の既定値・窓のずれ・注意文・快適上限マーカーがいずれも正しく出て、短くしてもGenerateは止まらないこと。
- [ ] **G5 マスク描画**: Generateを押して進捗とその後のタイムラインを確認する → 進捗表示・一時オブジェクトの後片付け・「元に戻す」・書き出したmp4の白黒がいずれも正しいこと。
...
- [ ] **G10 回帰**: `outputs/inpaint_regression/`のツールで改修前後のフィンガープリントをLTX 2.3・2.5の両エンジンで比較し、撮り直し・追尾・cutoutRangeも操作する → 出力SHA-256が両エンジンで改修前と一致し、他機能も従来どおり動くこと。**監督がHTTP API経由で実行中**（結果は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §105.8へ記録）。
...
- [ ] **③G10の改修前SHAの採取方法**: mainを一時的に起動して`outputs/inpaint_regression/`のツールで改修前のフィンガープリントを採るかどうかの判断。**現在は監督がHTTP API経由で実行中。**
