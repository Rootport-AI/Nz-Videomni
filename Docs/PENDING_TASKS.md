...
| §3-127 | `scripts/setup.ps1`の完了案内が実在しない`.au2pkg.zip`を案内している（実体は`AviUtl2-Plugin\NzVideomni.aux2`のドラッグ＆ドロップ）。文面の差し替えだけで直る | 未着手・オーナー判断待ち（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.4） |
| §3-128 | `engine/venv-engine.freeze.txt`が`hf-xet`を固定しておらず、直接指定の段で入った版がそのまま残る | 未着手・裁定待ち＝固定するか意図的に外すか（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.7） |
...
#### 3-127. `scripts/setup.ps1`の完了案内が、実在しない`.au2pkg.zip`を案内している（起票：2026-08-30）

- **概要**: 本体インストーラ（`setup.bat`→`scripts/setup.ps1`）の完了案内は、AviUtl2へ入れるプラグインとして`.au2pkg.zip`を指している。**いまの配布物は`AviUtl2-Plugin\NzVideomni.aux2`をドラッグ＆ドロップする形なので、この文を読んだ利用者は存在しないファイルを探すことになる。** **導入を終えた利用者は必ずこの文を読む**ため放置できない。直すのは文面だけで、スクリプトの挙動には触れない。**受入条件**は、完了案内が実在するファイル（`AviUtl2-Plugin\NzVideomni.aux2`）とその置き方だけを案内し、[`README.md`](../README.md)の導入手順と食い違わないこと。
- **状態**: 未着手・オーナー判断待ち（文面の確定）。
- **出典**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.4、`scripts/setup.ps1`（完了案内の出力）。

#### 3-128. `engine/venv-engine.freeze.txt`が`hf-xet`を固定していない（起票：2026-08-30）

- **概要**: LTX 2.3側のエンジン用仮想環境の固定版一覧（freeze）に`hf-xet`（HuggingFaceの高速転送ライブラリ）の行が無く、インストールの途中で直接指定された版がそのまま残る。**freezeの記載と実環境が食い違う**状態で、実際に開発機の古い`.venv-engine`には`hf-xet`自体が入っていない。決めるべきは**固定するのか、意図的に外すのか**である（固定すれば環境が揃うが、転送ライブラリは上流の更新が速い）。**受入条件**は、どちらの結論でも**freezeの記載と実際に入る版が一致していることを読み手が確かめられる**状態にすること——固定するなら行を足し、外すなら理由を1行残して「書き漏れ」と読まれないようにする。
- **状態**: 未着手・裁定待ち。
- **出典**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §82.7、`engine/venv-engine.freeze.txt`。
