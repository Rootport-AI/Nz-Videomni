...
- 作成: 2026-07-15／最終更新: 2026-09-04（**§3-36〔音声のfilePathの二重機構〕の一本化を実装・ビルド・デプロイまで済ませ、オーナーの画面目視待ちで§2節を立て直した〔§2-6〕**——挙動不変のリファクタで、経緯と根拠の正本はフロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §109。§3-36は§2へ移送したので、「再訪条件つきでクローズした項目」の節からは削除してある。**番号を§2-1へ振り直さず§2-6を採ったのは、同じ2026-09-04に§2-5を削除したばかりで、同じ日に同じ番号を二度使うと読み違えのもとになるためである**〔§2-1〜§2-5はいずれも欠番〕）
...
### 2-6. A2Vの音声ファイルの場所を覚える仕組みを1つへ寄せた件の画面目視 — 本書§3-36の移送先（起票：2026-07-20〔出自は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-24の「将来の掃除課題」〕／実装：2026-09-04）

Createタブの音声スロットが「音声ファイルがどこにあるか」を覚えるやり方が**2つ**あったので、古いほう（ファイル選択ダイアログの応答を横から覗き見て控えておく仕組み）を撤去し、共有フック`useSourceUpload`が状態として持っている`filePath`ただ1つへ寄せた。**画面の見え方も、送る要求も、何ひとつ変えていない純粋な整理である**（機能の追加も仕様の変更も含まない）。**経緯と根拠の正本はフロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §109で、本書には書き写さない。**

**機械ゲート（型検査・vitest・lint）は全合格**で、ビルドと2箇所へのデプロイも済んでいる（ビルド成果物・実機・バックエンドリポジトリ配布コピーのSHA-256が3値一致）。**テストは期待値もアサートも1つも変えずに通っている**ので、挙動が変わっていないことの機械的な裏づけはそろっている。**残るのは実機の画面で3つの入口が今までどおり動くことを見るだけで、この表がその正本である。**

**共通の合格条件**: どの入口でも、**音声カードにファイル名が出て**、**そのカードに音声の長さ（`12.3s`のような秒数）が表示され**、**尺（コマ数）がその長さに合わせて自動で動く**こと。**「今までと同じ」であれば合格**である（新しい見え方は無い）。

- [ ] **V1（ドラッグ＆ドロップ）**: Createタブの音声カードへ、`.wav`ファイルをエクスプローラから直接ドロップする。
- [ ] **V2（動画から音声を抽出）**: タイムラインの動画オブジェクトを右クリックし、**#3「動画から音声を抽出してA2V」**を選ぶ。
- [ ] **V3（音声オブジェクトから）**: タイムラインの音声オブジェクトを右クリックし、**#7**（音声をA2Vの素材にする項目）を選ぶ。**さらに、その音声のまま生成を1本走らせて正常に完了すること**（3つの入口のうち、実際に生成まで通すのはこの1本でよい）。

**合格後**: ①本項を`3-36-02`として[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移送しクローズする（**同書には`旧§3-36`〔モデルの再ホスト〕が既にあるため、採番規定どおり`-02`を付ける**）。②同書冒頭の「空になった節の扱い」の段落へ、§2をまた節ごと削除した旨と§2-6が欠番になる旨を追記する。③フロントエンド[`RIGHTCLICK_REDESIGN_SPEC.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/RIGHTCLICK_REDESIGN_SPEC.md)の「音声を引き継いだとき」の説明が、撤去した旧機構（`lastAudioFilePathRef`の初期化）を現行の仕組みとして書いたままなので、現状へ直す（**これだけは合格を待たずに直してよい**）。
