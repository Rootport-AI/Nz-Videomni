...
- **快適上限の線そのものを扱う課題**——§3-130（LTX 2.3既定構成の境界のモデル化）・§3-133（参照動画つき単発生成の実測）・§3-134（画角拡張の予算の較正）。マルチエンジン化の後続ではなく、快適上限マーカーのテーマから出たものである。
...
- **E1で確定した原因（2026-09-03・オーナー実機）**: 突き合わせた結果、差分は`[Object.0]`ブロックの2キーのみだった——**`音声付き`**（D&D=1／🎞挿入=0）と**`再生位置`の第2値**（D&D=ソース尺10.042／挿入=0.000）。`[Object.1]`（映像再生ブロック）は完全一致し、ファイルパスの違いは無関係である。したがって「表示種別は内部フラグでSDK経由では直せない」という仮説は外れ、**原因は`create_object_from_media_file`が`音声付き`を立てないことに確定した**。着手時の入口（E1）が示した「差分があればその行が答え」のとおり、`get_object_alias`で正解の文字列を採取し`create_object_from_alias`方式（`音声付き=1`・`再生位置`第2値をソース尺へ補正して流用）で修正できる。
- **採取したエイリアスの差分（証拠の転記）**: 実測エイリアスはリポジトリ外（AviUtl2ポータブルの`data\Alias\from_d_and_d.object`／`from_insert_button.object`、2026-09-03採取）にあるため、核心部のみここへ転記する。`[Object.1]`とファイルパスは両者で完全一致し無関係なので省く。D&D由来（二色）の`[Object.0]`:
  ```
  [Object.0]
  effect.name=動画ファイル
...
- **実害の見立て**: **実害を確定した——2026-09-01時点の「見た目だけの問題と見られる」という見立ては誤りだった。** 青一色のオブジェクトは右クリックメニューに**「音声を分離」の項目自体が表示されない**（2026-09-03・オーナー実機確認）。ただし**操作パネルから生成物をダウンロードし、それをD&Dで置き直せば二色になり「音声を分離」も使える**——これが現状の回避経路である。
- **注記**: 「音声付き」というUI項目名自体は現行の設定項目一覧には存在しない（2026-09-01調査どおり）が、**エイリアス直列化のキーとしては現役である**。`TIMELINE_ALPHA_REQUIREMENTS.md`側の記載の再訂正は実装ステージS3で行う（本項目では台帳の記録に留める）。
- **状態**: 着手中（2026-09-03・修正計画承認済み。`create_object_from_media_file`から`create_object_from_alias`方式への差し替えで直す）。
- **出典**: フロントエンド`native/src/bridge.cpp`（`create_object_from_media_file`の呼び出し）、SDK非公式ミラー <https://github.com/aviutl2/aviutl2_sdk_mirror>（`plugin2.h`・CHANGELOG）、本体更新履歴 <https://docs.aviutl2.jp/changelog>（beta5・beta17）、本体実行時抽出のエフェクト項目表 <https://github.com/aviutl2/aviutl2_community_translation>（`extract/effects.json`）、UI対訳表 <https://github.com/nagotown/aviutl2-EN>、2026-09-01のオーナー実機確認（E0）、2026-09-03のオーナー実機確認（E1・エイリアス突き合わせ）。
