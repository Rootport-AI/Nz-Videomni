...
- 作成: 2026-07-15／最終更新: 2026-09-08（現在の構成は**§1 近日中の改修項目／§3 将来の研究課題／§4 スコープ外**の3節。§2〔実装済み・ユーザーのテスト待ち〕は該当項目が無いため、運用規則どおり見出しごと削除してある）
...
### 1-28. LTXシリーズのキーフレーム上限を5→10へ変更（起票：2026-09-08）

- **概要**: 公式LTX Desktop（ローカル実行・LTX 2.3/2.5共通）に揃えて、キーフレーム画像の上限枚数を5枚から10枚にする。上限の正本を`config.py`の定数`MAX_CONDITIONING_IMAGES`1箇所に集約し（`LimitsConfig.max_conditioning_images`は入力項目をやめて読み取り専用のcomputed fieldへ。古い`config.yaml`の5は無視される）、`api/models.py`の単発・連結バリデータを1ヘルパーへ共通化してスナップ後のframe_idx重複を422で拒否する。Gradio UIは10スロット化する。プラグインwebuiは`GET /config`の値を読むため値の追随のみで済む。エンジンは無変更。快適上限マーカーには反映しない（オーナー裁定）。
- **状態**: 実装中（featureブランチ`feature/keyframe-limit-10`）。
- **出典**: 計画の正本はこのセッションの計画（実装完了後に`VERIFICATION_LOG.md`の新節へ移す予定）。上限10の根拠は公式リポジトリ`Lightricks/LTX-Desktop`の`backend/api_types.py`（`LOCAL_MULTI_KEYFRAME_MAX_COUNT = 10`）。
