...
- 作成: 2026-07-15／最終更新: 2026-09-24（§3-165「Chained エンジンの潜在クリップサイズの追加」を起票。直近の追記は 2026-09-22 の §3-164 と §4-1）
...
#### 3-164. 生成条件の mp4 メタデータ埋め込みと読み出し（A1111 の「PNG Info」相当）（起票：2026-09-22）

- **概要**: お手本にしている A1111 SD WebUI は、生成条件を出力 PNG のテキストチャンクに書き込み、「PNG Info」から読み戻せる。同じ体験を mp4 で実現する。①出力 mp4 に生成条件を埋め込む、②任意の mp4 から条件を読み出して表示する、③読み出した条件を生成フォームへ流し込む、の3段に分けて考える。
- **現状**: `output.mp4` は `services/video_io.py` がシステムの ffmpeg を呼んで書き出している（クロップ出力の経路も同じ関数群で再エンコードする）。生成条件はジョブごとの `outputs/<ジョブ>/metadata.json` に残るが、mp4 自体には何も入っておらず、ファイルを移動・改名・共有すると条件が分からなくなる。
- **仕組み（技術的な壁は無い）**: MP4 コンテナのタグ領域（PNG のテキストチャンクに相当）へ ffmpeg の `-metadata キー=値` で書ける。標準キー（`comment`・`description`）はどのプレイヤーでも表示され、独自キー（例 `nz_videomni_parameters`）は `-movflags use_metadata_tags` を付けたときだけ保持される（付け忘れると mov 側の書き出しで捨てられる）。読み出しは `ffprobe -show_format` で足りる。値は UTF-8 の文字列で数十 KB でも問題なく、既存の mp4 への後付けは `-c copy` の再多重化で済む（再エンコード不要）。
- **段階と見積り**: ①埋め込み＝小（書き出しコマンドに引数を足すだけ。クロップ経路でもタグを引き継ぐ指定が要る）。②表示＝中（バックエンドに ffprobe を呼ぶ小さな入口を足し、webui に「MP4 Info」相当の表示を置く。自前の出力は `metadata.json` があるので、価値は外から持ち込んだ mp4 に対して出る）。③フォームへの流し込み＝中〜大（要求 JSON への逆変換。バッチ機能の「表（CSV）が正典」と同じ発想なのでゼロからではない）。
- **設計判断（着手時に決める）**: 埋める中身は絞る。現状の `metadata.json` は計測の記録なども含んで大きいので、A1111 の `parameters` に倣い「プロンプト・ネガティブプロンプト・幾何・seed・ステップ・モデル名・LoRA・加速設定」を 1 キーの JSON にまとめる。プロンプトが動画に載るので、埋め込まない設定（オプトアウト）を必ず付ける。
- **PNG と同じ宿命**: 再エンコードすると消える（AviUtl2 で編集して書き出した mp4 や、動画サイトへ上げたものからは読めない）。
- **状態**: 未着手（将来の研究課題）。
- **出典**: `services/video_io.py`（ffmpeg の呼び出し）、[`STORAGE_POLICY.md`](STORAGE_POLICY.md)（`outputs/` の位置づけ）、A1111 SD WebUI の「PNG Info」（発想の元）。

#### 3-165. Chained エンジンの潜在クリップサイズの追加（起票：2026-09-24）

- **概要**: 連結生成の第2段（Stage-2）は潜在クリップ（窓）を 22 潜在フレーム（`standard`）か 19（`high_resolution`）で刻む。**オーナーの見立て: ちらつき・モーフィング・ドリフトは窓の境界を跨ぐときに起きやすいので、同じ尺なら窓を広げて境界を跨ぐ回数を減らせば品質が上がるはず。** 低い解像度では VRAM に余裕があるので、22 より広い窓を Chained タブのドロップダウンから選べるようにする。
- **現状**: 窓はプリセット表（`chain_math.py` の `STAGE2_WINDOW_PRESETS`。`standard`＝(22, 18)・`high_resolution`＝(19, 12)・`full_length`＝(61, 61)）からの名前引きで、解像度からは決めていない。「解像度に応じた窓の自動選択」は起票のうえ実測して退けた（[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-69。数値指標と目視が逆を向き、自動で選ばせる判定が信用できなかった）。22 は最初に動いた値であって最適値ではなく、送り 18＝6.000 秒が音声潜在に丸め誤差なく乗る構成（[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) §1）。広げる向きは未検討で、22 より広い多タイル用プリセットは無い（`full_length` は A2V 1 クリップ専用の 1 タイル）。
- **予算上の見込み（フェルミ推定・未実測）**: 1 タイルのトークン数＝(幅÷32)×(高さ÷32)×窓。連結の線（`ltx` 40,000・`ltx25` 44,880）を当てると、1280×768（960 マス）では 2.3 で 41 潜在フレーム・2.5 で 46 潜在フレームまで入る計算になる。逆に窓 25 なら約 1.64〜1.84 Mpx、窓 28 なら約 1.46〜1.64 Mpx が上限の目安。
- **案（オーナー方針）**:
  - ドロップダウンに広い窓を追加し、ラベルに解像度の目安を添える（例「46f（LTX 2.5 1280×768）」）。
  - **未較正の窓では快適上限マーカーを表示しない**（混乱を防ぐ）。全窓の較正は時間がかかるので最初から目指さず、窓ごとに段階的に較正して解禁する。
- **検討点（起票時の整理）**:
...
- **状態**: 未着手（将来の研究課題）。着手順は「`full_length` との比較で品質仮説を確かめる → プリセット追加＋マーカー非表示 → 窓ごとの較正で段階的にマーカー解禁」。
- **出典**: [`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) §1・§1-A・§1-B・§7・§12、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-69、[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §11、`chain_math.py`、`api/models.py`、`tokenBudget.ts`。
