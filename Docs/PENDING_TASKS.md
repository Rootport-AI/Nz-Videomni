...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**7件**） | 部分完了。**次に着手するのは高速化第3弾（SageAttention）**（高速化第1弾＝fusedカーネルと先読みblock swapは2026-08-24に完了。**高速化第2弾＝モデル骨格の常駐〔テキストエンコーダ側〕は2026-08-25に完了し、`unsupported_features`が8件→7件になった**。スタイルLoRAとIC-LoRAは2026-08-25のオーナー目視合格で完了） |
...
- **済み（2026-08-25実装。ただし**オーナー目視待ち**）**: **高速化第3弾＝SageAttention（`attention_backend`）**。**既定は`"sdpa"`のまま据え置いた**（利用者が設定画面で明示的に`sage`を選んだときだけ効く）。**実測は1280×768・2クリップ×121フレームの連結生成で1.1017倍**（交互3組の平均。合格線1.10倍に対してぎりぎりの合格）。**本弾だけは「選ぶと出来上がる絵の細部が変わる」種類の高速化なので、固定ベンチマークのSHA-256一致では合否を出せない**——判定は、すでに実績のあるLTX 2.3のsageを先に較正して作った**PSNR（ピーク信号対雑音比。元の絵とどれだけ違うかを1つの数値にまとめた指標で、単位はdB。数値が大きいほど元の絵に近い）とSSIM（構造類似度。2つの映像がどれだけ同じ構造をしているかを0〜1で表す指標で、1に近いほど同じ）の相対の帯**で行った（768p単発でPSNR 29.082dB／SSIM 0.891、512×320でPSNR 25.171dB／SSIM 0.863。いずれも合格）。**機械ゲートG0〜G3と実機ゲートG5（全11項）は全項目合格したが、オーナーの目視ゲート（G8）は未了である**——素材は`outputs/ltx25-sage-gate/g8_material/`に用意してあり、**音の継ぎ目1箇所の聴き取り**を併せて依頼している（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77.6）。**第2弾と同じく422の側から動作の側へ移ったので、`unsupported_features`は7件→6件**になり、設定画面のattentionの選択がLTX 2.5でも押せるようになった。分類は単発 4／5／16／3、連結 5／5／21／3。フロントエンド本体の変更は0行（触ったのはモックの2箇所だけ）。**あわせて既存の罠が1つ消えた**——これまではLTX 2.5を選んだまま設定画面でattentionを`sage`にすると、以後の**すべての**生成が422で断られていた（**ただし`vae_mode`側には同型の罠が残っている**。下記の判断材料(9)）。正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §77。
...
**同じ2026-08-25の高速化第3弾（SageAttention）も、第2弾と同じ側の作業だった。** 3つの表がそろって動き、`unsupported_features`は7件→6件になって、設定画面のattentionの選択がLTX 2.5でも押せるようになった。**ただし第3弾には、前の2弾に無い性質が1つある**——**選ぶと出来上がる絵の細部が変わる**ので、**固定ベンチマークのSHA-256一致では合否を出せず、オーナーの目視ゲートが要る**（本節執筆時点で未了＝目視待ち）。

- **状態**: 部分完了。**高速化第1弾（fusedカーネル・先読みblock swap）は2026-08-24に、高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）は2026-08-25に完了**（いずれも全ゲート合格・出力ビット不変のため目視ゲート無し。[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §75・§76）。**同じ2026-08-25に高速化第3弾（SageAttention）も実装し、機械ゲートG0〜G3と実機ゲートG5〔全11項〕に全項目合格した——ただし本弾は出力の絵が変わる種類の高速化なので、オーナーの目視ゲート（G8）が未了である＝目視待ち**（同 §77）。連結生成の本体・V2V継続・A2V〔長尺A2Vを含む〕は2026-08-23に実装し**オーナー合格済み**（連結生成は2026-08-23の目視〔[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §72.10〕、V2V継続とA2Vは2026-08-24にAviUtl2の操作パネルからの実機検証で合格し、長尺A2Vはオーナー裁定により合格扱い〔同 §73.10〕）。**スタイルLoRAとIC-LoRA〔長尺を含む〕は2026-08-24に実装して全ゲート合格し、2026-08-25にAviUtl2の操作パネルからの実機検証でオーナー合格した**（同 §74.12）。**残り6件は未着手である。高速化の前倒しは第3弾で打ち止め**（残るPrunaVAEDはLTX 2.5版の枝刈り済み重みが出るまで先送り）**なので、次は準必須級（Retake／End source／Outpainting）に入ることになるが、着手の順序と時期はオーナーの判断による。**
...
- **状態**: 未着手（将来の研究課題）。着手は高速化第3弾（SageAttention）のあとで判断する。
