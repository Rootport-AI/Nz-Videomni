...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**6件**） | 部分完了。**高速化の前倒しは2026-08-25の第3弾（SageAttention）で打ち止め**（`unsupported_features`は7件→6件。**第3弾のオーナー目視〔G8〕も2026-08-25に合格した**）。残るPrunaVAEDはLTX 2.5版の枝刈り済み重みが出るまで先送りなので、次に入るのは準必須級（Retake／End source／Outpainting）だが、着手の順序と時期はオーナー判断待ち |
