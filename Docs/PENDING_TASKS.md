...
- **トリガー**: 参照つきの長尺でIC-LoRAの制御の精度が落ちたと体感したとき、またはその報告が上がったとき。**新しい分岐が発火するのは参照自身の潜在トークン数がしきい値を超えたときで、1152×1536なら単一クリップ369フレーム以上がこれに当たる**（解像度が低いほど、発火に必要な尺は長くなる。正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §101.2）。
- **やること（3点）**:
