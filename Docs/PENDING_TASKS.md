...
- 作成: 2026-07-15／最終更新: 2026-09-01（素材fpsとMCP／モックの3件を実装したのに伴い台帳を組み替えた——§3-13〔素材fpsの取得〕は実装済み・オーナーの実機ゲート待ちとして**§2を復活させて§2-5へ移し**〔§3-13は欠番〕、§3-115・§3-116・§3-122の3件は**実機で確認できる要素が原理的に無いためオーナー裁定により§2を経ず直接クローズ**した〔[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-115・§3-116・§3-122。本書側の3番号は欠番。根拠記録は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §86〕。この直接クローズの例外は末尾の運用規則へ明文化してある。同日それ以前に、オーナーの台帳見直しにより§3-4・§3-10・§3-15・§3-37・§3-45・§3-46・§3-89・§3-91・§3-102・§3-118の10件をクローズし、§3-52を再訪条件つきの§4-31へ移した。同日それ以前に、🎞ボタンで挿入したオブジェクトが青一色になる症状を§3-140として起票し、軽微修正4件がオーナーの実機目視ゲートに合格したため§2は節ごと削除済み。同日さらに、§3-42を「1.86Mpx超の解像度におけるSingle長尺クリップの実現」と改めて再訪条件つきの§4-32へ移し〔§3-42は欠番〕、§3-13〔素材fpsの取得〕を新しい調査結論〔Media Foundation方式で実現可能〕とオーナー方針で書き直し、その方針の覚え書きを§3-71・§3-72へ1行ずつ足した）
...
実装は済んでいて、オーナー本人の実機・目視・実GPUテストが未了のもの。**書式は冒頭の「位置づけ」2番を参照。**

**採番について**: 本節は2026-09-01の軽微修正4件（§2-1〜§2-4）がすべて合格して一度は節ごと削除されたあと、同日中に本項のために立て直したものである。**同じ日のうちに同じ番号を二度使うと読み違えのもとになるので、削除済みの§2-1〜§2-4は欠番のままにして§2-5から続けている**（その4件の記録は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-136〜§3-139にある）。

### 2-5. 素材fpsの取得と操作パネルへの流し込み（ブリッジ契約v11。起票時は本書§3-13＝欠番）

- **何ができるようになったか**: 右クリックプリフィルのSettings方針「素材に合わせる」が、fps軸でも本当に**選択した素材そのもののフレームレート**を使うようになった。これまでは名前だけで、中身はプロジェクトのフレームレートの代用だった（2026-07-22のX1でグレーアウトして残してあったボタンを、今回そのまま復活させている）。**取得はWindowsのMedia Foundation**で、素材ファイルの宣言（最初の映像ストリームのネイティブ型）だけを読む——デコーダを作らず1フレームも復号しないので、動画が長くても遅くならない。**非整数のfpsは整数へスナップする**（29.97→30、23.976→24）。
- **正本**: フロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §101（実装記録）、同[`BRIDGE_CONTRACT.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/BRIDGE_CONTRACT.md) §4.13・§8（契約v11）、同[`RIGHTCLICK_REDESIGN_SPEC.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/RIGHTCLICK_REDESIGN_SPEC.md) §5-13（Settingsの仕様）、[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §86（自動ゲートの結果と物差し）。
- **先にやること**: **もう無い。** フロントエンドの`build.ps1`→`deploy.ps1`での配置（実機＋リポジトリ内の配布コピーの2箇所）は2026-09-01に済ませてある（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §86.6の追記）。**下のG1〜G8はいつでも見られる。**

#### fps軸「素材に合わせる」が実際に効いているか

- [ ] **G1**: Settingsでfps軸を「素材に合わせる」にし、**30フレーム毎秒の素材を24フレーム毎秒のプロジェクトへ置いて**右クリック生成する → 操作パネルのfps欄が**30**になる（プロジェクトの24ではない）。
- [ ] **G2**: 29.97フレーム毎秒の素材と23.976フレーム毎秒の素材で同じことをする → それぞれ**30**と**24**に丸められて入る（小数のままにならない）。
- [ ] **G6**: fps軸を「素材に合わせる」にしたままAviUtl2を再起動する → 設定が「素材に合わせる」のまま残っている（勝手に「プロジェクトに合わせる」へ戻らない）。

#### 読めない素材で固まらないか（**ハング検出型**のゲート）

- [ ] **G3**: mov・mkv・壊れたファイル・極端に大きいファイルを順に右クリックする → **どれでも固まらず、読めないものは即座にプロジェクトのフレームレートへ落ちる**（**mkvがプロジェクトfpsになるのは合格である**——Media Foundationはmkv/webmを読めないのが普通で、これは異常ではない）。**遅いのではなく「固まる」形で出るリスクを見るゲートなので、体感速度のG5とは別に見ること。** 固まったら報告してほしい（非同期化を別途起票し、本機能はフォールバック挙動のまま出す）。

#### 退行が無いか

- [ ] **G4**: fps軸を「プロジェクトに合わせる」「開発用」に切り替えて右クリックする → 従来どおりの値が入る。**サイズ軸の3択も従来どおり**動く（サイズとfpsで別々の方針を選んだ場合も含む）。
- [ ] **G7**: fps軸を「素材に合わせる」にしたまま、**撮り直し（Retake）**を右クリックから始める（素材が60フレーム毎秒など、プロジェクトと大きく違うものが分かりやすい） → 窓の長さ（フレーム数）が上限・下限で頭打ちになる挙動が、おかしな値にならない。
- [ ] **G8**: 🎞ボタンで生成結果をタイムラインへ挿入している最中に、別のオブジェクトを右クリックする → **どちらも正常に終わる**（動画の書き出しと素材fpsの読み取りが、同じプロセスの中で喧嘩しないことの確認）。

#### 体感速度

- [ ] **G5**: 大きめの動画（長尺・高解像度）を右クリックする → **メニューから操作パネルへの反応が今までと変わらない**（もたつきを感じたら報告してほしい。結果の使い回しやワーカーへの追い出しを別途起票する）。
...
**MCPサーバーとフロントエンドのモックに閉じた3件も2026-09-01にクローズした**——`submit_chain`への撮り直しの引数＝同§3-115、モックのクリップ数の下限＝同§3-116、`submit_generate`への画角拡張の引数＝同§3-122（**本書側の3番号は欠番**）。この3件は表の対象外として下に並べていたものである。
...
- **覚え書き（オーナー決定2026-09-01）**: 素材fpsのプリフィルでは**非整数fpsを整数へスナップする**（本書§2-5で実装済み）。ただしこれはプリフィルの値の話であり、**プロジェクトのfpsが29.97のままなら本項の422はそのまま起きる**——スナップは本項の代替にならない。
- **状態**: 将来の改修項目（着手時期未定）。
...
- **覚え書き（オーナー決定2026-09-01）**: 素材fpsのプリフィルでは**非整数fpsを整数へスナップする**（本書§2-5で実装済み）。**本項の音ズレは書き出し側の切り捨てなのでスナップでは解消しない**が、素材が29.97でもプリフィルが30を入れる分、利用者が非整数fpsを指定する機会は減る。
- **状態**: 将来の改修項目（着手時期未定）。
...
- **例外（オーナー裁定2026-09-01）: 実機で確認できる要素が原理的に無い項目——MCPサーバーやフロントエンドのモックに閉じた改修のように、GPUもAviUtl2本体も関与しないもの——は、自動ゲートが全部緑になった記録を根拠に、§2を経ず直接[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移してよい。** 根拠記録（どの物差しで何件通ったか）は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md)へ必ず残し、クローズ記録からそこを指すこと。初適用は同日の§3-115・§3-116・§3-122で、記録は同[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §86である。
