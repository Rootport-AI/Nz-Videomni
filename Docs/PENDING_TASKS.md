...
- [x] **G1 右クリック**: 部分フィルタと動画をそれぞれ右クリックする → 部分フィルタには「🖌 Inpainting」、動画には「🎬 Video」の項目が出ること（メニューはSDKの仕様でどの種別を右クリックしても表示され、対象外の種別を選んだときの断りはクリック後に出る——「対象外の種別には出ない」という従来の合否文言は誤りだったため修正した）。動画側の項目は「🎬 Video: 選択範囲を撮り直す (Retake)」のすぐ下に並ぶ（メニュー順序を変更）。**再確認待ち**（LTX 2.5選択中にInpaintingサブタブが見当たらなかった件はLTX 2.5未対応が原因とオーナーが確認済みで、変更はしない）。 **合格（2026-09-15。並びの修正を確認）**
- [x] **G2 順序不問**: 部分フィルタ→動画、動画→部分フィルタのどちらの順でも右クリックする → どちらの順序でもカード2枚が揃い、2回目で1回目の内容が消えず、対象動画の二重アップロードが起きないこと。**合格。**
- [x] **G3 解像度不一致**: プロジェクトと異なる解像度の動画を対象動画にする → 実数入りの専用文言が出てGenerateが押せなくなること。**不具合を検出**（部分フィルタが無い状態でも解像度不一致の注意文が出ていた）→ **両方のカードが揃っているときだけ判定するよう修正済み・再確認待ち。** **合格（2026-09-15。修正後に再確認）**
...
- [x] **G9 部分フィルタの消失**: カード表示後に部分フィルタを削除してGenerateを押す → 案内文で止まり生成が投入されないこと。**不具合を検出**（案内文が一瞬で消えて読めなかった）→ **トースト表示へ変更済み・再確認待ち。** **合格（2026-09-15。トーストで再確認）**
...
- [x] **①膨張の既定値**: 既定5／2は据え置き、画面の「マスク周囲の「のりしろ」」欄（Stage-1／Stage-2の整数、0〜15）でユーザーが指定できる形に変更して決着（2026-09-15。設計正本[`INPAINTING_DESIGN.md`](INPAINTING_DESIGN.md) §3.8）。
- [x] **②スパイクの目視**: 不要と裁定（G6の実機結果で合格扱い。2026-09-15）。
