...
- 作成: 2026-07-15／最終更新: 2026-09-15（バッチパネルのi2vモード追加〔§1-30〕により§1を立て直した。現在の構成は**§1 近日中の改修項目／§3 将来の研究課題／§4 スコープ外**の3節で、§2〔実装済み・ユーザーのテスト待ち〕は該当項目が無いため、運用規則どおり見出しごと削除してある）
...
### 1-30. バッチパネルへのi2vモード追加（起票：2026-09-15）

- **概要**: Create画面のバッチパネル（従来「バッチA2V」）に、音声フォルダを指定せず画像フォルダだけを指定したときの動き（i2vモード＝画像1枚につき動画を1本作るモード）を追加した。あわせて、行画像の生成強度の規則をa2v・i2v共通の1つ（Create画面冒頭のキーフレームカードの値、無ければ既定0.8）へ揃えた——従来のバッチa2vは行画像の強度が1.0固定だった。
- **設計の正本は[`BATCH_A2V_I2V_MODE.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/BATCH_A2V_I2V_MODE.md)である**（決まったこと・なぜそう決めたか。**本項には写さない**）。
- **状態**: 実装完了・機械ゲート全緑・実機ゲートG1〜G12合格・クローズ待ち（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §108）。
