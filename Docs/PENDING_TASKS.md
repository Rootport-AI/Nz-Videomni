...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り**6件**） | 部分完了。**高速化の前倒しは2026-08-25の第3弾（SageAttention）で打ち止め**（`unsupported_features`は7件→6件。**第3弾のオーナー目視〔G8〕も2026-08-25に合格した**）。残るPrunaVAEDはLTX 2.5版の枝刈り済み重みが出るまで先送り。**次に入るのは準必須級のうちRetakeとEnd sourceを先行**（2026-08-25裁定）**・Outpaintingは棚卸し済みで、着手時期はその2つの完成後にオーナーが再検討する**（棚卸し結果は§3-102の判断材料(10)） |
...
- **着手条件**: 将来のモデル世代交代でこの種の能力が追加されたときに再検討する。**LTX 2.5には対応済みで、素のクリップ連結（正順Chained）も2026-08-23から動くようになったが、本件の題材である逆順Chained＝End sourceはまだ422（`FEATURE_UNSUPPORTED`）で断っているため、2.5では検証できていない**（End sourceの2.5対応は本書§3-102の準必須級に入っている）。現時点では調査・実装のどちらも着手しない。
...
| 見るところ | 現在値 | 完了時にどうなるか |
|---|---|---|
| `GET /models`の`unsupported_features`（LTX 2.5） | **6件**（`retake`／`end_source`／`two_stage_hq`／`outpaint`／`nag`／`prune_vaed`） | 実装した機能の名前が1つ消える。0件になれば本項は完了。**`keep_resident`（第2弾）と`sage_attention`（第3弾）は2026-08-25にここを抜けた（高速化3弾の中でこの配列が動いたのはこの2つで、第2弾が最初。この配列そのものは2026-08-23・2026-08-24にも動いている）** |
| `CHAIN_REJECT_TABLE`（連結生成用） | **5行**（`retake`／`end_source`／`nag_enabled`／`pipeline`／`vae_mode`） | 該当行が1行消える。**`source_video`／`source_audio`は2026-08-23に、`loras`／`reference_video_id`は2026-08-24に、`keep_resident`と`attention_backend`は2026-08-25にここを抜けて動作側へ移った** |
