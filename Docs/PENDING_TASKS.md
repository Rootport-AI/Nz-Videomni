...
- 作成: 2026-07-15／最終更新: 2026-09-26（§3-167 を B-2 完結・次はオーナー目視 → B-3 へ更新）
...
- **状態**: **B-1（LTX 2.3）完結・オーナー目視合格（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §117）。B-2（LTX 2.5）は機械ゲートと実機確認まで完結（同 §118）。次＝B-2 のオーナー目視 → B-3。規則の正本は §117.3 と、その改定の §118.3 です。** 残りは次の 2 つです。
  1. **B-2 のオーナー目視**: 見る項目と出力の場所は [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §118.7・§118.10 にあります。
