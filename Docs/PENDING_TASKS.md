...
- **前提**: §3-97（マルチエンジン土台）。ただし**先に土台を完成させてから着手するのではなく、抱き合わせで進める**——2例目が無いまま土台だけを作り込むと過剰抽象化になるため（§3-97の検証方針を参照）。
- **設計正本**: [`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md)。**参考資料**: [`LTX25_RESEARCH_NOTES.md`](LTX25_RESEARCH_NOTES.md)（オーナーによる2026-08-20共有の事前調査。マルチエンジン設計の議論より前の調査であり、設計資料ではない）。
- **なぜ2例目として理想的か**: LTX 2.5 はエンジン系統としては ltx のままなので、専用の仮想環境もワーカーも新設せずに済む。**ベースモデルの軸だけを独立に検証できる**——一度に検証する軸が1つで済むため、失敗したときに原因が絞れる。エンジン系統の軸（別venv・別worker・別ペイロード）の実証はWan等の別系統を入れるときに行う。
- **主要な作業塊**:
