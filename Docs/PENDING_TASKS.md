...
- 作成: 2026-07-15／最終更新: 2026-09-05（現在の構成は**§1 近日中の改修項目／§2 実装済み・ユーザーのテスト待ち／§3 将来の研究課題／§4 スコープ外**の4節。§2は該当項目が出たので運用規則どおり節ごと立て直した）
...
### 2-8. モデル別のUI表示テーブルと、画角拡張の予算のサーバー配信化 — 本書§3-135の移送先（起票：2026-09-01／実装：2026-09-05）

フロントエンドが自前で抱えていた「モデルごとの知識」を2つとも手放した。①サーバーが名指しする機能名（`chain`・`outpaint`・`prune_vaed`…）が**どのUI部品を閉じ、閉じたあとにどの保存値をサーバー既定へ書き戻すか**を、`webui/src/shell/featureScope.ts`の1枚の宣言表だけで決める形にした。②**画角拡張（Outpainting）の快適上限のトークン予算**を、フロントエンドの自前の表から`GET /config`の配信（`limits.comfort_budgets[系統].outpaint_budget`）へ合流させた。**画面の見た目は何も変えておらず、線の値も1つも動いていない**（配る経路が変わっただけである）。

**実装・設計判断・ゲート実数の正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §99、フロントエンド側の実装記録は[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §111、予算の値の正本は[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §9で、本書には書き写さない。** **新しいベースモデルを足すときに触る場所の一覧は同 §99.9 にある**——**本テーマが減らしたのは、まさにその数である。**

**自動ゲートはすべて合格している**（バックエンドのpytest・フロントエンドの型検査／vitest／lint、ビルドとデプロイのSHA-256 3値一致）。`.aux2`は実機とバックエンドリポジトリ配布コピーの2箇所へ配置済みである。

> **確認の前の必須手順**: **バックエンドを停止し、C1以降のツリーで起動し直すこと。** そのうえで`GET /config`の`limits.comfort_budgets`に`outpaint_budget`が載っていることを確かめてから目視に入る。**サーバーが古いままだと画角拡張の警告がどの尺でも出なくなり、了承済みの退行（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §99.7）を実装の不具合と読み違える。**

**Settings（表から導かれる見え方）**

- [ ] **G-V1（2行の出没）**: ベースモデルを LTX 2.3 と LTX 2.5 で切り替え、Settingsの「生成高速化」を見る → **LTX 2.5 では映像復元器（PrunaVAED）の行が消え、LTX 2.3 では「Embeddings processorの常駐」の行が消えれば合格**（それぞれ、そのエンジンが断る設定である）。**実機で目に見えるのはこの2行だけである**——タブ・パネル・サブタブの無効化は、現行の2系統では発火する条件が揃わない。**そちらが以前と同じ挙動であることは、アサーションを1行も変えずに移設したテストが担保している**（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §99.5）。

**画角拡張（Outpainting）**

- [ ] **G-V2（警告の閾値）**: 両系統で、1280×768の素材へ余白を与えて尺だけを動かす（LTX 2.5 なら余白 左右320・上下128＝キャンバス1920×1024が、較正で測ったのと同じ形である） → **警告が出る尺と出ない尺の境目が[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §9の線どおりで、2026-09-05に確かめた境目から動いていなければ合格。** **「どの尺でも警告が出ない」となっていたら、上の必須手順を飛ばしている**——サーバーを入れ替えてからやり直すこと。

**書き戻し（隠れた行の保存値がサーバー既定へ戻ること）**

- [ ] **G-V3（PrunaVAED）**: LTX 2.3 で映像復元器を PrunaVAED にしてから、LTX 2.5 へ切り替えて1本生成する → **422（設定を断るエラー）が出なければ合格。** 行が隠れるだけでは足りず、`localStorage`に残った選択がサーバー既定へ書き戻されていることの確認である。
- [ ] **G-V4（埋め込み処理器の常駐）**: 逆向きに、LTX 2.5 で「Embeddings processorの常駐」をonにしてから、LTX 2.3 へ切り替えて1本生成する → **422が出なければ合格。** **書き戻しは2本の個別処理から表由来の1本へまとめてあるので、両方向を1回ずつ見ておく。**

**合格後**: ①本項を`3-135`として[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md)へ移送しクローズする（同書に§3-135の記録が無いことを確認済みなので、枝番は付けない）。②本書§4-6の「表駆動化そのものの受け皿」の参照を、本項から同書§3-135へ付け替える。③同書§3-139末尾の申し送り（「研究課題として同書§3-135へ起票してある」）の行き先も同じく付け替える。④同書冒頭の「空になった節の扱い」の段落へ、§2をまた節ごと削除した旨と§2-8が欠番になる旨を追記する。
...
- **設計裁定（2026-09-05・オーナー合意済み）**: 方式は**しきい値自動切替**に確定した。factor-2（canny・pose等のunion-control系）の参照が現行閾値（25,000トークン）以内であれば現行の一括エンコードのまま（バイト同一性を完全温存・追加コストゼロ）とし、閾値を超えたときだけ自動でタイル化へ切り替え、参照長のガードもタイル化前提の予算へ緩和する（緩和後の線は実装時に実測で引く）。
  - **UIノブは設けない**。チェックボックス案（factor-1は既定on固定・factor-2は操作可能）を検討したが、現行ガードがちょうど費用急変点（膝）の真上に立っているため「膝を越えるfactor-2ジョブは今日存在しない」ことが判明し、自動切替であれば既存ジョブのバイト同一性・回帰基準を一切壊さずに済む一方でユーザーが操作すべき判断も残らないため撤回した。参照の長さそのものが実質のスイッチになる（短い参照＝従来どおり、長い参照＝タイル化で新たに通る）。同型の前例はチャンク化アップサンプル（サイズに応じて自動・ノブなし）。
  - factor-1（deblur系）は現状維持（既に常時タイル化済み）。
  - **出荷条件**: 新規に通る長尺領域について、品質受入ゲートを先に合格させること。参照潜在をデコードして戻した動画にcannyとDWposeを再抽出し、輪郭のずれ・関節点距離を数値比較する検査に加え、同一シードでのA/B目視を課す。品質見込みの根拠（factor-1タイル化の前例と誤差実測、sage attentionとの比較）は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §49.9〜§49.11・§43.5・§72が正本（数値はここへ写さない）。
  - **将来への申し送り**: 現行の膝の位置はVRAM 16GB前提の値である。複数のVRAM容量へ対応するときは膝の位置に何らかの分岐が必要になるが、それは現時点では考えない（オーナー裁定）。
- **状態**: 将来の改修項目（設計は確定済み・着手時期は未定。参照つき高解像度長尺クリップの需要が具体的に出たときが自然な着手条件）。
...
- **2ジョブ目以降のVRAMせり上がりは§3-105で解消済みである**（F2は前処理区間のヘッドルームを広げるが、本項が扱うデコード段の境界は動かさない。正本は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §100）。
- **状態**: 将来の研究課題（着手時期未定）。
...
- **本項の扱い**: **実装するかどうかはオーナー判断であり、着手は決まっていない。** 直すなら`GET /models`の`unsupported_features`をGradio側でも読み、含まれる機能のコントロールを隠す（またはグレーアウトする）形になる。**表駆動化そのものは2026-09-05にフロントエンド側で実装済みで、受け皿だった本書§3-135は消化した**（実装の記録は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §99。**この表はWebUI専用でGradioからは使えない**ので、⑤を直すときの参考にはなっても再利用はできない。オーナーの画面目視の受け皿は本書§2-8で、合格後は[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-135へ移る）。
...
- **オーナーとの議論の結論（2026-09-05）**: 実装形はチャンク化が正解である。中間テンソルはタッチ回数こそ少ないが総バイト量が大きく、メインメモリへ展開する方式（activation offloading相当）はPCIe帯域がblock swap先読みの経路と衝突するため不採用。チャンク化は分割数を過剰にしても損失が数%で済むため、境界探索も不要である。
