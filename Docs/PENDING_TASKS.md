...
- **判断材料**: ①この予算は快適上限マーカーの線（単発生成用）とは**別軸である**——同じ式を使うが別ワークロードなので、統合してはならない（理由はコード側のコメントが正本）。②LTX 2.5 の復号は1920×1088×241フレームで退避することが分かっており（本書§4-34）、高解像度側の較正はその制約と重なる。③`webui/src/modes/edit/outpaintGeometry.ts`の`comfortTokenEstimate`は幅・高さ÷32を切り捨てずに掛けており、`comfortTable.ts`の切り捨て式と一致しない。較正時にどちらの式へ揃えるかを先に決めること。
- **状態**: 将来の研究課題（着手時期未定）。
- **出典**: `webui/src/modes/edit/outpaintGeometry.ts`（`COMFORT_TOKEN_BUDGET`とその注記）、バックエンド[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) §1.1・§6、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-95（本項の出自）、本書§4-34。
...
- **関連**: 本書§3-134（画角拡張の予算の較正。高解像度側の較正は本項の制約と重なる）・§3-59（画角拡張のチェーン対応）。
