...
- 作成: 2026-07-15／最終更新: 2026-09-26（§3-167 を B-2 完結＝オーナー目視全合格・次は B-3 へ更新）
...
- **状態**: **B-1（LTX 2.3）完結・オーナー目視合格（[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §117）。B-2（LTX 2.5）はオーナー目視 7 項目全合格で完結しました（同 §118.10・2026-09-26）。次＝B-3（fp8 の快適上限の較正・LTX 2.3 と LTX 2.5。計画承認済み・2026-09-26）です。** 規則の正本は §117.3 と、その改定の §118.3 です。残りは次のとおりです。
  1. **B-3**: fp8 用の快適上限の行を較正する（今は未較正で、マーカーは GGUF 用の目安のまま出ます）。Sulphur-2 Q6_K の w46 の VRAM 溢れを配信値へ反映するかの最終判断も、この較正の後に行います（[`COMFORT_LIMIT_TABLE.md`](COMFORT_LIMIT_TABLE.md) 第12.4節）。
...
- **出典**: [`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §117（裁定・設計・判定規則・VRAM の見立て・申し送り）・§118（LTX 2.5）、`sft_fp8_format.py`、`engine/fp8/`、`engine25/`、README「追加の transformer（GGUF／fp8 safetensors）/ LoRA を配置する」。
