...
- 作成: 2026-07-15／最終更新: 2026-09-02（**文書総整備（敵対的レビュー反映・Outputs-archive拡充）を実施**。失効した§4-12参照・本日分の壊れリンクの修正、`outputs/`のMarkdown2本を`Docs/Outputs-archive/`へ複写、`STORAGE_POLICY.md`へOutputs-archiveの入口を追記、`webui/src`の§3-47コメント全箇所を[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-47-02の表記へ更新）
...
### 2-1. バッチA2Vの走行が、Create画面のリマウントで見えなくなる件の解消

- **何を変えたか**: バッチA2Vのランナー（走行中のバッチそのもの）の置き場所を、画面の寿命に縛られるフックの中から、画面の外側にあるモジュールレベルの1個へ移した。バッチi2v-longが最初から採っている方式に揃えたもので、走行中にCreate画面が作り直されても、新しい画面はその走行へつなぎ直す。走行に使っているフォルダ・行の一覧・行ごとの画像の選択肢も一緒に戻る。走っていないときのリマウントは従来どおり白紙から始まる。
- **これまでどうなっていたか**: 走行中に右クリック操作でCreate画面が作り直されると、バッチは裏で走り続けるのに画面からは消えていた——Stopボタンも行の進捗も無くなり、止める手段が無かった（共有ロックのほうは2026-07-31に手当て済みで、滞留はしない）。
- **オーナー実機目視ゲート（G-P1）**:
  - [ ] バッチA2Vを走らせている最中に、タイムラインからCreate画面へ送るような右クリック操作をする → Stopボタンと行の進捗が消えずに表示されたままになっている（フォルダ欄も空にならない）。
  - [ ] そのままStopを押す → バッチが止まり、Chain画面のバッチi2v-longのStartが押せる状態に戻る（共有ロックが返っている）。
- **出典**: フロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §104、[`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) §3-47-02、[`BATCH_I2V_WORKORDER.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/BATCH_I2V_WORKORDER.md) §6.5。
