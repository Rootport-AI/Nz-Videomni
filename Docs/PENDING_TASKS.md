...
| 項目 | 内容 | 状態 |
|---|---|---|
| §3-102 | LTX 2.5でまだ使えない機能の対応（残り8件） | 部分完了。**次に着手するのは高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）**（高速化第1弾＝fusedカーネルと先読みblock swapは2026-08-24に完了。スタイルLoRAとIC-LoRAは実装完了・オーナー目視待ち） |
...
- **残っているもの（本項の実体）**: 下表の8件（`unsupported_features`の8件と1対1で対応する）。上から順に着手する（オーナー裁定2026-08-23の段階分け「必須級 → 高速化技術の前倒し → 準必須級 → 実験的」に沿う）。**必須級は2026-08-24で出尽くし、同日に高速化第1弾（fusedカーネル・先読みblock swap）も完了した。次に着手するのは表の先頭＝高速化第2弾（モデル骨格の常駐のテキストエンコーダ側）である。**

| 段階 | 残項目 | 断っているフィールド |
|---|---|---|
| **高速化技術の前倒し（第2弾）** | モデル骨格の常駐（`keep_resident`）のテキストエンコーダ側。**既定OFFのオプトイン**として入れる想定 | `keep_resident` |
| **高速化技術の前倒し（第3弾）** | SageAttention。出力の細部が変わるので**PSNRを基準にした判定**が要る（§43の注意書きと同じ） | `attention_backend` |
...
| **実験的** | NAG／VSF（ネガティブプロンプト系） | `nag_enabled`／`neg_method`／`vsf_scale`（下記の判断材料(1)） |
| **実験的** | 非蒸留モデル | `pipeline` |
...
- **判断材料（7）高速化第1弾で分かった、後続に効く事実（2026-08-24）**: ①**`PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`はWindowsのPyTorchでは拒否されており、一度も効いていなかった**（実測。torch 2.9では変数名自体が`PYTORCH_ALLOC_CONF`へ非推奨化されている）。VRAMの断片化を語るときはこの前提から始めること（本書§3-112へ起票済み）。②**VRAMの天井がある場所はエンジンごとに違う**——LTX 2.5はdenoiseの中（stage-2タイル）、LTX 2.3はdenoiseの外（デコード段）。「両エンジンに同じ挙動を」が成り立たない場面があるので、そのときは違いを隠さずフラグの名前で表に出すこと（`hold_arenas`が先例）。③**コードを変えたあとの再ゲートは必ずサーバー再起動から**（`POST /pipeline/load`はPythonモジュールを読み込み直さない）。④**小さいジョブの予約VRAMは履歴依存でMB単位の再現性が無い**ので、合否は「基準比+400MB以内」という予算で判定する。⑤**LoRA有りのジョブでは、denoise直後の受け皿の返却（約208MB）が効かない**（LoRAのA/Bバッファ236MBが受け皿を見続けるため。予算内の既知の限界で、直し方は本書§3-113）。詳細は[`VERIFICATION_LOG.md`](VERIFICATION_LOG.md) §75.7・§75.10・§75.11。
...
- **概要**: 先読みblock swapは、ノイズ除去（denoise）が終わった直後にGPU上の受け皿（アリーナ）を返すようにしてある（実測で約208MBの返却）。ところが**LoRAを使うジョブではこれが効かない**——LoRAのA/Bバッファ（実測236MB）が受け皿を見続けるため、先頭のスロットを返そうとした時点で例外に当たり、そこで処理が止まって後続のスロットも返らない。
