...
- [ ] **2-1. 表示と選択挙動の実機目視**: ヘッダーの静的な「Nz-Videomni」表示が「LTX 2.3」（既定）／「LTX 2.5」を選べるドロップダウンに置き換わっていることを目視する → 表示崩れが無ければ合格。「LTX 2.5」を選び、トースト・警告を出さず無言で「LTX 2.3」へ戻ることを確認する → 無言で復帰すれば合格（LTX 2.5は実体を持たないモックの選択肢で、将来のLTX 2.5対応の置き場所を先取りしただけ）。
  - **経緯**: フロントエンド[`DEVLOG.md`](../AviUtl2-Plugin/Nz-Videomni-frontend-AviUtl2/Docs/DEVLOG.md) §85（`webui/src/shell/AppShell.tsx`、文言は`strings.toolVersion`）。
  - **この先**: このドロップダウンを実配線にする設計は本書§3-97（マルチエンジン土台）・§3-98（LTX 2.5対応）で起票済み。設計正本は[`MULTI_ENGINE_DESIGN.md`](MULTI_ENGINE_DESIGN.md)。
...
- **主要な作業塊**:
  ...
  - フロントエンドのドロップダウン配線——現在ヘッダーにあるモックのドロップダウン（`webui/src/shell/AppShell.tsx:170`・本書§2-1）を実配線へ。切替＝即ロード、ジョブ実行中は不可（既存409 `JOB_BUSY` 流用）、未導入は導入案内、ヘッダーバッジに「読み込み中」を追加。
- **v1スコープ外**: エンジン別のUIパラメータ制約の切り替え（`8n+1` グリッド等。`chain_math.py:41-42` のLTX定数がAPIバリデーションまで連動している）。作業順序は「土台づくり → LTX 2.5実装・切替成功 → UI調整」。
...
- **関連**: §3-98（LTX 2.5対応）が本項の直接の実証相手。本書§2-1（ヘッダーのモデル名ドロップダウン。現在はモックで、選ぶと無言で戻る）が本項で実体を得る。[`MODEL_MANAGEMENT_DESIGN.md`](MODEL_MANAGEMENT_DESIGN.md)（凍結）が現行4カテゴリのドロップダウンの設計記録。
...
- **関連**: §3-97（土台）。本書§2-1（現在モックの「LTX 2.5」選択肢が本項で実体を得る）。本書§3-91（到着時刻の拘束）は「将来のモデル世代交代（例: LTX 2.5等）でこの種の能力が追加されたとき」を着手条件としているため、本項の実装中にLTX 2.5の能力を確かめる機会がある。
