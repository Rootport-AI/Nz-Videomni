...
> **【ステータス追記 2026-07-11】本ワークオーダーは 2026-07-06 に実装完了（VERIFICATION_LOG §31・目視ゲートもクローズ）。** なお、当時スコープ外とした連結タブの LoRA 対応（`GenerateChainRequest.loras` 加算＋worker 配線）は **2026-07-11 に実装済み**（VERIFICATION_LOG §32）。したがって本文中の「連結タブでは `<lora:...>` トークンを除去＋警告」という挙動の記述（§2③・§4・§6 等）は**現在は無効**＝現在の Clip Chain タブはトークンを `loras` として API に送出し、実際に LoRA が効く。本文は起票当時のワークオーダー＝史料として不変。
...
- **連結タブで LoRA を実際に効かせるバックエンド改修**＝**将来の研究課題**。実体＝`GenerateChainRequest` に `loras` フィールドを加算（凍結 API の加算的変更の型）＋ worker 配線（全体共通適用か、クリップ毎適用かの設計判断を含む）。今回は「除去＋警告」でフロントのみに留める。
  - **→ 2026-07-11 実施済み（NEXT_SESSION_HANDOFF 冒頭ブロック／VERIFICATION_LOG §32）**: この「研究課題」は実装された。`GenerateChainRequest.loras` を加算し worker まで配線・強度はチェーン全体で共通（クリップ毎は v1 見送り）。Clip Chain タブは `<lora:...>` トークンを除去せず `loras` として送出するようになり、**「連結は LoRA 未対応（除去＋警告）」という本ワークオーダー時点の記述は現在は無効**。本節は起票当時のスコープ記録として残す。
