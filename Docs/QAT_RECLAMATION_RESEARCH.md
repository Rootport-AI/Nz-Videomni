...
> **22.7GB 回収 DONE（models/ 50.9GB→28.15GB）。** 本調査は「マルチモーダル `Gemma3ForConditionalGeneration` を保ったまま重みロードを回避する」前提（案B=loader パッチ／案A=vision 実抽出）だったが、実装時に **より深い根因** が判明した：**我々は `language_model` の hidden_states しか使わないのに、wheel が vision_tower＋multi_modal_projector 込みのフルのマルチモーダルモデルを構築していた**（vision は最初から死蔵重み・QAT shard がそれを供給し隠していた）。
...
> - **検証**: full-QAT baseline と **バイト完全一致**（T2V `23844b4e…6bb7bf`／最小I2V `a511eda4…c217`・QAT 在/不在の3経路）、peak_vram 8440MB（<baseline 9164）、退行なし。commit `93696b4`→`694ca54`→**main へ `--no-ff` マージ済 `826e76f`**（branch `refactor/qat-reclamation`）。詳細検証ログ＝`Docs/VERIFICATION_LOG.md` §14。
