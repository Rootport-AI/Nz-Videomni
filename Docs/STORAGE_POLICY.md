...
- **保存物を作らない機能は本書の適用対象外である。** 物体追尾は`outputs/`にも`uploads/`にも何も書かない（成果物はAviUtl2のタイムライン上のオブジェクトの書き換えだけである）。詳細は[`OBJECT_TRACKING_DESIGN.md`](OBJECT_TRACKING_DESIGN.md)を参照。
...
## 4. 将来の改善候補（台帳: [`PENDING_TASKS.md`](PENDING_TASKS.md) §4-23）
