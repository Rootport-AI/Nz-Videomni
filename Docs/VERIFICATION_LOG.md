...
**準備**: `audio_strength`が実際に効くことを確認するには、**音声側の重みキーを持つLoRA**を使うこと。実測で確認できたのは以下3本（網羅ではない）：`DR34ML4Y_LT3X_V3`／`LTX-2.3-Henshin`／`LTX2.3-MysticXXX`。**登録済みIC-LoRA 3本と`Pixar_Toon`は音声キーがゼロのためno-op**（`audio_strength`を指定しても何も起きない）で、本ゲートの検証には使えない。
...
- **バックエンドの pytest（モック・アプリ venv）**: **899 passed**。失敗1件は `test_backend_status_...` で、**ポート 18620 でバックエンドが稼働中だと必ず落ちる既知の環境依存テスト**（§47.2 と同一。本改修と無関係であることは確認済み）。
