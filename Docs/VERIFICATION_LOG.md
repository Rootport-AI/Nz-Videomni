...
**API**: `GenerateChainRequest.stage2_window: Literal["standard","high_resolution"] = "standard"`。
...
## 57. ★長尺 IC-LoRA（1 本の参照動画で連結クリップを駆動する）— 実装完了・機械検証は全 PASS（pytest 1189 件＋エンジン 55 件、vitest 2232 件、mock 実サーバー通し）。**実機ゲート G1〜G8 は全 PASS**（2026-08-11。G4 は同日中に較正計測を実施）
...
**G6（前処理コスト）。** canny は約 7.8ms/フレーム（11544 フレーム級へ外挿するとおよそ 91 秒）。dwpose は 417 フレームの実測で 20.12 秒（1 フレームあたり 48.25ms）、11544 フレーム級への外挿は 4.2〜9.3 分——実測点が 1 つしかないため幅を持たせている。あわせて G3 のログから「参照が尽きた分は前処理も走らない」（上限 1873 に対し実処理は 921 フレームのみ）ことも裏取りできた。
