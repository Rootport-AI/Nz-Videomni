...
- 既存 caseA（複雑×男声）＋新 3 本で 2×2 の 4 隅が 720p・同一 seed で完備（＋低解像スパイク run_A）。音声一致 r≈1.0・全 run spill/OOM なし。**リップシンク判定はユーザー試聴＝OPEN**。読み方: caseC>caseA なら声質/音源要因・caseD>caseA ならシーン要因・caseE が高精度ならスパイク結果の 720p 再現。
- 注: 3 本は比較可能性のため**改善ノブ適用前**の条件（`lip sync` 明示・10s+ 尺などは未適用）。ノブ適用版の追加検証はユーザー試聴後に判断。
