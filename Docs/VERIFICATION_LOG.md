...
**(3) 短縮幅は事前の見積りより良かった。** 計画は「GGUF の読み込み 3.9秒が消え、GPU への転送 約1.4秒は残るので `10b_ep_build` は 1.2〜1.5秒」と予測していたが、**実測は 0.58秒**である。**予測が悲観側に外れた理由は本段では切り分けていない**（転送がこのフェーズの内側にあるという前提そのものが実測と合っていない可能性がある）。**次に触る人は、まずこの前提を疑うこと。**
