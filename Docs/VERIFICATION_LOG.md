...
## 82. ★ベースモデル別インストールバッチ `install-LTX25.bat` の新設（台帳 [`PENDING_TASKS.md`](PENDING_TASKS.md) §3-111）＝開発機ゲート A0〜A8 全合格／**サブマシンゲート G0〜G7 は未実施**（2026-08-30）

**結論から言うと、`install-LTX25.bat` をダブルクリックすれば LTX 2.5 の重みが入る、という導線を作り、開発機で確かめられる範囲（A0〜A8）は全て合格した。実ダウンロードを伴う確認（G0〜G7）はサブマシンの担当で、まだ実施していない。** したがって**本テーマはまだクローズしていない**——台帳 §3-111 は「実装完了・サブマシンゲート待ち」の状態で残してある。
...
### 82.4 サブマシンゲート G0〜G7（**未実施**・実施後にここへ追記する）

**実ダウンロードを伴う検証はサブマシンで行う【オーナー裁定 2026-08-23】。** 開発機の `hf.exe` が壊れていること、開発機には既に5ファイルが揃っていて「初めて落とす」経路を通れないことの2点による。**下表は手順と合格条件であり、結果欄はまだ空である。実施したら、この節に結果を追記すること（表そのものは当時のまま残す）。**

対象機はサブマシン（RTX 3080 mobile 16GB・`setup.bat` 導入済み）。受入条件との対応は G1・G5→(a)、G2・G3→(b)、G4・G5→(c)。

| ゲート | 手順 | 合格条件 | 結果 |
|---|---|---|:---:|
| G0 | リポジトリ同期。`.venv-engine\Scripts\hf.exe --help` の終了コード。`models\LTX25\` に5ファイルが無いこと。空き容量 | exit 0・5ファイル未配置・必要量以上の空き | 未実施 |
| G1 (a) | `setup.bat` | 検証テーブルが A0 のベースラインと同一（**行順を含む**）・全 PASS・**LTX 2.5 の行は0**・全ダウンロード SKIP・exit 0 | 未実施 |
| G1.5 | **回線を切断して** `install-LTX25.bat` | 前提確認は通過 → hf のダウンロードが失敗 → **日本語の失敗案内**が出る／非0 終了／`models\.dl\LTX25-1` が残る（再開用） | 未実施 |
| G7a | 回線復旧。`run.bat` → UI で「LTX 2.5（未導入）」を選ぶ | トーストに `install-LTX25.bat` を含む・選択が 2.3 に戻る・`POST /pipeline/load` は送られない | 未実施 |
| G2 (b) | UI とサーバーを閉じ `install-LTX25.bat` | 5ファイルのサイズが HuggingFace と**バイト一致**（正本は [`LTX25_RESEARCH_NOTES.md`](LTX25_RESEARCH_NOTES.md) 10節の表）・テーブルは担当分だけで全 PASS・2.3 の行は0・`models\.dl\` が消える・`LTX25\TextEncoder\` に GGUF 以外が無い・exit 0 | 未実施 |
| G3 (b) | `run.bat` → UI を**開き直して**「LTX 2.5」を選ぶ | 「（未導入）」表記が消えている・`switched` トースト・`GET /models` の `installed` が真 | 未実施 |
| G4 (c) | `install-LTX25.bat` を再実行 | 2エントリとも SKIP・短時間で終了・5ファイルの更新日時が不変・exit 0 | 未実施 |
| G5 (a,c) | 続けて `setup.bat` | G1 と完全一致・LTX 2.5 の5ファイルの更新日時が不変 | 未実施 |
| G6 | G2／G4／G5 の直後の `models/INSTALLED_PATHS.txt` | **3回とも同一内容**（生成物そのものを比較する。行数は書き写さない） | 未実施 |
| G7b | LTX 2.5 で短尺生成を1本 | 完走。`*.assets.safetensors` が初回ロードで自動生成される | 未実施 |
...
**もう1つ、`setup.bat` の画面表示が1行だけ変わっている。** ダウンロード段の見出しから容量の数字を外し、`"Model downloads (public repos, no token needed)"` にした。**担当範囲が引数で変わるようになった以上、見出しに固定の総量を書けないためである**（必要量は `setup.ps1` と `install_model.ps1` がそれぞれの文脈で表示する）。**検証テーブルの行は1行も変わっていない**（A1 で確認済み）。

### 82.6 状態

**§3-111 はクローズしていない。** 開発機ゲート A0〜A8 は全合格したが、**受入条件(b)(c) を直接見るのはサブマシンゲート G2〜G6 であり、それが未実施だからである。** G0〜G7 が全て合格した時点で、台帳 §3-111 を [`PENDING_TASKS_CLOSED.md`](PENDING_TASKS_CLOSED.md) へ移し、§3 の表からも外す。

**GPU には触れていない。** 本テーマの作業は実装・文書・上記 A0〜A8 の全てが CPU のみである。
...
**サブマシンゲート G-R**: 未実施。`setup.bat` を再実行して再試行が実際に効くことを見る（G1.5 は5試行分＝約75秒待ってから失敗案内に到達することを含む）。
