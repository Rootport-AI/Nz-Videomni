...
判定線は **緑残存＝0.0／勾配比 2.0 以下／音声の最大差＝符号器の床の2倍以内／凍結証明は凍結した腕で 0.0・凍結しなかった腕で `None`／VRAM 16,000 MB 未満** である。**予約VRAMは最悪でも 8,966 MB（O8）でこぼれは起きていない。** ホスト側メモリ（RSS）のピークは 20.8〜22.9 GiB、pad帯の平坦度は全腕 25.6〜57.2 だった。
...
- **O4（stage-2を3ステップ）**: 既定の2ステップと比べ keep 核 PSNR は **+0.19 dB**、勾配比は 1.14 → 1.05 とわずかに良く、所要時間は 79.7秒 → 88.9秒（**+11%**）だった。**差は小さく、既定を変える根拠にはならない**——**監督の推奨は既定（2ステップ）の維持**である。これもG8の裁定材料である。
...
**(6) の所要時間の比較（同一条件）**: 台所 2.3＝**108.2秒** / 2.5＝**77.2秒**、街路 2.3＝**124.3秒** / 2.5＝**90.2秒**。**LTX 2.5 のほうが約3割速い**という結果だった。この 2.3 の出力はそのままG8の対比素材になっている。
