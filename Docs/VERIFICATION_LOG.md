...
## 57. ★長尺 IC-LoRA（1 本の参照動画で連結クリップを駆動する）— 実装完了・機械検証は全 PASS（pytest 1189 件＋エンジン 55 件、vitest 2232 件、mock 実サーバー通し）。**実機ゲート G1〜G8 は保留**（2026-08-11）
...
### 57.6 残っていること（実機ゲート G1〜G8 は保留）

**GPU を使う確認はまだ 1 件も実施していない。** デプロイも未実施である。保留中の項目は次のとおり（台帳の [`PENDING_TASKS.md`](../../Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md) §2-1 と同じもの）。

| # | 内容 | 合格条件 |
|---|---|---|
| G1 | 4 クリップ + canny + 長い参照動画（本命） | 全クリップが参照へ追従し、つなぎ目で被写体が飛ばない |
| G2 | G1 + 長尺 A2V の併用 | 音声が全編維持され、映像も参照へ追従する |
| G3 | 4 クリップに 2 クリップぶんの参照 | 422 にならず完走し、後半は参照なしで生成される |
| G4 | stage-1 の負荷閾値の較正（1152x1536 の 361 / 481 フレーム） | 暫定閾値 **25000 トークン**が実測の膝と合う。ずれていれば `CHAIN_STAGE1_COMFORT_TOKEN_BUDGET` と TS ミラーを同時に更新する。**[`CHAIN_STAGE2_RESEARCH_NOTES.md`](CHAIN_STAGE2_RESEARCH_NOTES.md) 6 節が名指しで必須としている項目** |
| G5 | 単発 IC-LoRA の回帰（同一シード） | **出力がビット同一**（§57.2 のリファクタが動きを変えていない証明） |
| G6 | 前処理コストの実測（dwpose / canny × 長尺。depth は v1 対象外） | 所要時間とメモリが許容範囲 |
| G7 | アップロード時のトリム | 11544 フレーム超だけが切り詰められ、上限以下は無変換 |
| G8 | depth × 多クリップの 422 | 画面側の先回りブロックと API の 422 の両方が明快な文言を出す |
