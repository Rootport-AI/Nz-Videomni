...
## 58. ★Single a2vの全長stage-2化（stage-2窓の「全長」プリセット追加）＝§1-19実装完了・機械検証（pytest・型検査・vitest）全PASS・デプロイ済み。**残るのはオーナー実機ゲートのみ**（台帳 `Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md` §1-19・§2-1）（2026-08-12）
...
本節時点で実機ゲートは未実施である。合否判定の項目（G-B1〜G-B7）は台帳フロントエンド[`PENDING_TASKS.md`](../../Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md) §2-1に列挙した。要点は、960×576/481f・1280×768/257f・1280×768/481f（対照）・1920×1088/153fの完走とVRAM同等性確認、512×320/481fでのつなぎ目ゼロの目視確認、改修前生成物との比較によるつなぎ目消失の判定、任意で12fps×481fの音声破綻観測の7点。実測が`config.yaml`の`spill_free_frames`テーブルとずれた場合のみ、同ファイル182〜187行を実測値へ更新する追加作業が発生する（A2Vは音声VAE分がVRAMに乗るため、快適上限が下がる可能性が最有力の追加作業）。

## 59. ★Chainedの快適上限マーカー（解像度スライダーに「ここまでが快適」の線を引く）＝確認実験3Run完了＋実装完了・機械検証（pytest・型検査・vitest・lint）全PASS。**残るのはオーナー実機ゲートのみ**（台帳 `Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md` §1-20・§2-3）（2026-08-12）
...
| frontend `npm test`（vitest） | **2312 passed / 10 skipped**（既存skip） |
...
### 59.6 実機ゲート — 未実施（オーナー実機テスト待ち）

本節時点でオーナーの目視ゲートは未実施である。項目は台帳フロントエンド[`PENDING_TASKS.md`](../../Nz-LTX23-frontend-AviUtl2/Docs/PENDING_TASKS.md) §2-3に9項目として列挙した。要点は、線の位置がつまみと実際に重なるかの実測、仕上げ工程の切替に線が追随するか、赤い線の逆算が実地で正しいか、明暗どちらのテーマでも白い線が見えるか、参照動画添付時に線が128の倍数へ切り下がるか、そして`config.yaml`の値を30000／60000に変えて再起動すると線が動くか（ハードコード禁止の実地確認）である。
