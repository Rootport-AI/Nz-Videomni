...
## 49. ★IC-LoRA Depth（深度制御）・Deblur（ぼけ除去）の追加＝実装完了・機械検証（pytest・自己診断・型検査・vitest）全PASS・**G1（深度制御信号の性質ゲート）全項目PASS・G2（mock 通し）全項目PASS（2026-08-03）。G3はDepth合格・DeblurはOOMで調査中。G4は未実施**（2026-08-03実装。**テーマ未完結**）
...
**デプロイも未実施である。** コミット・プッシュ済み（backend `fd6d43f` / frontend `d375028`、2026-08-03）。G4は引き続き未実施。

### 49.9 G3 途中経過（2026-08-03オーナー実機）

- **Depth（①通常経路・②A2V経由）＝合格。** オーナー実機で、参照動画の特徴（動きや構図）を踏襲した動画を問題なく生成できることを確認した。本2項目はクローズする。
- **Deblur（③）＝OOM発生。** `t2v`・1280x768・257フレームでジョブ `92a95e94` を実行したところ、生成開始から約50秒後にCUDA out of memoryで停止した。落下地点（stage-1のどの時点でVRAMを使い切っているか）を調査中であり、④のVRAM実測はこのOOM調査待ちのため未着手。
- **計画段階のリスクR4との関係**: §49.1／本ワークオーダー§4で記録したとおり、Deblurは`reference_downscale_factor=1`（参照を縮小せずに条件付けに使う）であるため、stage-1の総トークン数が既存の制御系（縮小係数2）比で約1.6倍に増える見立てだった。今回のOOMは、この見立てが1280x768/257fという条件下で現実化した可能性がある事実として記録する。ただし解像度・フレーム数の限界なのか他の要因が重なっているのかはまだ切り分けておらず、断定はしない。
