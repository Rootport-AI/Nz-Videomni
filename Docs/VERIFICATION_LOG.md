...
ユーザーが動画の一区間を選ぶと、サーバーはその区間の前後に「のりしろ」を足した**窓**を元ファイルからフレーム単位きっかりに切り出し、窓の**両端を凍結したまま真ん中だけを作り直して**、**窓まるごと**を返す。返ってきた窓は、タイムライン上で元の動画に重ね置きして使う。窓の長さは 73〜169 フレーム、のりしろの既定は頭 25 フレーム／尾 24 フレームである。
...
| 項目 | 結果 |
|---|---|
| backend `pytest`（app の仮想環境） | **1108 PASS／15 skip／0 FAIL**（着手前 1045 件 → 1113 件・**+68 件**・退行ゼロ） |
...
§55.10 で「残っているもの」としていた実 GPU での生成を実施した。手順書は `outputs/retake_gate/RUNBOOK_RETAKE_GATE.md`、実行スクリプトは同ディレクトリの `run_retake_gate.ps1`（ポート 18620・アームごとにサーバーを起動して必ず自分で停める作法。W1 ゲート `outputs/stage2_window_optin/run_w1_gate.ps1` の踏襲）。判定は各アームの `outputs/retake_gate/runs/<日時>-<アーム名>/summary.json` に機械が読める形で残る。
