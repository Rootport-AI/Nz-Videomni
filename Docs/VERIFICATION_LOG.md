...
**帯幅の法則（模型の訂正）。** 最終出力でマスクの外側が描き替わる帯の幅は、Stage-1の膨張 r1 に依らず、**Stage-2の膨張 r2 だけ**で決まる——`r2 × 長辺 ÷ 64 ＋ 約18 px`（実測 38/58/98 px 対 r2＝1/2/4）。計画で置いた「r1 の帯が±20%」という掛け算の模型は形が違っていた（低解像度で膨張してから拡大する双一次補間が一定の定数項を足す）。既定 5/2 では長辺1280で約58 px。**帯を狭めたいなら r2 を下げるのが効く。** 既定値は画角拡張と同じ 5/2 で据え置く。

**復元は最終段だけで足りる（敵対的レビュー C3 の裏づけ）。** 復元前の汚染幅（54〜57 px）と復元後の帯幅（57〜58 px）がほぼ同じで、Stage-1のにじみはStage-2のブレンドがキャンバスで上書きして消している。半解像度で復元を挟む必要は無い。
