...
> **チェーンの `end_source`（素材（末尾））の補足（2026-08-16追加、2026-08-17に窓内モードへ書き換え）**: `GenerateChainRequest` の `end_source`（`EndSourceSpec`。`video_id` / `image_id` のどちらか一方と `context_frames`）は、**クリップの本数だけで挙動が2つに分かれる**。判定は `chain_math.compute_chain_layout` が1箇所で行い、`metadata.json`（§6.6）の `end_source.mode` として公開する（リクエスト側にモードを指定するフィールドは無い）。
>
> - **クリップ1本＝`"in_window"`（窓内モード。推奨）**: 素材の先頭 `context_frames` フレームが**クリップ自身の末尾**として凍結される。stage-1 はクリップ1本を1つのデノイズ窓として処理するため、凍結した素材が窓の全区間から注意機構に見え、生成は最初から素材へ向かって進む。**配信尺はクリップの `num_frames` そのもの**で、恒等式は `total_px == clips_total_px` である（`source_video` を併用したときだけ、あちらが凍結した頭のぶんが差し引かれる）。帯とV2Vの頭が同じ窓を食い合って生成すべき潜在が1枚も残らない構成は、**通るクリップ長を名指しした 422**（`VALIDATION_ERROR`）で拒否する。
> - **クリップ2本以上＝`"internal_segment"`（旧方式。非推奨）**: 帯はクリップ列の後ろへ足される専用セグメントで、恒等式は `total_px == clips_total_px + end_context_px`、すなわち**配信尺が帯のぶん伸びる**。帯のセグメント境界を越えて注意が働かないため、生成結果が素材へ**クロスフェード**する既知問題がある。互換のために温存しているが、新規の利用は避けること。
> - **両モード共通**: `context_frames` は**8の倍数**（8n+1 ではない）で [8,136]、既定72。両端が別の格子に乗るのは映像VAEが因果的で、潜在0が画素0だけを覆う単独のキーフレームだからである。素材は `context_frames + 1` フレーム以上必要（先頭1枚は因果VAEのキーフレーム潜在に消費され、出力には現れない）。総尺上限 `MAX_CHAIN_TOTAL_PIXEL_FRAMES` は**帯を除いたクリップ合計**に掛かる。`overlap_frames >= 2` が必須（窓内モードにはつなぎ目が無く根拠は消えているが、機能の規則をモード別の例外で増やさないため維持している）。`retake` / `source_audio` / `reference_video_id` とは排他で、`source_video` との併用（＝冒頭と末尾を与えた補間）は可能である。
> - **凍結 API 契約への影響**: 2026-08-16 の追加そのものは加算のみだったが、**2026-08-17 の窓内モードで単一クリップの意味と受理範囲が変わった**（改版履歴 v0.5.13）。キーの追加・型変更・エラーコードの増減は無い。
> - 詳細な設計判断・実装内容・実機実験4ラウンド20ジョブの正本は[`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §61（v2＝内部区画方式の歴史記録は同 §60）、契約の記述はフロントエンド`Docs/API_REFERENCE.md` §5.2。
