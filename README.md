...
> オーナーの目視確認にも合格しました。撮り直しと素材（末尾）は目視確認がまだ済んでいません**）。**LTX 2.5 でまだ使えないのは、NAG／VSF・PrunaVAED・
> 非蒸留モデル用の高品質パイプラインの3つ**です。
...
**LTX 2.5 でまだ使えないのは3つです**——**NAG／VSF（非CFGネガティブプロンプト）**・
**PrunaVAED（枝刈り版の映像VAEデコーダ）**・**非蒸留モデル用の高品質パイプライン（`two_stage_hq`）**。
**どれも画面のタブやパネルとして出ている「機能」ではありません。** PrunaVAED は
**LTX 2.5 版の枝刈り済み重みが世の中に無い**ため先送りです
...
6. **クリップ連結（`submit_chain`）も LTX 2.5 で使えます**（2026-08-23 から）。**`source_video_id`（V2V）と
   ...
   **なお撮り直しは、そもそも `submit_chain` に引数がありません**——**LTX 2.3 でも同じ**で、LTX 2.5 で
   ...
   いま 422 になるのは、NAG／VSF（`nag_enabled`）・
   PrunaVAED（`vae_mode`）・非蒸留パイプライン（`pipeline`）の3つだけです（§7.1）。
