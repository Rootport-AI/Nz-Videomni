...
3. **画面の見え方で戸惑いやすいところが2つあります。**
   ...
   **なお 2026-09-02 に、LoRA の強さの数え方を ComfyUI と同じ（指定した値がそのまま効く）に改めました。これは LTX 2.3・LTX 2.5 の両方に関わる変更です。**
   `Pixar_Toon` と `LTX-2.3-Henshin` はそれまで指定の半分の強さでしか効いていなかったので、
   これからは作った人が意図したとおりの強さになります（以前と同じ効きにしたいときは `0.5` を指定してください）。
   また **kohya 形式の LoRA はそれまで指定しても何も起こりませんでしたが、これからは効くようになります**（上記「LoRA」の項）。
