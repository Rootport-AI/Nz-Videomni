...
> **撮り直し（Retake）と素材（末尾）は 2026-08-26 から、キャンバス拡張（画角拡張・Outpainting）は 2026-08-29 から、
> ネガティブプロンプト（NAG／VSF）は 2026-08-30 から LTX 2.5 でも使えます**（§7「制限事項」の LTX 2.5 の項）。
> **この3つはいずれもオーナーの目視確認に合格しています**（キャンバス拡張は 2026-08-29、
> 撮り直しと素材（末尾）およびネガティブプロンプトは 2026-08-30）。
> **LTX 2.5 でまだ使えないのは、PrunaVAED と非蒸留モデル用の高品質パイプラインの2つだけ**です。
...
スタイル LoRA・IC-LoRA（参照動画による制御）と、キャンバス拡張（画角拡張・Outpainting）と、高速化4つ（GGUF逆量子化の1カーネル化・先読み block swap・
モデル骨格の常駐・SageAttention）です。**
**このうち撮り直しと素材（末尾）には 2026-08-26 に、キャンバス拡張には 2026-08-29 に対応しました。**
...
素材（末尾）が、2026-08-29 にキャンバス拡張が加わったことで、単発生成でも連結生成でも断られるモードは
1つも無くなりました**（上の表に残る3つは、いずれもモードではなく、エンジンが持っていない機能を指す項目です）。
**したがって、いま画面で灰色になるタブ・パネル・サブタブは1つもありません**（Edit タブの2つのサブタブは
どちらも開きます）。
...
**この2つは 2026-08-30 にオーナーによる目視確認にも合格しています**（出来上がる動画そのものが増える改修なので、
機械の検証だけでは採否を決められず、AviUtl2 の操作パネルから実際に撮り直しと逆順の連結生成を回してもらいました。
結果は [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §78.14、実測は同 §78）。
**なお、素材（末尾）を2本以上のクリップで使う「逆順」は、以前からの案内どおり推奨しません**——
つなぎ目の音が段差状に聞こえます。これはオーナー確認のうえで**仕様として許容**しているもので、
推奨のクリップ1本の使い方には影響しません。

**2026-08-29、キャンバス拡張（画角拡張・Outpainting）も LTX 2.5 で使えるようになりました。** Edit タブの
「キャンバス拡張」サブタブです。**上の表からもう1行消えて、画面の灰色が最後の1つ解けます**——
**これで単発生成の側でも断られるモードは1つも無くなりました。** 作り方は**公式の LTX 2.5 のやり方をそのまま
移したもの**で、LTX 2.3 が使っているのと**同じ In/Outpainting の LoRA ファイル**を LTX 2.5 の本体に載せて動かします
（公式のやり方自体がこの組み合わせなので、LTX 2.3 用の重みを流用しているわけではありません）。
実機での確認では、同じ依頼が LTX 2.3 で 108.2秒、LTX 2.5 で 77.2秒でした——ただし
**この2つを「LTX 2.5 のほうが速い」という意味に受け取らないでください。** LTX 2.3 側は
ベースモデルを切り替えた直後の1本目で、**モデルをディスクから読み直す時間が丸ごと乗っています**
（同じ条件でも2本目は5秒ほど速くなります）。**速さを比べるための測り方をしていない数字**なので、
ここでは「どちらのエンジンでも問題なく最後まで動いた」ことの記録として載せています。
**この機能は 2026-08-29 に目視確認まで済んで完成しています**
（実測は [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §79、目視の結果は同 §79.11）。
**同じ日に、描き足した部分がうっすら緑がかって見える現象も直しました**——合成のときに
下敷きとして使っていた緑の余白を、その場面で描かれた絵そのものに置き換えるという直し方で、
**LTX 2.3 と LTX 2.5 の両方に入っています**（同 §79.10）。**元の映像を残す部分の再現も、
以前より良くなりました。****使うときに1つ知っておいてください**——
**生成の進み具合の表示が 50% あたりで15秒ほど止まって見えます**が、不具合ではありません
（内部で映像を組み直している区間で、進み具合の計算がこの区間を数えないためです。**LTX 2.3 でも同じ場所で止まります**）。
...
**LTX 2.5 でまだ使えないのは2つです**——**PrunaVAED（枝刈り版の映像VAEデコーダ）**・
**非蒸留モデル用の高品質パイプライン（`two_stage_hq`）**。
**どちらも画面のタブやパネルとして出ている「機能」ではありません。** PrunaVAED は
**LTX 2.5 版の枝刈り済み重みが世の中に無い**ため、非蒸留モデルは
**それを動かせる計算機が手元に無い**ため、どちらも先送りです
（[`Docs/PENDING_TASKS.md`](Docs/PENDING_TASKS.md) §3-102）。

> **2026-08-30、ネガティブプロンプト（NAG／VSF）が LTX 2.5 でも使えるようになりました。**
> 「こういう絵にはしないでほしい」を言葉で指定する機能で、**NAG と VSF の2つの方式を
> `neg_method` で選べます**（つまみも LTX 2.3 と同じ `nag_scale` / `nag_tau` / `nag_alpha` /
> `vsf_scale` です）。**使い分けの目安は、正確さを保ちたいなら NAG、特定の要素を強く追い出したいなら VSF**
> （詳しくは [`Docs/VSF_README_NOTES.md`](Docs/VSF_README_NOTES.md)）。
> **1つだけ注意があります**——**`nag_alpha` を 0 にしても「NAG なし」とまったく同じ絵にはなりません。**
> 切りたいときは**チェックボックス（`nag_enabled`）を外してください。**
> **この機能は 2026-08-30 にオーナーの目視確認にも合格しています。**
...
（[`Docs/PENDING_TASKS.md`](Docs/PENDING_TASKS.md) §3-102。**クリップ連結・V2V・A2V は 2026-08-23 に、スタイル LoRA と IC-LoRA と高速化2つは 2026-08-24 に、モデル骨格の常駐と SageAttention は 2026-08-25 に、撮り直しと素材（末尾）は 2026-08-26 に、キャンバス拡張（Outpainting）は 2026-08-29 に対応済みです。高速化で残っているのは PrunaVAED だけで、これは LTX 2.5 版の枝刈り済み重みが存在しないため先送りです**）。**なお LTX 2.5 の重みは HuggingFace で公開済みですが、`setup.bat` の取得対象ではありません**——
