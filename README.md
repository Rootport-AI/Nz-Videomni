...
V2V（元動画からの継続生成）・end source（指定した画像・動画で**終わる**動画の生成。**実験的機能で、現在はUI非公開**——§7参照）・A2V（音声から動画生成）・IC-LoRA／スタイルLoRA
などを加算的に拡張しています。
...
- **旧「未実装（Phase 2以降）」一覧の現状**（Phase 1 当時の一覧はその後の拡張で大半が実装済みになりました）:
  ...
  - **2026-08-16に実装（ただし実験的機能で、現在はフロントエンドのUIに公開していません）**: 終了フレーム指定は **end source（`POST /generate/chain` の `end_source`）** として実現しました。**指定した画像・動画そのもので終わる動画**を作る機能で、末尾の区間（帯）は素材をVAEエンコードして凍結します。帯の長さは8の倍数で最大136フレーム、**出力の尺はクリップ合計＋帯**になります（クリップ側は新規生成分のまま）。素材（冒頭）＝`source_video` と併用すると、冒頭と末尾を与えた補間になります。**ただし実際の生成結果は「本体はプロンプトどおりに進み、そこからクロスフェードで素材へ接続する」形になり、指定した素材で終わる動画にはなりません**（末尾の凍結帯の情報を前方のフレームへ伝える経路が無いため）。この既知問題により、2026-08-16のオーナー裁定で**AviUtl2フロントエンドのUIからは非公開**とし、**APIと内部実装だけを将来の改修に備えて温存**しています。API から直接使うことは技術的には可能ですが推奨しません。詳細はフロントエンド `Nz-LTX23-frontend-AviUtl2/Docs/API_REFERENCE.md` §5.2、経緯と実測は [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §60。なお**キーフレーム画像による**最終フレームちょうどの条件付けも引き続きできません（`frame_idx` は `num_frames-8` にクランプされます）。
