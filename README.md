...
V2V（元動画からの継続生成）・end source（素材（末尾）＝添付した画像・動画へ**繋がる**動画の生成。**クリップ1本のときが推奨**——§7参照）・A2V（音声から動画生成）・IC-LoRA／スタイルLoRA
などを加算的に拡張しています。
...
> **この表は主要なものだけを載せた抜粋です。** 全ルートの一覧と各フィールドの詳細は、フロントエンド側の
> `Nz-LTX23-frontend-AviUtl2/Docs/API_REFERENCE.md` が正本です。稼働中のサーバーであれば
...
- **旧「未実装（Phase 2以降）」一覧の現状**（Phase 1 当時の一覧はその後の拡張で大半が実装済みになりました）:
  ...
  - **2026-08-16に実装し、2026-08-17に作り替えました**: 終了フレーム指定は **end source（素材（末尾）。`POST /generate/chain` の `end_source`）** として実現しました。**添付した画像・動画へ繋がる動画**を作る機能です。**クリップが1本のとき（窓内モード。こちらが推奨）**は、素材の先頭フレームがクリップ**自身の末尾**として凍結され、生成の最初から素材へ向かって進みます——**出力の尺は伸びず、指定したクリップの長さそのもの**です。凍結するフレーム数（錨）は実機比較で**8フレームが最良**と分かっており、AviUtl2フロントエンドは常に8を送ります。**クリップが2本以上のとき（旧方式。非推奨）**は、帯をクリップ列の後ろへ継ぎ足す旧来の幾何になり、**出力の尺がクリップ合計＋帯へ伸びる**うえ、生成結果が素材へ**クロスフェード**する既知問題があります——互換のために温存しているだけなので、新規の利用は避けてください。素材（冒頭）＝`source_video` と併用すると、冒頭と末尾を与えた補間になります。**既知の限界**: 素材が本体のシーンと意味論的に遠いと、クロスフェードやカットで繋がります（モデルの限界であり、素材の選び方で回避します）。また、クリップが仕上げ工程のタイル1枚（標準169フレーム・高解像度145フレーム）を超えると境界にちらつきが出るため、フロントエンドが警告を出します（サーバーは拒否しません）。詳細はフロントエンド `Nz-LTX23-frontend-AviUtl2/Docs/API_REFERENCE.md` §5.2、実装と実機実験は [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §61（旧方式の記録は同 §60）。なお**キーフレーム画像による**最終フレームちょうどの条件付けは引き続きできません（`frame_idx` は `num_frames-8` にクランプされます）。
  - **引き続き未実装**: 本格的なジョブキュー（単一ユーザー想定のため「1ジョブ＋busy 409」を正式仕様とし、仕様書 §13.5 でスコープ削除）／AviUtl2 拡張フロントエンド（別リポジトリで開発・本リポジトリは汎用 REST API のまま）。
...
3. `/mcp` コマンドで `nz-ltx23` サーバーと 22 個のツールが一覧に出れば成功。
