...
> LTX 2.3 側のコードパスの名前で、LTX 2.5 のエンジンはそれを持っていないためです。NAG・LoRA・撮り直し（Retake）・
> 素材（末尾）なども同様に LTX 2.5 では使えません（§7「制限事項」の LTX 2.5 の項）。**クリップ連結（Chained）・
> V2V・A2V（長尺 A2V・バッチ A2V を含む）は 2026-08-23 に対応済み**で、LTX 2.5 でもそのまま使えます
> （操作パネルからの実機確認済み）。
...
V2V（素材（冒頭）に動画を使って続きを作る）と、A2V（音声から動画）です。**
**このうち後ろの3つ（クリップ連結・V2V・A2V）には 2026-08-23 に対応しました**——それまではクリップ連結そのものが
丸ごと断られていましたが、いまは複数のクリップを1本に繋ぐ生成も、動画の続きを作る生成も、音声に合わせた生成も、
そのまま動きます（基本生成はそれ以前から使えます）。
**A2V は Single 画面の音声アコーディオン・複数クリップにまたがる長尺 A2V・バッチ A2V のどれも使えます。**
**いずれも AviUtl2 の操作パネルからの実機確認で動作を確認済みです**（クリップ連結は 2026-08-23、V2V と A2V は
2026-08-24）。
下記は要求すると **422 で断られます**
...
断り方は「入口ごとまとめて」ではなく項目ごとです。** 素のクリップ連結・V2V・A2V はそのまま走り、その上に重ねる
この2つのモードだけが断られます。画面でも Chained タブは開き、灰色になるのはその中の2つのカード（素材（末尾）・
参照動画）だけです。
...
（[`Docs/PENDING_TASKS.md`](Docs/PENDING_TASKS.md) §3-102。**クリップ連結・V2V・A2V は 2026-08-23 に対応済みで、次に着手するのはスタイル LoRA です**）。**なお LTX 2.5 の重みは HuggingFace で公開済みですが、`setup.bat` の取得対象ではありません**——
