...
> **これらはオーナーによる実機確認・目視確認にすべて合格しています**（**2026-09-03 に加わった埋め込み処理器の常駐だけは、まだオーナーの確認を経ていません**）。
...
「埋め込み処理器の常駐」が加わりました）。**このうち LTX 2.3 で効くのは5つです**——6つ目だけは LTX 2.5 専用で、
...
Acceleration 区画にある「埋め込み処理器の常駐（LTX 2.5）」トグルで切り替えます。**既定は off** です。
...
**ただし 2026-09-03 に加わった埋め込み処理器の常駐だけは、まだオーナーの確認を経ていません**（実測とゲートの記録は同 §92）。
