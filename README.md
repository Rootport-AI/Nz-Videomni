...
>
> **Nz-Videomni は製品の名前、LTX 2.3 はモデルの名前**です。**LTX 2.5 には 2026-08-22 に対応しました**（画面上部のドロップダウンで切り替えます。対応範囲は基本生成〔テキストから動画・画像から動画〕と、2026-08-23 に加わったクリップ連結〔Chained〕まで）。さらに Wan 2.x など別のモデルも載せられる基盤を目指しているため、製品名にモデル名を含めていません。
...
> LTX 2.5 では使えません（§7「制限事項」の LTX 2.5 の項）。**クリップ連結（Chained）だけは 2026-08-23 に対応済み**で、
> LTX 2.5 でもそのまま使えます。
...
**LTX 2.5 で使えるのは基本生成（テキストから動画・画像から動画）と、クリップ連結（Chained）です。**
**クリップ連結には 2026-08-23 に対応しました**——それまでは連結そのものが丸ごと断られていましたが、いまは
複数のクリップを1本に繋ぐ生成がそのまま動きます。下記は要求すると **422 で断られます**
（AviUtl2 の画面側でも該当するタブとパネルが灰色になるので、通常の操作でここへ到達することはありません）。

| 断られる機能 | 対応するリクエスト項目 |
|--------------|------------------------|
| 撮り直し（Retake） | `retake` |
| 素材（末尾）＝end source | `end_source` |
| V2V（素材（冒頭）に動画を使って続きを作る） | `source_video` |
| A2V（音声から動画） | `source_audio` |
...
**上の4つ（撮り直し・素材（末尾）・V2V・A2V）は連結生成と同じ入口（`POST /generate/chain`）を通りますが、
断り方は「入口ごとまとめて」ではなく項目ごとです。** 素のクリップ連結はそのまま走り、その上に重ねるこれらの
モードだけが断られます。画面でも Chained タブは開き、灰色になるのはその中の4つのカード（素材（冒頭）の動画・
素材（末尾）・参照動画・音声）だけです。
...
6. **クリップ連結（`submit_chain`）も LTX 2.5 で使えます**（2026-08-23 から）。ただし V2V・A2V・撮り直し・素材（末尾）の
   引数を添えると 422 になります（§7.1）。
