...
LTX 2.3 と共用前処理器だけ**なので、この 15 行にも LTX 2.5 の行は出ません。LTX 2.5 は専用の `install-LTX25.bat` で
別に導入します——下の「models フォルダの構成」を参照してください）。ここが MISSING のまま気づかないと、
...
> **`setup.bat` は LTX 2.5 のぶんを取りにいきません（意図的にそうしてあります）。** インストール最後の検証テーブルにも LTX 2.5 の行は出ません。
> これは「本体のインストーラは LTX 2.3 だけを導入し、別のベースモデルは専用のバッチで足す」という設計によるもので、
> 重みが手に入らないからではありません。**LTX 2.3 だけを使うぶんには何の影響もありません**
> （LTX 2.5 は、`install-LTX25.bat` を実行するまで「選べるが未導入」として画面に出ます）。
>
...
> キャンバス拡張〔画角拡張〕・ネガティブプロンプト〔NAG／VSF〕、および生成の高速化の4項目）。
...
> ②**PrunaVAED を LTX 2.3 で on にしたまま LTX 2.5 へ切り替えると、以後のすべての生成が 422 で断られます**
> （設定画面で Default に戻してください）。③**素材（末尾）は「クリップ1本」での使用を推奨します**
...
生成の高速化4項目（GGUF逆量子化の1カーネル化・先読み block swap・モデル骨格の常駐・SageAttention）——は
**すべて LTX 2.5 でそのまま使えます**。**画面で灰色になるタブ・パネル・サブタブは1つもありません。**
**これらはオーナーによる実機確認・目視確認にすべて合格しています**
...
**LTX 2.3 を選んでいるあいだは、これらはすべて従来どおり使えます。** LTX 2.5 の重みは
...
`models/LTX25/<カテゴリ>/` へ置くと認識されます（§1「models フォルダの構成」の LTX25 の項）。
