...
> 選ぶと 1280×768 の連結生成が約1.10倍。**ただし `sage` は選ぶと絵の細部が変わります**）。NAG・撮り直し（Retake）・
...
**なお LTX 2.5 での `sage` は、開発者による検証はすべて終わっていますが、オーナーによる目視確認がまだ済んでいません。**
...
断り方は「入口ごとまとめて」ではなく項目ごとです。** 素のクリップ連結・V2V・A2V・LoRA・参照動画はそのまま走り、
その上に重ねるこの2つのモードだけが断られます。画面でも Chained タブは開き、灰色になるのはその中の1つのカード
（素材（末尾））だけです。
...
**なお、この機能はオーナーによる目視確認がまだ済んでいません**（絵が変わる種類の高速化なので、機械の検証だけでは
採否を決められないためです）。
...
（[`Docs/PENDING_TASKS.md`](Docs/PENDING_TASKS.md) §3-102。**クリップ連結・V2V・A2V は 2026-08-23 に、スタイル LoRA と IC-LoRA と高速化2つは 2026-08-24 に、モデル骨格の常駐と SageAttention は 2026-08-25 に対応済みです。高速化で残っているのは PrunaVAED だけで、これは LTX 2.5 版の枝刈り済み重みが存在しないため先送りです**）。**なお LTX 2.5 の重みは HuggingFace で公開済みですが、`setup.bat` の取得対象ではありません**——
...
6. **クリップ連結（`submit_chain`）も LTX 2.5 で使えます**（2026-08-23 から）。**`source_video_id`（V2V）と
   ...
   **なお LTX 2.5 での `sage` は、開発者による検証はすべて終わっていますが、オーナーによる目視確認がまだ
   済んでいません**（§5「生成の高速化（Acceleration）」と同じ断りです）。
   いま 422 になるのは、撮り直し（`retake`）・素材（末尾）（`end_source`）・キャンバス拡張（`outpaint`。
   単発生成のみ）と、NAG／VSF（`nag_enabled`）・PrunaVAED（`vae_mode`）・
