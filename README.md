...
- **効くのは2本目以降です。** 1本目は組み立てるので、これまでどおりの時間がかかります。**2本目からはその組み立てが
  省かれます**（**短縮の実測値は [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §92 が正本です**）。
- **出来上がる動画は1バイトも変わりません。** 同じ部品を作り直さずに使い回すだけなので、速くなるだけです。
- **GPU（VRAM）は増えません。** メインメモリと引き換えの機能なので、GPU 側には何も置きません。
- **代償はメインメモリで、常駐は約 4.6GiB です**（ワーカーのログでの実測値。**実機での確定値は
  [`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §92 を参照してください**）。
- **上の「モデル骨格の常駐」とは別のスイッチで、常駐する相手も別物です**（あちらは文章を読み取る部分＝テキスト
  エンコーダの重み）。**両方 on にすると、メインメモリの増分は足し算になります**——片方が他方を含んでいるわけでは
  ありません。**メモリに余裕がない場合は、片方だけを on にする使い方もできます。**
- **off に戻すと、そのジョブの先頭で解放されます**（「キーを送らないこと」がそのまま解放の指示です）。
