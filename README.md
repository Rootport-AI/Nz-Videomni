...
> **メインメモリを約416MB（207.9MB × 2枠）、生成中ずっと使います。** 重みを CPU から GPU へ速く渡すために、
> OS が動かせない形で確保した受け渡し用の枠（ページ固定メモリ）を2つ持ち続けるためです。この枠はサーバーを
> 起動しているあいだ使い回されます（枠の大きさはモデルの最も大きなブロックに合わせて決まるので、LTX 2.3 では
> 253.8MB × 2枠になります）。メインメモリが逼迫しているときは off にすると解放されます。
