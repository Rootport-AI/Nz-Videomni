...
| 項目 | 値 |
|------|----|
| 版 | **v0.5.62**（**正本は下の「改訂履歴」の最終行である。本欄はその写しなので、履歴へ1行足したら必ずここも合わせること**——過去に2度、履歴だけ進んで本欄が取り残された） |
| 日付 | **2026-09-18**（v0.5 本体は 2026-07-02。以後の更新は下の改訂履歴を参照） |
...
#### (d) `GenerateRequest` の各フィールドの扱い（4 分類・全 30 件。**2026-09-15 に 422 系 2・無視 2・動作 26・従属 0 へ改訂**〔同じ 2026-09-15 のうちに 2 度動いた。まず `inpaint` の新設で 422 系が 1 つ増えて 422 系 3・動作 25 の全 30 件になり、同日の台帳 §3-150 で `inpaint` が「動作」へ移って現在の姿になった。総数 30 件は変わらない。それ以前は 2026-09-03 の 422 系 2・無視 2・動作 25・従属 0 で全 29 件〕）
