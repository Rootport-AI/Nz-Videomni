...
| `nag_enabled` | bool | `false` | — | **2026-07-28追加**。NAG（Normalized Attention Guidance＝CFGを使わずcross-attention出力レベルでネガティブプロンプトを効かせる非CFG手法）の有効化。**エンジン系統 `ltx25`（LTX 2.5）でも 2026-08-30 から使える**（§6.10(d)）——**計算式は LTX 2.3 のものを共有しており、置換する `Attention.forward` だけが 2.5 用に書き下ろされている。** **`nag_alpha=0` にしても、出力が「NAGなし」とビット単位で一致することは保証していない**（負のプロンプトを1件足すぶんテキストエンコーダのバッチが1行から2行になり、行列積のカーネル選択が変わりうるため。形も乱数消費も行間の相互作用も変わらない）——**無効化したいときは本フィールドを `false` にすること。** 詳細は `Docs/VERIFICATION_LOG.md` §38（LTX 2.3）・**§80（LTX 2.5 開通の全ゲート。オーナーの目視ゲート G8 は未了）** |
