...
- **実測と設計判断の正本**: `Docs/VERIFICATION_LOG.md` §43（LTX 2.3）・**§77（LTX 2.5 開通の全ゲート。オーナーの目視ゲートは未了＝目視待ち）**。利用者向け説明は `README.md`「生成の高速化（Acceleration）」節。
...
設計正本は `Docs/MULTI_ENGINE_DESIGN.md` §5.6、実測記録は `Docs/VERIFICATION_LOG.md` §69（v1）・§72（連結生成）・§73（V2V・A2V）・§74（LoRA・参照動画）・**§75（高速化第 1 弾＝fused カーネルと先読み block swap）**・**§76（高速化第 2 弾＝`keep_resident`）**・**§77（高速化第 3 弾＝SageAttention。**オーナーの目視ゲートは未了＝目視待ち**）、起票とクローズ記録は `Docs/PENDING_TASKS_CLOSED.md` §3-98。**連結生成・V2V 継続・A2V〔Single・長尺・バッチ〕は 2026-08-23 に対応済みで、オーナーの実機確認にも合格している**（連結生成は 2026-08-23、V2V 継続と A2V は 2026-08-24。**長尺 A2V はオーナー裁定で合格扱い**——裁定の全文は `Docs/VERIFICATION_LOG.md` §73.10(2)）。**スタイル LoRA と IC-LoRA は 2026-08-24 に対応し、機械ゲート・実機ゲートの全項目合格に加えて、2026-08-25 にオーナーの実機確認にも合格した**（同 §74.12）。**したがって LTX 2.5 で現在使える範囲のうち、SageAttention を除くすべてがオーナー確認まで済んでいる**——**高速化第 3 弾（SageAttention）だけは、機械ゲートと実機ゲートに全項目合格したものの、オーナーの目視ゲートが未了である＝目視待ち**（`Docs/VERIFICATION_LOG.md` §77.6）。 **生きている後続課題の一覧は台帳 `Docs/PENDING_TASKS.md` §3 を見ること**（件数と項目番号は本書に書かない。同じ件数が文書ごとに食い違う事故を繰り返したため、列挙は台帳の1箇所に集約してある）。
