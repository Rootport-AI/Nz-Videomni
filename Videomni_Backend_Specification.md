...
| v0.5.61 | 2026-09-15 | **Inpainting（マスクによる部分再生成）が、エンジン系統 `ltx25`（LTX 2.5）でも使えるようになったことを反映（拒否が減る方向の変更で、凍結 API 契約〔§6〕のフィールド・型・既定・応答形・エラーコードのいずれにも変更は無い。これまで通っていた要求が新たに落ちることも無い）**。**要求の形は両系統で同一である**——`InpaintSpec` の 4 フィールドも、エンドポイントの 6 検査も、キャンバスが 128 の倍数であるという制約も、余白をサーバーが素材の実寸から導くことも、緑のキャンバスを作るアプリ層の処理（`services/video_io.py` の `fill_mask_green_mp4`）も共有のままである。新しく書いたのは LTX 2.5 側の駆動部（`engine25/inpaint25.py`）だけで、**使う制御アダプタは LTX 2.3 が配布する In-Outpainting IC-LoRA そのもの**である（公式の手引きと公式のワークフローも同じ組み合わせである）。**§6.2**（`inpaint` の行から「LTX 2.5 は本機能を持たない」を削除）／**§6.3**（`InpaintSpec` の「エンジン系統による違い」を、両系統で効く現行の姿へ書き直し）／**§6.6**（`inpaint` ブロックが LTX 2.5 では厚くなること＝画角拡張と同じ音声の記録と `restore_peak_vram_mb` が加わることを追記）／**§6.10**（(a) に日付つきの段落・(b) の `unsupported_features` を **3 件 → 2 件**へ・(c) は変更なし・(d) を 422 系 2・無視 2・動作 26・従属 0 の全 30 件へ）を更新した。**画面では Edit タブの「Inpainting」サブタブの灰色が解け、LTX 2.5 で灰色になるサブタブは 1 つも無くなった。** 設計の正本は `Docs/INPAINTING_DESIGN.md` §6.3、機械ゲート・G10 の回帰指紋・実 GPU ゲートの記録は `Docs/VERIFICATION_LOG.md` **§107**、台帳は `Docs/PENDING_TASKS.md` §3-150 である。 |
...
**【2026-09-15・同日の続き】そして同じ日のうちに、Inpainting は LTX 2.5 でも動くようになった。** 上の段落が「対応は予定しているが時期は決めていない」と書いた工事が、その日のうちに済んだためである（台帳 `Docs/PENDING_TASKS.md` §3-150）。**`GET /models` の `unsupported_features` は 3 件 → 2 件へ戻り**（`two_stage_hq` / `prune_vaed`）、**この配列にモード名は 1 つも残っていない。** **新しく書いたのは LTX 2.5 側の駆動部（`engine25/inpaint25.py`）だけである**——仕様層は LTX 2.3 から一字も変えておらず、`InpaintSpec` の 4 フィールドも 6 検査も、緑のキャンバスを作るアプリ層の処理（`services/video_io.py` の `fill_mask_green_mp4`）も共有のままである。**制御アダプタは LTX 2.3 が配布する In-Outpainting IC-LoRA そのもの**で、公式の手引き（docs.ltx.io）と公式の ComfyUI 用ワークフローも同じ組み合わせである。**画面では Edit タブの「Inpainting」サブタブの灰色が解け、LTX 2.5 で灰色になるサブタブは 1 つも無くなった。** **使うときの注意が 1 つある**: **`metadata.json` の `inpaint` ブロックは LTX 2.5 のほうが厚い**——画角拡張と同じ音声の記録と、外側の復元の VRAM のピーク `restore_peak_vram_mb` が加わる（§6.6）。**実測（実 GPU ゲート・G10 の回帰指紋・機械検証）の正本は `Docs/VERIFICATION_LOG.md` §107 である**（本節には書き写さない）。**オーナーの目視は未実施である。**
