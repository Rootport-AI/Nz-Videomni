...
| v0.5.61 | 2026-09-15 | **Inpainting（マスクによる部分再生成）が、エンジン系統 `ltx25`（LTX 2.5）でも使えるようになったことを反映（拒否が減る方向の変更で、凍結 API 契約〔§6〕のフィールド・型・既定・応答形・エラーコードのいずれにも変更は無い。これまで通っていた要求が新たに落ちることも無い）**。**要求の形は両系統で同一である**——`InpaintSpec` の 4 フィールドも、エンドポイントの 6 検査も、キャンバスが 128 の倍数であるという制約も、余白をサーバーが素材の実寸から導くことも、緑のキャンバスを作るアプリ層の処理（`services/video_io.py` の `fill_mask_green_mp4`）も共有のままである。新しく書いたのは LTX 2.5 側の駆動部（`engine25/inpaint25.py`）だけで、**使う制御アダプタは LTX 2.3 が配布する In-Outpainting IC-LoRA そのもの**である（公式の手引きと公式のワークフローも同じ組み合わせである）。**§6.2**（`inpaint` の行から「LTX 2.5 は本機能を持たない」を削除）／**§6.3**（`InpaintSpec` の「エンジン系統による違い」を、両系統で効く現行の姿へ書き直し）／**§6.6**（`inpaint` ブロックが LTX 2.5 では厚くなること＝画角拡張と同じ音声の記録と `restore_peak_vram_mb` が加わることを追記）／**§6.10**（(a) に日付つきの段落・(b) の `unsupported_features` を **3 件 → 2 件**へ・(c) は変更なし・(d) を 422 系 2・無視 2・動作 26・従属 0 の全 30 件へ）を更新した。**画面では Edit タブの「Inpainting」サブタブの灰色が解け、LTX 2.5 で灰色になるサブタブは 1 つも無くなった。** 設計の正本は `Docs/INPAINTING_DESIGN.md` §6.3、機械ゲート・G10 の回帰指紋・実 GPU ゲートの記録は `Docs/VERIFICATION_LOG.md` **§107**、台帳は `Docs/PENDING_TASKS_CLOSED.md` §3-150 である。 |
| v0.5.62 | 2026-09-18 | **§12.2（Gradio 検証UI）の Acceleration 節の記述を実装へ追いつかせた（文書のみ。凍結 API 契約〔§6〕への変更は無い——`keep_resident_embeddings` は v0.5.52 で既に契約側に存在しており、今回動いたのはこの検証用UIの記述と実装だけである）**。**「5項目すべて実装済み」を6項目へ改め**、埋め込み処理器の常駐（`keep_resident_embeddings`。既定 off。LTX 2.5 専用）をモデル骨格の常駐の直下・VAE の直前に追加した（`gradio_ui/ui.py` / `gradio_ui/feature_scope.py` / `gradio_ui/handlers.py`、コミット `3e9bab8`）。あわせて、**VAE（LTX 2.5 で非表示）と埋め込み処理器の常駐（LTX 2.3 で非表示）は、読み込み中のベースモデルが対応しないと行ごと非表示になる**こと（`GET /models` の `base_models[].unsupported_features` に基づく判定。§6.10(b)）を1文で補った——**VAE 側の非表示は前日 2026-09-17（コミット `d0dc9ba`）の出荷分だが本節の記述がそれまで追いついておらず、本版でまとめて反映した**。台帳の §4-6 の⑤・⑥は、どちらも 2026-09-18 のオーナー実機ゲート合格を受けて `Docs/PENDING_TASKS_CLOSED.md` §3-161・§3-162 としてクローズした（検証の記録は `Docs/VERIFICATION_LOG.md` §114）。 |
| v0.5.63 | 2026-09-24 | **生成条件の mp4 埋め込みと読み出し（台帳 `Docs/PENDING_TASKS_CLOSED.md` §3-164）を反映（加算のみ。既存フィールドの意味・型・応答形は変わらず、これまで通っていた要求が新たに落ちることも無い）**。完成した `output.mp4`／`joined.mp4` のコンテナの `comment` タグへ `metadata.json` と同じ JSON を書き込み、読み出し口 `POST /utils/mp4-info` を新設した。**§6.1**（エンドポイント表へ1行・読み出し口の補足・認証の要る経路の記述を件数から経路名の列挙へ改め、記載漏れだった物体追尾の経路も加えた）／**§6.2**（`embed_mp4_metadata` を加算）／**§6.6**（名前が `path` で終わるキーの値は保存領域からの相対で記録・`comment` タグとの関係・`file_size_bytes` は埋め込み前の値）／**§6.8**（`LOCAL_ONLY`／`MEDIA_NOT_FOUND`／`MEDIA_UNREADABLE` を追加・ファクトリ件数を更新（49→52））／**§6.10 (d)(f)**（`embed_mp4_metadata` を「動作」へ。アプリ層で動作し、アダプタは読まない）／**§8.1**（`output.mp4`／`joined.mp4` のタグ・結合は元動画のタグを引き継がない）／**§12.2**（Gradio の「MP4 Info」タブと Settings の「出力」チェックボックス）／**§12b.2・付録B.1**（ツール本数の記述を正本参照へ）。実測とゲートは `Docs/VERIFICATION_LOG.md` §115。 |
...
| `./.venv-utils`（**任意導入**・2026-09-11 追加） | ユーティリティAI（物体追尾）の worker（`tracking/worker.py`）。**エンジンではない**ので、ベースモデルの切り替えにも「worker は同時に1つ」の数にも関与しない | **CPU 版の torch**（cu128 ビルドと同居できないため別 venv。GPU は使わない） |
...
- **LTX 2.5（`ltx25`）では、fp8 safetensors を選んでもペイロードの形は変わらない（2026-09-26〜・§3-167 B-2）。** transformer は GGUF でも fp8 safetensors でも同じキー `transformer_path` 1 本で渡り、ワーカー側の `engine25/pipeline25.py` が拡張子で読み方を振り分ける。新しいキーは足していない（設計は `Docs/VERIFICATION_LOG.md` §118.2）。
...
`_real_available()`（`services/engines/ltx/adapter.py:807`）は **torch / `ltx_*` を一切 import せずに**、real GGUF + component-file 経路が実際に開くファイル群の存在だけを確認する（失敗は握り潰して `False` → auto は mock へフォールバック、かつ torch 無しの `.venv` で `import services.ltx_runner` が安全に保たれる）。
...
### 4.7 ユーティリティAIモジュール（物体追尾・2026-09-11 新設）

**動画を生成しないAIを、本バックエンドが初めて抱えた。** 物体追尾は、AviUtl2 のタイムラインに置いた**部分フィルタ**（画面の一部だけに効果を掛けるフィルタオブジェクト）の枠を追跡AIに追わせ、結果を**中間点**としてそのオブジェクトへ書き戻す機能である。**設計・契約・設定の正本は [`Docs/OBJECT_TRACKING_DESIGN.md`](Docs/OBJECT_TRACKING_DESIGN.md) であり、本節はアーキテクチャ上の位置づけだけを述べる**（同書の §4 が API 契約、§5 がプラグイン側、§6 がバックエンド側、§7 が重みと導入の正本である）。

**(a) プロセス構成。** 追尾は `./.venv-utils` 上の**第4のプロセス**（`python -m tracking.worker`）で動く。§4.1 のエンジン worker とは次の点で性格が違う。

- **ベースモデルの切り替えに巻き込まれない。** エンジン worker は「同時に1つ」という規律を持つが、ユーティリティワーカーはその数に入らず、生成用 worker の起動・終了とは無関係に生き続ける。
- **起動は遅延で、終了処理は足さない。** アプリの起動時には起こさず、最初のセッション要求が来たときに起動する。終了は標準入力が閉じたことで自分から行う（既存の worker と同じ作法）。
- **GPU を使わない。** 装置は常に CPU であり、生成ジョブと計算資源を取り合わない。
- **アプリ側のログ名は `ltx.tracking`、ワーカーの標準エラー出力は `logs/utils_worker.log` へ落とす。**

**(b) プロトコル。** worker との往復は `@@TRK@@` を目印とする 1 行 JSON で、JSON が `"bytes": N` を持つときはその直後に生のバイト列が N バイト続く。§4.2 の `@@LTX@@` と**同じ発想の別系統**であり、合図の集合も往復の対も §4.2 とは共有しない（合図の一覧は設計書 §6.2）。**送受信の対は鍵で直列化する**——API の枠組みが同時要求を別スレッドで走らせるため、鍵が無いと「長さを前に置く」形の枠付けが壊れて復帰できなくなる（既存の生成用アダプタが同じ理由で同じ作法を採っている）。

**(c) API。** エンドポイントは `/api/v1/utils/track/` の名前空間に 3 本（セッションを開く / フレームを 1 枚送る / セッションを閉じる）。**生成の名前空間とは分けてある**——追尾はジョブを作らず、キューの枠を取らず、`outputs/` にも `uploads/` にも何も書かない。`GET /status` には既存のどのブロックにも畳まない `tracking` ブロックを 1 つ足す（形は §6.5・§6.5b の流儀に倣うが、**凍結キー集合ではない**）。**したがって §6 の凍結 API 契約には一切の変更が無い**——加算されたのは別名前空間のエンドポイントと `/status` の非凍結ブロックだけで、`GenerateRequest` にも `GenerateChainRequest` にも `JobResponse` にもフィールドは増えていない。

**(d) エラー。** 専用のコードを 5 つ持つ（`TRACK_BUSY` 409 / `TRACK_UNAVAILABLE` 503 / `TRACK_SESSION_NOT_FOUND` 404 / `TRACK_FRAME_INVALID` 400 / `TRACK_FAILED` 503）。**封筒の形は §6.8 の既存エラーと同じ**で、生成側の `FEATURE_UNSUPPORTED` は流用しない（追尾は「このエンジンでは使えない機能」ではなく、別系統の機能だからである）。本文の形そのものが誤っている場合は、この 5 つに入らず通常の 422 で返る。

**(e) 設定。** `config` に区画 `tracking:` を 1 つ足す（項目は 4 つ。正本は設計書 §6.4）。**`config.yaml.example` にはこの区画を書かない**——追尾は任意導入であり、導入した環境では既定値がそのまま正しいため、運用者が書き込むものが無いからである。1 枚あたりのバイト数の上限と放置セッションの期限は設定ではなく**ソースコード上の定数**である。

**(f) 保存物。** **無い。** 成果物は AviUtl2 のタイムライン上のエイリアス 1 本の書き戻しだけで、一時ファイルも作らない。したがって [`Docs/STORAGE_POLICY.md`](Docs/STORAGE_POLICY.md) の適用対象外であり、§8「出力ファイル」にも追加は無い。

**(g) 導入。** 任意である。`setup.bat` は `.venv-utils` も重みも作らず、`install-UETrack.bat` を実行した環境にだけ現れる。記述子 `scripts/manifests/30-uetrack.json` は**エンジン系統を宣言しない**——バックエンドはこれをベースモデルとして読まない（§4.3・§5.1 の系統判定には関与しない）。未導入の環境では `/status` の `tracking` が「導入されていない」と答え、操作パネルが案内を出す。
...
| POST | `/api/v1/utils/mp4-info` | サーバーを動かしている PC 上の動画ファイルのパスを受け取り、コンテナの `comment` タグを返す（**2026-09-24 加算**。§6.6 の埋め込みの読み出し口。ffprobe で読むだけで、ジョブは作らない） | 要 | 200 / 403(LOCAL_ONLY) / 404(MEDIA_NOT_FOUND) / 422(MEDIA_UNREADABLE, VALIDATION_ERROR) |
...
- **`POST /utils/mp4-info`（`api/media_info.py::mp4_info`、2026-09-24 加算・タグ `utils`）** は本文 `{"path": "<絶対パス>"}` を受け取り `{"comment": str | null}` を返す（タグが無ければ `null`）。**ループバック（`127.0.0.1`／`::1`）からの要求だけに答え、それ以外は `403 LOCAL_ONLY`**——本文でサーバー側のパスを指定させる入口なので、`--listen`（§3.2）で LAN に開いていても外からファイルを探らせない。**UNC パス（`\\server\share`・`//server/share`・`\\?\UNC\…`・区切りの混在など、`pathlib` がドライブを `\\` 始まりと解釈する形すべて）はファイルに触れる前に `422 MEDIA_UNREADABLE`**（副作用として長いパス用の `\\?\C:\…` とデバイスパス `\\.\…` も断られる）。ネットワークドライブを割り当てたドライブ文字は文字列から区別できないので断らない（既知の制約）。存在しない・通常のファイルでない → `404 MEDIA_NOT_FOUND`、ffprobe が読めない → `422 MEDIA_UNREADABLE`。**ローカル運用向け**であり、クライアントの接続先が別 PC なら手元のパスは読めない。
...
| `inpaint` | InpaintSpec \| null | `null` | — | **2026-09-15追加（Inpainting）**。マスク動画の白い領域の内側だけを描き替える指定。`null` なら従来リクエストとバイト単位で同一。`reference_video_id`（描き替える対象の動画）が必須で、`outpaint`・`conditioning_images`・`crop_output` とは排他（下記凍結制約 13）。**`width`/`height` は「キャンバス」**——素材の実寸を 128 の倍数へ切り上げた値で、余白（右と下）はサーバーが素材の実寸から導く。配信される解像度は素材の実寸、音声は窓で切り出した元の音声である。詳細は §6.3 の InpaintSpec。**LTX 2.3・LTX 2.5 のどちらのエンジン系統でも使える**（2026-09-15。それ以前は `ltx25` が 422 `FEATURE_UNSUPPORTED` で断っていた。§6.10(a)） |
| `embed_mp4_metadata` | bool | `true` | — | **2026-09-24追加（台帳 `Docs/PENDING_TASKS_CLOSED.md` §3-164）**。完成した `output.mp4`（と、このジョブを結合した `joined.mp4`）のコンテナの `comment` タグへ、`metadata.json` と同じ JSON 文字列を書き込むか（書き込む条件と規則は §6.6 冒頭の注記が正本）。`false` なら何も書かず、結合でも元動画のタグを引き継がない。**`GenerateChainRequest` にも同じ名前・型・既定で存在し、`to_clip_request` で各クリップの記録へ転記される**。既定が `true` なので、クライアントは `false` のときだけ送る。**ワーカーには届かない**アプリ層の後処理で、両系統で効く（§6.10 (d)(f)） |
...
> **チェーンの `end_source`（素材（末尾））の補足（2026-08-16追加、2026-08-17に窓内モードへ書き換え、2026-08-18に逆順Chained・錨への素材音声の凍結を追加、2026-09-07にブリッジモードを追加）**: `GenerateChainRequest` の `end_source`（`EndSourceSpec`。`video_id` / `image_id` のどちらか一方と `context_frames` / `strength`）は、**クリップの本数と素材（冒頭）（`source_video`）の有無で挙動が4つに分かれる**。判定は `chain_math.compute_chain_layout` が1箇所で行い、`metadata.json`（§6.6）の `end_source.mode` として公開する（リクエスト側にモードを指定するフィールドは無い）。通常のAPIリクエストで到達できる経路は**窓内モード（クリップ1本）**・**逆順Chained（クリップ2本以上で素材（冒頭）なし）**・**ブリッジモード（クリップ2本以上で素材（冒頭）あり）**の3つで、旧方式（`"internal_segment"`）は到達不能な死蔵コードである。**素材（末尾）だけを使うときの推奨はクリップ1本の窓内モードのみ**（2026-08-18のオーナー裁定）。逆順Chainedは受理されるが推奨外で、クリップの境目・末尾に品質劣化が出ることがあり仕様として許容する（詳細は下記「オーナー裁定」の項）。**素材（冒頭）を併用するブリッジモードはこの裁定の対象外である**（用途の違う別のモードで、詳細は下記の項）。
>
...
> - **クリップ2本以上＝`"reverse"`（逆順Chained。受理されるが推奨外、2026-08-18追加）**: stage-1 のみを依存順（タイムライン末尾のクリップから先頭へ向けて）に生成する。`chain_math` が発行する3本の表（`seg_generation_order`＝生成順・`seg_head_source`＝頭を凍結する元・`seg_tail_source`＝尾を凍結する元）でループを駆動し、各セグメントは自分より未来側のセグメントの頭を自分の尾のりしろとして凍結する——正順チェーンの頭凍結を鏡写しにした形で、既存の `freeze_mask_values`（`tail_mask_value=None` のとき4値すべてを鏡写しソフト凍結にする）をそのまま流用するため**新規の凍結機構は無い**。生成し終えた各セグメントの潜在は連結してから、既存の一括 stage-2 へそのまま渡す（stage-2 は無改修）。**配信尺はクリップ合計そのもの**で、恒等式は窓内モードと同じ `total_px == clips_total_px`（帯は最終クリップの内側）。**意味拡張（既存フィールド）**: `overlap_frames` が逆順の継ぎ目の幅そのものになり、このモードだけ `overlap_frames >= 2` の下限を免除する（内部区画を追加しないため音声のりしろ予算の消費が素のチェーンと同一になり、下限の根拠が消滅するため。既定値は1潜在）。`overlap_strength` は逆順の継ぎ目にも正順と同じつまみとして効く。**受理範囲の後方非互換変更（新たに422になる構成）**: 最終クリップの潜在数が `kv + n_end_v` 以下になる構成を拒否する（逆向きに運ぶべき新規生成内容がゼロになるため。旧方式では通っていた構成が本モードでは422になる）。**かつてここに①として並べていた「`source_video`（素材（冒頭））との2クリップ以上での併用を拒否」は、2026-09-07に下記のブリッジモードとして受理へ転じたので、いまは無い。**`metadata.json` へ `end_source.generation_order`（幾何が宣言する生成順）・`end_source.stage1_order`（Stage-1ループの実測順）・`end_source.stage1_freezes`（各セグメントの実行時凍結記録）が新出——`stage1_order == generation_order` の一致が、エンジンが実際に逆順で走ったことの確認になる。
> - **クリップ2本以上で素材（冒頭）あり＝`"bridge"`（ブリッジモード。2026-09-07追加）**: 全クリップを**正順**（タイムラインの先頭から末尾へ）に生成し、**最終クリップだけを両側で条件付けする**。頭は1つ前のクリップからののりしろ、尾は素材（末尾）の末尾の凍結フレームである。`chain_math` が発行する3本の表は**素材（末尾）を持たない普通の正順チェーンとまったく同じ値**で（`seg_generation_order` は先頭から順、`seg_head_source` は1つ前のクリップ、`seg_tail_source` は全クリップとも `null`）、表の作り方には新しい腕を足していない。**恒等式は窓内モード・逆順Chainedと同じ `total_px == clips_total_px`**（末尾の凍結フレームは最終クリップの内側にあるので加算されない）で、配信尺はそこから素材（冒頭）が凍結した頭のぶんを差し引いた長さになる（`source_video` を併用したときの通常の規則と同じ）。**意味拡張（既存フィールド）**: `overlap_frames` の下限 `>= 2` は逆順Chainedと同じ理由で免除される（区画を継ぎ足さないため音声のりしろ予算の消費が素のチェーンと同一になり、下限の根拠が消滅するため）。**受理範囲の変更（加算方向）**: 2026-08-18から422で拒否していた「`source_video` × `end_source` × 2クリップ以上」が**受理へ転じた**（これまで通っていた構成が新たに落ちることは無い）。最終クリップの潜在数が `kv + n_end_v` 以下になる構成を422で拒否する検査は、逆順Chainedと同じく本モードにも掛かる。**用途は「内容の似ている2本の動画の間の欠落区間を補完する」ことである。** 2本の内容が離れていると最終クリップの中でクロスフェードや不自然なモーフが起きるが、**これは仕様として許容する**（2026-09-07のオーナー裁定。フロントエンドもこの場合に品質の警告を出さない）。`metadata.json` の `end_source.mode` は `"bridge"`、`end_source.generation_order` と `end_source.stage1_order` はどちらも正順の並びになる。**本モードは実験的な機能という位置づけである**（2026-09-07のオーナー裁定。最終クリップを長くすると早期収束による静止が出ることがあり、これも仕様として受け入れている。考察は`Docs/CHAIN_STAGE2_RESEARCH_NOTES.md` §11の【2026-09-07】の項）。実装・機械検証・実機ゲートの正本は[`Docs/VERIFICATION_LOG.md`](Docs/VERIFICATION_LOG.md) §102、設計の理由は`Docs/CHAIN_STAGE2_RESEARCH_NOTES.md` §11の【2026-09-07】の項である。
...
> **エンジン系統による違い（2026-09-15）**: 上の表の内容は**エンジン系統に依存しない**。4 つのフィールドの意味・既定・制約も、エンドポイントの 6 検査も、キャンバスが 128 の倍数であるという制約も、余白をサーバーが素材の実寸から導くことも、緑のキャンバスを作るアプリ層の処理も、両系統で同一である。**同じ 2026-09-15 のうちに 2 度動いた欄である**——午前は `ltx25` が 422 `FEATURE_UNSUPPORTED` で断っており（本機能は LTX 2.3 専用として出荷された）、同日の `Docs/PENDING_TASKS_CLOSED.md` §3-150 で LTX 2.5 側の駆動部（`engine25/inpaint25.py`）が入って**その拒否は無くなった**（§6.10(a)）。**ただし `metadata.json` の `inpaint` ブロックは LTX 2.5 のほうが厚い**——画角拡張と同じ音声の記録と、外側の復元の VRAM のピークが加わる（§6.6）。実測は `Docs/VERIFICATION_LOG.md` §107。
...
`services/pipeline_manager.py::_write_metadata` が書き出す（`output.save_metadata_json` が true のとき）。

> **パスの記録と mp4 への埋め込み（2026-09-24、台帳 `Docs/PENDING_TASKS_CLOSED.md` §3-164）**
> - **名前が `path` で終わるキーの文字列値は、保存領域からの相対で記録する**（入れ子の辞書・リストの中も含む。`services/recipe_paths.py::relativize_recipe_paths`）。規則は上から順に、①相対パスはそのまま（`output.path` がこれ）、②`config.output_dir` の配下なら `outputs/<配下の相対>`、③`config.upload_dir` の配下なら `uploads/<配下の相対>`（②③の両方の配下にあるときは、より深い方の区画を採る）、④それ以外は区切りの中で最後に現れる `uploads`／`outputs` から後ろ、それも無ければファイル名だけ。ドライブ文字の無い `\` 始まりのパスも絶対パスと同じに扱う。区切りは `/`。**絶対パスは書かない**——フォルダの改名・移設で壊れ、共有した mp4 から PC のユーザー名が読めてしまうためである。既存の `metadata.json` は書き換えない。
> - **埋め込みは `metadata.json` を書き出すとき（`output.save_metadata_json` が true）にだけ行う**（`_embed_recipe` はその分岐の内側で呼ばれる）。そのうえで要求の `embed_mp4_metadata` が真（既定）なら、この JSON と同じ文字列（`services/video_io.py::recipe_text`）を完成した `output.mp4` のコンテナの `comment` タグへ書き込む（ffmetadata ファイル経由の再多重化。`-c copy`・`+faststart`。映像・音声は再エンコードしない）。中間ファイルには書かない。**埋め込みに失敗しても警告ログを残すだけで、元の mp4 とジョブの完了は保たれる。** mp4 を再エンコードするとタグは消える。読み出しは `POST /utils/mp4-info`（§6.1）。
> - **結合（`POST /jobs/{id}/join`）でも、元ジョブの記録を `joined.mp4` に書き込む**（結合後のファイルの説明ではない。読み込んだ記録に上の相対化を通してから書く）。**書き込むかどうかは元ジョブの `request.embed_mp4_metadata` に従い、この項目が無い以前のジョブは真として扱う。** 書き込まないときも、元の動画のタグは引き継がない（§8.1）。
> - **`output.file_size_bytes` は埋め込み前の大きさである**（埋め込む JSON 自身がこの値を含むため、埋め込み後の大きさとは一致させられない）。
...
| `LOCAL_ONLY` | 403 | `POST /utils/mp4-info` への要求がループバック以外から来た（**2026-09-24 新設**。`--listen` 時も断る） |
| `MEDIA_NOT_FOUND` | 404 | `POST /utils/mp4-info` のパスが存在しない、または通常のファイルでない（**2026-09-24 新設**） |
| `MEDIA_UNREADABLE` | 422 | `POST /utils/mp4-info` のファイルを ffprobe が読めない、またはパスが UNC 形式（長いパス用の `\\?\…` とデバイスパス `\\.\…` を含む。ファイルに触れる前に拒否）（**2026-09-24 新設**） |
...
**【2026-09-15・同日の続き】そして同じ日のうちに、Inpainting は LTX 2.5 でも動くようになった。** 上の段落が「対応は予定しているが時期は決めていない」と書いた工事が、その日のうちに済んだためである（台帳 `Docs/PENDING_TASKS_CLOSED.md` §3-150）。**`GET /models` の `unsupported_features` は 3 件 → 2 件へ戻り**（`two_stage_hq` / `prune_vaed`）、**この配列にモード名は 1 つも残っていない。** **新しく書いたのは LTX 2.5 側の駆動部（`engine25/inpaint25.py`）だけである**——仕様層は LTX 2.3 から一字も変えておらず、`InpaintSpec` の 4 フィールドも 6 検査も、緑のキャンバスを作るアプリ層の処理（`services/video_io.py` の `fill_mask_green_mp4`）も共有のままである。**制御アダプタは LTX 2.3 が配布する In-Outpainting IC-LoRA そのもの**で、公式の手引き（docs.ltx.io）と公式の ComfyUI 用ワークフローも同じ組み合わせである。**画面では Edit タブの「Inpainting」サブタブの灰色が解け、LTX 2.5 で灰色になるサブタブは 1 つも無くなった。** **使うときの注意が 1 つある**: **`metadata.json` の `inpaint` ブロックは LTX 2.5 のほうが厚い**——画角拡張と同じ音声の記録と、外側の復元の VRAM のピーク `restore_peak_vram_mb` が加わる（§6.6）。**実測（実 GPU ゲート・G10 の回帰指紋・機械検証）の正本は `Docs/VERIFICATION_LOG.md` §107 である**（本節には書き写さない）。**オーナーの目視も同日に合格した**（台帳 `Docs/PENDING_TASKS_CLOSED.md` §3-150）。
...
- **2026-09-15 の Inpainting で、この配列は 2 件 → 3 件になり、同じ日のうちに 3 件 → 2 件へ戻った。** **名前が増えるのは 2 回目、モードが増えるのは初めてだった**（1 回目は 2026-09-03 の `keep_resident_embeddings` で、そちらは LTX 2.3 の側の配列だった）。**そして `inpaint` は、この配列の歴史で唯一「入ってから出ていった」名前である**——午前の §3-55 で本機能が LTX 2.3 専用として出荷されて入り、同日の §3-150 で LTX 2.5 側の駆動部ができて出ていった。**画面では Edit タブの「Inpainting」サブタブが一度灰色になり、同じ日のうちに灰色が解けた**——2026-08-26 に作ったサブタブ単位のグレーアウトの仕組みは、この数時間だけ実際に使われ、いままた一度も使われない状態で生きている。**現在値は 2 件で、モード名は 1 つも無い**（上の (a) の【2026-09-15・同日の続き】）。
...
#### (d) `GenerateRequest` の各フィールドの扱い（4 分類・全 31 件。**2026-09-24 に `embed_mp4_metadata` の新設で動作が 1 つ増え、422 系 2・無視 2・動作 27・従属 0 になった**。それ以前は **2026-09-15 に 422 系 2・無視 2・動作 26・従属 0 へ改訂**〔同じ 2026-09-15 のうちに 2 度動いた。まず `inpaint` の新設で 422 系が 1 つ増えて 422 系 3・動作 25 の全 30 件になり、同日の台帳 §3-150 で `inpaint` が「動作」へ移って現在の姿になった。総数 30 件は変わらない。それ以前は 2026-09-03 の 422 系 2・無視 2・動作 25・従属 0 で全 29 件〕）
...
#### (f) 連結生成（`POST /generate/chain`）の対応範囲（2026-08-23 新設・同日 V2V／A2V の開通を反映・2026-08-24 スタイル LoRA／IC-LoRA の開通を反映・**2026-08-26 撮り直し／素材（末尾）の開通を反映**・**2026-08-30 非CFGネガティブプロンプトの開通を反映**・**2026-09-03 埋め込み処理器の常駐〔`keep_resident_embeddings`〕の新設を反映**・**2026-09-24 生成条件の mp4 埋め込み〔`embed_mp4_metadata`〕の新設を反映**・**2026-09-25 Stage-2 の窓〔`stage2_window` の `w25`〜`w61`〕の追加を反映**）
...
- **`stage2_window` の `"full_length"` は、素の連結生成の本体には到達しない。** API 層が「クリップちょうど 1 本」かつ「`source_audio` 必須」の 2 条件を 422 で強制するためで、**これは LTX 2.3 でも同じ制約**であり LTX 2.5 固有の制限ではない。**`source_audio` が使えるようになった 2026-08-23 以降、LTX 2.5 でもこの経路は実際に通る**——Single タブの A2V がこれで、フロントエンドは 1 クリップの連結生成に `full_length` を添えて投げている。`source_audio` を伴わない素の連結生成で使えるのは、`full_length` を除く 15 名（`"standard"`〔潜在 22 フレーム・既定〕・`"high_resolution"`〔潜在 19 フレーム〕と、2026-09-25 に加わった `"w25"`〜`"w61"`〔名前の数字が潜在フレーム数〕）である。**窓は全部で 16 名あり、数値の正本は `chain_math.py` の `STAGE2_WINDOW_PRESETS`、実装の記録は `Docs/VERIFICATION_LOG.md` §116 である。** `w61` は潜在 61 フレームで `full_length` と同じ長さだが、送りを持つ多タイル用の窓であり、`full_length` の 2 条件は掛からない。これらの窓も LTX 2.3 と LTX 2.5 で共有である。
...
outputs/
└── {job_id}/
    ├── output.mp4               # H.264 / yuv420p。音声ありジョブは AAC を mux。comment タグに metadata.json と同じ JSON（既定）
    ├── metadata.json            # 生成メタデータ（save_metadata_json: true のとき）
    ├── joined.mp4               # V2V 継続ジョブを POST /jobs/{id}/join で結合したとき。comment タグに元ジョブの記録（既定）
...
- **`output.mp4` と `joined.mp4` は、既定で `metadata.json` と同じ JSON をコンテナの `comment` タグに持つ**（要求の `embed_mp4_metadata` で止められる。中間ファイルには書かない。規則は §6.6 冒頭の注記が正本）。結合は元動画のグローバルタグを引き継がない（`-map_metadata -1`）ので、`joined.mp4` のタグは「元ジョブの記録」か「無し」のどちらかである。
...
タブ構成は **Generate | Clip Chain | Style LoRA | Jobs | MP4 Info | Settings**。上段の共通バーは **server status 表示 + Refresh のみ**（旧 Load Model / Unload ボタンは廃止。モデルのロードは Settings→Models、アンロードは Settings→Danger zone に一本化）。**プロンプト**入力はタブの上に常時表示され、Generate / Clip Chain 双方で共用する。
...
**Settings タブ**: 言語/テーマ・接続情報・ポーリング設定・サーバー config ビューアに加え、**Models** セクション（見出し直下に**ベースモデル（LTX 2.3 ／ LTX 2.5 の系統）を選ぶドロップダウン**、その下にカテゴリ別ドロップダウン＋Load。選択肢と初期値は `GET /models` の `base_models[]` ・ `active_base_model` から作り、系統を変えると4カテゴリの選択肢がその系統のものへ入れ替わる。Load は `base_model` を常に送る。`default` 選択肢は実ファイル名を併記した `default — <ファイル名>` 表示）と **Danger zone**（Unload 等・チェックボックスで解錠）を持つ。あわせて **Acceleration（生成の高速化）** 区画があり、**6項目すべて実装済み**の切替——Fused GGUF Dequantization Kernel（GGUF 逆量子化の1カーネル化）／attention（`sdpa`・`sage`）／Block-swap prefetch（先読み block swap）／モデル骨格の常駐（`keep_resident`）／埋め込み処理器の常駐（`keep_resident_embeddings`。既定 off。LTX 2.5 専用。**2026-09-18 に追加**）／VAE（Default・PrunaVAED。**2026-08-05 に実装**。既定は Default で恒久的に反転しない）——を並べる。**このうち VAE の PrunaVAED は LTX 2.5 で、埋め込み処理器の常駐は LTX 2.3 で、それぞれ読み込み中のベースモデルが対応しないため行ごと非表示になる**（`GET /models` の `base_models[].unsupported_features` に基づく判定。§6.10(b)）。**常時グレーアウトのプレースホルダは 2026-08-05 をもって1件も無くなった**。**値はジョブ単位でリクエストに載る**（サーバーの再起動もパイプラインの再読み込みも要らない。フィールドは §6.2 を参照）。Acceleration の直後には **「Metadata output（メタデータ出力）」** 区画があり、チェックボックス「生成したmp4に生成条件をメタデータとして書き込む」（既定 on。off のときだけ `embed_mp4_metadata: false` を単発・連結・A2V バッチの要求へ載せる）を置く。

**MP4 Info（mp4 情報）タブ（2026-09-24追加）**: Jobs と Settings の間にある。左に動画ファイルの受け口（`gr.File`）、右に読み取り専用のテキスト欄（欄の名前は「Metadata (Generation conditions)／メタデータ（生成条件）」）を置き、受け取ったファイルのパスを `POST /utils/mp4-info` へ渡して `comment` タグの文字列をそのまま表示する。タグが無ければ「生成条件のメタデータが見つかりませんでした」。コピー専用のボタンは無い（欄の文字を選択してコピーする）。
...
テストは `tests/test_mcp_*.py`（7ファイル）が `./.venv` で完結し、GPU・実バックエンドプロセスを必要としない（`httpx.ASGITransport` でモック FastAPI アプリに直結）。機械検証・実機ゲート（承認フロー・ツール一覧の表示・T2V/I2V/A2Vバッチ等の実操作）の実施記録は `Docs/VERIFICATION_LOG.md` §39 が正本（本書執筆時点でのゲート状況もそちらを参照。本書では都度古くなる実施状況を複製しない）。
...
| **MCP（Model Context Protocol）** | AIエージェントが外部ツールを呼び出すための標準規格。本プロジェクトは `mcp_server/` パッケージでツール群を公開する（§12b。本数の正本は `Docs/MCP_SERVER_DESIGN.md` §8）。 |
