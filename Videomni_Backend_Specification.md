...
| 項目 | 値 |
|------|----|
| 版 | **v0.5.64**（**正本は下の「改訂履歴」の最終行である。本欄はその写しなので、履歴へ1行足したら必ずここも合わせること**——過去に2度、履歴だけ進んで本欄が取り残された） |
| 日付 | **2026-09-24**（v0.5 本体は 2026-07-02。以後の更新は下の改訂履歴を参照） |
...
#### (f) 連結生成（`POST /generate/chain`）の対応範囲（2026-08-23 新設・同日 V2V／A2V の開通を反映・2026-08-24 スタイル LoRA／IC-LoRA の開通を反映・**2026-08-26 撮り直し／素材（末尾）の開通を反映**・**2026-08-30 非CFGネガティブプロンプトの開通を反映**・**2026-09-03 埋め込み処理器の常駐〔`keep_resident_embeddings`〕の新設を反映**・**2026-09-24 生成条件の mp4 埋め込み〔`embed_mp4_metadata`〕の新設を反映**）
