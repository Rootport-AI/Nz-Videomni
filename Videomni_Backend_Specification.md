...
| v0.5.62 | 2026-09-18 | **§12.2（Gradio 検証UI）の Acceleration 節の記述を実装へ追いつかせた（文書のみ。凍結 API 契約〔§6〕への変更は無い——`keep_resident_embeddings` は v0.5.52 で既に契約側に存在しており、今回動いたのはこの検証用UIの記述と実装だけである）**。**「5項目すべて実装済み」を6項目へ改め**、埋め込み処理器の常駐（`keep_resident_embeddings`。既定 off。LTX 2.5 専用）をモデル骨格の常駐の直下・VAE の直前に追加した（`gradio_ui/ui.py` / `gradio_ui/feature_scope.py` / `gradio_ui/handlers.py`、コミット `3e9bab8`）。あわせて、**VAE（LTX 2.5 で非表示）と埋め込み処理器の常駐（LTX 2.3 で非表示）は、読み込み中のベースモデルが対応しないと行ごと非表示になる**こと（`GET /models` の `base_models[].unsupported_features` に基づく判定。§6.10(b)）を1文で補った——**VAE 側の非表示は前日 2026-09-17（コミット `d0dc9ba`）の出荷分だが本節の記述がそれまで追いついておらず、本版でまとめて反映した**。台帳は `Docs/PENDING_TASKS.md` §4-6⑤・§4-6⑥（実機ゲート後にクローズ予定）。 |
