...
| v0.5.34 | 2026-08-25 | **LTX 2.5 の高速化第 2 弾（`keep_resident`＝テキストエンコーダの常駐。既定 off）を反映（文書のみ。凍結 API 契約〔§6〕のフィールド・型・既定の応答形への変更は無い）**。`keep_resident` が **422 の側から動作の側へ移った**。**これは第 1 弾とは種類の違う移動で、`unsupported_features` が 8 件 → 7 件になった最初の事例である**（第 1 弾で動いたのは 4 分類の対応表だけだった）。**§6.2**（`keep_resident` の行を「LTX 2.5 では 422」から開通の記述へ。**契約は LTX 2.3 と同じだが実装は別物**である旨と、2.5 が常駐させるのは Gemma 4 テキストエンコーダの state dict ただ 1 つ〔実測 **7.68GiB**〕である旨、および 2.5 側には併用制限も降格経路も無い旨を明記）／**§6.10(a)**（対応範囲へ高速化第 2 弾を追加。効くのは 2 本目以降で、生成時間は実測 **27.6 秒 → 20.5 秒＝約 25% 短縮**）／**§6.10(b)**（`unsupported_features` の例から `"keep_resident"` を削除して**現在値 7 件**へ。**422 側から項目が抜けた初めての事例**である旨の注記を追加）／**§6.10(d)**（`GenerateRequest` 全 28 フィールドの 4 分類を **422 系 5・無視 5・動作 15・従属 3** へ改訂）／**§6.10(f)**（`GenerateChainRequest` 全 34 フィールドの 4 分類を **422 系 6・無視 5・動作 20・従属 3** へ改訂）／**§6.6**（`keep_resident_used` のエコーは LTX 2.5 では `"on"` / `"off"` の 2 値しか出ない旨を追記。契約は 3 値のまま据え置く）／**§6.10 末尾**（実測記録へ §76 を追加）を更新した。実測記録は `Docs/VERIFICATION_LOG.md` §76、設計正本は `Docs/MULTI_ENGINE_DESIGN.md` §5.6・§8.5。 |
...
### 6.10 LTX 2.5（エンジン系統 `ltx25`）の対応範囲（2026-08-22。連結生成の対応範囲＝下記 (f) を 2026-08-23 に加筆、LoRA と参照動画・高速化 2 つを 2026-08-24 に加筆）
...
- **2026-08-25 の高速化第 2 弾では、この配列が初めて動いた（8 件 → 7 件）。** `keep_resident` は**もともと 422 で断っていた機能**なので、開通させると `unsupported_features` から名前が 1 つ消える——**「断っていた機能を開ける」ときと「無視していた項目を効かせる」ときとで、動く表が違う**という区別に注意すること。前者では (d)・(f) の 4 分類の対応表に加えてこの配列も動き、**画面のグレーアウトが 1 つ解ける**。後者（第 1 弾）では 4 分類の対応表しか動かない。
...
| 分類 | 件数 | フィールド | 扱い |
|---|---:|---|---|
| **422 系** | 5 | `pipeline`（`"distilled"` 以外）／`outpaint`／`nag_enabled`／`vae_mode`（`"default"` 以外）／`attention_backend`（`"sdpa"` 以外） | `FEATURE_UNSUPPORTED`。**判定は常に「既定値と違うか」であって「フィールドが有るか」ではない**——クライアントは毎回スキーマ全体を送るため、既定値のまま届いたフィールドは利用者が求めたものではない。**`loras` / `reference_video_id` は 2026-08-24 に、`keep_resident` は 2026-08-25 にこの分類を抜けて「動作」へ移った。`keep_resident` の移動はこの分類から出た最初の 1 件で、これにより `unsupported_features` が 8 件 → 7 件になった**（同日の高速化第 1 弾ではこの分類は 1 件も動いていない） |
