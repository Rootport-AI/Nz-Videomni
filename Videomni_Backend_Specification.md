...
### 6.10 LTX 2.5（エンジン系統 `ltx25`）の対応範囲（2026-08-22。連結生成の対応範囲＝下記 (f) を 2026-08-23 に加筆、LoRA と参照動画・高速化 2 つを 2026-08-24 に加筆、モデル骨格の常駐〔`keep_resident`〕と SageAttention〔`attention_backend`〕を 2026-08-25 に加筆、**撮り直し〔Retake〕と素材（末尾）〔End source〕を 2026-08-26 に、画角拡張〔Outpainting〕を 2026-08-29 に、非CFGネガティブプロンプト〔NAG／VSF〕を 2026-08-30 に加筆**）
...
#### (d) `GenerateRequest` の各フィールドの扱い（4 分類・全 28 件。**2026-08-30 に 422 系 2・無視 2・動作 24・従属 0 へ改訂**）
...
#### (f) 連結生成（`POST /generate/chain`）の対応範囲（2026-08-23 新設・同日 V2V／A2V の開通を反映・2026-08-24 スタイル LoRA／IC-LoRA の開通を反映・**2026-08-26 撮り直し／素材（末尾）の開通を反映**・**2026-08-30 非CFGネガティブプロンプトの開通を反映**）

**結論から言うと、LTX 2.5 では `POST /generate/chain` のモードがすべて走る**——素の連結生成・V2V 継続・A2V に加えて、スタイル LoRA と IC-LoRA（参照動画による制御。1 本の参照動画をクリップごとに配る長尺 IC-LoRA を含む）、そして **2026-08-26 に加わった撮り直し（Retake）と素材（末尾）（End source）**である。**断るのは、このエンジンが持たない機能を名指しするフィールドだけになった**（**2026-08-30 以降は `pipeline` / `vae_mode` の 2 つ**。`nag_enabled` は同日この一覧から抜けた）。**判断は 1 箇所（`services/engines/ltx25/adapter.py`）で行い、API 層は系統名とリクエスト本体を渡して取り次ぐだけ**である（`engines.reject_chain(family, request)`）。宣言していない系統＝ LTX 2.3 は素通りするので、**LTX 2.3 から見た振る舞いは 1 バイトも変わらない**。
