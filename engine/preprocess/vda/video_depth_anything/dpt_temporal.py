...
class DPTHeadTemporal(DPTHead):
    def __init__(self, 
        ...
        assert num_frames > 0
        # VENDORING CHANGE: upstream wraps this in ``easydict.EasyDict`` but only
        # ever splats it as ``**kwargs`` below, so a plain dict is equivalent and
        # drops the easydict dependency (absent from the engine venv).
