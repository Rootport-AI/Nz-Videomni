...
# VENDORING CHANGE: the xFormers import guard is gone (bare ``print`` on the
# worker's STDOUT protocol channel; xFormers is not installed). The attention
# branch it gated is removed below -- ``CrossAttention._attention`` is SDPA now.
...
class TemporalAttention(CrossAttention):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, cached_hidden_states=None):
        ...
        # VENDORING CHANGE: the xFormers branch is gone; this is the exact path
        # upstream already took whenever xFormers was absent (the G0 smoke
        # configuration), with SDPA inside ``CrossAttention._attention``.
