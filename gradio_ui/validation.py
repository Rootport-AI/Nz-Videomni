...
else None. Note: because a single clip is capped at 481 frames and a chain at 24
clips, the 11544 cap is (just) unreachable through the 24-slot UI once the join
overlaps are subtracted; the check is defence-in-depth that mirrors the server
and also surfaces chain_math's degenerate-geometry ValueError (e.g. clips too
short for a continuous audio cross-fade) before any API call.
"""
...
MAX_CHAIN_TOTAL_PIXEL_FRAMES = 24 * 481  # 11544; mirrors api/models.py
