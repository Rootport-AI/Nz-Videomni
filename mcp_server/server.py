...
■ end_source（素材（末尾））はクリップ何件でも使えます
  submit_chain の end_source_video_id / end_source_image_id を指定すると、
  clips が1件のときは窓内モード（素材がそのクリップ自身の末尾として凍結され、
  1つのデノイズ窓の中で素材へ到達するように生成されます）、2件以上のときは
  逆順Chained（最後のクリップから順に生成し、各クリップは1つ後ろのクリップの
  冒頭を自分の末尾として引き継ぎます）になります。出力の長さはどちらの場合も
  クリップの合計であって、素材の分だけ伸びることはありません。
