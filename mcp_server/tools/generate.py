...
専用**で、埋め込み処理器のジョブ間常駐。台帳 §3-114。他の5項目と違い、
    ...
    **LTX 2.5 専用**です（既定off）。**これは向きが逆で、LTX 2.3 を選んで
    ...
    Args:
        ...
        keep_resident_embeddings: **LTX 2.5 専用**。2.5 の埋め込み処理器
            ...
            実測4.66GiB が常駐します（正本はバックエンド
            Docs/VERIFICATION_LOG.md §92）。``keep_resident`` とは別のスイッチなので、
    ...
    V2V / A2V:
      ...
      * ``end_source_video_id`` / ``end_source_image_id``（素材（末尾））は
        ...
        **クリップが1件のときは窓内モードで、生成は1つのデノイズ窓の中で
        その素材へ到達するように行われます（推奨）**。**2件以上で
        ``source_video_id`` を併用しないときは
        ...
        **出力の長さはどちらの場合もクリップの合計**であって、素材の分だけ
        ...
        とは排他です。``source_video_id`` との併用はクリップ何件でも受理され
        ます。**1件のとき**は頭と尾の両方を固定して補間する構図（窓内モード）。
        **2件以上のとき**はブリッジモードで、クリップは通常どおり正順に生成
        され、最後のクリップだけが両側で条件付けされます（冒頭＝1つ前の
        クリップからののりしろ、末尾＝素材（末尾）の凍結フレーム）。内容の
        似ている2本の動画の間の欠落区間を補完する用途です。2本の内容が離れて
        いると最後のクリップ内でクロスフェードや不自然なモーフが起きますが、
        これは仕様として許容しています。**なおこの組み合わせは実験的な機能で、
        最後のクリップを長くすると映像がほぼ静止する時間が出ることがあります。**
