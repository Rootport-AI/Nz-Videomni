...
# Source file: lib/models/uetrack/decoder.py
# Modifications are marked with "NZ:" comments. See tracking/VENDOR_NOTICE.md.
import torch.nn as nn
import torch
import torch.nn.functional as F
from .box_ops import box_xyxy_to_cxcywh  # NZ: was lib.utils.box_ops
...
class Corner_Predictor(nn.Module):
    ...
    # NZ: added `device` (default CPU). This project never builds this head
    # (DECODER.TYPE is CENTER), but the two coordinate grids below held the
    # only CUDA-pinning calls in this file, and a CPU-only port carries none.
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False, device="cpu"):
        ...
        with torch.no_grad():
            ...
            self.coord_x = self.indice.repeat((self.feat_sz, 1)) \
                .view((self.feat_sz * self.feat_sz,)).float().to(device)  # NZ: was a CUDA pin
