#!/usr/bin/env bash
# Detached spike launcher. Sets isolated env, runs the measured spike, tees to log.
set -u
...
export SPIKE_BLOCKS_ON_GPU=8

cd "$PWD_ROOT/vendor/LTX-Desktop-LOW-VRAM/backend"
LOG="$PWD_ROOT/outputs/forkenv_spike_bs8_cpuenc.log"
echo "=== launching spike bs8 cpuenc at $(date) ===" > "$LOG"
