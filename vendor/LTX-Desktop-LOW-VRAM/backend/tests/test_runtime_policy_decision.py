...
def test_windows_with_low_vram_forces_api() -> None:
    assert decide_force_api_generations(system="Windows", cuda_available=True, vram_gb=7) is True
...
def test_linux_with_low_vram_forces_api() -> None:
    assert decide_force_api_generations(system="Linux", cuda_available=True, vram_gb=7) is True
